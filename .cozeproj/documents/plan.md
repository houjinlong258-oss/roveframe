...
**模型路由**：所有 AI 调用统一经过模型路由层——用户已在设置中接入自己的大模型（**Claude / Anthropic（主力，原生 Messages 协议）**、OpenAI、Gemini、DeepSeek、豆包、Kimi、通义千问、智谱 GLM、Grok、自定义 OpenAI 兼容接口）时优先调用用户模型（流式 SSE），未接入时回落平台内置 LLM 能力。路由层内置各服务商预设（默认 Base URL + 模型列表），Claude 走 Anthropic Messages 协议适配，其余统一走 OpenAI 兼容协议。**自动模式（默认）**：按任务复杂度分流——评论回复、邮件分类等轻量任务走轻量模型，经营分析、方案生成等复杂任务走旗舰模型，节省成本；用户也可为每种能力固定指定模型。

| API | 能力 |
|-----|------|
| `/api/agent/chat` | AI COO 流式对话：自动拉取营收/订单/客户摘要注入上下文，回答经营问题并给出行动建议 |
| `/api/insights` | Dashboard AI 洞察：基于真实数据生成经营解读与异常提示；`?type=briefing` 生成每日简报（经营总结 + 优先事项） |
...
**布局**：顶部导航栏（Logo + 导航项 + 告警铃铛 + 设置入口）；主内容区自上而下：AI 今日简报卡（每日 06:30 自动生成经营总结 + 3 条优先事项，点击直达对应页面）→ KPI 卡片区（今日营收/今日订单/客流量/好评率）→ 图表区（近 7 日营收趋势折线图 + 热销菜品 Top5 横向柱状图 + 订单渠道占比饼图）→ 下半区左右分栏：AI 经营洞察流（左，卡片流，含正/负/建议三类标记，每卡含一键行动按钮 + 问 AI）+ 实时告警列表（右）。
...
| 今日简报优先事项 | 点击 | 按类型跳转 @page(/reviews)?filter=pending / @page(/reservations) / @page(/customers)?filter=churn | filter | 一键直达待办 |
| AI 洞察卡片 | 点击一键行动 | 按洞察类型跳转 @page(/marketing)?brief / @page(/reviews)?filter=negative 并预填上下文 | brief/filter | 行动闭环 |
