...
| 维度 | 审计时 | **本轮修复后** | 依据 |
|---|---|---|---|
| 核心业务功能（20 页 + 91 API） | 约 85% | **约 90%** | 落地页补齐对外入口；91 路由 0 未授权 |
| AI / Agent 主链路 | 约 90% | **约 95%** | 联网检索接通（此前静默不可用）；平台回落可用 |
| 数据库 schema | 约 80% | **约 90%** | 51/51 表；迁移清单补齐 + 列级守卫 |
| 部署链 | 约 45% | **约 70%** | 4 个在用迁移入链 + 单一事实源；`output: standalone` 未做 |
| 安全（应用层） | 约 85% | **约 90%** | 0 未授权路由 + 高风险入口补行为测试 |
| 安全（数据库层） | 约 10% | **约 85%** | RLS 从"启用却零策略"变为 **105 条策略**，零交叉验证通过 |
| 可观测性 | 约 25% | **约 60%** | 指标端点 + 告警规则 + 备份调度说明；无 APM / 日志聚合 |
| 可水平扩展 | 约 20% | **约 40%** | 契约显式化且可检测；共享后端仍未接（需跨模块改造） |
| 测试（有效性） | 约 60% | **约 70%** | 724 TS + 815 Python；新增 5 组守卫均验证过"能失败" |
| 运维手册 / 回滚 | 约 70% | **约 75%** | 新增监控告警手册；真实恢复演练仍未做 |
| 外部集成 | 约 45% | **约 65%** | ERPNext 误导性状态已修；搜索对 Agent 可用；RAG 仍耦合平台 |
| 产品化（面向外部客户） | 约 35% | **约 50%** | 落地页 + 三语 + 价格占位；**计费闭环仍未做** |

**综合**：单商户试点 **约 90%**（审计时 75%）；对外 SaaS **约 55–60%**（审计时 35–40%）。

**剩余差距集中在两处**，都不是"代码没写"：

1. **计费闭环** —— 无订阅开通、无支付网关对接，价格仍是占位数字（需业务决策）；
2. **水平扩展** —— 限流/并发槽仍是进程内存，多副本会成倍放宽。
   接入共享后端要把 12 处调用点从同步改为 await，属**跨模块改造**，
   不是换一个依赖。

另有三项需要**外部系统或人工操作**，本轮只能做到"可检测 + 有文档"：
APM / 日志聚合、凭据轮换（只能在控制台做）、真实恢复演练。

### 2.1 三个"看起来能用、其实不能"的地方 —— **本轮已全部修复**

审计时这三项的共同点：**UI 或文档声称的能力，后端并不具备**，
测试全绿也发现不了，只能靠读代码 + 查系统目录。

| 项 | 审计时的实况 | 本轮处置 |
|---|---|---|
| 数据库层租户隔离 | 33 张表启用 RLS 却**零策略**，`migrate-rls.sql` 从未成功执行过 | **已修**：迁移本身有类型错误（`varchar = uuid`），修好后应用，策略 2 → **105**；零交叉验证通过 |
| ERPNext 集成 | 设置页显示"已连接"，同步端点却是 `not implemented` | **已修**：单一能力事实源 + `connectivity_only` 状态 + UI 明示"数据不会同步"；HTTP 实测 5 项全过 |
| RAG / 知识库 | 嵌入凭据来自平台运行时身份，自部署下抛错 | **未修（有意）**：属架构决策（P1-5 嵌入 provider 抽象），需先选定第二个嵌入后端才能验证抽象是否成立 |
