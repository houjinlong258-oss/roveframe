"""Windows UTF-8 bootstrap for RoveAgent entry points.
...
should be imported at the very top of every RoveAgent entry point
(``roveagent``, ``roveagent-agent``, ``roveagent-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``roveagent_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``roveagent_bootstrap``, never ``roveagent_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing RoveAgent modules.

    RoveAgent ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real RoveAgent source root
    ...
    """
    root = src_root or os.environ.get("ROVEAGENT_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``ROVEAGENT_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("ROVEAGENT_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import roveagent_bootstrap``
# (or ``from roveagent_bootstrap import apply_windows_utf8_bootstrap``) at
