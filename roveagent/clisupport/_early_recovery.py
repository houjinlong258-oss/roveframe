"""Dependency-light venv recovery that runs BEFORE roveagent_cli.main's imports.

The ``roveagent`` console entry point is ``roveagent_cli.main:main``.  Importing
``roveagent_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``roveagent_cli.env_loader``, ``yaml`` via ``roveagent_cli.config``, ...).  In
...
a corrupted venv.  ``roveagent_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``roveagent_cli.main`` to become
...
#
# ``roveagent update`` renames the live ``roveagent*.exe`` shims aside
# (``roveagent.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``roveagent`` on PATH, and the command that would repair it IS ``roveagent
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY roveagent
    invocation, and ``roveagent acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which RoveAgent already vendors
    (``~/.roveagent/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".roveagent" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``roveagent acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``roveagent_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``roveagent_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``roveagent update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `roveagent update`: CLI and Desktop retries preserve
...
# every `roveagent` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``roveagent update`` (or the self-lock preflight, #83569) left the
    ...
    ``roveagent_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``roveagent acp`` included).
    ...
    """
    try:
        from roveagent.clisupport import _install_repair as ir
        ...
        try:
            ...
            print(
                "  The next launch will retry; roveagent will keep working from "
