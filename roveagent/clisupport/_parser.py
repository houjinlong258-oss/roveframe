"""
Top-level argparse construction for the roveagent CLI.
...
# argparse runs (it sets ``ROVEAGENT_HOME`` and strips itself from ``sys.argv``),
...
# tests/roveagent_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``roveagent --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``roveagent_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    roveagent                        Start interactive chat
    roveagent chat -q "Hello"        Single query mode
    roveagent --tui                  Launch the modern TUI (or set display.interface: tui)
    roveagent --cli                  Force the classic REPL (overrides display.interface: tui)
    roveagent -c                     Resume the most recent session
    roveagent -c "my project"        Resume a session by name (latest in lineage)
    roveagent --resume <session_id>  Resume a specific session by ID
    roveagent --resume latest        Resume the most recent session (same as -c)
    roveagent --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    roveagent setup                  Run setup wizard
    roveagent logout                 Clear stored authentication
    roveagent auth add <provider>    Add a pooled credential
    roveagent auth list              List pooled credentials
    roveagent auth remove <p> <t>    Remove pooled credential by index, id, or label
    roveagent auth reset <provider>  Clear exhaustion status for a provider
    roveagent model                  Select default model
    roveagent fallback [list]        Show fallback provider chain
    roveagent fallback add           Add a fallback provider (same picker as `roveagent model`)
    roveagent fallback remove        Remove a fallback provider from the chain
    roveagent config                 View configuration
    roveagent config edit            Edit config in $EDITOR
    roveagent config set model gpt-4 Set a config value
    roveagent gateway                Run messaging gateway
    roveagent -s roveagent-agent-dev,github-auth
    roveagent -w                     Start in isolated git worktree
    roveagent gateway install        Install gateway background service
    roveagent sessions list          List past sessions
    roveagent sessions browse        Interactive session picker
    roveagent sessions rename ID T   Rename/title a session
    roveagent logs                   View agent.log (last 50 lines)
    roveagent logs -f                Follow agent.log in real time
    roveagent logs errors            View errors.log
    roveagent logs --since 1h        Lines from the last hour
    roveagent debug share             Upload debug report for support
    roveagent console                Open the safe RoveAgent command console
    roveagent update                 Update to latest version
    roveagent dashboard              Start web UI dashboard (port 9119)
    roveagent dashboard --stop       Stop running dashboard processes
    roveagent dashboard --status     List running dashboard processes

For more help on a command:
    roveagent <command> --help
"""
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="roveagent",
        description="RoveAgent Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `roveagent chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ROVEAGENT_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `roveagent setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to ROVEAGENT_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.roveagent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with RoveAgent Agent",
    )
    ...
    # top-level parser: when the user writes `roveagent -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/roveagent_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also ROVEAGENT_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.roveagent/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from RoveAgent itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
