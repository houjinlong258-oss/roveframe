"""``roveagent_cli/_scan_venv_blockers.py`` — Standalone venv-process scan for JSON consumption.

Invoked by the Desktop Electron app::

    venv\\Scripts\\python.exe -m roveagent_cli._scan_venv_blockers
...
def _redact_sensitive_cmdline(cmdline: str) -> str:
    ...
    try:
        from roveagent.core.redact import redact_sensitive_text  # noqa: PLC0415
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    roveagent_cli.main gateway run`` and the uv-side interpreter re-running the
    same argv). Reporting those as blockers dead-ends the Desktop update:
    the preflight aborts with ``venv-blocked`` *before* spawning
    ``roveagent-setup``, so the CLI updater's own
    ``_pause_windows_gateways_for_update()`` — which exists precisely to
    stop these processes (and is always active: ``roveagent-setup`` invokes
    ``roveagent update --yes --gateway``) — never gets the chance to run.
    ...
    *value* shadowed the subcommand token. An import failure counts as
    ...
    """
    try:
        from roveagent.gateway.status import looks_like_gateway_command_line  # noqa: PLC0415
    except Exception:
        return False
...
def main() -> None:
    ...
    try:
        from roveagent.clisupport.main import _detect_venv_python_processes  # noqa: PLC0415
