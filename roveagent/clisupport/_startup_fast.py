...
This module is imported by ``roveagent_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no roveagent_cli.config,
...
``roveagent_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(roveagent_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ROVEAGENT_HOME?"""
    active_profile = os.path.join(roveagent_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    roveagent_home = os.environ.get("ROVEAGENT_HOME", "").strip()
    if roveagent_home:
        return roveagent_home
    return os.path.join(os.path.expanduser("~"), ".roveagent")
...
def container_mode_may_be_active() -> bool:
    ...
    roveagent_home = os.environ.get("ROVEAGENT_HOME", "").strip()
    if roveagent_home:
        if os.path.exists(os.path.join(roveagent_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(roveagent_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(roveagent_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".roveagent")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect ROVEAGENT_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``roveagent --version`` output (also used by /version).
    ...
    ``roveagent version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        ...
        from roveagent.clisupport import __release_date__, __version__

        print(f"RoveAgent Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from roveagent.clisupport.config import detect_install_method
    ...
    try:
        from roveagent.clisupport.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``roveagent --version`` before the heavy import wall.
    ...
    container. Termux keeps the ROVEAGENT_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("ROVEAGENT_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
