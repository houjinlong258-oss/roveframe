...
RoveAgent is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_ROVEAGENT_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `roveagent
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from RoveAgent' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    RoveAgent shells out to git from many non-interactive contexts — MCP catalog
...
def _text_names_roveagent(text: str) -> bool:
    """True when *text* names RoveAgent at a path-segment / token boundary.

    A bare ``"roveagent" in text`` substring test would also match unrelated
    processes whose paths merely contain the letters (``...\\sroveagenta\\...``),
    ...
    that *starts with* ``roveagent`` (``roveagent``, ``roveagent.exe``, ``roveagent_cli``,
    ``roveagent-agent``, ``roveagent-runtime``) or the hidden-dir form
    ``.roveagent``/``.roveagent-runtime``.
    """
    for token in re.split(r"[\\/\s=,;\"']+", text.lower()):
        if token.startswith("roveagent") or token.startswith(".roveagent"):
            return True
    return False


def _process_command_is_roveagent(pid: int) -> bool:
    """Best-effort check that *pid* currently runs RoveAgent code."""
    try:
        import psutil
        ...
        return _text_names_roveagent(f"{command} {executable}")
    except Exception:
        return False


def pid_is_roveagent(
    pid: int,
    *,
    ...
    The PID must be valid, currently exist, and identify a RoveAgent process. When
    ...
    if (
        ...
        return False
    try:
        return _process_command_is_roveagent(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # ``pid_is_roveagent`` guard is for BARE pids from state files or process
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``roveagent update`` on slow-WMI machines
