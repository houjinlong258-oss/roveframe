    ...
    if isinstance(config, dict):
        if "max_concurrent_sessions" in config:
            ...
            if isinstance(gateway_cfg, dict):
                ...
                key = "roveagent.gateway.max_concurrent_sessions"
    else:
...
def active_session_limit_message(
    ...
    return (
        f"RoveAgent is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _registry_home(registry_home: str | Path | None = None) -> Path:
    return Path(registry_home) if registry_home is not None else Path(get_roveagent_home())
...
class ActiveSessionLease:
    ...
    # root ``ROVEAGENT_HOME`` must release against the same registry even when
...
def release_active_session(lease: ActiveSessionLease) -> None:
    ...
    # running under a profile ROVEAGENT_HOME override (#85431).
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``roveagent dashboard`` / ``serve``) never trips that
