"""roveagent import-agent — import Claude Code / Codex CLI setups into RoveAgent.

Usage:
    roveagent import-agent                       # auto-detect ~/.claude or ~/.codex
    roveagent import-agent claude-code           # import from ~/.claude
    roveagent import-agent codex                 # import from ~/.codex
    roveagent import-agent claude-code --dry-run # preview only, no changes
    roveagent import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``roveagent claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_roveagent.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ROVEAGENT_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ROVEAGENT_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ROVEAGENT_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ROVEAGENT_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ROVEAGENT_HOME/skills/codex-imports/<name>/
...
deliberately via ``roveagent setup`` or config.yaml.
"""
...
from roveagent.utils import atomic_write_text, atomic_yaml_write

logger = logging.getLogger(__name__)

# Same entry delimiter as the RoveAgent memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `roveagent config edit` (or move it aside), then "
            f"re-run the import."
        ) from exc
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`roveagent config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_roveagent.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → RoveAgent command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a RoveAgent glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct RoveAgent equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if name in existing and not self.overwrite:
                self.record(kind, name, f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in RoveAgent config")
                continue

            roveagent_srv: Dict[str, Any] = {}
            if srv.get("command"):
                roveagent_srv["command"] = srv["command"]
                if srv.get("args"):
                    roveagent_srv["args"] = srv["args"]
                env_kept, env_stripped = sanitize_mcp_env(srv.get("env"))
                if env_kept:
                    roveagent_srv["env"] = env_kept
                if env_stripped:
                    self.stripped_secrets.extend(
                        f"mcp_servers.{name}.env.{k}" for k in env_stripped
                    )
                if srv.get("cwd"):
                    roveagent_srv["cwd"] = srv["cwd"]
            if srv.get("url"):
                roveagent_srv["url"] = srv["url"]
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → ROVEAGENT_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``roveagent import-agent`` (invoked from roveagent_cli.main)."""
    from roveagent.clisupport.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: roveagent import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: roveagent import-agent claude-code   or   roveagent import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ RoveAgent — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: roveagent import-agent "
    ...
    print_info("Secrets:     never imported — run 'roveagent setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: roveagent import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'roveagent setup' "
               "to configure providers, or add them to ~/.roveagent/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from roveagent.clisupport.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'roveagent setup' or ~/.roveagent/.env.")
        print()
