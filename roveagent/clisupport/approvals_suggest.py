"""``roveagent approvals suggest`` — mine approval history into allowlist proposals.

RoveAgent has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.roveagent/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, roveagent self-management, …) are eligible.
...
def scan_approval_history(
    ...
    """
    from roveagent.tools.approval import detect_dangerous_command, detect_hardline_command
...
def normalize_command(command: str) -> str:
    """Fold user/roveagent home prefixes and collapse whitespace.
    ...
    from roveagent.tools.approval import (
        _rewrite_resolved_roveagent_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_roveagent_home(command))
...
def derive_glob(normalized: str) -> Optional[str]:
    ...
    unsafe root binary.
    """
    from roveagent.tools.approval import _has_allowlist_shell_operator
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  roveagent approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.roveagent/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``roveagent approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``roveagent approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from roveagent.clisupport.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: roveagent approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `roveagent approvals <subcommand> -h` for details."
    )
    return 1
