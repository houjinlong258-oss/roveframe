"""
Backup and import commands for roveagent CLI.

`roveagent backup` creates a zip archive of the entire ~/.roveagent/ directory
(excluding the roveagent-agent repo and transient files).

`roveagent import` restores from a backup zip, overlaying onto the current
ROVEAGENT_HOME root.
"""
...
from roveagent.constants import get_default_roveagent_root, get_roveagent_home, display_roveagent_home
...
from roveagent.clisupport.sizefmt import format_bytes as _format_size
...
# Where ``roveagent backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``roveagent-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/roveagent-agent/``
...
# ROVEAGENT_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "roveagent-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under ROVEAGENT_HOME) —
...
# File names that ``roveagent import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ROVEAGENT_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the RoveAgent backup slot."""
...
@contextmanager
def _backup_operation_lock(roveagent_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = roveagent_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another RoveAgent backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ROVEAGENT_HOME, resolved from config only (no network, no init).
    ...
    """
    try:
        from roveagent.plugins.memory import _get_active_memory_provider, load_memory_provider
    except Exception:
        return []
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to roveagent root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``roveagent-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/roveagent-agent/`` — must be preserved.
        if part == "roveagent-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ROVEAGENT_HOME.
...
# to the user as a hung `roveagent update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # roveagent_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the RoveAgent home directory."""
    roveagent_root = get_default_roveagent_root()

    if not roveagent_root.is_dir():
        print(f"Error: RoveAgent home directory not found at {roveagent_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(roveagent_root):
            _run_backup_locked(args, roveagent_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, roveagent_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"roveagent-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"roveagent-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_roveagent_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(roveagent_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(roveagent_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``roveagent-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "roveagent-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(roveagent_root)
    ...
    # outside ROVEAGENT_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_roveagent_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: roveagent import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a RoveAgent backup.
    ...
    # Look for telltale files that a roveagent home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a RoveAgent backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.roveagent/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a roveagent dir name
        if prefix in {".roveagent", "roveagent"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``roveagent import``, which is the
    ...
    so without the chown a ``sudo roveagent import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``roveagent import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to RoveAgent' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_roveagent_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (roveagent_root / "config.yaml").exists()
        has_env = (roveagent_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has RoveAgent configuration.")
        ...
        roveagent_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ROVEAGENT_HOME.
            ...
            target = roveagent_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(roveagent_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_roveagent_home()}."
            )
        ...
        profiles_dir = roveagent_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # roveagent_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: roveagent profile list  (after installing roveagent)")

        # Guidance
        print()
        if not (roveagent_root / "roveagent-agent").is_dir():
            print("Note: The roveagent-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: roveagent update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  roveagent -p {pname} gateway install")
        ...
        try:
            from roveagent.clisupport.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your RoveAgent configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and roveagent backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ROVEAGENT_HOME).
...
# — `roveagent update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ROVEAGENT_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ROVEAGENT_HOME and the entry is silently
...
def _quick_snapshot_root(roveagent_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: roveagent snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(roveagent_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside ROVEAGENT_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``roveagent update``.
    ...
    Args:
        ...
        roveagent_home: Override for the RoveAgent home directory (tests).
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, roveagent_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(roveagent_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for roveagent backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_roveagent_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, roveagent_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(roveagent_root):
            return _write_full_zip_backup_locked(out_path, roveagent_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, roveagent_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``roveagent_root`` to ``out_path``.
...
def _pre_update_backup_dir(roveagent_home: Optional[Path] = None) -> Path:
    home = roveagent_home or get_roveagent_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ROVEAGENT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ROVEAGENT_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``roveagent update``) should continue even if the backup fails.
    """
    roveagent_root = roveagent_home or get_default_roveagent_root()
    if not roveagent_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(roveagent_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, roveagent_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `roveagent claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ROVEAGENT_HOME under ``backups/`` before a
    ``roveagent claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``roveagent import <archive>``.  Writes to
    ``<ROVEAGENT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `roveagent import` and the
