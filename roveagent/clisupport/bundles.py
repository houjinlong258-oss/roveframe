"""Implementation of the ``roveagent bundles`` CLI subcommand.

Mirrors the structure of ``roveagent_cli/skills_hub.py`` but for skill
...
from roveagent.core.skill_bundles import (
    _bundles_dir,
    delete_bundle,
    get_bundle,
    list_bundles,
...
def _console() -> Console:
    # Bind to stderr so piping `roveagent bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  roveagent bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``roveagent bundles`` argparse tree.

    Called from ``roveagent_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `roveagent bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``roveagent bundles <subcommand>`` to the right handler."""
