...
into prompt_toolkit's event loop. Each function takes the RoveAgentCLI instance
...
from roveagent.clisupport.banner import cprint, _DIM, _RST
from roveagent.clisupport.config import save_env_value_secure
...
def clarify_callback(cli, question, choices, multi_select=False):
    ...
    from roveagent.tools.clarify_gateway import resolve_clarify_timeout
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.roveagent/.env and never exposed to the model.
    """
