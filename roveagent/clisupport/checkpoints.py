"""`roveagent checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.roveagent/checkpoints/``.  Actions:

    roveagent checkpoints               # same as `status`
    roveagent checkpoints status        # total size, project count, breakdown
    roveagent checkpoints list          # per-project checkpoint counts + workdir
    roveagent checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    roveagent checkpoints clear [-f]    # nuke the entire base (asks first)
    roveagent checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    roveagent checkpoints
    roveagent checkpoints prune --retention-days 3 --max-size-mb 200
    roveagent checkpoints clear -f
...
from roveagent.clisupport.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    from roveagent.tools.checkpoint_manager import store_status
    ...
    if legacy:
        ...
        print()
        print("Clear with: roveagent checkpoints clear-legacy")
    return 0
...
def cmd_prune(args: argparse.Namespace) -> int:
    from roveagent.tools.checkpoint_manager import prune_checkpoints, store_status
...
def cmd_clear(args: argparse.Namespace) -> int:
    from roveagent.tools.checkpoint_manager import CHECKPOINT_BASE, clear_all, store_status
...
def cmd_clear_legacy(args: argparse.Namespace) -> int:
    from roveagent.tools.checkpoint_manager import clear_legacy, store_status
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``roveagent checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `roveagent checkpoints` → status
