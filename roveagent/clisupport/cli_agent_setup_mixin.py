"""Agent-construction and session-resume display methods for ``RoveAgentCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.roveagent/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``RoveAgentCLI``. ``self.*``
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    ...
    ``roveagent chat -q`` runs one turn without ever building the
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``RoveAgentCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("   Run 'roveagent model' to choose a provider, or "
                      "'roveagent setup' for first-time setup.")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: roveagent setup")
            return False
        ...
        # `roveagent chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `roveagent auth add openai-codex`
        # without `roveagent model`), fall back to the provider's first catalog
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        exact same flow as ``roveagent model`` (which fronts Quick Setup / Nous
        ...
        try:
            ...
            answer = "n"
        if answer in {"n", "no"}:
            _cprint("  Skipped. Run 'roveagent model' or 'roveagent setup' any time.")
            return False
        ...
        try:
            ...
            print()
            _cprint("  Setup cancelled. Run 'roveagent model' any time.")
            return False
        except Exception as exc:
            logger.debug("first-run provider setup failed: %s", exc)
            _cprint(f"  ⚠️  Provider setup failed: {exc}")
            _cprint("  Run 'roveagent model' to try again.")
            return False
        ...
        if self._runtime_credentials_ready():
            ...
            return True
        _cprint("  Provider setup didn't complete. Run 'roveagent model' to retry.")
        return False
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from roveagent.clisupport.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (roveagent sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (roveagent sessions list).{_RST}")
                return False
        ...
        try:
            ...
            from roveagent.constants import partial_update_hint
    ...
    def _display_resumed_history(self):
        ...
        from roveagent.tools.ansi_strip import sanitize_display_text as _sanitize_display_text
        ...
        for i, (role, text) in enumerate(entries):
            if role == "event":
                ...
                lines.append("  ◆ RoveAgent: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ RoveAgent: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines() or [""]
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
