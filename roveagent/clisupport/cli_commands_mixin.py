...
``cli.py``'s ``RoveAgentCLI`` class. ``RoveAgentCLI`` inherits ``CLICommandsMixin`` so
...
from roveagent.constants import display_roveagent_home, is_termux as _is_termux_environment
...
def _print_lightpanda_engine_status() -> None:
    ...
    """
    try:
        from roveagent.tools.browser_tool import lightpanda_engine_status, _using_lightpanda_engine
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after RoveAgent did)
            /rollback diff <N>        — preview changes since checkpoint N
            /rollback <N> <file>      — restore a single file from checkpoint N
        """
        from roveagent.tools.checkpoint_manager import format_checkpoint_list
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: roveagent --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything RoveAgent changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — RoveAgent hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for RoveAgent config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_roveagent_home()}/state-snapshots/):\n")
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from roveagent.clisupport.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `roveagent profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: roveagent -p {imported}")
    ...
    def _handle_stop_command(self):
        ...
        try:
            from roveagent.tools.async_delegation import active_count, interrupt_all
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `roveagent journey`).
        ...
        from cli import _cprint
        from roveagent.clisupport.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from roveagent.clisupport.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from roveagent.clisupport.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run roveagent chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in roveagent_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        if not pcfg or not pcfg.enabled:
            ...
            relay_fronts = False
            try:
                from roveagent.gateway.relay import relay_platform_identities
        ...
        # "Is `roveagent gateway` running?" is the correct diagnosis, and the
        ...
        # CLI printed "Is `roveagent gateway` running?" while the gateway was
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `roveagent gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `roveagent sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from roveagent.clisupport.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `roveagent sessions list` to see available sessions.")
            return
        ...
        if self.agent:
            ...
            if hasattr(self.agent, "_todo_store"):
                try:
                    from roveagent.tools.todo_tool import TodoStore
        ...
        from roveagent.core.context_compressor import is_user_originated_turn
        ...
        # same directory as a startup `roveagent -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``roveagent -w``).

        ``prune`` is the same attended reclaim as ``roveagent worktree prune``
        (roveagent_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"prune", "gc", "clean"}:
            ...
            from roveagent.clisupport import worktree_gc
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `roveagent -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `roveagent -w`.
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=os.environ.get("ROVEAGENT_SESSION_SOURCE", "cli"),
                model=self.model,
                model_config={
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from roveagent.clisupport.config import cfg_get, read_raw_config
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        """
        from roveagent.core.pet import store
        from roveagent.core.pet.manifest import ManifestError
        from roveagent.clisupport.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        """
        from roveagent.core.pet import store
        from roveagent.core.pet.generate import orchestrate
        from roveagent.core.pet.generate.imagegen import GenerationError
        from roveagent.clisupport.pets import _set_active
    ...
    def _handle_cron_command(self, cmd: str):
        ...
        import shlex
        from roveagent.tools.cronjob_tools import cronjob as cronjob_tool
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from roveagent.clisupport.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from roveagent.clisupport.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to roveagent_cli.curator so the CLI and the `roveagent curator`
        ...
        try:
            from roveagent.clisupport.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to roveagent_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from roveagent.clisupport.skills_hub import handle_skills_slash
    ...
    def _handle_learn_command(self, cmd: str):
        ...
        """
        from roveagent.core.learn_prompt import build_learn_prompt
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from roveagent.clisupport.init_command import build_init_prompt_for_cwd
    ...
    def _handle_background_command(self, cmd: str):
        ...
        def run_background():
            ...
            try:
                ...
                if response:
                    try:
                        from roveagent.clisupport.skin_engine import get_active_skin
                        _skin = get_active_skin()
                        label = _skin.get_branding("response_label", "⚕ RoveAgent")
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``roveagent bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: roveagent bundles create "
        ...
        _cprint(
            ...
            f"Manage with `roveagent bundles`.{_RST}"
        )
    ...
    def _handle_browser_command(self, cmd: str):
        ...
        if sub == "use" or sub.startswith("use "):
            ...
            from roveagent.tools.registry import invalidate_check_fn_cache
            ...
            if arg == "on":
                ...
                from roveagent.tools.browser_use_cli import BACKEND_DISABLED
        ...
        if sub.startswith("connect"):
            ...
            try:
                from roveagent.tools.browser_tool import cleanup_all_browsers
            ...
            try:
                from roveagent.tools.browser_tool import _ensure_cdp_supervisor  # type: ignore[import-not-found]
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a RoveAgent-managed isolated debug "
            ...
            if current:
                ...
                try:
                    from roveagent.tools.browser_tool import cleanup_all_browsers, _stop_cdp_supervisor
            ...
            print()
            try:
                from roveagent.tools.browser_use_cli import is_browser_use_cli_mode
            ...
            if current:
                ...
                try:
                    from roveagent.tools.browser_tool import _get_cloud_provider
                ...
                if provider is not None:
                    ...
                    try:
                        from roveagent.tools.browser_tool import _get_browser_engine
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `roveagent cron`.
        """
        from cli import _DIM, _RST, _cprint
        from roveagent.clisupport.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from roveagent.clisupport.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this RoveAgent process runs — use `roveagent cron` for "
    ...
    def _handle_review_command(self, cmd: str) -> None:
        ...
        snapshot = list(getattr(self, "conversation_history", None) or [])
        try:
            from roveagent.core.review_engine import format_dispatch_note, start_review
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"RoveAgent keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from roveagent.clisupport.loops import dispatch_loop_command
        ...
        if result.get("created"):
            try:
                from roveagent.clisupport.loops import goal_blocks_loop_tick
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from roveagent.clisupport.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_roveagent_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="roveagent_prompt_")
        try:
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from roveagent.clisupport.colors import Colors as _Colors
        from roveagent.clisupport.focus_view import (
            FOCUS_CONFIG_KEY,
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from roveagent.clisupport.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from roveagent.clisupport.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from roveagent.clisupport.focus_view import format_hidden_line
    ...
    def _handle_reasoning_command(self, cmd: str):
        ...
        if explicit_global and save_config_value("roveagent.core.reasoning_effort", arg):
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while RoveAgent is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while RoveAgent is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while RoveAgent is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from roveagent.constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from roveagent.clisupport.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
        ...
        if explicit_global and save_config_value("roveagent.core.service_tier", saved_value):
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        """
        from roveagent.clisupport.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update RoveAgent Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``roveagent update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update RoveAgent Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update RoveAgent Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update RoveAgent Agent",
            detail="This will exit the current session and run `roveagent update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey RoveAgent' hotword listener.
    ...
    def _persist_wake_word_enabled(self, enabled: bool):
        ...
        try:
            from roveagent.tools.wake_word import load_wake_word_config
