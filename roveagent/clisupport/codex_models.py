...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in roveagent_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # RoveAgent uses. Removed here. If OpenAI re-enables them on Codex backend,
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # entitlement; RoveAgent does not.
...
def _add_context_variants(model_ids: List[str]) -> List[str]:
    ...
    The suffix is RoveAgent-side only — it is stripped before the model id hits
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while RoveAgent openai-codex talks to the same
