...
Toggles `model.openai_runtime` between "auto" (= chat_completions, RoveAgent'
...
The actual runtime resolution happens in roveagent_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "roveagent"}:
        return "auto", []
...
def check_codex_binary_ok() -> tuple[bool, Optional[str]]:
    ...
    try:
        from roveagent.core.transports.codex_app_server import check_codex_binary
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, roveagent-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate RoveAgent' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into RoveAgent for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from roveagent.clisupport.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the roveagent-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "roveagent-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + RoveAgent tool callback are always-on production
            ...
            if "roveagent-tools" in mig_report.migrated:
                msg_lines.append(
                    "RoveAgent tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default RoveAgent runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "RoveAgent tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default RoveAgent runtime.")
