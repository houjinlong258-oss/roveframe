"""Slash command definitions and autocomplete for the RoveAgent CLI.
...
from roveagent.constants import INDICATOR_STYLES
...
def _personalities_from_cli_config() -> Dict[str, Any]:
    ...
    from roveagent.clisupport.personality import available_personalities
...
class CommandDef:
    ...
    # Names a key in ``roveagent_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``roveagent_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of RoveAgent config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal RoveAgent works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .roveagent/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey RoveAgent' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while RoveAgent is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.roveagent/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update RoveAgent Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show RoveAgent Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`roveagent_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/roveagent`` subcommand mapping, and
    ...
    """
    try:
        from roveagent.clisupport.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. RoveAgent ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full RoveAgent
...
def _collect_gateway_skill_entries(
    ...
    try:
        from roveagent.core.skill_commands import get_skill_commands
        from roveagent.tools.skills_tool import SKILLS_DIR
        from roveagent.core.skill_utils import get_external_skills_dirs, get_project_skills_dirs
        ...
        # ``roveagent skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``roveagent skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``roveagent skills list`` and
...
# unrelated command landed. These claim their slots right after /roveagent,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /roveagent <command>).
...
# native slot, the alias spelling stays reachable via /roveagent reset).
...
# ``/roveagent <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/roveagent" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /roveagent topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /roveagent moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /roveagent debug on Slack.
#   - egress: Docker-only proxy status; reachable as /roveagent egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /roveagent init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /roveagent version on
...
#   - diff: git working-tree diff; reached via /roveagent diff on Slack so it
...
#     /roveagent update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /roveagent heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /roveagent refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /roveagent pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /roveagent whoami on Slack.
...
#     /roveagent platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_ROVEAGENT_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform", "insights"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/roveagent <verb>`` subcommand.
    ...
    can still reach them via ``/roveagent <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/roveagent`` is always reserved as the first entry so the
    legacy ``/roveagent <subcommand>`` form keeps working for anything that
    ...
    # Reserve /roveagent as the catch-all top-level command.
    entries.append(("roveagent", "Talk to RoveAgent or run a subcommand", "[subcommand] [args]"))
    seen.add("roveagent")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_ROVEAGENT_ONLY:
            # Intentionally Slack-via-/roveagent only (see _SLACK_VIA_ROVEAGENT_ONLY).
            return
    ...
    # everything except /roveagent, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://roveagent-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /roveagent handler.

    Maps both canonical names and aliases so /roveagent bg do stuff works
    the same as /roveagent bg do stuff.

    Plugin-registered slash commands are included so ``/roveagent <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _stacked_skill_completions(self, text: str):
        ...
        """
        try:
            from roveagent.core.skill_commands import _MAX_STACKED_SKILLS as _cap
        except Exception:
            _cap = 5
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from roveagent.clisupport.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via roveagent_cli.personality."""
        try:
