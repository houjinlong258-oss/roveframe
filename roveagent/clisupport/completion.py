"""Shell completion script generation for roveagent CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_roveagent_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# RoveAgent Agent bash completion
# Add to ~/.bashrc:
#   eval "$(roveagent completion bash)"

_roveagent_profiles() {{
    local profiles_dir="$HOME/.roveagent/profiles"
...
_roveagent_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_roveagent_profiles)" -- "$cur"))
        return
    fi
...
complete -F _roveagent_completion roveagent
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef roveagent
# RoveAgent Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(roveagent completion zsh)"

_roveagent_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.roveagent/profiles" ]]; then
        profiles+=($HOME/.roveagent/profiles/*(N/:t))
    fi
...
_roveagent() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_roveagent_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'roveagent command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# RoveAgent Agent fish completion",
        "# Add to your config:",
        "#   roveagent completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __roveagent_profiles",
        "    echo default",
        "    if test -d $HOME/.roveagent/profiles",
        "        for d in $HOME/.roveagent/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c roveagent -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c roveagent -f -s p -l profile"
        " -d 'Profile name' -xa '(__roveagent_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__roveagent_profiles)' -d 'Profile name'"
                )
