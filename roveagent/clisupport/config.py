"""
Configuration management for RoveAgent Agent.

Config files are stored in ~/.roveagent/ for easy access:
- ~/.roveagent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.roveagent/.env         - API keys and secrets

This module provides:
- roveagent config          - Show current configuration
- roveagent config edit     - Open config in editor
- roveagent config get      - Print a resolved configuration value
- roveagent config set      - Set a specific value
- roveagent config unset    - Remove a user configuration value
- roveagent config wizard   - Re-run setup wizard
"""
...
from roveagent.clisupport.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``roveagent config set`` (which rewrites
    ...
    ``config.yaml`` in place: roveagent never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.roveagent/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``roveagent logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (roveagent config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  roveagent config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` RoveAgent makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. RoveAgent itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``roveagent update``, the TUI build.
...
#   on every plugin install / ``roveagent update``.
...
# * ``ROVEAGENT_HOME`` / ``ROVEAGENT_PROFILE`` / ``ROVEAGENT_CONFIG`` /
#   ``ROVEAGENT_ENV`` — RoveAgent runtime location flags. Writing these into
...
#
# IMPORTANT: ``ROVEAGENT_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (ROVEAGENT_LANGFUSE_PUBLIC_KEY,
# ROVEAGENT_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # RoveAgent runtime location — never via dashboard env writer.
    # NOT a ROVEAGENT_* blanket: integration credentials (ROVEAGENT_GEMINI_*,
    # ROVEAGENT_LANGFUSE_*, ROVEAGENT_SPOTIFY_*, ...) ARE allowed.
    "ROVEAGENT_HOME", "ROVEAGENT_PROFILE", "ROVEAGENT_CONFIG", "ROVEAGENT_ENV",
    "ROVEAGENT_CONFIG_PATH", "ROVEAGENT_ENV_PATH",
    ...
    "ROVEAGENT_COPILOT_ACP_COMMAND", "ROVEAGENT_COPILOT_ACP_ARGS",
    # RoveAgent security policy / approval-routing context. These remain available
    ...
    "ROVEAGENT_YOLO_MODE", "ROVEAGENT_ACCEPT_HOOKS", "ROVEAGENT_REDACT_SECRETS",
    "ROVEAGENT_INTERACTIVE", "ROVEAGENT_EXEC_ASK", "ROVEAGENT_GATEWAY_SESSION",
    "ROVEAGENT_CRON_SESSION", "ROVEAGENT_SINGLE_QUERY_SESSION",
    "ROVEAGENT_SESSION_KEY", "ROVEAGENT_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or RoveAgent runtime location "
            "and security policy (ROVEAGENT_HOME, ROVEAGENT_YOLO_MODE, ...) "
            "cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.roveagent/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # ROVEAGENT_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `roveagent plugins enable
    # observability/langfuse` or `roveagent tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ROVEAGENT_LANGFUSE_ENV",
    "ROVEAGENT_LANGFUSE_RELEASE",
    "ROVEAGENT_LANGFUSE_SAMPLE_RATE",
    "ROVEAGENT_LANGFUSE_MAX_CHARS",
    "ROVEAGENT_LANGFUSE_CAPTURE",
    "ROVEAGENT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "ROVEAGENT_ACP_AUTH_METHOD",
    "ROVEAGENT_ACP_AUTO_APPROVE",
    "ROVEAGENT_COPILOT_ACP_COMMAND",
    "ROVEAGENT_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from roveagent.clisupport.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set ROVEAGENT_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ROVEAGENT_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_roveagent_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # ROVEAGENT_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if RoveAgent is running in package-manager-managed mode.

    Two signals: the ROVEAGENT_MANAGED env var (set by the systemd service),
    or a .managed marker file in ROVEAGENT_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update RoveAgent through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``roveagent_cli/`` — i.e. the git checkout for source
    installs, ``/opt/roveagent`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ROVEAGENT_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how RoveAgent was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$ROVEAGENT_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. ROVEAGENT_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.roveagent``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$ROVEAGENT_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.roveagent:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$ROVEAGENT_HOME``, a
    ...
    ``docker`` and ``roveagent update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/roveagent-agent`` image bakes a ``docker``
        stamp into ``/opt/roveagent`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $ROVEAGENT_HOME.
    try:
    ...
    #    host install whose shared $ROVEAGENT_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``roveagent update``.
    try:
        method = (
            (get_roveagent_home() / ".install_method")
    ...
    # detect Nix installs that don't set ROVEAGENT_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``roveagent_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ROVEAGENT_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/roveagent`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``roveagent update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# roveagent_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side RoveAgent, it doesn't update
...
✗ ``roveagent update`` doesn't apply inside the Docker container.

RoveAgent Agent runs as a published image (nousresearch/roveagent-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/roveagent-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate roveagent-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/roveagent-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/roveagent-agent/tags
  • Your config and session history live under ``$ROVEAGENT_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``roveagent update`` inside Docker.
...
def format_managed_message(action: str = "modify this RoveAgent installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this RoveAgent installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall RoveAgent."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ROVEAGENT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, roveagent_bin
    or None if container mode is not active, we're already inside the
    container, or ROVEAGENT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ROVEAGENT_DEV") == "1":
        return None
    ...
    container_mode_file = get_roveagent_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "roveagent-agent")
    exec_user = info.get("exec_user", "roveagent")
    roveagent_bin = info.get("roveagent_bin", "/data/current-package/bin/roveagent")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "roveagent_bin": roveagent_bin,
    }
...
# Re-export from roveagent_constants — canonical definition lives there.
from roveagent.constants import get_roveagent_home, get_process_roveagent_home  # noqa: F811,E402
from roveagent.utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_roveagent_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_roveagent_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_roveagent_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the ROVEAGENT_UID / ROVEAGENT_GID env vars set by Docker deployments.

    Docker containers running RoveAgent commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level ROVEAGENT_HOME once, but
    subdirectories created at runtime by ``ensure_roveagent_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("ROVEAGENT_UID", "").strip()
    gid_str = os.environ.get("ROVEAGENT_GID", "").strip()
    try:
...
def _chown_to_roveagent_uid(path) -> None:
    """Chown ``path`` to ``ROVEAGENT_UID:ROVEAGENT_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_roveagent_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_roveagent_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the roveagent group can
    share state with the gateway service.

    The mode can be overridden via the ROVEAGENT_HOME_MODE environment variable
    (e.g. ROVEAGENT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ROVEAGENT_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``ROVEAGENT_UID``/``ROVEAGENT_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ROVEAGENT_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_roveagent_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When RoveAgent runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ROVEAGENT_CONTAINER") or os.environ.get("ROVEAGENT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ROVEAGENT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ROVEAGENT_HOME, upgrading legacy empty templates.
...
# ensure_roveagent_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_ROVEAGENT_HOME_ENSURED: set = set()


def ensure_roveagent_home():
    """Ensure ~/.roveagent directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``ROVEAGENT_HOME`` we only re-run
    ...
    ``get_roveagent_home()`` and therefore re-run for the new path.
    """
    home = get_roveagent_home()
    key = str(home)

    # Named profiles must be created explicitly (e.g. ``roveagent profile create``).
    # Check tombstones before the memo so a stale empty shell cannot skip
    # the deleted-profile guard.
    from roveagent.constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _ROVEAGENT_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        old_umask = os.umask(0o007)
        try:
            _ensure_roveagent_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _ROVEAGENT_HOME_ENSURED.add(key)


def _ensure_roveagent_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ROVEAGENT_HOME {home} does not exist."
        )
...
from roveagent.clisupport.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``roveagent config set`` uses ``.`` as the nesting separator, so a key that
    ...
    (#84064).  A backslash escapes a dot::
        ...
        _split_key_path("roveagent.providers.qwen3\\.5-397b.api_key")
...
def _set_nested(config, dotted_key: str, value):
    ...
    Supports both dict and list navigation:
      ...
      _set_nested(c, "roveagent.providers.1", "x") → c["providers"][1] = "x"
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `roveagent config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.roveagent.config`` entries, then checks
    which ones are absent or empty under ``skills.config.<key>`` in the user's
    config.yaml.  Returns a list of dicts suitable for prompting.
    """
    try:
        from roveagent.core.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
    ...
    try:
        ...
        # should never break `roveagent update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but RoveAgent' own config writer has historically emitted it
    ...
    for camel, snake in _CAMEL_ALIASES.items():
        if camel in entry and snake not in entry:
            _warn_once_per_provider(
                ...
                "roveagent.providers.%s: camelCase key '%s' auto-mapped to '%s' "
    ...
    if unknown:
        _warn_once_per_provider(
            ...
            "roveagent.providers.%s: unknown config keys ignored: %s",
    ...
    for url_key in ("base_url", "url", "api"):
        ...
        if isinstance(raw_url, str) and raw_url.strip():
            ...
            if parsed.scheme and parsed.netloc:
                ...
                logger.warning(
                    "roveagent.providers.%s: '%s' value '%s' is not a valid URL "
    ...
    # RoveAgent (``_save_discovered_models_to_config``), not hand-curated.
    # Older RoveAgent versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older RoveAgent versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``roveagent_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (roveagent_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (roveagent_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (roveagent_cli/setup.py)
    "known_plugin_toolsets", # written/read by roveagent_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (roveagent_cli/nous_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'roveagent setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — RoveAgent won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, roveagent send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'roveagent doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        ...
        hint_path = display_roveagent_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "roveagent update / roveagent -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from roveagent.clisupport.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ roveagent config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in roveagent_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from roveagent.clisupport.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        from roveagent.toolsets import validate_toolset
        from roveagent.clisupport.toolset_validation import validate_platform_toolsets
        from roveagent.clisupport.toolset_scope import toolset_allowed_for_platform
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: roveagent config set <key> <value>")
    ...
    # `roveagent update`, but only the version bump is persisted.
    ...
    # metadata.roveagent.config in their SKILL.md frontmatter.
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = read_raw_config()
            try:
                from roveagent.core.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.roveagent/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_roveagent_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``RoveAgentCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``roveagent config set``
    ...
    backends) — while display paths (``roveagent status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the ROVEAGENT_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _roveagent_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in roveagent_cli.personality.
from roveagent.clisupport.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`roveagent_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from roveagent.clisupport.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from roveagent.clisupport.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``ROVEAGENT_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`roveagent_cli.personality` (single owner).
    """
    from roveagent.clisupport.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.roveagent/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_roveagent_home``, tui profile override, multi-profile probes).
    """
...
def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    ...
    """
    from roveagent.utils import atomic_yaml_write
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.roveagent/config.yaml.
    ...
    (which change ``ROVEAGENT_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the RoveAgent host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/roveagent/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_roveagent_dotenv()
...
#   openai-codex (OAuth — roveagent auth) — OpenAI Codex
#   nous         (OAuth — roveagent auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.roveagent/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset RoveAgent writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.roveagent/.env.
    ...
    interactive menu render (`roveagent tools`, `roveagent setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `roveagent tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.roveagent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.roveagent/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``ROVEAGENT_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"ROVEAGENT_CUSTOM_{slug}_API_KEY" if slug else "ROVEAGENT_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.roveagent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.roveagent/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to RoveAgent — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known RoveAgent vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.roveagent/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.roveagent/.env`` over ``os.environ``.

    Used for RoveAgent-managed credentials where a deliberate edit to ``.env``
...
def redact_key(key: str) -> str:
    ...
    """
    from roveagent.core.redact import mask_secret
...
def show_config():
    ...
    print(color("│              ⚕ RoveAgent Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from roveagent.clisupport.auth import get_anthropic_key
    ...
    # Warn on stale ROVEAGENT_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("ROVEAGENT_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale ROVEAGENT_MAX_ITERATIONS={_env_ghost} "
                f"(run 'roveagent doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from roveagent.clisupport.personality import active_personality_name
    ...
    # Skill config
    try:
        from roveagent.core.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
    ...
    print(color("  roveagent config edit     # Edit config file", Colors.DIM))
    print(color("  roveagent config set <key> <value>", Colors.DIM))
    print(color("  roveagent setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``ROVEAGENT_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("ROVEAGENT_MODEL", ""))
...
def build_cron_model_impact(
    *,
    ...
    if jobs is None:
        try:
            from roveagent.cron.jobs import load_jobs
    ...
    from roveagent.cron.jobs import is_job_runnable
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `roveagent cron list`, then pin the "
        "intended values with `roveagent cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (roveagent_cli/plugins_cmd.py, roveagent_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `roveagent config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``roveagent config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``roveagent config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``roveagent config set --force``.
    """
    if is_managed():
        managed_error("set configuration values")
        return
    # Reject malformed dotted keys with empty segments (leading/trailing/
    # double dots). ``"roveagent.core."`` split to ["agent", ""] and _set_nested wrote
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   roveagent config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   roveagent config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``roveagent config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``roveagent config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    roveagent config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    roveagent config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    try:
    ...
    # so a fresh `roveagent config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_roveagent_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `roveagent config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `roveagent config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: roveagent config get <key> [--json]")
            print()
            print("Examples:")
            print("  roveagent config get model")
            print("  roveagent config get terminal.backend")
            print("  roveagent config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: roveagent config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  roveagent config set model anthropic/claude-sonnet-4")
            print("  roveagent config set terminal.backend docker")
            print("  roveagent config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: roveagent config unset <key>")
            print()
            print("Examples:")
            print("  roveagent config unset model")
            print("  roveagent config unset terminal.backend")
            print("  roveagent config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'roveagent config migrate' to add them")
        ...
        print("  roveagent config           Show current configuration")
        print("  roveagent config edit      Open config in editor")
        print("  roveagent config get <key>          Print a resolved config value")
        print("  roveagent config set <key> <value>   Set a config value")
        print("  roveagent config unset <key>        Remove a config value")
        print("  roveagent config check     Check for missing/outdated config")
        print("  roveagent config migrate   Update config with new options")
        print("  roveagent config path      Show config file path")
        print("  roveagent config env-path  Show .env file path")
        sys.exit(1)
...
def _inject_profile_env_vars() -> None:
    ...
    try:
        from roveagent.providers import list_providers
...
# ``_inject_profile_env_vars`` surfaces them in ``roveagent config`` UI so users
