"""Default configuration data for RoveAgent Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from roveagent_cli/config.py. Must not import from roveagent_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["roveagent-cli"],
    # SQLite journal mode used by every RoveAgent database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running RoveAgent server processes.
    ...
    "session": {
        # Per-terminal `roveagent -c`: each CLI session drops a breadcrumb file
        # under $ROVEAGENT_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by roveagent_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `roveagent chat --run-budget N`.
        ...
        # an interactive `roveagent gateway restart` must never block for hours
        ...
        # the RoveAgent-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps RoveAgent
        ...
        # identity slot (SOUL.md). Empty by default. The ROVEAGENT_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, RoveAgent adds a coding operating brief
        ...
        # `roveagent status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``ROVEAGENT_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for RoveAgent' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # RoveAgent uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at ROVEAGENT_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `roveagent -p <bot> chat -Q` recipient
        ...
        #             ROVEAGENT_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ROVEAGENT_HOME/home when it exists (old strict
        ...
        # skipped. When empty, RoveAgent auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), RoveAgent sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when RoveAgent
        ...
        #  "/home/user/.roveagent/cache/documents:/output"]
        ...
        # bundled RoveAgent image, which drops to the `roveagent` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `roveagent tools` picker's
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes RoveAgent spawn
        ...
        # `roveagent doctor` say so. Also settable via AGENT_BROWSER_ENGINE.
        "engine": "auto",
        ...
        # browser tools) runs on a RoveAgent-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by RoveAgent' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.roveagent/browser-profile/) so copied
        ...
        # approval does it run `roveagent browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, RoveAgent sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and RoveAgent should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``roveagent chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.roveagent/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: roveagent sweeps the checkpoint base at startup
        ...
        # ``roveagent checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .roveagent.md, or .cursorrules before RoveAgent applies
    ...
    # Single-query (``roveagent -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, RoveAgent truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so RoveAgent' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); roveagent =
                                      # RoveAgent' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. RoveAgent' local
    ...
    "auxiliary": {
        ...
        # Invoked by ``roveagent kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``roveagent kanban decompose`` and the kanban
        ...
        # ``roveagent profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `roveagent model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `roveagent` (and `roveagent chat`) launches by default:
        ...
        # REPL and `--tui` (or ROVEAGENT_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `roveagent --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `roveagent -c` muscle memory.  Default off so existing
        # users aren't surprised.  ROVEAGENT_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `roveagent --tui` drops a one-time hint
        ...
        # model — see roveagent_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to ROVEAGENT_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/roveagent`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `roveagent pets`. Disabled until
        ...
        "pet": {
            ...
            # get_roveagent_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see roveagent_cli/web_server.py — an event-loop stall
        ...
        # ROVEAGENT_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (ROVEAGENT_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``ROVEAGENT_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ROVEAGENT_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ROVEAGENT_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ROVEAGENT_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ROVEAGENT_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/roveagent``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; RoveAgent appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.roveagent/cache/piper-voices/
    ...
    # "Hey RoveAgent" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey roveagent",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_roveagent" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.roveagent/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.roveagent/cache/delegation/
    ...
    # response. If not, RoveAgent feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before RoveAgent auto-pauses the goal and
    ...
    "moa": {
        ...
        # <roveagent_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.roveagent/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.roveagent/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``roveagent skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``roveagent skills trust`` / ``roveagent skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${ROVEAGENT_SKILL_DIR} and ${ROVEAGENT_SESSION_ID} in SKILL.md
        ...
        # (`roveagent skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.roveagent/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.roveagent/.curator_backups/blobs/. Enables `roveagent curator ledger`
        # and single-mutation `roveagent curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `roveagent curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `roveagent curator run --consolidate` overrides
        ...
        # normally restored on every `roveagent update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `roveagent curator purge` deletes
        ...
        # skipped), snapshot ~/.roveagent/skills/ into
        # ~/.roveagent/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `roveagent curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for roveagent-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *RoveAgent Agent*" header.
    ...
    # dangerous command. -q runs export ROVEAGENT_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; ROVEAGENT_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace RoveAgent' built-in platform hint for a single messaging platform
    ...
    # ``roveagent plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.roveagent/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ROVEAGENT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .roveagent config) always require human
        ...
        # longer trigger the startup banner. Add via `roveagent doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``roveagent_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow RoveAgent to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ROVEAGENT_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ROVEAGENT_CRON_MAX_PARALLEL env var.
        ...
        # ROVEAGENT_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # ROVEAGENT_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via ROVEAGENT_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `roveagent -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `roveagent` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `roveagent kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     roveagent-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core RoveAgent tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.roveagent/logs/.
    ...
    # update model picker lists without shipping a roveagent-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://roveagent-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `roveagent model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the RoveAgent provider id (as used elsewhere in
    ...
    "monitoring": {
        ...
        "gateway_health_export": {
            ...
            "resource_attributes": {
                "service.name": "roveagent-gateway",
    ...
    "gateway": {
        ...
        # internal ROVEAGENT_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.roveagent/sessions/sessions.json entirely.
        ...
        # the ROVEAGENT_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.roveagent.gateway` that defenses 1-2 don't
        ...
        # `roveagent update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``ROVEAGENT_GATEWAY_MAX_STARTS`` / ``ROVEAGENT_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.roveagent/.env,
        ...
        # behavior: files must live under the RoveAgent cache, under
        ...
        # user. Bridged to ROVEAGENT_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the RoveAgent
        # cache (~/.roveagent/cache/{documents,images,audio,video,screenshots})
        ...
        # to ROVEAGENT_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ROVEAGENT_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ROVEAGENT_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.roveagent/state.db.
    ...
    "sessions": {
        ...
        # the default of ``roveagent sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.roveagent/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `roveagent sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `roveagent update` prints a one-line notice with
        ...
        # ~/.roveagent/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ROVEAGENT_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ROVEAGENT_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `roveagent sessions export`. Checked
    ...
    # ``roveagent doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `roveagent doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``roveagent update`` behaviour.
    "updates": {
        ...
        #     DBs) into <ROVEAGENT_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``roveagent backup``-style zip
        #     of ROVEAGENT_HOME into <ROVEAGENT_HOME>/backups/, restorable with
        #     ``roveagent import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `roveagent update` does with uncommitted local changes to the
        ...
        # When `roveagent update` finds the source checkout parked on a feature
        ...
        #                         merge. `roveagent update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<ROVEAGENT_HOME>/lsp/bin/`` on first use.
    ...
    # `roveagent tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.roveagent/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.roveagent/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.roveagent/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, RoveAgent may use this encrypted
            ...
            # ~/.roveagent/bin/ on first use.  When False you must install
            ...
            # `roveagent secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.roveagent/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. RoveAgent disables it for our users unless they
        # explicitly opt in here. When false (default), RoveAgent sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; RoveAgent also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each RoveAgent computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". RoveAgent passes the
    ...
    #
    # Configure with `roveagent egress setup`.  Disabled by default — the rest of
    # RoveAgent works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.roveagent/bin/ on
    ...
    # RoveAgent Desktop (Electron app) launch options. These only affect
    # `roveagent desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the ROVEAGENT_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the ROVEAGENT_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials RoveAgent mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'roveagent model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the RoveAgent auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey RoveAgent' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use RoveAgent, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @roveagent:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ROVEAGENT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #roveagent)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: roveagent-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'roveagent-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote RoveAgent API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote RoveAgent API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote RoveAgent API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ROVEAGENT_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ROVEAGENT_TOOL_PROGRESS_MODE for backward
    ...
    "ROVEAGENT_PREFILL_MESSAGES_FILE": {
    ...
    "ROVEAGENT_EPHEMERAL_SYSTEM_PROMPT": {
