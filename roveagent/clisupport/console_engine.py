"""Safe RoveAgent Console command engine.

This module backs ``roveagent console`` and is intentionally narrower than the
full RoveAgent CLI. It exposes a curated set of native adapters that can later be
...
from roveagent.tools.ansi_strip import strip_ansi as _strip_ansi
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["RoveAgentConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'roveagent doctor'")
        and last.startswith("Run 'roveagent setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from roveagent.clisupport._subprocess_compat import split_command_line
...
def _format_job(job: dict, action: str) -> str:
    from roveagent.cron.jobs import effective_job_state
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="roveagent", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `roveagent "):
        return ""
    return summary
...
# dashboard opens a fresh RoveAgentConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("roveagent.clisupport.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["RoveAgentConsoleEngine", list[str]], str]:
    def handler(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "RoveAgentConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["RoveAgentConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `roveagent {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `roveagent {usage}`?",
        )


class RoveAgentConsoleEngine:
    """Curated line-command executor for RoveAgent Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "roveagent":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "RoveAgent Console does not run shell syntax. Use one supported "
                    "RoveAgent command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show RoveAgent component status.", _status)
        self.register(("version",), "version", "Show RoveAgent version information.", _version)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent RoveAgent logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update RoveAgent configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for RoveAgent Console."""

        extracted = {
            "dump": (
                "roveagent.clisupport.subcommands.dump",
                "build_dump_parser",
                "cmd_dump",
                [()],
                set(),
            ...
            "debug": (
                "roveagent.clisupport.subcommands.debug",
                "build_debug_parser",
                "cmd_debug",
                [("share",), ("delete",)],
                {("share",), ("delete",)},
            ...
            "prompt-size": (
                "roveagent.clisupport.subcommands.prompt_size",
            ...
            "insights": (
                "roveagent.clisupport.subcommands.insights",
            ...
            "security": (
                "roveagent.clisupport.subcommands.security",
            ...
            "backup": (
                "roveagent.clisupport.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "roveagent.clisupport.subcommands.import_cmd",
            ...
            "config": (
                "roveagent.clisupport.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "tools": (
                "roveagent.clisupport.subcommands.tools",
                "build_tools_parser",
                "cmd_tools",
            ...
            "plugins": (
                "roveagent.clisupport.subcommands.plugins",
            ...
            "skills": (
                "roveagent.clisupport.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "mcp": (
                "roveagent.clisupport.subcommands.mcp",
                "build_mcp_parser",
                "cmd_mcp",
                [
                    ("list",),
            ...
            "memory": (
                "roveagent.clisupport.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "auth": (
                "roveagent.clisupport.subcommands.auth",
                "build_auth_parser",
                "cmd_auth",
                [
                    ("list",),
            ...
            "pairing": (
                "roveagent.clisupport.subcommands.pairing",
            ...
            "webhook": (
                "roveagent.clisupport.subcommands.webhook",
            ...
            "hooks": (
                "roveagent.clisupport.subcommands.hooks",
                "build_hooks_parser",
                "cmd_hooks",
            ...
            "slack": (
                "roveagent.clisupport.subcommands.slack",
                "build_slack_parser",
                "cmd_slack",
                [("manifest",)],
                set(),
            ...
            "profile": (
                "roveagent.clisupport.subcommands.profile",
            ...
            "cron": (
                "roveagent.clisupport.subcommands.cron",
                "build_cron_parser",
                "cmd_cron",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update RoveAgent configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "roveagent.clisupport.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("roveagent.clisupport.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "roveagent.clisupport.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("roveagent.clisupport.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
            ...
            handler_factory=lambda fixed: _builder_handler(
                "project",
                fixed,
                "roveagent.clisupport.projects_cmd",
                "build_parser",
                "cmd_project",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("roveagent.clisupport.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
        ...
        registered = {
            ...
            "checkpoints": (
                "roveagent.clisupport.checkpoints",
                "register_cli",
                None,
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported RoveAgent Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in RoveAgent Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`roveagent {first}` is not available in RoveAgent Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in RoveAgent Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in RoveAgent Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in RoveAgent Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in RoveAgent Console.",
            ("skills", "publish"): "`skills publish` is not available in RoveAgent Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in RoveAgent Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in RoveAgent Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in RoveAgent Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in RoveAgent Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in RoveAgent Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in RoveAgent Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in RoveAgent Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in RoveAgent Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in RoveAgent Console.",
            ("kanban", "gc"): "`kanban gc` is not available in RoveAgent Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in RoveAgent Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in RoveAgent Console.")
...
def _version(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "version")
    from roveagent.clisupport._startup_fast import print_fast_version_info

    return _capture_output(lambda: print_fast_version_info(check_updates=True))


def _status(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in RoveAgent Console.")
    ...
    from roveagent.clisupport.logs import list_logs, tail_log
...
def _sessions_list(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from roveagent.clisupport.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from roveagent.clisupport.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
...
def _config_migrate(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _sessions_export(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from roveagent.state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="roveagent.clisupport.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from roveagent.clisupport.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from roveagent.clisupport.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from roveagent.cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from roveagent console")
...
def _cron_resume(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    ...
    from roveagent.cron.jobs import AmbiguousJobReference, _roveagent_now, rearm_oneshot, resume_job

    try:
        if ns.at or ns.run_now:
            job = rearm_oneshot(ns.job, _roveagent_now().isoformat() if ns.run_now else ns.at)
        else:
...
def _cron_run(_engine: RoveAgentConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron run <job>")
    from roveagent.cron.jobs import AmbiguousJobReference, trigger_job
...
def run_console_repl(
    *,
    ...
    """Run the local ``roveagent console`` REPL."""
    ...
    engine = RoveAgentConsoleEngine()
    if interactive:
        print("RoveAgent Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("roveagent> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
