"""Unified provider-credential lifecycle across every store RoveAgent reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.roveagent/.env``                     — the canonical secret store
    2. ``~/.roveagent/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.roveagent/config.yaml``              — inline mirrors written by the
...
def _scrub_config_yaml_mirrors(old_value: str, new_value: str | None) -> List[str]:
    ...
    if not old_value:
        return []
    from roveagent.utils import atomic_yaml_write, fast_safe_load
    ...
    if isinstance(keyed_providers, dict):
        for provider_id, entry in keyed_providers.items():
            _fix(entry, f"roveagent.providers.{provider_id}", fields=("api_key",))
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `roveagent auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``roveagent auth add``.
    ...
    ran ``roveagent auth add <provider> --type api-key`` separately. This makes
    the Desktop save's effect on disk match what ``roveagent auth add`` does.
    """
    from roveagent.clisupport.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from roveagent.clisupport.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from roveagent.clisupport.config import load_env, remove_env_value
