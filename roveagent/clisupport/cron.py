"""
Cron subcommand for roveagent CLI.
...
# commands at execution time when ``_ROVEAGENT_GATEWAY=1``.
from roveagent.cron.lifecycle_guard import (  # noqa: F401  (re-exported for terminal_tool)
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: roveagent gateway install", Colors.DIM))
    print(color("                    sudo roveagent gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  roveagent cron status", Colors.DIM))


def cron_list(show_all: bool = False):
    """List all scheduled jobs."""
    from roveagent.cron.jobs import list_jobs

    jobs = list_jobs(include_disabled=show_all)

    if not jobs:
        print(color("No scheduled jobs.", Colors.DIM))
        print(color("Create one with 'roveagent cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_tick():
    ...
    from roveagent.cron.scheduler import CronTickYielded, tick
    try:
        tick(verbose=True)
        ...
        print("  Check `roveagent cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_runs(job_id: Optional[str] = None, limit: int = 20):
    ...
    from roveagent.cron.executions import list_executions
...
def cron_incidents(args) -> int:
    ...
    ``roveagent cron incidents [--state <s>]`` lists incidents (the stored error
    is redacted and truncated at write time, safe for terminal display);
    ``roveagent cron incidents ack <id>`` closes one so its failure ping stays
    silent until the error signature changes.
    """
    from roveagent.cron.incidents import ack_incident, list_incidents
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(
                color(
                    "✗ Incident ID required: roveagent cron incidents ack <incident_id>",
                    Colors.RED,
                )
            )
            return 1
    ...
    print(
        color(
            ...
            "roveagent cron incidents ack <id>",
            Colors.DIM,
        )
    )
    return 0
...
def cron_status():
    ...
    if not pids:
        ...
        try:
            from roveagent.gateway.status import get_running_pid, is_gateway_runtime_lock_active
    ...
    if pids or gateway_alive_via_lock:
        ...
        from roveagent.cron.scheduler import _is_fd_exhaustion_text as _cron_is_fd_exhaustion_text
        ...
        if hb_age is None:
            ...
            print("  If the gateway just started, wait ~60s and re-run `roveagent cron status`.")
            print("  If heartbeat never appears, restart: roveagent gateway restart")
            ...
            if pids:
                print(f"  PID: {', '.join(map(str, pids))}")
            print("  Cron jobs may NOT be firing. Restart: roveagent gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec roveagent "
                        "roveagent cron ...`). Fix ownership to match the "
        ...
        print("    roveagent gateway install    # Install as a user service")
        print("    sudo roveagent gateway install --system  # Linux servers: boot-time system service")
        print("    roveagent gateway            # Or run in foreground")
...
def _scripts_dir_for_cron() -> Path:
    ...
    Prefer ``cron.jobs.CRON_DIR.parent`` over a fresh ``get_roveagent_home()`` call
    so tests and profile-aware callers that monkeypatch cron storage inspect the
    same RoveAgent home the jobs were loaded from.
    """
    from roveagent.cron.jobs import CRON_DIR
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside ROVEAGENT_HOME/scripts: {script!r}"
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `roveagent cron doctor` again.", Colors.DIM))
    return 1
...
def cron_edit(args):
    from roveagent.cron.jobs import AmbiguousJobReference, resolve_job_ref
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (ROVEAGENT_SESSION_KEY);
        ...
        try:
            from roveagent.gateway.session_context import _SESSION_ASYNC_DELIVERY
...
def cron_resume(args) -> int:
    ...
    from roveagent.cron.jobs import AmbiguousJobReference, _roveagent_now, rearm_oneshot

    run_at = _roveagent_now().isoformat() if args.run_now else args.run_at
    try:
...
def cron_notepad(args) -> int:
    """Handle ``roveagent cron notepad <job_id> [get|set|delete|list]``.
    ...
    """
    from roveagent.cron import notepad
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: roveagent cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: roveagent cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: roveagent cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: roveagent cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
