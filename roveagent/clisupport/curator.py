"""CLI subcommand: `roveagent curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`roveagent curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `roveagent curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`roveagent curator status` and run `roveagent curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.set_pinned(args.skill, True):
        print(
            ...
            "`roveagent curator list-unmanaged` shows which skills the curator tracks."
        )
        return 1
    if not skill_usage.is_curator_managed(args.skill):
        ...
        print(
            ...
            f"`roveagent curator adopt {args.skill}` to put it under curator "
            "management)"
        )
        return 0
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `roveagent curator adopt <name>`, "
          "or all with `roveagent curator adopt --all-unmanaged`")
    return 0
...
def _cmd_restore(args) -> int:
    from roveagent.tools import skill_ledger, skill_usage
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`roveagent curator unpin {args.skill}`"
        )
        return 1
    tok = skill_ledger.set_ledger_actor("user")
    try:
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.roveagent/skills/.curator_backups/{snap.name}")
    return 0


def _cmd_ledger(args) -> int:
    """List per-mutation audit ledger entries (newest first)."""
    from roveagent.tools import skill_ledger
    ...
    print(
        "\nRoll back a single mutation with `roveagent curator rollback <id>`; "
        "whole-tree snapshots remain available via `roveagent curator rollback --list`."
    )
    return 0
...
def _cmd_purge(args) -> int:
    ...
    from roveagent.tools import skill_ledger
    from roveagent.tools.skill_usage import _archive_dir
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        if entry is None:
            print(
                ...
                "See `roveagent curator ledger` for entry ids, or use "
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`roveagent curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.roveagent/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.roveagent/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.roveagent/skills/ from a curator snapshot, or a single "
             "mutation by ledger entry id (see `roveagent curator ledger`)",
    )
    p_rollback.add_argument(
        "entry_id", nargs="?", default=None,
        help="Ledger entry id for single-mutation rollback (from "
             "`roveagent curator ledger`). Omit for whole-tree snapshot rollback.",
    )
    p_rollback.add_argument(
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by roveagent_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="roveagent curator")
    register_cli(parser)
