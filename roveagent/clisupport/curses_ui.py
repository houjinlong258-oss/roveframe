"""Shared curses-based UI components for RoveAgent CLI.

Used by `roveagent tools` and `roveagent skills` for interactive checklists.
...
_MENU_NAVIGATION_HANDLER: ContextVar[MenuNavigationHandler | None] = ContextVar(
    "roveagent_menu_navigation_handler", default=None
)
_NUMBERED_BACK_ENABLED: ContextVar[bool] = ContextVar(
    "roveagent_numbered_back_enabled", default=False
)
...
def _read_numbered_input(prompt_text: str) -> str | _NumberedNavigation:
    ...
    from roveagent.clisupport.pt_input_extras import install_modify_other_keys_aliases
