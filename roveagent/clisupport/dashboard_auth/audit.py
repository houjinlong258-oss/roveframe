...
Profile-aware location: ``$ROVEAGENT_HOME/logs/dashboard-auth.log``.
...
from ``roveagent_constants`` or other roveagent_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$ROVEAGENT_HOME/logs/dashboard-auth.log``.

    Uses ``roveagent_constants.get_roveagent_home()`` (a leaf module — no import
    ...
    return get_roveagent_home() / "logs" / "dashboard-auth.log"
