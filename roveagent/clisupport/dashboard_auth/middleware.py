...
from roveagent.clisupport.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /roveagent``, the
    ``login_url`` is prefixed (``/roveagent/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from roveagent.clisupport.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["roveagent.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``roveagent_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``roveagent_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from roveagent.clisupport.dashboard_auth.cookies import clear_session_cookies
        from roveagent.clisupport.dashboard_auth.prefix import prefix_from_request
