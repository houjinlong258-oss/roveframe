...
prefix (e.g. ``mission-control.tilos.com/roveagent/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /roveagent`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``ROVEAGENT_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``roveagent.example.com`` instead of
    ``https://roveagent.example.com``) — the single most common cause of
    "I set ROVEAGENT_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "roveagent.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/roveagent"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# ROVEAGENT_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``ROVEAGENT_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("ROVEAGENT_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("ROVEAGENT_DASHBOARD_PUBLIC_URL env var", env_raw)
