"""``roveagent debug`` debug tools for RoveAgent Agent.

Currently supports:
    roveagent debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.roveagent/logs/*.log`` are not leaked into
...
from roveagent.utils import atomic_replace
...
_REDACTION_BANNER = (
    "[roveagent debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.roveagent/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``roveagent debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``roveagent debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `roveagent debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "roveagent-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``roveagent debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.roveagent/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``roveagent debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "roveagent-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----RoveAgentDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from roveagent.clisupport.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_roveagent_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `roveagent debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by RoveAgent Desktop on the machine running the app, not by this "
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``roveagent debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``roveagent dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``roveagent dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "roveagent-debug-share/1"
...
def collect_share_bundle(
    ...
    Returns ``{"report": ..., "roveagent.core.log": ..., "roveagent.gateway.log": ...,
    ...
    if agent_log:
        bundle["roveagent.core.log"] = agent_log
    if gateway_log:
        bundle["roveagent.gateway.log"] = gateway_log
    if gui_log:
...
def build_nous_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    ...
    The JSON shape is what the discord-support viewer (Repo 3) parses::
        ...
        {"format": "roveagent-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
         "files": {"report": ..., "roveagent.core.log": ..., ...}}
    """
...
class DebugShareResult:
    ...
    urls: dict  # label -> paste URL (e.g. {"Report": "...", "roveagent.core.log": "..."})
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``roveagent debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "roveagent debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    for label in ("roveagent.core.log", "roveagent.gateway.log", "gui.log", "desktop.log"):
...
def run_debug_share(args):
    ...
    if local_only:
        ...
        print(bundle["report"])
        for title, label in (
            ("FULL agent.log", "roveagent.core.log"),
            ("FULL gateway.log", "roveagent.gateway.log"),
    ...
    try:
        ...
        print("\nRun `roveagent debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  roveagent debug delete <url>")

    print("\nShare these links with the RoveAgent team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/RoveAgent version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``roveagent debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from roveagent.clisupport.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "roveagent debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `roveagent debug share --local` to print the report instead, "
            "or `roveagent debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(
        ...
        "  GitHub Issues        https://github.com/NousResearch/roveagent-agent/issues\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: roveagent debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'roveagent debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``roveagent debug`` call.
    ...
    # best-effort — any failure is swallowed so ``roveagent debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: roveagent debug <command>")
        print()
        print("Commands:")
