...
from roveagent.constants import agent_browser_runnable, find_node_executable
from roveagent.tools.environments.local import roveagent_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $ROVEAGENT_HOME/node is
    ...
    "browser": lambda: (
        ...
        or _has_roveagent_agent_browser()
...
def _has_roveagent_agent_browser() -> bool:
    ...
    # install.sh installs globally into $ROVEAGENT_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-RoveAgentHome", str(get_roveagent_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = roveagent_subprocess_env(inherit_credentials=False)
