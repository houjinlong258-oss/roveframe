"""
Doctor command for roveagent CLI.

Diagnoses issues with RoveAgent Agent setup.
"""
...
from roveagent.constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
ROVEAGENT_HOME = get_roveagent_home()
_DHH = display_roveagent_home()  # user-facing display path (e.g. ~/.roveagent or ~/.roveagent/profiles/coder)

# Load environment variables from ~/.roveagent/.env so API key checks work
_env_path = get_env_path()
load_roveagent_dotenv(roveagent_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from roveagent.clisupport.colors import Colors, color
from roveagent.clisupport.models import _ROVEAGENT_USER_AGENT
from roveagent.clisupport.vercel_auth import describe_vercel_auth
from roveagent.constants import OPENROUTER_MODELS_URL
from roveagent.utils import base_url_host_matches
...
from roveagent.constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all RoveAgent containers"
...
def _roveagent_database_paths(roveagent_home: Path) -> list[tuple[str, Path]]:
    """Return (display name, path) pairs for RoveAgent-managed SQLite databases."""
    # backup.py owns the canonical list of per-profile stores; reuse it.
    from roveagent.clisupport.backup import _QUICK_STATE_FILES

    entries = [
        (name, roveagent_home / name)
        for name in _QUICK_STATE_FILES
        if name.endswith(".db")
    ]
    # Non-default kanban boards each keep their own kanban.db.
    for board_db in sorted((roveagent_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(roveagent_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``roveagent_cli.sqlite_safe_read``).
    ...
    """
    from roveagent.clisupport.sqlite_safe_read import (
        has_live_connection,
...
def _format_db_size(db_path: Path) -> str:
    ...
    from roveagent.clisupport.backup import _format_size
...
def _report_database_journal_modes(
    roveagent_home: Path | None = None,
    version_info: tuple[int, ...] | None = None,
) -> None:
    """List each database's journal mode; warn on WAL under a vulnerable SQLite."""
    from roveagent.state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = roveagent_home if roveagent_home is not None else ROVEAGENT_HOME
    try:
        databases = _roveagent_database_paths(home)
    except Exception as exc:
        check_warn(f"Could not list RoveAgent databases: {exc}")
        return
    exposed = []
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.roveagent/.env contains provider auth/base URL settings."""
...
def _honcho_is_configured_for_doctor() -> bool:
    ...
    try:
        from roveagent.plugins.memory.honcho.client import HonchoClientConfig
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("ROVEAGENT_KANBAN_TASK"):
...
def _doctor_web_capability_rows() -> list[tuple[str, str, str]]:
    ...
    try:
        from roveagent.core.web_search_registry import (
            get_active_extract_provider,
            get_active_search_provider,
        )
        from roveagent.tools.web_tools import _ensure_web_plugins_loaded, _provider_is_ready
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from roveagent.clisupport.auth import get_minimax_oauth_auth_status
...
from roveagent.clisupport.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'roveagent sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'roveagent sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    # ROVEAGENT_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor removed its only consumer (the v3→4 migration) — it is silently
    # ignored. ROVEAGENT_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("ROVEAGENT_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("ROVEAGENT_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed RoveAgent Relay plugin."""
    from roveagent.clisupport.relay_plugin_cutover import (
    ...
    if isinstance(raw_config, dict):
        ...
        if isinstance(plugins, dict):
            for key in legacy_relay_plugin_keys(plugins.get("enabled")):
                findings.append(
                    (
                        f"roveagent.plugins.enabled: {key}",
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches roveagent_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``roveagent --version`` reporting a stale version while
    ...
    """
    try:
        from roveagent.clisupport import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != roveagent_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'roveagent update', or set "
            "roveagent_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-roveagent and dashboard static services are up
    ...
    for static in ("main-roveagent", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `roveagent profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    """
    try:
        from roveagent.core.ssl_guard import verify_ca_bundle_with_fallback
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `roveagent doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        from roveagent.providers.base import ProviderProfile as _PP
        try:
            from roveagent.clisupport.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the ROVEAGENT_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("ROVEAGENT_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via ROVEAGENT_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.nousresearch.roveagent"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `roveagent update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (roveagent desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `roveagent desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.nousresearch.roveagent` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch RoveAgent once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/RoveAgent.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/RoveAgent.app``
    is deliberately not probed: it is the separately-signed RoveAgent-Setup
    launcher (``com.nousresearch.roveagent.setup``, certificate-anchored), whose
    grants are stable by construction and unaffected by rebuilds.
    """
    root = Path(__file__).resolve().parents[1]
    release_dir = root / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/RoveAgent.app") if p.is_dir()]
    if not candidates:
        return None
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    """
    try:
        from roveagent.clisupport import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and RoveAgent will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and RoveAgent.app if you use Desktop), "
        "and restart them once. With RoveAgent' stable signing identities the "
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `roveagent`.
    os.environ.setdefault("ROVEAGENT_INTERACTIVE", "1")

    # Handle `roveagent doctor --ack <id>` as a fast path. Persist the ack and
    # return without running the rest of the diagnostics — the user has
    # already seen the advisory and just wants to silence it.
    if ack_target:
        from roveagent.clisupport.security_advisories import (
            ADVISORIES,
            ack_advisory,
        )
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.roveagent/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 RoveAgent Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`roveagent doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from roveagent.state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: RoveAgent already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and roveagent_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.roveagent/.env (primary location for user config)
    env_path = ROVEAGENT_HOME / '.env'
    if env_path.exists():
        ...
        # matches roveagent_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'roveagent setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'roveagent setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'roveagent setup' to create one")
                issues.append("Run 'roveagent setup' to create .env")
    
    # Check ~/.roveagent/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = ROVEAGENT_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from roveagent.clisupport.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from roveagent.clisupport.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'roveagent config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from roveagent.clisupport.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.roveagent/.env or run 'roveagent setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'roveagent setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'roveagent config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from roveagent.clisupport.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from roveagent.clisupport.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'roveagent setup' to migrate config")
                else:
                    issues.append("Run 'roveagent doctor --fix' or 'roveagent setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from roveagent.clisupport.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'roveagent doctor --fix'")
        except Exception:
            pass

        # Detect stale ROVEAGENT_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives ROVEAGENT_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from roveagent.clisupport.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("ROVEAGENT_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"ROVEAGENT_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"roveagent.core.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `roveagent setup` run)",
                )
                if should_fix:
                    if remove_env_value("ROVEAGENT_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale ROVEAGENT_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove ROVEAGENT_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the ROVEAGENT_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale ROVEAGENT_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'roveagent doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from roveagent.clisupport.config import load_env as _load_env_depr
            from roveagent.clisupport.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses RoveAgent' own device-code flow — the Codex CLI is
    ...
    roveagent_home = ROVEAGENT_HOME
    if roveagent_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        roveagent_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = roveagent_home / subdir_name
    ...
    soul_path = roveagent_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give RoveAgent a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# RoveAgent Agent Persona\n\n"
                "<!-- Edit this file to customize how RoveAgent communicates. -->\n\n"
                "You are RoveAgent, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = roveagent_home / "memories"
    ...
    state_db_path = roveagent_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from roveagent.state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'roveagent doctor --fix' "
                        "(or 'roveagent sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from roveagent.state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'roveagent doctor --fix' "
                        "(or 'roveagent sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every RoveAgent
        ...
        try:
            from roveagent.state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'roveagent sessions optimize-storage' "
    ...
    wal_path = roveagent_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'roveagent doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "roveagent"
        ...
        _cmd_link = _cmd_link_dir / "roveagent"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(roveagent not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/roveagent → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/roveagent points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/roveagent → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/roveagent — run 'roveagent doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/roveagent exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/roveagent not found",
                    "(roveagent command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/roveagent → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/roveagent symlink — run 'roveagent doctor --fix'")
    ...
    try:
        from roveagent.constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "(pip install 'roveagent-agent[vercel]')",
                "Install the Vercel optional dependency: pip install 'roveagent-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/RoveAgent-managed install)
        ...
        try:
            from roveagent.tools.browser_tool import _find_agent_browser, _is_npx_agent_browser_sentinel
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `roveagent update` does, so a session's first browser call
            ...
            # `roveagent update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `roveagent doctor` invocation.
                from roveagent.tools.browser_tool import (
                    _chromium_installed,
                    _is_camofox_mode,
                    _get_cloud_provider,
    ...
    try:
        from roveagent.tools.browser_tool import _using_lightpanda_engine, lightpanda_engine_status
        from roveagent.tools.browser_lightpanda import LIGHTPANDA_INSTALL_HINT, find_lightpanda_binary
    except Exception:
        pass
    else:
        ...
        if _using_lightpanda_engine():
            ...
            if not _lp_used:
                ...
                check_info(
                    "Fix: pick Lightpanda in `roveagent tools` → Browser Automation, "
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable ROVEAGENT_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a RoveAgent one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'roveagent config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from roveagent.core.auxiliary_client import _to_openai_base_url
            ...
            headers = {
                ...
                "User-Agent": _ROVEAGENT_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``roveagent doctor``.
    ...
    def _probe_bedrock() -> _ConnectivityResult:
        try:
            from roveagent.core.bedrock_adapter import (
                has_aws_credentials,
    ...
    # never the legitimate source for `roveagent doctor`.
    ...
    try:
        ...
        from roveagent.model_tools import check_tool_availability, TOOLSET_REQUIREMENTS
        ...
        if api_disabled or web_not_ready:
            issues.append("Run 'roveagent setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = ROVEAGENT_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: roveagent skills list)")
    ...
    try:
        from roveagent.clisupport.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = ROVEAGENT_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            from roveagent.plugins.memory.honcho.client import HonchoClientConfig, resolve_config_path
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: roveagent memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: roveagent memory setup",
                    "No Honcho API key — run 'roveagent memory setup'",
                    issues,
                )
            else:
                from roveagent.plugins.memory.honcho.client import get_honcho_client, reset_honcho_client
                reset_honcho_client()
                try:
        ...
        try:
            from roveagent.plugins.memory.mem0 import _load_config as _load_mem0_config
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run roveagent memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: roveagent memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: roveagent memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from roveagent.clisupport.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "roveagent -p" in content:
                            _m = _re.search(r"roveagent -p (\S+)", content)
    ...
    # `roveagent doctor --live` (real network calls; bounded + read-only).
    try:
        from roveagent.clisupport.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'roveagent doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
