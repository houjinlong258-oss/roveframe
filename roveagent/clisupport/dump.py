"""
Dump command for roveagent CLI.

Outputs a compact, plain-text summary of the user's RoveAgent setup
...
from roveagent.clisupport.config import get_roveagent_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.roveagent/.env.
    ...
    exports. ``roveagent debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.roveagent_build_sha`` by the Dockerfile's
    ``ROVEAGENT_GIT_SHA`` build-arg (see ``roveagent_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``roveagent dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from roveagent.clisupport.gateway import get_gateway_runtime_snapshot
...
def _count_skills(roveagent_home: Path) -> int:
...
def _cron_summary(roveagent_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = roveagent_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    load_roveagent_dotenv(
        roveagent_home=env_path.parent,
    ...
    lines.append(f"roveagent_home:      {display_roveagent_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.roveagent/.env: a managed
        ...
        # A credential added via `roveagent auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `roveagent auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["roveagent-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(roveagent_home)}")
    lines.append(f"  skills:             {_count_skills(roveagent_home)}")
