"""
roveagent fallback — manage the fallback provider chain.
...
Subcommands:
  roveagent fallback [list]   Show the current fallback chain (default when no subcommand)
  roveagent fallback add      Pick provider + model via the same picker as `roveagent model`,
                           then append the selection to the chain
  roveagent fallback remove   Pick an entry to delete from the chain
  roveagent fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.roveagent/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from roveagent.clisupport.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `roveagent model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  roveagent fallback add")
        print()
        return
    ...
    print("  Docs: https://roveagent-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `roveagent model`, then append the selection to the chain."""
    from roveagent.clisupport.main import _require_tty, select_provider_and_model
    ...
    print("  `roveagent model` — select the provider + model you want as a fallback.")
    print()
    ...
    from roveagent.core.backend_identity import BackendIdentity, same_deployment
    ...
    print()
    print("  Run `roveagent fallback list` to view, or `roveagent fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``roveagent fallback [subcommand]``."""
