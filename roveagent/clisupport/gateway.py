"""
Gateway subcommand for roveagent CLI.

Handles: roveagent gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from roveagent.gateway.config import coerce_systemd_watchdog_seconds, load_gateway_config
from roveagent.gateway.status import terminate_pid
from roveagent.gateway.restart import (
...
# display_roveagent_home is imported lazily at call sites to avoid ImportError
# when roveagent_constants is cached from a pre-update version during `roveagent update`.
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``ai.roveagent.gateway*`` LaunchAgent — the update path needs the whole
    ...
    if is_macos():
        ...
        if all_profiles:
            ...
            # mirrors the systemd branch's ``roveagent-gateway*`` pattern above.
        ...
        if all_profiles:
            ...
            # ``launchctl list`` prefix scan also catches ai.roveagent.gateway*
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "roveagent CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
# — restart may fail", and `roveagent update` can deadlock behind it.  The loop
...
def _probe_loop_tick_socket(
    ...
    """
    try:
        from roveagent.gateway.shutdown_watchdog import get_loop_tick_socket_path
...
def probe_gateway_loop_liveness(
    ...
    *,
    ...
    try:
        from roveagent.gateway.shutdown_watchdog import get_loop_heartbeat_path
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``roveagent gateway status`` from falsely counting the ``roveagent`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``roveagent gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_roveagent_home().resolve())
    # Forward slashes on both sides of the ROVEAGENT_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"roveagent_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. ROVEAGENT_HOME
        ...
        # ROVEAGENT_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "roveagent_home=" in command_lc
            and f"roveagent_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``roveagent update`` hung exactly there
            ...
            # watchdog probe.
            from roveagent.clisupport._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``roveagent update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            RoveAgent profile are returned.
    """
    _exclude = set(exclude_pids or set())
    pids: list[int] = []
    if not all_profiles:
        try:
            from roveagent.gateway.status import get_running_pid
...
def find_profile_gateway_processes(
    ...
    *,
    ...
    """Return running gateway PIDs mapped to RoveAgent profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from roveagent.gateway.status import get_running_pid, get_running_pid_identity_strict
        from roveagent.clisupport.profiles import list_profiles
    except Exception:
        if strict:
            raise
        return processes
    ...
    for profile in profiles:
        try:
            if strict:
                identity = get_running_pid_identity_strict(profile.path / "roveagent.gateway.pid")
                pid = identity[0] if identity else None
                create_time = identity[1] if identity else 0.0
            else:
                pid = get_running_pid(profile.path / "roveagent.gateway.pid", cleanup_stale=False)
                create_time = 0.0
...
def find_windows_gateway_services(
    *,
    ...
    validated profile gateway PID comes from RoveAgent's own PID file, and its
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "roveagent.clisupport.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m roveagent_cli.main gateway
    ...
    try:
        from roveagent.gateway.status import looks_like_gateway_command_line
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``roveagent update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting RoveAgent's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose ROVEAGENT_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``roveagent update`` users
    # with no running gateway until they re-invoke ``roveagent gateway``
    ...
    from roveagent.clisupport._subprocess_compat import (
    ...
    # interpreter and captures the stable cwd + env overlay (ROVEAGENT_HOME,
    ...
    if sys.platform == "win32":
        try:
            from roveagent.clisupport.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / ROVEAGENT_HOME) the windowless
        # base interpreter needs to import roveagent_cli.  Empty on POSIX, where
...
def _roveagent_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``ROVEAGENT_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("ROVEAGENT_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_roveagent_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``ROVEAGENT_HOME``.

    Under ``sudo``, ``ROVEAGENT_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_roveagent_home` falls back to ``/root/.roveagent`` — the wrong
    profile. The unit file pins ``ROVEAGENT_HOME`` for the actual gateway
    ...
    unit_home = (_roveagent_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("ROVEAGENT_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("ROVEAGENT_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["ROVEAGENT_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}roveagent gateway status\n"
...
def _systemd_restart_wait_timeout(system: bool = False) -> float:
    ...
    from roveagent.gateway.shutdown_forensics import parse_systemd_duration_to_us
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}roveagent gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from roveagent.clisupport.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: roveagent gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `roveagent gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``roveagent gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from roveagent.clisupport.profiles import list_profiles, get_active_profile_name
    except Exception:
    ...
    for prof in profiles:
        ...
        if prof.gateway_running:
            try:
                ...
                pid = get_running_pid(prof.path / "roveagent.gateway.pid", cleanup_stale=False)
                if pid:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (roveagent_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (RoveAgent_Gateway /
            # RoveAgent_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `roveagent gateway install` deployment.
            from roveagent.clisupport.gateway_windows import get_task_name

            _task_name = get_task_name()
        except Exception:
            _task_name = "RoveAgent_Gateway"
        if _windows_scheduled_task_supervises(_task_name):
            return False

    from roveagent.gateway.status import _pid_exists, write_planned_stop_marker
    ...
    try:
        ...
        # whole ai.roveagent.gateway* fleet — not just the current profile's
    ...
    # healthy standalone gateway (no service supervisor — e.g. `roveagent
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ROVEAGENT_HOME-scoped).
    ...
    """
    try:
        from roveagent.gateway.status import get_running_pid, remove_pid_file
    except ImportError:
        return False
    ...
    try:
        from roveagent.gateway.status import write_planned_stop_marker
...
from roveagent.constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our RoveAgent Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``roveagent gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``ROVEAGENT_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("ROVEAGENT_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from roveagent.clisupport._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
_SERVICE_BASE = "roveagent-gateway"
SERVICE_DESCRIPTION = "RoveAgent Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ROVEAGENT_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ROVEAGENT_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from roveagent.constants import get_default_roveagent_root

    home = get_roveagent_home().resolve()
    default = get_default_roveagent_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ROVEAGENT_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(roveagent_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when ROVEAGENT_HOME is a named profile.

    For ``~/.roveagent/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        roveagent_home: Optional explicit ROVEAGENT_HOME path. Defaults to the current
            ``get_roveagent_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional RoveAgent root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_roveagent_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from roveagent.constants import get_default_roveagent_root

    home = Path(roveagent_home or str(get_roveagent_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_roveagent_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(roveagent_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".roveagent"
    try:
        Path(roveagent_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(roveagent_home, default_root=target_root)
    except ValueError:
        return _profile_arg(roveagent_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this ROVEAGENT_HOME.

    Default ``~/.roveagent`` returns ``roveagent-gateway`` (backward compatible).
    Profile ``~/.roveagent/profiles/coder`` returns ``roveagent-gateway-coder``.
    Any other ROVEAGENT_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older RoveAgent installs that predate the
# roveagent-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (roveagent-gateway-*.service) and unrelated third-party
# "roveagent" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("roveagent.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "roveagent.clisupport.main gateway",
    "roveagent_cli/main.py gateway",
    "gateway/run.py",
    " roveagent gateway ",
    "/roveagent gateway ",
)
...
def _find_legacy_roveagent_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy RoveAgent gateway units.

    Detects unit files installed by older RoveAgent versions that used a
    different service name (e.g. ``roveagent.service`` before the rename to
    ``roveagent-gateway.service``). When both a legacy unit and the current
    ``roveagent-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``roveagent-gateway-coder.service`` and unrelated third-party
      ``roveagent-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``roveagent.service`` running an unrelated
...
def has_legacy_roveagent_units() -> bool:
    """Return True when any legacy RoveAgent gateway unit files exist."""
    return bool(_find_legacy_roveagent_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy RoveAgent gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_roveagent_units()
    if not legacy:
        return
    print_warning("Legacy RoveAgent gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current roveagent-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    roveagent gateway migrate-legacy")


def remove_legacy_roveagent_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy RoveAgent gateway unit files.

    Iterates over whatever ``_find_legacy_roveagent_units()`` returns — which is
    ...
    legacy = _find_legacy_roveagent_units()
    if not legacy:
        print("No legacy RoveAgent gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy RoveAgent gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: roveagent gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo roveagent gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    roveagent gateway uninstall")
    print_info("    sudo roveagent gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: roveagent gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``roveagent setup`` (end of wizard)
    and ``roveagent import``: if this host supports a service manager and no
    ...
    ``roveagent gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   roveagent gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: roveagent gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: roveagent gateway install")
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode RoveAgent often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.roveagent`` → ``ai.roveagent.gateway.plist`` (backward compatible).
    Profile ``~/.roveagent/profiles/coder`` → ``ai.roveagent.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.roveagent.gateway-{suffix}" if suffix else "ai.roveagent.gateway"
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    ``get_default_roveagent_root()``), NOT by globbing ``~/Library/LaunchAgents``:
    the LaunchAgents directory is shared per-user, so a sandboxed
    ``ROVEAGENT_HOME`` (tests, capture sandboxes, side-by-side installs) must
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.roveagent.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.roveagent.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # roveagent_constants cached from before this symbol existed. See
            # _reload_roveagent_constants() in roveagent_cli/managed_uv.py.
            from roveagent.clisupport.managed_uv import _reload_roveagent_constants

            venv_python_path = _reload_roveagent_constants().venv_python_path
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.roveagent/roveagent-agent  -> /home/alice/.roveagent/roveagent-agent
      /opt/roveagent                 -> /opt/roveagent  (kept as-is)
...
def _roveagent_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ROVEAGENT_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_roveagent_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.roveagent                    → /home/alice/.roveagent
      /root/.roveagent/profiles/coder     → /home/alice/.roveagent/profiles/coder
      /opt/custom-roveagent               → /opt/custom-roveagent  (kept as-is)
    """
    current_roveagent_raw = os.environ.get("ROVEAGENT_HOME", "").strip()
    current_roveagent = (
        Path(current_roveagent_raw).expanduser()
        if current_roveagent_raw
        else get_roveagent_home()
    )
    ...
    # different ROVEAGENT_HOME into the generated service unit.
    current_default = Path.home() / ".roveagent"
    target_default = Path(target_home_dir) / ".roveagent"

    # Default ~/.roveagent → remap to target user's default
    if current_roveagent == current_default:
        return str(target_default)

    # Profile or subdir of ~/.roveagent → preserve the relative structure
    try:
        relative = current_roveagent.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.roveagent) — keep as-is
        return str(current_roveagent)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    roveagent_node = roveagent_home / "node" / "bin"
    if _is_dir(roveagent_node):
        candidates.append(str(roveagent_node))
    roveagent_nm = roveagent_home / "node_modules" / ".bin"
    if _is_dir(roveagent_nm):
        candidates.append(str(roveagent_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m roveagent_cli.main``, so module
    ...
    dir, or a clone that ``roveagent update`` later relocates/removes — the path
    ...
    ``ROVEAGENT_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if ROVEAGENT_HOME
...
def _systemd_watchdog_seconds(roveagent_home: str | Path | None = None) -> int:
    """Resolve the managed-overlay-aware watchdog setting for a service home."""
    override_token = None
    reset_home_override = None
    if roveagent_home is not None:
        from roveagent.constants import (
            reset_roveagent_home_override,
            set_roveagent_home_override,
        )

        override_token = set_roveagent_home_override(roveagent_home)
        reset_home_override = reset_roveagent_home_override
    try:
...
def _systemd_watchdog_service_fields(
    roveagent_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(roveagent_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], roveagent_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The RoveAgent-managed Node under ``$ROVEAGENT_HOME/node`` goes first when it
    ...
    *roveagent_root* is the RoveAgent home the unit will run against. System units
    ...
    """
    from roveagent.constants import (
        roveagent_managed_node_tree_present,
        iter_roveagent_node_dirs,
    )

    managed_node_present = roveagent_managed_node_tree_present(roveagent_root)
    for directory in iter_roveagent_node_dirs(roveagent_root) if managed_node_present else ():
        entry = str(directory)
        try:
    ...
    # target RoveAgent home provides managed Node, consulting the invoker's PATH
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's RoveAgent home is known — probing here would stat the calling
    ...
    if system:
        ...
        roveagent_home = _roveagent_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            roveagent_home
        )
        profile_arg = _profile_arg_for_target_user(roveagent_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's ROVEAGENT_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(roveagent_home) if roveagent_home else _remap_path_for_user(working_dir, home_dir)
        venv_dir = _remap_path_for_user(venv_dir, home_dir)
        path_entries = [_remap_path_for_user(p, home_dir) for p in path_entries]
        # Managed Node for the TARGET user's tree (see the skip above): probe
        # the remapped roveagent_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(roveagent_home) if roveagent_home else None
        )
        path_entries = [
...
Group={group_name}
ExecStart={python_path} -m roveagent_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="ROVEAGENT_HOME={roveagent_home}"
Environment="ROVEAGENT_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
"""
    ...
    roveagent_home = str(get_roveagent_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        roveagent_home
    )
    profile_arg = _profile_arg(roveagent_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m roveagent_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="ROVEAGENT_HOME={roveagent_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    # ── ROVEAGENT_HOME sync chokepoint ──────────────────────────────────────
    ...
    # pinned ROVEAGENT_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo roveagent gateway … --system``, ROVEAGENT_HOME is often stripped
    # and falls back to ``/root/.roveagent``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / ROVEAGENT_HOME comparisons
    ...
    _sync_roveagent_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir ROVEAGENT_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved ROVEAGENT_HOME
    ...
    exported a throwaway ``ROVEAGENT_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``roveagent gateway restart``
    with ``ROVEAGENT_HOME=/tmp/roveagent-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="ROVEAGENT_HOME=..."`` lines and launchd
    ``<key>ROVEAGENT_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'ROVEAGENT_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>ROVEAGENT_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp ROVEAGENT_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: ROVEAGENT_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported ROVEAGENT_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single ROVEAGENT_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only ROVEAGENT_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp ROVEAGENT_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../roveagent_test`` path into
    # ``Environment="ROVEAGENT_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/roveagent_test"' in new_unit
        or "/roveagent_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # ROVEAGENT_HOME into the unit (manual E2E homes like /tmp/roveagent-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current RoveAgent install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo roveagent gateway uninstall --system")
    print_info("         roveagent gateway install")
    print_info("         roveagent gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("ROVEAGENT_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def _get_cron_drain_timeout() -> float:
    ...
    env_raw = os.getenv("ROVEAGENT_CRON_DRAIN_TIMEOUT")
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("ROVEAGENT_RESTART_AFTER_TURN_TIMEOUT")
    if env_raw is not None and str(env_raw).strip() != "":
...
def systemd_install(
    ...
    # Offer to remove legacy units (roveagent.service from pre-rename installs)
    # before installing the new roveagent-gateway.service. If both remain, they
    ...
    if has_legacy_roveagent_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_roveagent_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin ROVEAGENT_HOME; adopt it before any
    ...
    # ``sudo roveagent gateway install --system --force`` would bake /root/.roveagent
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}roveagent gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}roveagent gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}roveagent gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # ROVEAGENT_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    _sync_roveagent_home_from_systemd_unit(system=system)
    try:
        from roveagent.gateway.status import get_running_pid, write_planned_stop_marker
    ...
    try:
        ...
        print(
            ...
            "check `roveagent gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `roveagent gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}roveagent gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}roveagent gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}roveagent gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}roveagent gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.roveagent.gateway-{suffix}" if suffix else "ai.roveagent.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``roveagent gateway start/stop/status`` call).
...
#      background process (the `nohup roveagent gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_roveagent_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_roveagent_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m roveagent_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active ROVEAGENT_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "roveagent.clisupport.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``roveagent update`` sees the flag on the live grandchild argv and hands
    ...
    return [
        get_python_path(),
        "-m",
        "roveagent.clisupport.stderr_timestamp",
        "--error-log",
        str(error_log),
        "--",
        *inner,
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup roveagent gateway run --replace`
    ...
    working.
    """
    from roveagent.clisupport._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_roveagent_home() / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    out_path = log_dir / "roveagent.gateway.log"
    err_path = log_dir / "roveagent.gateway.error.log"
    try:
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup roveagent gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `roveagent gateway start`)
    ...
    nofile_block = ""
    try:
        from roveagent.clisupport.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>ROVEAGENT_HOME</key>
        <string>{roveagent_home}</string>
        <key>ROVEAGENT_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.roveagent/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See roveagent-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_roveagent_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current RoveAgent install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  roveagent gateway status             # Check status")
    from roveagent.constants import display_roveagent_home as _dhh
...
def launchd_stop():
    ...
    # `roveagent gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def _wait_for_gateway_exit(
    ...
    """
    import time
    from roveagent.gateway.status import get_process_start_time, get_running_pid
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `roveagent update`
...
def launchd_status(deep: bool = False):
    ...
    # RoveAgent PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the RoveAgent PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current RoveAgent install")
    else:
        print("⚠ Service definition is stale relative to the current RoveAgent install")
        print("  Run: roveagent gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: roveagent gateway stop")
            else:
    ...
    if deep:
        log_file = get_roveagent_home() / "logs" / "roveagent.gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/roveagent"
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``ROVEAGENT_S6_SUPERVISED_CHILD``.
...
def named_profile_served_by_running_multiplexer() -> bool:
    ...
    try:
        from roveagent.constants import get_default_roveagent_root
        default_root = get_default_roveagent_root()
    except Exception:
        return False

    try:
        from roveagent.gateway.status import _read_pid_record

        default_pid_path = default_root / "roveagent.gateway.pid"
        rec = _read_pid_record(default_pid_path)
        if not rec:
            return False
        from roveagent.gateway.status import _pid_exists, _pid_from_record
        pid = _pid_from_record(rec)
        if not pid or not _pid_exists(pid):
            return False

        from roveagent.gateway.config import _env_multiplex_profiles_override
        ...
        from roveagent.gateway.config import _normalize_multiplex_profile_allowlist
        from roveagent.clisupport.profiles import normalize_profile_name
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``roveagent gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``roveagent gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``roveagent gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other ROVEAGENT_HOME roots.
    """
    ...
    if pid is None:
        ...
        # ROVEAGENT_HOME (via _pid_record_belongs_to_current_profile). When no
        ...
        try:
            from roveagent.gateway.status import _read_pid_record, _pid_record_belongs_to_current_profile

            stale = _read_pid_record()
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                stale_home = stale.get("roveagent_home", "<unknown>")
                logger.warning(
                    "PID file belongs to another profile (roveagent_home=%s). "
    ...
    print("  Use 'roveagent gateway restart' to replace it,")
    print("  or 'roveagent gateway stop' first.")
    print("  Or use 'roveagent gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("ROVEAGENT_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the RoveAgent gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'roveagent' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/roveagent/docker/entrypoint.sh before the RoveAgent command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$ROVEAGENT_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set ROVEAGENT_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `roveagent gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set ROVEAGENT_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("ROVEAGENT_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `roveagent gateway restart` which already
    ...
    # `roveagent gateway start/restart` — leaving the gateway vulnerable to
    ...
    from roveagent.gateway.run import start_gateway

    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ RoveAgent Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # ROVEAGENT_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("ROVEAGENT_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``ROVEAGENT_GATEWAY_MAX_STARTS`` /
    # ``ROVEAGENT_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    # Set max_starts <= 0 to disable. Best-effort: a bookkeeping failure must
    # never block startup.
    try:
        import time as _time

        from roveagent.gateway.status import record_start_and_check_storm
        ...
        try:
            _env_starts = os.getenv("ROVEAGENT_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("ROVEAGENT_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``roveagent gateway run`` enters through this CLI wrapper, not through
        ...
        from roveagent.gateway.run import _exit_after_graceful_shutdown
    ...
    try:
        ...
        raise
    if not success:
        _exit_diag("roveagent.gateway.exit_nonzero")
        _hard_exit_after_gateway_teardown(1)
    _exit_diag("roveagent.gateway.exit_clean")
    _hard_exit_after_gateway_teardown(0)
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. roveagent) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where RoveAgent delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. RoveAgent connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: roveagent pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and RoveAgent will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``roveagent setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run roveagent under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.roveagent/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if entry is not None:
        ...
        if entry.is_connected is not None:
            try:
                from roveagent.gateway.config import PlatformConfig
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_roveagent_home() / "whatsapp" / "session" / "creds.json"
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        from roveagent.gateway.status import parse_active_agents
...
def _setup_standard_platform(platform: dict):
    ...
    from roveagent.clisupport.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from roveagent.clisupport.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'roveagent pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: roveagent pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'roveagent gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. RoveAgent will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. RoveAgent will store the returned account_id/token in ~/.roveagent/.env."
    )
    print_info(
    ...
    try:
        from roveagent.gateway.platforms.weixin import check_weixin_requirements, qr_login
    except Exception as exc:
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `roveagent gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_roveagent_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `roveagent pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if method_idx == 0:
        ...
        try:
            from roveagent.gateway.platforms.qqbot import qr_register
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `roveagent pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "RoveAgentAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``roveagent_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from roveagent.clisupport import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.roveagent/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.roveagent/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: roveagent gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: roveagent gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: roveagent gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo roveagent gateway install --system"
                        )
                    print_info("  Or run in foreground:  roveagent gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: roveagent gateway run")
                print_info(
                    "  For persistence:   tmux new -s roveagent 'roveagent gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: roveagent gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup roveagent gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'roveagent gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`roveagent_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from roveagent.clisupport.service_manager import (
    ...
    if profile is None:
        ...
        # ROVEAGENT_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``roveagent gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``roveagent gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``roveagent gateway run`` are unaffected.
      2. ``ROVEAGENT_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``roveagent gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``ROVEAGENT_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("ROVEAGENT_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    ...
    # `docker logs` and ${ROVEAGENT_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if ROVEAGENT_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or ROVEAGENT_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-roveagent service's
    # pattern in docker/s6-rc.d/main-roveagent/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: roveagent gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: roveagent gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s roveagent 'roveagent gateway run'"
                )
                print()
            ...
            print()
            print(
                "  roveagent gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s roveagent 'roveagent gateway run'         # persistent via tmux"
            )
            print(
                "  nohup roveagent gateway run > ~/.roveagent/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from roveagent.clisupport.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  roveagent profile create <name>     # creates the s6 service slot")
                print("  roveagent -p <name> gateway start   # bring it up via s6")
                print("  roveagent status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: roveagent gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: roveagent gateway run")
            sys.exit(1)
        ...
        from roveagent.tools.process_registry import _is_supervised_gateway_process
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `roveagent gateway uninstall` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: roveagent gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  roveagent profile delete <name>     # tears down the s6 service slot")
                print("  roveagent -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `roveagent -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: roveagent gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        # The supervised probe also PASSES a plain foreground `roveagent gateway run`
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `roveagent gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `roveagent gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: roveagent gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  roveagent gateway install")
                else:
                    print("To install as a service:")
                    print("  roveagent gateway install")
                    print("  sudo roveagent gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  roveagent gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup roveagent gateway run > ~/.roveagent/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s roveagent 'roveagent gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  roveagent gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  roveagent gateway install  # Install as user service")
                    print(
                        "  sudo roveagent gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy RoveAgent gateway unit files from
        # pre-rename installs (e.g. roveagent.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_roveagent_units(interactive=not yes, dry_run=dry_run)
