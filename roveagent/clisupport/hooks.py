"""roveagent hooks — inspect and manage shell-script hooks.

Usage::

    roveagent hooks list
    roveagent hooks test <event> [--for-tool X] [--payload-file F]
    roveagent hooks revoke <command>
    roveagent hooks doctor

Consent records live under ``~/.roveagent/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.roveagent/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``roveagent hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: roveagent hooks {list|test|revoke|doctor}")
        print("Run 'roveagent hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    from roveagent.core import outbound_webhooks, shell_hooks
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.roveagent/config.yaml.")
        print("See `roveagent hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.roveagent/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `roveagent hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `roveagent hooks test` and `roveagent hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (RoveAgent wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `roveagent hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `roveagent hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `roveagent hooks doctor`.")
