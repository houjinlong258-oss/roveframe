"""Image-authored deployment provenance for immutable RoveAgent runtimes.

The published image bakes ``/etc/roveagent/image-provenance.json`` outside both
``$ROVEAGENT_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/roveagent/image-provenance.json")
