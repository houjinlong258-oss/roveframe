...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      ...
      mirroring the ``roveagent model`` CLI picker. Adds network calls
    ...
    """
    from roveagent.clisupport.model_switch import list_authenticated_providers
    ...
    try:
        from roveagent.clisupport.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``roveagent_cli/moa_cmd.py``.
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    """
    try:
        from roveagent.core.models_dev import get_model_info
    except Exception:
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`roveagent_cli.providers.custom_provider_aliases` — lets the frontend
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from roveagent.clisupport.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `roveagent model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: RoveAgent' own Anthropic device flow
    ...
    try:
        ...
        roveagent_creds = read_roveagent_oauth_credentials() or {}
        if roveagent_creds.get("accessToken"):
            return True
    ...
    try:
        from roveagent.core.credential_pool import AUTH_TYPE_OAUTH
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for RoveAgent.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (RoveAgent device flow / Claude Code) are
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's RoveAgent overlay declares it keyless."""
    try:
        from roveagent.clisupport.providers import ROVEAGENT_OVERLAYS
        overlay = ROVEAGENT_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `roveagent model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from roveagent.clisupport.models import CANONICAL_PROVIDERS
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`roveagent_cli.model_switch.list_picker_providers`) so the
