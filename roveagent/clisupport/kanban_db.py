...
``<root>`` is the **shared RoveAgent root** (the parent of any active
...
``roveagent -p <profile>`` joins the same board as the dispatcher that
...
the profile's RoveAgent home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``ROVEAGENT_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``ROVEAGENT_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``roveagent kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.roveagent``. In Docker / custom
deployments where ``ROVEAGENT_HOME`` points outside ``~/.roveagent`` (e.g.
``/opt/roveagent``), ``<root>`` is ``ROVEAGENT_HOME``. Legacy env-var
overrides still work:

* ``ROVEAGENT_KANBAN_DB`` — pin the database file path directly.
* ``ROVEAGENT_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``ROVEAGENT_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``ROVEAGENT_KANBAN_DB``,
``ROVEAGENT_KANBAN_WORKSPACES_ROOT``, and ``ROVEAGENT_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/roveagent-kanban-v1-spec.pdf`` for the full
...
from roveagent.clisupport.sqlite_util import add_column_if_missing as _add_column_if_missing
from roveagent.toolsets import get_toolset_names
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``roveagent_constants.VALID_REASONING_EFFORTS`` plus
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    """
    try:
        from roveagent.core.delegation_context import is_delegated_child_process_context

        delegated = is_delegated_child_process_context()
    except Exception:
        delegated = bool(os.environ.get("ROVEAGENT_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active ROVEAGENT_HOME so dispatcher- and
    worker-side hooks both carry the right profile without the caller plumbing
    it through.
    """
    try:
        from roveagent.clisupport.lifecycle import invoke_hook
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from roveagent.clisupport.lifecycle import has_hook
...
# ``ROVEAGENT_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``ROVEAGENT_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("ROVEAGENT_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``ROVEAGENT_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("ROVEAGENT_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``ROVEAGENT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "ROVEAGENT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "roveagent_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``roveagent-agent``
...
def kanban_home() -> Path:
    """Return the shared RoveAgent root that anchors the kanban board.

    Resolution order:

    1. ``ROVEAGENT_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_roveagent_root()``, which already returns ``<root>``
       when ``ROVEAGENT_HOME`` is ``<root>/profiles/<name>``, and returns
       ``ROVEAGENT_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``ROVEAGENT_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("ROVEAGENT_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from roveagent.constants import get_default_roveagent_root
    return get_default_roveagent_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``roveagent kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``ROVEAGENT_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``roveagent kanban boards
    ...
    env = os.environ.get("ROVEAGENT_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``roveagent kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``ROVEAGENT_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("ROVEAGENT_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``ROVEAGENT_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("ROVEAGENT_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``ROVEAGENT_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("ROVEAGENT_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``roveagent kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``roveagent_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``ROVEAGENT_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (roveagent_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``ROVEAGENT_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("ROVEAGENT_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``roveagent_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from roveagent.clisupport.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``roveagent gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from roveagent.clisupport.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see roveagent_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from roveagent.clisupport.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources RoveAgent treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``roveagent kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``ROVEAGENT_KANBAN_DB`` env → ``ROVEAGENT_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `roveagent kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from roveagent.state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        from roveagent.state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See roveagent_state._WAL_INCOMPAT_MARKERS for detection logic.
                from roveagent.state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``roveagent kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``ROVEAGENT_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from roveagent.clisupport.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"roveagent_nested_{secrets.token_hex(8)}"
...
def create_task(
    ...
    *,
    ...
    each name to ``roveagent --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from roveagent.clisupport import projects_db as _pdb
    ...
    # `roveagent --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``roveagent kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `roveagent kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``roveagent kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``roveagent kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``roveagent kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``roveagent kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``roveagent kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``ROVEAGENT_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold RoveAgent' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside RoveAgent-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``roveagent kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the RoveAgent checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, RoveAgent treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, RoveAgent creates/reuses that linked worktree. With
      no ``workspace_path``, RoveAgent anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, RoveAgent uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``ROVEAGENT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a RoveAgent
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``roveagent kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``roveagent kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real RoveAgent profile.
    ...
    if not rows:
        return False
    try:
        from roveagent.clisupport.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because RoveAgent ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. RoveAgent
...
def _system_memory_sample() -> dict:
    ...
    """
    try:
        from roveagent.gateway.lifecycle_ledger import sample_memory
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``roveagent kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``ROVEAGENT_KANBAN_DB``
...
def _memory_pressure_level(sample: Optional[Mapping[str, Any]] = None) -> str:
    ...
    if not sample:
        return "unknown"
    try:
        from roveagent.gateway.memory_status import classify_pressure
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from roveagent.clisupport.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from roveagent.clisupport.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real RoveAgent profile.
        # `_default_spawn` invokes ``roveagent -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a RoveAgent
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `roveagent kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because RoveAgent bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from roveagent.clisupport.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_roveagent_argv() -> list[str]:
    """Return the interpreter-bound RoveAgent CLI invocation."""
    # ``roveagent_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``roveagent`` package — there is no
    # ``roveagent`` package to import.
    return [sys.executable, "-m", "roveagent.clisupport.main"]


def _absolute_roveagent_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved RoveAgent shim."""
...
def _roveagent_path_argv(path: str) -> list[str]:
    """Return argv for a resolved RoveAgent executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_roveagent_argv()
    return [_absolute_roveagent_path(path)]


def _resolve_roveagent_argv() -> list[str]:
    """Resolve the ``roveagent`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$ROVEAGENT_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("roveagent")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\roveagent.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m roveagent_cli.main`` — fallback for setups where
       RoveAgent is launched from a venv and the ``roveagent`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_roveagent_bin`` for the same reason. Kept
    local (not imported from gateway) because ``roveagent_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("ROVEAGENT_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _roveagent_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _roveagent_path_argv(resolved_env_bin)
        return _module_roveagent_argv()

    roveagent_bin = _safe_which_no_cwd("roveagent") if _IS_WINDOWS else shutil.which("roveagent")
    if roveagent_bin:
        return _roveagent_path_argv(roveagent_bin)
    return _module_roveagent_argv()
...
def _resolve_worker_cli_toolsets(roveagent_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``ROVEAGENT_KANBAN_TASK`` is set.
    """
    ...
    try:
        ...
        token = set_roveagent_home_override(roveagent_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for ROVEAGENT_HOME=%r (%s)",
            roveagent_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``roveagent -p <profile> chat -q ...`` subprocess.
    ...
    ``ROVEAGENT_KANBAN_DB`` / ``ROVEAGENT_KANBAN_BOARD`` / workspaces_root env
    ...
    from roveagent.gateway.session_context import _VAR_MAP
    for key in _VAR_MAP:
        env.pop(key, None)

    # Inject ROVEAGENT_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `roveagent -p <name>` the
    # _apply_profile_override() runs *before* roveagent_constants is imported.
    # If ROVEAGENT_HOME is absent from the child's env, get_roveagent_home() falls
    # back to Path.home() / ".roveagent" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from roveagent.clisupport.profiles import resolve_profile_env
    try:
        env["ROVEAGENT_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via ROVEAGENT_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # ROVEAGENT_HOME never had profiles created.
        pass
    if task.tenant:
        env["ROVEAGENT_TENANT"] = task.tenant
    env["ROVEAGENT_KANBAN_TASK"] = task.id
    env["ROVEAGENT_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `roveagent kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["ROVEAGENT_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["ROVEAGENT_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ROVEAGENT_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ROVEAGENT_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["ROVEAGENT_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ROVEAGENT_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`roveagent -p <name>`
    # rewrites ROVEAGENT_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_roveagent_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["ROVEAGENT_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["ROVEAGENT_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ROVEAGENT_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["ROVEAGENT_KANBAN_BOARD"] = resolved_board
    # ROVEAGENT_PROFILE is the author the kanban_comment tool defaults to.
    # `roveagent -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["ROVEAGENT_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited ROVEAGENT_TUI=1
    ...
    # older roveagent builds on PATH that predate the flag's precedence.
    env.pop("ROVEAGENT_TUI", None)

    cmd = [
        *_resolve_roveagent_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped ROVEAGENT_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("ROVEAGENT_HOME"))
    if worker_toolsets:
    ...
    # `roveagent kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`roveagent` executable not found on PATH. "
            "Install RoveAgent Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``roveagent kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``roveagent kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `roveagent kanban dispatch` do (OOF-30): explicit
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled ROVEAGENT_PROFILE values like "roveagent-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default RoveAgent root exists

    Reads profile paths directly so this module has no import dependency on
    ``roveagent_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from roveagent.constants import get_default_roveagent_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``roveagent kanban assignees`` for the terminal.
