...
Backs ``roveagent kanban export|import``, the matching ``/boards/{slug}/export``
...
ARCHIVE_FORMAT = "roveagent-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "roveagent_version": str(roveagent_version),
            "includes": {
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError(
            "archive is not a RoveAgent kanban board export (no manifest.json)"
        )
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a RoveAgent kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this RoveAgent "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update RoveAgent and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "roveagent_version": manifest.get("roveagent_version"),
        },
        "counts": counts,
