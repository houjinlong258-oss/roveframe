"""RoveAgent lifecycle dispatch for first-party observers and plugins."""
...
def invoke_hook(hook_name: str, **kwargs: Any) -> List[Any]:
    ...
    try:
        from roveagent.clisupport.observability import observe_lifecycle

        observe_lifecycle(hook_name, **kwargs)
    except Exception:
        logger.warning("Built-in observability hook failed", exc_info=True)

    from roveagent.clisupport import plugins
...
def has_hook(hook_name: str) -> bool:
    ...
    try:
        from roveagent.clisupport.observability import handles_hook
