"""Install and remove the Linux desktop entry (``roveagent.desktop``).

``roveagent desktop`` builds and launches the Electron app. On Linux, a
...
``roveagent uninstall --gui`` removes the entry again.

Two values must be absolute for the entry to work:

  - ``Exec`` — the launcher runs without shell ``PATH`` customizations, so
    a bare ``roveagent desktop`` fails when roveagent lives in ``~/.local/bin``
...
DESKTOP_ENTRY_NAME = "roveagent.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``roveagent.desktop`` entry lives."""
...
def _can_import_roveagent_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``roveagent_cli.main`` unaided.
    ...
    try:
        result = subprocess.run(
            [key, "-I", "-c", "import roveagent_cli.main"],
...
def _running_interpreter_fallback() -> str:
    ...
    The RUNNING interpreter by definition has ``roveagent_cli`` importable
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``roveagent desktop``.

    Prefer the real ``roveagent`` executable (argv[0] or PATH). When RoveAgent
    ...
    bytes. ``resolve_roveagent_bin()`` prefers ``sys.argv[0]``, which differs
    ...
    """
    from roveagent.clisupport.relaunch import resolve_roveagent_bin

    bin_path = _resolve_roveagent_bin_for_desktop_entry(
        resolve_roveagent_bin, checkout_root=project_root
    )
    interpreter = _running_interpreter()
    if not _can_import_roveagent_cli(Path(interpreter)):
        # The candidate interpreter cannot actually import roveagent_cli.main
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `roveagent` script with
            ...
            # sys.executable is the interpreter actually running RoveAgent (the
...
def _resolve_roveagent_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`roveagent_cli.relaunch.resolve_roveagent_bin` with one
    desktop-entry-specific rule: an ``argv[0]`` that points inside this
    checkout is a launch-context artifact (the repo ``roveagent`` script the
    ...
    if resolve_fn is None:
        from roveagent.clisupport.relaunch import resolve_roveagent_bin as resolve_fn
    ...
    def _inside_checkout(candidate: str) -> bool:
        try:
            ...
            return False
        # The repo `roveagent` script and anything else shipped in the tree is
        ...
        for root in {checkout_root, resolved_root}:
            if root is not None and (path == root or root in path.parents):
                return True
        # The `python -m roveagent_cli.main` relaunch context surfaces the
    ...
    # interpreter is an external launcher (e.g. /opt/.../bin/roveagent from
    ...
    if rerouted is None and primary:
        # argv[0] was checkout-internal AND PATH had no `roveagent` — common
...
def _wrapper_targets_checkout(wrapper: Path, checkout_root: Path) -> bool:
    ...
    ``<checkout>/venv/bin/python <checkout>/roveagent``; a venv console
    ...
    for root in roots:
        ...
        # <root>/roveagent …): a path-continuation boundary is also a match.
        if root + "/" in text:
            return True
    return False
...
def _known_wrapper_candidates():
    ...
    (``$PREFIX/bin``). The wrapper is always named ``roveagent``.
    """
    candidates = []
    home = Path.home()
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "roveagent")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/roveagent"))
    candidates.append(home / ".local" / "bin" / "roveagent")
    return candidates


def _project_root() -> Path:
    """This file lives at ``<checkout>/roveagent_cli/linux_desktop_entry.py``.
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see RoveAgent' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=RoveAgent\n"
        "GenericName=RoveAgent Desktop\n"
        "Comment=Launch RoveAgent Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=RoveAgent\n"
    )
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    ...
    The freedesktop icon lookup finds an installed ``apps/roveagent.png``
    by the unqualified name ``roveagent``, so the entry can reference the
    ...
    try:
        ...
        dest = _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "roveagent.png"
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the RoveAgent desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "roveagent"
    if icon.is_file() and _install_icon_to_hicolor(icon):
        icon_value = "roveagent"
