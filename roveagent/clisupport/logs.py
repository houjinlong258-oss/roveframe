"""``roveagent logs`` — view and filter RoveAgent log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.roveagent/logs/``.

Usage examples::

    roveagent logs                    # last 50 lines of agent.log
    roveagent logs -f                 # follow agent.log in real time
    roveagent logs errors             # last 50 lines of errors.log
    roveagent logs gateway -n 100    # last 100 lines of gateway.log
    roveagent logs gui -f            # follow gui.log (dashboard/pty/ws)
    roveagent logs desktop -f        # follow desktop.log (Electron app boot/backend)
    roveagent logs --level WARNING    # only WARNING+ lines
    roveagent logs --session abc123   # filter by session ID substring
    roveagent logs --component tools  # only tool-related lines
    roveagent logs --since 1h         # lines from the last hour
    roveagent logs --since 30m -f     # follow, starting 30 min ago
"""
...
LOG_FILES = {
    "agent": "roveagent.core.log",
    "errors": "errors.log",
    "gateway": "roveagent.gateway.log",
    "gui": "gui.log",
    "desktop": "desktop.log",
...
def tail_log(
    ...
    *,
    ...
    log_path = get_roveagent_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when RoveAgent runs — try 'roveagent chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_roveagent_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_roveagent_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_roveagent_home()}/logs/")
        return

    print(f"Log files in {display_roveagent_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'roveagent chat' to generate logs)")
