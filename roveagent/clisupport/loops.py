...
Design notes / invariants (same contract as ``roveagent_cli/goals.py``):
...
- This module has zero hard dependency on ``cli.RoveAgentCLI``, the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    """
    try:
        from roveagent.clisupport.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from roveagent.clisupport.goals import _warn_dropped_write
...
def goal_blocks_loop_tick(session_id: str) -> bool:
    ...
    """
    try:
        from roveagent.clisupport.goals import GoalManager
