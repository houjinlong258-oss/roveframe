"""RoveAgent-managed uv and Python runtime repair.

RoveAgent owns its own uv binary at ``$ROVEAGENT_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$ROVEAGENT_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every RoveAgent
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.roveagent-runtime/python``. A
...
from roveagent.clisupport.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".roveagent-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.nousresearch.roveagent.managed-python"
...
def managed_uv_path() -> Path:
    """Return the path where RoveAgent keeps *its* uv binary.

    ``$ROVEAGENT_HOME/bin/uv`` on POSIX, ``$ROVEAGENT_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for RoveAgent-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever RoveAgent provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: RoveAgent keeps databases "
            "out of WAL mode on this SQLite build. The next `roveagent update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``roveagent update`` runs
    the call site from the *old*, already-imported ``roveagent_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``roveagent update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under ROVEAGENT_HOME
    caches the last successful self-update time.
    """
    try:
        from roveagent.constants import get_roveagent_home

        stamp = get_roveagent_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``roveagent update`` so the managed copy stays current.
...
def _reload_roveagent_constants():
    """Re-execute ``roveagent_constants`` from disk and return the fresh module.

    ``roveagent update`` imports ``roveagent_constants`` from the OLD checkout,
    ...
    contains it, which is what made this read as a contradiction:
        ...
        cannot import name 'venv_python_path' from 'roveagent_constants'
        (~/.roveagent/roveagent-agent/roveagent_constants.py)
    ...
    """
    from roveagent import constants as roveagent_constants

    return importlib.reload(roveagent_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the RoveAgent generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `roveagent update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("roveagent.clisupport.main")
    ...
    if holders:
        ...
        return True, f"other RoveAgent processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the RoveAgent-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``roveagent update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``roveagent update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$ROVEAGENT_HOME/bin/`` instead of ``~/.local/bin/``.
    """
