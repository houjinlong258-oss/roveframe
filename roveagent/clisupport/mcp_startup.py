...
def _has_configured_mcp_servers() -> bool:
    ...
    try:
        ...
        if isinstance(mcp_servers, dict) and len(mcp_servers) > 0:
            return True
        from roveagent.clisupport.agent_plugins import has_enabled_agent_plugin_mcp
...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local ROVEAGENT_HOME override (profile
        ...
        def _discover() -> None:
            token = None
            try:
                from roveagent.constants import set_roveagent_home_override
            ...
            try:
                ...
                if token is not None:
                    try:
                        from roveagent.constants import reset_roveagent_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``roveagent -z "..."`` / ``-q``), the larger
    ...
    try:
        from roveagent.clisupport.config import load_config, DEFAULT_CONFIG
...
def _discover_mcp_tools_without_interactive_oauth() -> None:
    ...
    try:
        from roveagent.tools.mcp_oauth import suppress_interactive_oauth
    except Exception:
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``roveagent dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``roveagent -z``) can construct
