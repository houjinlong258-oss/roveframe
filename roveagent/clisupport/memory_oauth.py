...
def _resolve_flow(provider: str):
    ...
    try:
        return importlib.import_module(f"roveagent.plugins.memory.{provider}.oauth_flow")
    except ImportError:
...
def _scope_to_profile(profile: Optional[str]):
    ...
    token = set_roveagent_home_override(str(profiles_mod.get_profile_dir(requested)))
    try:
        yield
    finally:
