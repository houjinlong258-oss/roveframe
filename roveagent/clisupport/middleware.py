"""RoveAgent middleware contract helpers.
...
OBSERVER_SCHEMA_VERSION = "roveagent.observer.v1"
MIDDLEWARE_SCHEMA_VERSION = "roveagent.middleware.v1"
...
def apply_llm_request_middleware(
    ...
    **context: Any,
    ...
    provider kwargs before RoveAgent sends them.
    """
...
def _invoke_middleware(kind: str, **kwargs: Any) -> List[Any]:
    from roveagent.clisupport.plugins import invoke_middleware

    return invoke_middleware(kind, **middleware_payload(**kwargs))


def _has_middleware(kind: str) -> bool:
    from roveagent.clisupport.plugins import has_middleware
