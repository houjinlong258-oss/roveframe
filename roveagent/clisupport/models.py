...
Add, remove, or reorder entries here — both `roveagent setup` and
`roveagent` provider-selection will pick up the change automatically.
"""
...
from roveagent.clisupport.urllib_security import open_credentialed_url, url_origin
from roveagent.utils import atomic_json_write, base_url_host_matches

logger = logging.getLogger(__name__)

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_ROVEAGENT_USER_AGENT = f"roveagent-cli/{_ROVEAGENT_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from roveagent.clisupport.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `roveagent model`
    flow without maintaining a separate static list.
    """
    from roveagent.clisupport.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $ROVEAGENT_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append RoveAgent-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $ROVEAGENT_HOME/models_dev_cache.json directly (no network). Falls
    back to ``_XAI_STATIC_FALLBACK`` when the cache is empty or unreadable.
    """
    try:
        from roveagent.core.models_dev import _load_disk_cache
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see roveagent_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older RoveAgent that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older RoveAgent that doesn't ship
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_roveagent_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$ROVEAGENT_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  roveagent model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `roveagent model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `roveagent model` TUI
...
try:
    from roveagent.providers import list_providers as _list_providers_for_canonical
...
#
# Some vendors expose several RoveAgent provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `roveagent model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``roveagent model``, the
...
# In-repo fallback for the model RoveAgent silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from roveagent.clisupport.model_catalog import get_default_model_from_cache
...
# ``roveagent model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in roveagent_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``roveagent auth add openai-codex`` without
    ``roveagent model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``roveagent model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    roveagent-agent is tool-calling-first — every provider path assumes the model
...
# copy, `roveagent -p`, a cron job, or a freshly booted gateway answers
...
def _reasoning_caps_disk_path() -> Path:
    from roveagent.constants import get_roveagent_home
    return get_roveagent_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _ROVEAGENT_USER_AGENT}
    try:
...
from roveagent.core.reasoning_effort import EFFORT_LADDER as _REASONING_EFFORT_ORDER
from roveagent.core.reasoning_effort import clamp_effort as _clamp_effort
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from roveagent.clisupport.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — roveagent-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model RoveAgent lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from roveagent.clisupport.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from roveagent.constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return roveagent-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; roveagent's picker expects
...
def _fireworks_pricing_from_models_dev(
    *,
    ...
    result: dict[str, dict[str, str]] = {}
    try:
        from roveagent.core.models_dev import _get_provider_models
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from roveagent.clisupport.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``roveagent model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from roveagent.clisupport.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:roveagent-3                           →  ("nous", "roveagent-3")
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _ROVEAGENT_USER_AGENT}
...
def _resolve_static_model_alias(
    ...
    try:
        from roveagent.clisupport.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to RoveAgent' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``roveagent_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether RoveAgent should expose the /fast toggle for this model."""
    from roveagent.core.model_metadata import is_grok_46_family
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``roveagent auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.roveagent/.env``.
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from roveagent.clisupport.model_search import model_alias_canonical
...
def _merge_with_models_dev(provider: str, curated: list[str]) -> list[str]:
    ...
    """
    try:
        from roveagent.core.models_dev import list_agentic_models
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a RoveAgent release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from roveagent.clisupport.codex_models import get_codex_model_ids
        ...
        # a RoveAgent release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from roveagent.clisupport.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a RoveAgent release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``roveagent model`` shows.
            from roveagent.clisupport.providers import is_official_openai_host
    ...
    if normalized == "bedrock":
        try:
            from roveagent.core.bedrock_adapter import bedrock_model_ids_or_none
...
#
# Cache strategy:
#   - One JSON file at $ROVEAGENT_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from roveagent.constants import get_roveagent_home
    return get_roveagent_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$ROVEAGENT_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `roveagent config set
    ...
    if provider == "ollama":
        ...
        provider_cfg = _get_provider_config_dict("ollama")
        parts.append(
            "roveagent.providers.ollama.base_url="
            f"{provider_cfg.get('base_url', '') or provider_cfg.get('api', '') or provider_cfg.get('url', '')}"
        )
        parts.append(f"roveagent.providers.ollama.api_key={provider_cfg.get('api_key', '')}")
        key_env = provider_cfg.get("key_env") or provider_cfg.get("api_key_env") or ""
        parts.append(f"roveagent.providers.ollama.key_env={key_env}")
        if key_env:
        ...
        parts.append(
            "roveagent.providers.ollama.extra_headers="
...
def _resolve_anthropic_pool_catalog_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from roveagent.core.credential_pool import AUTH_TYPE_API_KEY
        from roveagent.clisupport.auth import read_credential_pool
...
def _fetch_anthropic_models(
    ...
    *,
    ...
    """
    try:
        from roveagent.core.anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from roveagent.core.anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _ROVEAGENT_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: RoveAgent' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through RoveAgent' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from roveagent.clisupport import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
        ...
        "User-Agent": f"RoveAgentAgent/{_v}",
    }
...
def _fetch_opencode_free_models(
    ...
    from roveagent.clisupport.urllib_security import open_credentialed_url
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _ROVEAGENT_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"roveagent-agent/{_ROVEAGENT_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=roveagent"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from roveagent.constants import get_roveagent_home
    return get_roveagent_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `roveagent moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "RoveAgent will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"RoveAgent will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a RoveAgent
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so RoveAgent cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models RoveAgent actually ships.  (#46850)
            #
            ...
            # `roveagent chat` already accepts these models via
    ...
    if normalized == "bedrock":
        try:
            from roveagent.core.bedrock_adapter import discover_bedrock_models, resolve_bedrock_runtime_region
