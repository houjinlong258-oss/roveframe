...
from roveagent.clisupport.config import get_env_value, load_config
...
# coverage category (roveagent_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `roveagent tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``roveagent status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # RoveAgent-managed Node dirs (Windows installer / POSIX $ROVEAGENT_HOME/node)
        ...
        managed_path = with_roveagent_node_path().get("PATH", "")
        if managed_path:
...
def _local_browser_runnable() -> bool:
    ...
    if not _has_agent_browser():
        return False
    try:
        from roveagent.tools.browser_tool import _chromium_installed, _using_lightpanda_engine
    except Exception:
        # If the runtime probe can't be imported, fall back to binary presence
        # (prior behaviour) rather than crashing the setup/status surface.
        return True
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("ROVEAGENT_LOCAL_STT_COMMAND"):
        return True
    try:
        from roveagent.tools.transcription_tools import _HAS_FASTER_WHISPER
    ...
    *,
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("ROVEAGENT_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("ROVEAGENT_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from roveagent.clisupport.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`roveagent tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``roveagent tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``roveagent model``'s Nous
    login, this:
