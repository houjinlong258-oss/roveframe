"""Durable aggregation and local export for RoveAgent shared metrics."""
...
from roveagent.clisupport.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "roveagent.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_roveagent_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "roveagent_version": row["roveagent_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across RoveAgent processes.
            with write_txn(connection):
    ...
    def _create_counter_aggregates_table(connection: sqlite3.Connection) -> None:
        connection.execute(
            ...
            CREATE TABLE IF NOT EXISTS counter_aggregates (
                ...
                roveagent_version TEXT NOT NULL,
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "roveagent_version": period_row["roveagent_version"],
