"""Bounded product contract for the first RoveAgent shared-metrics slice."""
...
from roveagent.core.relay_runtime import (
    LOGICAL_LLM_SCOPE,
...
SCHEMA_KEY = "roveagent.metrics.schema_version"
SCHEMA_VERSION = "roveagent.metrics.event.v2"
MODEL_CALL_SCOPE = "roveagent.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "roveagent.task_run"
TOOL_CALL_SCOPE = "roveagent.tool_call"
CLIENT_ACTIVE_MARK = "roveagent.client.active"
TOOL_APPROVAL_MARK = "roveagent.tool_approval"
SKILL_LIFECYCLE_MARK = "roveagent.skill.lifecycle"
SKILL_LOAD_MARK = "roveagent.skill.load"
SUBSCRIBER_NAME = "roveagent.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "roveagent.client.active"
LEGACY_MODEL_CALL_METRIC = "roveagent.model_call.count"
MODEL_ROUTE_METRIC = "roveagent.model_route.count"
TASK_STARTED_METRIC = "roveagent.task_run.started"
TASK_FINISHED_METRIC = "roveagent.task_run.finished"
TOOL_CALL_METRIC = "roveagent.tool_call.count"
TOOL_APPROVAL_METRIC = "roveagent.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "roveagent.skill.lifecycle.count"
SKILL_LOAD_METRIC = "roveagent.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted RoveAgent installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(roveagent_version or "").strip()
    ...
    return {
        ...
        "roveagent_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("roveagent_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its RoveAgent logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("roveagent.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from roveagent.clisupport.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map RoveAgent terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map RoveAgent registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "roveagent-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal RoveAgent tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to RoveAgent."""
