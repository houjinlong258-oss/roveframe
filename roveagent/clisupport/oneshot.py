...
configured for "cli" in `roveagent tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (ROVEAGENT_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `roveagent chat`:
...
from roveagent.gateway.session_context import declare_stateless_channel
...
def _build_preloaded_skills_prompt(skills: object = None) -> str | None:
    ...
    from roveagent.core.skill_commands import build_preloaded_skills_prompt
    ...
    if missing_skills:
        ...
        if loaded_skills:
            logging.warning(
                ...
                "List available skills with `roveagent skills list`.",
                missing_display,
                ", ".join(loaded_skills),
            )
        else:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"roveagent -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "roveagent -z: --toolsets all enables every toolset; "
    ...
    if unresolved:
        try:
            ...
            from roveagent.clisupport.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"roveagent -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "roveagent -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "roveagent -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to ROVEAGENT_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("ROVEAGENT_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "roveagent -z: --provider requires --model (or ROVEAGENT_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"roveagent -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("roveagent -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``roveagent -z`` / oneshot mode.

    Oneshot bypasses ``RoveAgentCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when roveagent is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from roveagent.clisupport.config import load_config
    from roveagent.clisupport.models import detect_provider_for_model
    ...
    env_model = os.getenv("ROVEAGENT_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from roveagent.clisupport import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("ROVEAGENT_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # RoveAgentCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from roveagent.clisupport.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                ROVEAGENT_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via ROVEAGENT_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via ROVEAGENT_YOLO_MODE=1
