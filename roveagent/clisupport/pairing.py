...
Usage:
    roveagent pairing list              # Show all pending + approved users
    roveagent pairing approve <platform> <request-id|code>  # Approve a pairing request
    roveagent pairing revoke <platform> <user_id> # Revoke user access
    roveagent pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle roveagent pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: roveagent pairing {list|approve|revoke|clear-pending}")
        print("Run 'roveagent pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: roveagent pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.roveagent/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'roveagent pairing list' to see pending requests.\n")
