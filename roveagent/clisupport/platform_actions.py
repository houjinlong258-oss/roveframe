...
"roveagent.gateway.platform_actions")`` at call time. The capability maps to the
...
CAPABILITY_ID = "roveagent.gateway.platform_actions"
...
class PlatformActions:
    ...
    def _capability_granted(self) -> bool:
        try:
            from roveagent.clisupport.plugin_capabilities import plugin_capability_granted
    ...
    def _gate(self, platform: str, **required: Any):
        ...
        if not self._capability_granted():
            return None, _err(
                ...
                f"roveagent.plugins.entries.{self._plugin_id}.allow_platform_actions)",
            )
