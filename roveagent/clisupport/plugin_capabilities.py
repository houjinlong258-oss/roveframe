...
this. Capabilities govern the *host API surfaces* RoveAgent hands out (which
...
until the user re-consents (``roveagent plugins update`` surfaces the diff).
...
CAPABILITY_REGISTRY: Dict[str, CapabilitySpec] = {
    ...
    for spec in (
        ...
        CapabilitySpec(
            id="roveagent.gateway.platform_actions",
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer RoveAgent may declare ids this build doesn't know; they can
