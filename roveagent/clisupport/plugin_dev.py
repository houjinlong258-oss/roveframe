"""Runtime-backed validation behind ``roveagent plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary ROVEAGENT_HOME
    ...
    try:
        ...
        # removes it instead of stranding a roveagent-plugin-doctor-* directory
        # until interpreter GC (or never, on a hard exit).
        temporary_home = stack.enter_context(
            tempfile.TemporaryDirectory(prefix="roveagent-plugin-doctor-")
        )
        ...
        stack.enter_context(
            patch.dict(
                os.environ,
                {
                    ...
                    "ROVEAGENT_BUNDLED_PLUGINS": str(bundled),
                    "ROVEAGENT_ENABLE_PROJECT_PLUGINS": "0",
                },
                clear=False,
            )
        )
    ...
    from roveagent.clisupport.plugins import PluginManager
    ...
    modules_before = {
        ...
        if name == "roveagent_plugins" or name.startswith("roveagent_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"RoveAgent discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "roveagent_plugins" or name.startswith("roveagent_plugins."))
            ):
...
def _holds_plugin(path: Path) -> bool:
    ...
    confusing error: ``roveagent plugins doctor`` with no argument defaults to
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        user_root = get_roveagent_home() / "plugins"
        ...
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".roveagent" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from roveagent.clisupport.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this RoveAgent supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — RoveAgent never auto-installs plugin dependencies; "
    ...
    if schema:
        from roveagent.clisupport.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through RoveAgent' real scanner and registration path."""
    try:
