"""
RoveAgent Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with roveagent-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.roveagent/plugins/<name>/``
3. **Project plugins** – ``./.roveagent/plugins/<name>/`` (opt-in via
   ``ROVEAGENT_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``roveagent_agent.plugins``
   entry-point group.
...
from roveagent.registration_lifecycle import replacement_coordinator
from roveagent.utils import env_var_enabled, fast_safe_load
from roveagent.clisupport.config import cfg_get, load_config_readonly
from roveagent.clisupport.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from roveagent.clisupport.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``ROVEAGENT_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("ROVEAGENT_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``ROVEAGENT_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.roveagent/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ROVEAGENT_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ROVEAGENT_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other RoveAgent loggers.
    """
    ...
    logger.debug(
        "ROVEAGENT_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. RoveAgent' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by roveagent_cli.kanban_db when a task
    ...
    # separate `roveagent -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `roveagent kanban dispatch`),
    ...
    # ``roveagent kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: roveagent_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINTS_GROUP = "roveagent_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "roveagent_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``roveagent_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- roveagent-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- roveagent-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- roveagent-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``roveagent:<event>``.
ROVEAGENT_EVENT_NAMESPACE = "roveagent"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable RoveAgent home."""
    token = set_roveagent_home_override(home)
    try:
        yield
    finally:
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "roveagent", "depends",
}

# Highest manifest schema version this RoveAgent understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this RoveAgent "
    ...
    for skey, spec in schema.items():
        ...
        if not present:
            if spec.get("required") and "default" not in spec:
                warnings.append(
                    f"roveagent.plugins.entries.{plugin_id}.settings.{skey} is required "
        ...
        if expected is not None:
            ...
            if not ok:
                warnings.append(
                    f"roveagent.plugins.entries.{plugin_id}.settings.{skey} should be "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `roveagent plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.roveagent/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``roveagent plugins list``. For a flat plugin at
    ...
    # ``roveagent_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — RoveAgent never
    ...
    # (``roveagent plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full RoveAgent paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_roveagent_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from roveagent.clisupport import config as config_mod
        ...
        # threads. Other RoveAgent config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from roveagent.clisupport.platform_actions import PlatformActions
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from roveagent.core.plugin_llm import PluginLlm
    ...
    def subagent_lifecycle(self) -> Any:
        ...
        if self._subagent_lifecycle is None:
            from roveagent.core.subagent_lifecycle import (
    ...
    def profile_name(self) -> str:
        """Return the active RoveAgent profile name (e.g. ``"default"``).

        Derived from ``ROVEAGENT_HOME`` via
        :func:`roveagent_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.roveagent/profiles/<name>``, or ``"custom"`` when
        ``ROVEAGENT_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_tool(
        ...
        if override and not self._tool_override_allowed(name):
            ...
            raise PluginToolOverrideError(
                ...
                f"{name!r}. Set "
                f"roveagent.plugins.entries.{plugin_id}.allow_tool_override: true "
    ...
    def has_capability(self, capability: str) -> bool:
        ...
        if source == "bundled" and capability == "roveagent.tools.override":
            return True
        plugin_id = self.manifest.key or self.manifest.name
    ...
    def call_mcp(
        ...
        if server not in allowlist:
            raise PermissionError(
                ...
                f"roveagent.plugins.entries.{plugin_id}.mcp_allowlist in config.yaml "
        ...
        from roveagent.tools.mcp_tool import _make_tool_handler
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with RoveAgent core) are trusted by default —
        ...
        return plugin_capability_granted(plugin_id, "roveagent.tools.override", config=cfg)
        ...
        *,
        ...
        if not self._gateway_injection_allowed():
            ...
            logger.warning(
                ...
                "roveagent.plugins.entries.%s.allow_gateway_injection: true to allow it",
                plugin_id,
                plugin_id,
            )
            return False
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``roveagent honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``roveagent <subcommand>``
        ...
        try:
            from roveagent.clisupport.commands import resolve_command
    ...
    def register_context_engine(self, engine) -> Optional[PluginRegistration]:
        ...
        from roveagent.core.context_engine import ContextEngine
    ...
    def register_context_reference(self, provider) -> None:
        ...
        """
        from roveagent.core.context_references import (
    ...
    def register_memory_provider(self, provider) -> None:
        ...
        """
        from roveagent.core.memory_provider import MemoryProvider
    ...
    def register_image_gen_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        tool calls.
        """
        from roveagent.core.image_gen_provider import ImageGenProvider
        from roveagent.core.image_gen_registry import (
            register_provider,
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`roveagent_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from roveagent.clisupport.dashboard_auth import DashboardAuthProvider
        from roveagent.clisupport.dashboard_auth.registry import (
    ...
    def register_video_gen_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        tool calls.
        """
        from roveagent.core.video_gen_provider import VideoGenProvider
        from roveagent.core.video_gen_registry import (
    ...
    def register_web_search_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        tool calls.
        """
        from roveagent.core.web_search_provider import WebSearchProvider
    ...
    def register_browser_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        """
        from roveagent.core.browser_provider import BrowserProvider
        from roveagent.core.browser_registry import (
    ...
    def register_terminal_environment_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        """
        from roveagent.core.terminal_env_provider import TerminalEnvironmentProvider
        from roveagent.core.terminal_env_registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_roveagent_dotenv()`` startup — after
        ``~/.roveagent/.env`` loads, before RoveAgent reads credentials — when
        ...
        NOTE ON TIMING: ``load_roveagent_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_roveagent_dotenv``) so the first process sees them (#64177).
        ...
        """
        from roveagent.core.secret_sources.base import SecretSource
        from roveagent.core.secret_sources.registry import (
            register_source,
            restore_registration,
            snapshot_registration,
        )
    ...
    def register_tts_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        """
        from roveagent.core.tts_provider import TTSProvider
        from roveagent.core.tts_registry import (
    ...
    def register_transcription_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        **but only when**:
        ...
        backends).
        """
        from roveagent.core.transcription_provider import TranscriptionProvider
        from roveagent.core.transcription_registry import (
    ...
    def register_platform(
        ...
        **entry_kwargs: Any,
        ...
        """
        from roveagent.gateway.platform_registry import platform_registry, PlatformEntry
    ...
    def register_slack_action_handler(
        ...
        RoveAgent' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        RoveAgent' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``roveagent model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: roveagent_cli.main imports plugins indirectly
        from roveagent.clisupport.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_redaction_patterns(self, patterns) -> int:
        ...
        """
        from roveagent.core.redact import register_redaction_patterns as _register
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``roveagent:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``roveagent:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, ROVEAGENT_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{ROVEAGENT_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``roveagent:<event>``
    ...
    def register_skill(
        ...
        ``~/.roveagent/skills/`` tree and is **not** listed in the system
        ...
        """
        from roveagent.core.skill_utils import _NAMESPACE_RE
...
def _resolve_hook_callback_timeout() -> float:
    ...
    try:
        ...
        logger.warning(
            "roveagent.plugins.hook_callback_timeout is not a number; using default %gs",
    ...
    if timeout < 0:
        logger.warning(
            "roveagent.plugins.hook_callback_timeout=%g is negative; using default %gs",
            timeout,
            _HOOK_CALLBACK_TIMEOUT_SECS,
        )
    ...
    if timeout > _MAX_HOOK_CALLBACK_TIMEOUT_SECS:
        logger.warning(
            "roveagent.plugins.hook_callback_timeout=%g exceeds max %gs; clamping",
            timeout,
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or roveagent_home_key()
        ...
        # by resolved roveagent home). The ledger is therefore keyed per manager
        # — i.e. per (roveagent_home, plugin_id) — and every release/restore
        ...
        # contributed, so `roveagent plugins list` still attributes them once the
        # full plugin loads.
    ...
    def _unload_scoped(
        ...
        if unload_all:
            ...
            if preledger_tools:
                try:
                    from roveagent.tools.registry import registry as tool_registry
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if force:
                ...
                self.unload()
            if env_var_enabled("ROVEAGENT_SAFE_MODE"):
                logger.info("ROVEAGENT_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_roveagent_dotenv() already ran at import time. Re-pull so the
    ...
    def _re_register_shell_hooks_after_force(self) -> None:
        ...
        try:
            from roveagent.core.shell_hooks import re_register_config_hooks
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        """
        try:
            from roveagent.core.secret_sources.registry import list_plugin_sources
            from roveagent.clisupport.env_loader import load_roveagent_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed RoveAgent plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the RoveAgent core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by RoveAgent core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed RoveAgent Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with roveagent and must
            ...
            # all ~20 of them added several seconds to every `roveagent`
            # invocation — including plain `roveagent chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform RoveAgent ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `roveagent plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from roveagent.clisupport.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_roveagent_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_roveagent_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.roveagent/plugins/)
        user_dir = get_roveagent_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.roveagent/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("ROVEAGENT_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".roveagent" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ROVEAGENT_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("ROVEAGENT_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from roveagent.clisupport.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from roveagent.clisupport.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every RoveAgent process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``roveagent_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``roveagent plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`roveagent plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``roveagent tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        if plugin_dir is None or not (plugin_dir / "roveagent.tools.py").is_file():
        ...
        try:
            ...
            # `roveagent plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: RoveAgent validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. RoveAgent does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from roveagent.tools.registry import registry as _registry
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `roveagent plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from roveagent.clisupport.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_roveagent_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``roveagent_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``roveagent_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different RoveAgent home (profile switch reusing a slug
        # like "roveagent-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"roveagent-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved RoveAgent home -> PluginManager. RoveAgent supports
# multiple profiles via different ROVEAGENT_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_roveagent_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# roveagent_constants.set_roveagent_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_roveagent_home() / "plugins"`` and some
    plugins (notably context engines such as roveagent-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch RoveAgent home (env var *or* the context-local
    ``set_roveagent_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active RoveAgent home instead of
    being one process-wide singleton.
    """
    try:
        return get_roveagent_home().expanduser().resolve()
    except Exception:
        return get_roveagent_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``roveagent_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``roveagent_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active RoveAgent profile/home.
    ...
    profile switch — via ``ROVEAGENT_HOME`` or the context-local
    ``set_roveagent_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=roveagent_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_roveagent_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from roveagent.clisupport.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="roveagent-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``roveagent:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``roveagent tools`` TUI so plugin-provided toolsets appear
