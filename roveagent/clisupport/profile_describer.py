...
Used by ``roveagent profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``roveagent_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the RoveAgent Agent kanban board.
...
Rules:
  ...
  - Never write "RoveAgent Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_roveagent_home())
        else:
    ...
    try:
        from roveagent.core.auxiliary_client import call_llm  # type: ignore
    except Exception as exc:
