"""Profile distributions — shareable, packaged RoveAgent profiles via git.

A distribution is a RoveAgent profile published as a git repository (or
...
* ``roveagent profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``roveagent skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``roveagent profile``, not a parallel tree):

    roveagent profile install <source> [--name N] [--alias] [--force] [--yes]
    roveagent profile update  <name>  [--force-config] [--yes]
    roveagent profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "roveagent-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            roveagent_requires=str(data.get("roveagent_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.roveagent_requires:
            out["roveagent_requires"] = self.roveagent_requires
        if self.author:
...
def check_roveagent_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires RoveAgent {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this RoveAgent distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``roveagent profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a RoveAgent profile distribution."
            )
...
def plan_install(
    ...
    from roveagent.clisupport import __version__ as roveagent_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a RoveAgent distribution."
        )

    # Version check up-front so we fail fast
    check_roveagent_requires(manifest.roveagent_requires, roveagent_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.roveagent).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="roveagent_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `roveagent profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`roveagent profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="roveagent_dist_update_") as tmp:
        plan = plan_install(
