"""
Profile management for multiple isolated RoveAgent instances.

Each profile is a fully independent ROVEAGENT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.roveagent/profiles/<name>/`` by default.

The "default" profile is ``~/.roveagent`` itself — backward compatible,
zero migration needed.

Usage::

    roveagent profile create coder          # fresh profile + bundled skills
    roveagent profile create coder --clone  # also copy config, .env, SOUL.md, skills
    roveagent profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    roveagent -p coder chat                 # or via flag
    roveagent profile use coder             # set as sticky default
    roveagent profile delete coder          # remove profile + alias + service
"""
...
from roveagent.core.skill_utils import is_excluded_skill_path
from roveagent.clisupport.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See roveagent_constants.get_subprocess_home().
    "home",
]
...
#   backups             — `roveagent backup` archives
...
# Marker file written by `roveagent profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `roveagent
...
# `roveagent skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         RoveAgent infrastructure directories that only the default profile
         (``~/.roveagent``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_roveagent_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.roveagent) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "roveagent-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
    ...
    "roveagent.gateway.pid", "gateway_state.json", "processes.json",
...
# Allow-list for ``export_profile("default")``: when ROVEAGENT_HOME equals the
...
# into the export. The set below identifies the *known RoveAgent profile
# artifacts* at the root of ROVEAGENT_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``roveagent_constants``.
...
_RESERVED_NAMES = frozenset({
    "roveagent", "default", "test", "tmp", "root", "sudo",
})

# RoveAgent subcommands that cannot be used as profile names/aliases
_ROVEAGENT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the roveagent root, NOT to the current ROVEAGENT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ROVEAGENT_HOME points outside
    ``~/.roveagent``, profiles live under ``ROVEAGENT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_roveagent_home() / "profiles"


def _get_default_roveagent_home() -> Path:
    """Return the default (pre-profile) ROVEAGENT_HOME path.

    In standard deployments this is ``~/.roveagent``.
    In Docker/custom deployments where ROVEAGENT_HOME is outside ``~/.roveagent``
    (e.g. ``/opt/data``), returns ROVEAGENT_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_roveagent_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the RoveAgent installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ROVEAGENT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_roveagent_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current RoveAgent home
    (:func:`roveagent_constants.get_roveagent_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, roveagent subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _ROVEAGENT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a roveagent subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            roveagent_exe = shutil.which("roveagent") or "roveagent"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(roveagent_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running RoveAgent. If we
    ...
    try:
        ...
        # not be migrated. The next `roveagent doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``roveagent -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``roveagent profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``roveagent -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``roveagent profile list`` for an unrelated profile.
    """
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        source = get_roveagent_home() / "config.yaml"
        ...
        # Creation must not fail over this; `roveagent model` still sets it later.
        pass
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``ROVEAGENT_HOME``.
    """
    try:
        from roveagent.gateway.status import get_running_pid
        if (
            get_running_pid(profile_dir / "roveagent.gateway.pid", cleanup_stale=False)
            is not None
        ):
            return True
    except Exception:
...
# ``config.yaml``. ``config.yaml`` is the user-facing RoveAgent config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_roveagent_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, roveagent_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own ROVEAGENT_HOME. When
    ...
    The returned ``roveagent_home`` is the path to pass to
    ``set_roveagent_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_roveagent_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``roveagent update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.roveagent)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.roveagent/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `roveagent -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this RoveAgent profile.\n"
    ...
    if not soul_path.exists():
        try:
            from roveagent.clisupport.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `roveagent update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`roveagent profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `roveagent update`.\n",
                encoding="utf-8",
            )
        except OSError:
            pass  # best-effort — the feature still works via the empty skills/ dir

    # Cloned configs can be older than the running RoveAgent (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `roveagent profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `roveagent -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ROVEAGENT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``roveagent profile create
    ...
    only the essential skills (e.g. ``roveagent-agent``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ROVEAGENT_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_roveagent_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running RoveAgent *backends* bound to this profile.
    ...
    names it, so find it by inspection: a RoveAgent backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``ROVEAGENT_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `roveagent -p <canon> profile
    ...
    roveagent_markers = ("roveagent.clisupport.main", "roveagent-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('roveagent')` resolution) spawns the
    ...
    # interpreter, not "roveagent", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _ROVEAGENT_CONSOLE_SCRIPT_NAMES = frozenset({"roveagent", "roveagent-agent", "roveagent-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a RoveAgent process: either an entrypoint marker in argv, a
            # resolved executable named `roveagent`, or a python interpreter
            # directly exec'ing a `roveagent`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_roveagent = (
                any(marker in joined for marker in roveagent_markers)
                or exe_name == "roveagent"
                or exe_name.startswith("roveagent")
            )
            if not is_roveagent and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: roveagent, roveagent-agent,
                # roveagent-acp) rather than a bare `startswith("roveagent")` --
                ...
                # "roveagent-notes.py" or "roveagent-unrelated-tool" as the shim,
                ...
                is_roveagent = script_stem in _ROVEAGENT_CONSOLE_SCRIPT_NAMES
            if not is_roveagent:
                continue
            ...
            # ...or by ROVEAGENT_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("ROVEAGENT_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.roveagent).\n"
            "To remove everything, use: roveagent uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_roveagent_home
    ...
    try:
        from roveagent.plugins.memory.holographic.store import MemoryStore as _MemoryStore
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``ROVEAGENT_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from roveagent.clisupport.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ROVEAGENT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ROVEAGENT_HOME")
    try:
        os.environ["ROVEAGENT_HOME"] = str(profile_dir)
        from roveagent.clisupport.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ROVEAGENT_HOME"] = old_home
        elif "ROVEAGENT_HOME" in os.environ:
            del os.environ["ROVEAGENT_HOME"]
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    pid_file = profile_dir / "roveagent.gateway.pid"
    ...
    try:
        ...
        from roveagent.gateway.status import terminate_pid as _terminate_pid
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.roveagent/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: roveagent profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ROVEAGENT_HOME.

    Returns ``"default"`` if ROVEAGENT_HOME is not set or points to ``~/.roveagent``.
    Returns the profile name if ROVEAGENT_HOME points into ``~/.roveagent/profiles/<name>``.
    Returns ``"custom"`` if ROVEAGENT_HOME is set to an unrecognized path.
    """
    from roveagent.constants import get_roveagent_home
    roveagent_home = get_roveagent_home()
    resolved = roveagent_home.resolve()

    default_resolved = _get_default_roveagent_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      ROVEAGENT_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-RoveAgent file the
      user may have lying alongside ROVEAGENT_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``roveagent sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``ROVEAGENT_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.roveagent itself — its parent is ~/ and its
        # directory name is ".roveagent", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: roveagent profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.roveagent itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.roveagent). "
            "Specify a different name: roveagent profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="roveagent_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"roveagent_{old_name}"
    legacy_old_host = f"roveagent.{old_name}"
    new_host = f"roveagent_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_roveagent_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("roveagent_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ROVEAGENT_HOME path string.

    Called early in the CLI entry point, before any roveagent modules
    are imported, to set the ROVEAGENT_HOME environment variable.

    When ROVEAGENT_HOME is already set, the configured spelling IS the
