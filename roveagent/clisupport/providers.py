"""
Single source of truth for provider identity in RoveAgent Agent.
...
2. **RoveAgent overlays** — transport type, auth patterns, aggregator flags,
...
# -- RoveAgent overlay ----------------------------------------------------------
# RoveAgent-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class RoveAgentOverlay:
    """RoveAgent-specific provider metadata layered on top of models.dev."""
...
ROVEAGENT_OVERLAYS: Dict[str, RoveAgentOverlay] = {
    "moa": RoveAgentOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": RoveAgentOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "nous": RoveAgentOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.nousresearch.com/v1",
    ),
    "openai-codex": RoveAgentOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": RoveAgentOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": RoveAgentOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": RoveAgentOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ROVEAGENT_QWEN_BASE_URL",
    ),
    "lmstudio": RoveAgentOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "azure-foundry": RoveAgentOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": RoveAgentOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "roveagent", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. RoveAgent overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + RoveAgent overlay
    ...
    try:
        from roveagent.core.models_dev import get_provider_info as _mdev_provider
    ...
    overlay = ROVEAGENT_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + roveagent extra
    ...
    if overlay is not None:
        # RoveAgent-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and ROVEAGENT_OVERLAYS, so
    ...
    try:
        from roveagent.providers import get_provider_profile as _profile
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The RoveAgent overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in ROVEAGENT_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact RoveAgent provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from roveagent.clisupport.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
            ...
            if _pcfg is not None:
                ...
                if len(_collapsed_siblings) > 1:
                    return ProviderDef(
                        ...
                        source="roveagent-auth-registry",
                    )
        except Exception:
            pass
