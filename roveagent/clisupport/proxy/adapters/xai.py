...
from roveagent.core.credential_pool import CredentialPool, PooledCredential, load_pool
from roveagent.clisupport.auth import DEFAULT_XAI_OAUTH_BASE_URL
from roveagent.clisupport.proxy.adapters.base import UpstreamAdapter, UpstreamCredential

logger = logging.getLogger(__name__)

_POOL_PROVIDER = "xai-oauth"

# xAI's public API is OpenAI-compatible for the endpoints RoveAgent commonly
# uses. The Responses endpoint is included because RoveAgent' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via RoveAgent-managed OAuth credentials."""

    auth_hint = "roveagent auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`roveagent auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`roveagent auth reset xai-oauth` or re-authenticate with "
                    "`roveagent auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `roveagent auth add xai-oauth --type oauth`."
            )
