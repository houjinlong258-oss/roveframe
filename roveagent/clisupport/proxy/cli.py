"""CLI handlers for the ``roveagent proxy`` subcommand."""
...
from roveagent.clisupport.proxy.adapters import ADAPTERS, get_adapter
from roveagent.clisupport.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "roveagent proxy requires aiohttp. Run `roveagent setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"roveagent auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting RoveAgent proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("RoveAgent proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: roveagent proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``roveagent proxy <subcommand>``."""
    ...
    print(
        "roveagent proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  roveagent proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  roveagent proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  roveagent proxy providers\n"
