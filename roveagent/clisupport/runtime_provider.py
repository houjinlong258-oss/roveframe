...
from roveagent.clisupport import auth as auth_mod
...
from roveagent.clisupport import config as _config_mod
from roveagent.clisupport.providers import custom_provider_aliases, custom_provider_slug
from roveagent.constants import OPENROUTER_BASE_URL
from roveagent.clisupport.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`roveagent_cli.config.load_config`.

    Deliberately NOT a module-level ``from roveagent_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``roveagent_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``roveagent_cli.config.load_config`` OR
    ``roveagent_cli.runtime_provider.load_config`` behaves as expected.
    """
...
from roveagent.utils import base_url_host_matches, base_url_hostname, env_int
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from roveagent.clisupport.auth import resolve_provider as _resolve_provider
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    # host_mandated_api_mode clause in roveagent_cli/providers.py so the
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from roveagent.clisupport.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of RoveAgent' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from roveagent.clisupport.config import _canonical_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from roveagent.clisupport.models import azure_foundry_model_api_mode
        ...
        if opencode_provider_family(provider) is not None:
            ...
            # Refs #16878.
            from roveagent.clisupport.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("ROVEAGENT_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/roveagent_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'roveagent doctor' for details."
        )
        return None
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``ROVEAGENT_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("ROVEAGENT_INFERENCE_PROVIDER", "").strip()
...
def is_routable_provider(provider: Optional[str]) -> bool:
    ...
    if name.lower() == "custom":
        ...
        return False
    try:
        from roveagent.clisupport.providers import resolve_provider_full
...
def _resolve_named_custom_runtime(
    *,
    ...
    if _oc_family is None:
        try:
            from roveagent.utils import base_url_hostname
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'roveagent model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.roveagent/.env or run 'roveagent model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'roveagent model')."
        )
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "anthropic":
        ...
        api_key = explicit_api_key
        if not api_key:
            from roveagent.core.anthropic_adapter import resolve_anthropic_token
    ...
    if provider == "nous":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("ROVEAGENT_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("ROVEAGENT_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    from roveagent.clisupport.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `roveagent auth list`). If the key is
        ...
        if provider == "nous" and entry is not None:
            min_ttl = max(60, env_int("ROVEAGENT_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "roveagent-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from roveagent.clisupport.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (RoveAgent canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most RoveAgent-compatible importers).
