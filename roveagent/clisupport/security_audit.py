"""On-demand supply-chain audit for RoveAgent Agent installs.

Scans three surfaces a RoveAgent user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The RoveAgent venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.roveagent/plugins``
...
def _discover_plugins(roveagent_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.roveagent/plugins``.
    ...
    """
    plugins_dir = roveagent_home / "plugins"
...
def _discover_components(
    *,
    ...
    home = roveagent_home or Path(get_roveagent_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `roveagent security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, roveagent_home=home
    )
