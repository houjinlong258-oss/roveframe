...
3. Running inside a container with no persistent volume mount over the
   ROVEAGENT_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("roveagent.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run RoveAgent as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("ROVEAGENT_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(roveagent_home: Optional[Path]) -> Optional[str]:
...
def _network_listener_without_auth(config: Optional[dict]) -> list[str]:
    ...
    findings: list[str] = []
    try:
        from roveagent.gateway.platforms.base import is_network_accessible
    except Exception:
        return findings
...
def run_security_audit(
    *, roveagent_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(roveagent_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(roveagent_home=roveagent_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
