"""CLI subcommand: ``roveagent send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"roveagent send: {file_path} is not a text file. --file reads the "
                ...
                f'  roveagent send --to telegram "MEDIA:{file_path}"\n'
                f'  roveagent send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  roveagent send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"roveagent send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"roveagent send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    """
    try:
        from roveagent.gateway.channel_directory import (
            format_directory_for_display,
            load_directory,
        )
    except Exception as exc:
        print(f"roveagent send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"roveagent send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `roveagent send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"roveagent send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `roveagent gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.roveagent/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_roveagent_env() -> None:
    """Populate ``os.environ`` from ``~/.roveagent/.env`` AND bridge top-level
    ...
    startup that ``roveagent send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.roveagent/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.roveagent/config.yaml`` into
       ...
       via ``roveagent config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``roveagent send``
    ...
    try:
        from roveagent.clisupport.config import _expand_env_vars
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.roveagent/.env and ~/.roveagent/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "roveagent send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  roveagent send --to telegram \"hello\"\n"
            "  roveagent send --to discord:#ops --file report.md\n"
            "  roveagent send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "roveagent send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `roveagent send --help` stays fast and does not pull in
    # the full tool registry / gateway config stack.
    from roveagent.tools.send_message_tool import send_message_tool
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform RoveAgent "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.roveagent/.env + ~/.roveagent/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  roveagent send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | roveagent send --to telegram:-1001234567890\n"
            "  roveagent send --to discord:#ops --file /tmp/report.md\n"
            "  roveagent send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  roveagent send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  roveagent send --list                  # all platforms\n"
            "  roveagent send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `roveagent send --list telegram`.",
    )
