...
the existing module-level functions in roveagent_cli.gateway and
roveagent_cli.gateway_windows directly. This protocol is a thin facade
...
``roveagent gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``roveagent gateway start/stop/restart``).
    """
    ...
    # `roveagent gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged roveagent
    ...
    detection always fails. Since every RoveAgent runtime call inside the
    container drops to roveagent via ``s6-setuidgid``, that silent failure
...
# in ``roveagent_cli.gateway`` (systemd/launchd) and ``roveagent_cli.gateway_windows``
...
# active (set via the ``roveagent -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in roveagent_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from roveagent.clisupport.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from roveagent.clisupport.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from roveagent.clisupport.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from roveagent.clisupport.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in roveagent_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from roveagent.clisupport.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from roveagent.clisupport.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from roveagent.clisupport.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from roveagent.clisupport.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``roveagent_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `roveagent profile create`
# runs inside the container (Phase 4). Static services (main-roveagent, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's ROVEAGENT_HOME blindly;
    derive the shared profile root from the current ROVEAGENT_HOME and map the
    ...
    roveagent_home = Path(os.environ.get("ROVEAGENT_HOME", "/opt/data"))
    if roveagent_home.parent.name == "profiles":
        root = roveagent_home.parent.parent
    else:
        root = roveagent_home
...
# UID/GID of the in-image ``roveagent`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``roveagent`` user — UID 10000 — running every RoveAgent runtime
    ...
    is dispatched as the roveagent user.
    ...
    with roveagent ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           roveagent:roveagent, 0755 (parent must already exist)
    ``svc_dir/event/``                     roveagent:roveagent, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 roveagent:roveagent, 0755
    ``svc_dir/supervise/event/``           roveagent:roveagent, 03730
    ``svc_dir/supervise/control``          roveagent:roveagent, 0660    (FIFO)
    ...
    roveagent user reads them fine.
    ...
    slot's supervise/ was roveagent-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _ROVEAGENT_UID, _ROVEAGENT_GID)
        except PermissionError:
            # Running as the roveagent user already — directory is roveagent-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _ROVEAGENT_UID, _ROVEAGENT_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is roveagent-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _ROVEAGENT_UID, _ROVEAGENT_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``roveagent -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`roveagent profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-roveagent, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources ROVEAGENT_HOME (and any extra env) via with-contenv —
             so e.g. ``-e ROVEAGENT_HOME=/data/roveagent`` is honored at run
          ...
          4. Drops to the roveagent user and exec's
             ``roveagent -p <profile> gateway run`` (or just ``roveagent
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``roveagent gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root ROVEAGENT_HOME profile" (the implicit profile that exists at
        the top of $ROVEAGENT_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``roveagent_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$ROVEAGENT_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``ROVEAGENT_HOME``,
        ...
        allocator in ``roveagent_cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/roveagent/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export ROVEAGENT_S6_SUPERVISED_CHILD=1")
        # Generalized supervisor marker (#74872) — same meaning for the
        # profile-redirect guard in roveagent_cli.main._apply_profile_override,
        # kept alongside the s6-specific sentinel for back-compat.
        lines.append("export ROVEAGENT_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's ROVEAGENT_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``roveagent gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-ROVEAGENT_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The ROVEAGENT_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own ROVEAGENT_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "roveagent gateway run --replace"
        else:
            gateway_cmd = f"roveagent -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid roveagent {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_finish_script() -> str:
        ...
        See #51228, #76435.
        """
        from roveagent.gateway.restart import GATEWAY_FATAL_CONFIG_EXIT_CODE
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${ROVEAGENT_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the ROVEAGENT_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e ROVEAGENT_HOME=/data/roveagent``
        gets its logs under /data/roveagent/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``roveagent logs`` reads and what
        ...
        return (
            ...
            f': "${{ROVEAGENT_HOME:=/opt/data}}"\n'
            f'log_dir="$ROVEAGENT_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as roveagent when
            # this script starts as root. Never chown or unlink roveagent-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is roveagent-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded roveagent-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid roveagent mkdir -p "$log_dir"\n'
            f'  s6-setuidgid roveagent rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid roveagent s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``roveagent gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``roveagent -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with roveagent ownership
            ...
            # the roveagent user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is roveagent-owned (see ``_seed_supervise_skeleton``), and on
