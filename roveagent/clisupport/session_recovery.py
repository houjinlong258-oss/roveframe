"""Offline, non-destructive recovery for a damaged RoveAgent session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``roveagent_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``roveagent_state._backup_db_file``.
    """
    from roveagent.clisupport.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="roveagent-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every RoveAgent process using this profile and retry. "
                "This includes the interactive `roveagent` CLI session this "
                ...
                "RoveAgent process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `roveagent` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from roveagent.clisupport.session_lost_and_found import (
