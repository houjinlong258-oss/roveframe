"""``roveagent sessions`` command — extracted from ``roveagent_cli/main.py``.
...
Helpers that stay in ``roveagent_cli.main`` (``get_roveagent_home``,
...
``roveagent_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def get_roveagent_home():
    return _m().get_roveagent_home()
...
#: Default age floor for `roveagent sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`roveagent sessions prune --never-active` — drop leaked/dead keyed rows.
    ...
    """
    from roveagent.clisupport.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    roveagent sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from roveagent.clisupport.session_recovery import (
    ...
    if action == "import":
        from roveagent.clisupport.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from roveagent.state import workspace_key as _ws_key
        ...
        from roveagent.clisupport.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from roveagent.clisupport.session_export_html import (
        ...
        if args.format == "trace":
            ...
            from roveagent.core.trace_upload import (
                TraceRedactionError,
                build_trace_jsonl,
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_roveagent_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from roveagent.clisupport.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_roveagent_home() / "session-exports"
        ...
        # `roveagent sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `roveagent sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `roveagent sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `roveagent sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: roveagent sessions pin <session_id>"
            )
            return
        ...
        from roveagent.core.skill_commands import describe_skill_invocation
        from roveagent.core.title_generator import generate_title
        ...
        # Launch roveagent --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`roveagent sessions optimize` later to reclaim freed space.)")
