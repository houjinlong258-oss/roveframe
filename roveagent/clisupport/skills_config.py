"""
Skills configuration for RoveAgent Agent.
`roveagent skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.roveagent/config.yaml under:
...
from roveagent.clisupport.config import cfg_get, load_config, save_config
from roveagent.clisupport.colors import Colors, color
from roveagent.clisupport.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``roveagent-agent``) are silently dropped from the
...
def skills_command(args=None):
    """Entry point for `roveagent skills`."""
    from roveagent.clisupport.curses_ui import curses_checklist
