#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the RoveAgent Skills Hub.

Powers both:
  - `roveagent skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    """
    from roveagent.tools.skills_hub import unified_search
    ...
    if len(exact) > 1:
        ...
        # skill: `roveagent skills install impeccable` should resolve to the
        ...
        # so users can copy it for `roveagent skills install`.
...
def _print_tier1_advisory(skill_dir, console) -> None:
    ...
    """
    try:
        from roveagent.tools.skillevaluator_scan import (
...
def _resolve_source_meta_and_bundle(identifier: str, sources):
    ...
    skill is how ``roveagent skills inspect owner/repo/skills/foo`` showed
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.roveagent/skills/`` that look
    ...
    """
    from roveagent.tools.skills_hub import SKILLS_DIR, _category_skill_dirs
    try:
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.roveagent/skills/<name>/)[/]"
        )
    from roveagent.clisupport.cli_output import line_input
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    """
    from roveagent.tools.skills_hub import GitHubAuth, create_source_router, unified_search
    ...
    # into `roveagent skills install` fail.
    ...
    c.print(table)
    c.print("[dim]Use: roveagent skills inspect <identifier> to preview, "
            "roveagent skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    """
    from roveagent.tools.skills_hub import (
    ...
    # skips the external API sources and serves everything from "roveagent-index".
    ...
    from roveagent.tools.skills_hub import _PROVIDER_FILTER_VALUES, _filter_results_by_provider
    ...
    # The identifier is what you pass to `roveagent skills install`. Browse used
    ...
    c.print("[dim]Tip: 'roveagent skills inspect <identifier>' to preview, "
            "'roveagent skills install <identifier>' to install, "
            "'roveagent skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    from roveagent.tools.skills_guard import scan_skill_cached, should_allow_install, format_scan_report
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]roveagent skills install {url} --name <your-name>[/]\n\n"
    ...
    try:
        ...
        from roveagent.tools.skills_hub import append_audit_log
    ...
    from roveagent.tools.skills_hub import HUB_DIR, source_url_for_bundle
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with roveagent-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_roveagent_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    try:
        ...
        append_audit_log("BLOCKED", bundle.name, bundle.source,
                         bundle.trust_level, "invalid_path", str(exc))
        return
    from roveagent.tools.skills_hub import SKILLS_DIR
    ...
    # metadata.roveagent.blueprint block, it is a runnable automation. Register it as
    ...
    try:
        from roveagent.tools.blueprints import BlueprintError, blueprint_spec_for_installed, register_blueprint_suggestion
        ...
        if spec is not None:
            ...
            if registered is not None:
                ...
                c.print(
                    ...
                    "or via[/] [bold]roveagent cron add[/][dim].[/]\n"
                )
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    from roveagent.tools.skills_hub import GitHubAuth, create_source_router
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="roveagent skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "roveagent-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"roveagent-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying roveagent-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``roveagent -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``ROVEAGENT_HOME`` at process
    start.  No explicit profile flag needed here.
    """
    from roveagent.tools.skills_hub import HubLockFile, ensure_hub_dirs
    from roveagent.tools.skills_sync import _read_manifest
    from roveagent.tools.skills_tool import _find_all_skills
    from roveagent.core.skill_utils import get_disabled_skill_names
...
def do_check(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from roveagent.tools.skills_hub import check_for_skill_updates
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    already get from ``roveagent update`` (ported from
    ...
    """
    from roveagent.tools.skills_hub import SKILLS_DIR, HubLockFile, check_for_skill_updates
    from roveagent.tools.skills_guard import content_hash
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: roveagent skills update <name> --force[/]\n")
...
def do_audit(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    are unchanged).
    """
    from roveagent.tools.skills_hub import HubLockFile, SKILLS_DIR
    from roveagent.tools.skills_guard import scan_skill, format_scan_report
    ...
    if deep:
        from roveagent.tools.skills_ast_audit import ast_scan_path, format_ast_report
...
def do_uninstall(name: str, console: Optional[Console] = None,
    ...
    from roveagent.tools.skills_hub import uninstall_skill
...
def do_reset(name: str, restore: bool = False,
    ...
    from roveagent.tools.skills_sync import reset_bundled_skill
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `roveagent update` keeps)."""
    from roveagent.tools.skills_sync import list_user_modified_bundled_skills
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `roveagent update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   roveagent skills diff <name>[/]")
    c.print("[dim]Resume updates: roveagent skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: roveagent skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    from roveagent.tools.skills_sync import diff_bundled_skill
    ...
    c.print()
    c.print(f"[dim]Revert with: roveagent skills reset {name} --restore[/]\n")
...
def do_opt_out(remove: bool = False,
    ...
    never touched.
    """
    from roveagent.tools.skills_sync import (
...
def do_repair_official(name: str, restore: bool = False,
    ...
    from roveagent.tools.skills_sync import restore_official_optional_skill
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    from roveagent.tools.skills_hub import TapsManager
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: roveagent skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: roveagent skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
...
def do_publish(skill_path: str, target: str = "github", repo: str = "",
    ...
    from roveagent.tools.skills_hub import GitHubAuth, SKILLS_DIR
    from roveagent.tools.skills_guard import scan_skill, format_scan_report
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: roveagent skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_roveagent_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via RoveAgent Skills Hub.\n\n"
                        f"This skill was scanned by the RoveAgent Skills Guard before submission.",
...
def do_snapshot_export(output_path: str, console: Optional[Console] = None) -> None:
    ...
    from roveagent.tools.skills_hub import HubLockFile, TapsManager
...
def skills_command(args) -> None:
    """Router for `roveagent skills <subcommand>` — called from roveagent_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: roveagent skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: roveagent skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: roveagent skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'roveagent skills <command> --help' for details.\n")
