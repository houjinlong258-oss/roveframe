...
``CommandDef.execute`` (roveagent_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/roveagent_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``roveagent_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_version(ctx: CommandContext) -> CommandReply:
    ...
    from roveagent.clisupport.banner import format_banner_version_label
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_roveagent_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    try:
        from roveagent.core.skill_bundles import _bundles_dir, list_bundles
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: roveagent bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from roveagent.clisupport.commands import gateway_help_lines

    lines = [
        t("roveagent.gateway.help.header"),
        *gateway_help_lines(),
    ]
    try:
        ...
        if skill_cmds:
            lines.append(t("roveagent.gateway.help.skill_header", count=len(skill_cmds)))
            # Show first 10, then point to /commands for the rest
            sorted_cmds = sorted(skill_cmds)
            for cmd in sorted_cmds[:10]:
                lines.append(f"`{cmd}` — {skill_cmds[cmd]['description']}")
            if len(sorted_cmds) > 10:
                lines.append(t("roveagent.gateway.help.more_use_commands", count=len(sorted_cmds) - 10))
    except Exception:
        pass
...
def _exec_commands(ctx: CommandContext) -> CommandReply:
    ...
    if raw_args:
        try:
            ...
            return CommandReply(t("roveagent.gateway.commands.usage"), format="markdown")
    else:
        requested_page = 1
    ...
    try:
        ...
        if skill_cmds:
            entries.append("")
            entries.append(t("roveagent.gateway.commands.skill_header"))
            for cmd in sorted(skill_cmds):
                desc = skill_cmds[cmd].get("description", "").strip() or t("roveagent.gateway.commands.default_desc")
                entries.append(f"`{cmd}` — {desc}")
    except Exception:
        pass

    if not entries:
        return CommandReply(t("roveagent.gateway.commands.none"), format="markdown")
    ...
    lines = [
        t("roveagent.gateway.commands.header", total=len(entries), page=page, total_pages=total_pages),
        "",
        *page_entries,
    ]
    if total_pages > 1:
        nav_parts = []
        if page > 1:
            nav_parts.append(t("roveagent.gateway.commands.nav_prev", page=page - 1))
        if page < total_pages:
            nav_parts.append(t("roveagent.gateway.commands.nav_next", page=page + 1))
        lines.extend(["", " | ".join(nav_parts)])
    if page != requested_page:
        lines.append(t("roveagent.gateway.commands.out_of_range", requested=requested_page, page=page))
    return CommandReply("\n".join(lines), format="markdown")
