"""
Status command for roveagent CLI.

Shows the status of all RoveAgent Agent components.
"""
...
from roveagent.clisupport.auth import AuthError, resolve_provider
from roveagent.clisupport.colors import Colors, color
from roveagent.clisupport.config import get_env_path, get_env_value, get_roveagent_home, load_config
from roveagent.clisupport.models import provider_label
...
from roveagent.clisupport.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``roveagent config``'s
...
def _estop_status_line():
    """One-line pause banner for `roveagent status`, or None when not paused.

    Cheap: a single stat on $ROVEAGENT_HOME/ESTOP via agent.estop.
    """
    try:
        from roveagent.core.estop import get_state
    except ImportError:
        return None
    state = get_state()
    if state is None:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `roveagent resume` to lift)"


def show_status(args):
    """Show status of all RoveAgent Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ RoveAgent Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `roveagent status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: roveagent portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: roveagent model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: roveagent auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: roveagent auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: roveagent model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from roveagent.clisupport.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'roveagent-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `roveagent status`).
        try:
    ...
    try:
        from roveagent.clisupport.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   roveagent gateway")
    ...
    jobs_file = get_roveagent_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_roveagent_home() / "sessions" / "sessions.json"
    ...
    try:
        from roveagent.clisupport.active_sessions import (
    ...
    print(color("  Run 'roveagent doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'roveagent setup' to configure", Colors.DIM))
    print()
