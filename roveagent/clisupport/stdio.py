...
on Japanese locales, etc.).  RoveAgent's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``roveagent_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. RoveAgent is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ROVEAGENT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ROVEAGENT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the RoveAgent-managed Git install directories so
    ...
    # already-running shells don't see the change — which means roveagent
    ...
    # install without RoveAgent-managed PortableGit).
    ...
    # stdin is re-configured for completeness; RoveAgent's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and RoveAgent's
    ``roveagent config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching RoveAgent (User env var in Windows Settings, or
    export in a PowerShell profile) and RoveAgent picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known RoveAgent-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\roveagent\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``roveagent`` from right after install) retain their old PATH.

    Any subprocess RoveAgent spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known RoveAgent-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "roveagent", "git", "cmd"),
        os.path.join(local_appdata, "roveagent", "git", "bin"),
        os.path.join(local_appdata, "roveagent", "git", "usr", "bin"),
        # RoveAgent venv Scripts directory — host of the roveagent.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes roveagent, but harmless to include.
        os.path.join(local_appdata, "roveagent", "roveagent-agent", "venv", "Scripts"),
