"""``roveagent debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for RoveAgent Agent. Use 'roveagent debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    roveagent debug share              Upload debug report (asks for confirmation)
    roveagent debug share --yes        Skip confirmation (for scripts/CI)
    roveagent debug share --lines 500  Include more log lines
    roveagent debug share --expire 30  Keep paste for 30 days
    roveagent debug share --local      Print report locally (no upload)
    roveagent debug share --no-redact  Disable upload-time secret redaction
    roveagent debug share --nous       Upload to Nous-internal storage (private)
    roveagent debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'roveagent debug share'",
    )
