"""``roveagent gateway`` and ``roveagent proxy`` subcommand parsers.

Extracted verbatim from ``roveagent_cli/main.py:main()`` (god-file Phase 2).
...
from roveagent.clisupport.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if ROVEAGENT_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "roveagent.gateway. In-chat restarts and updates exit back to that manager "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy roveagent.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy RoveAgent gateway unit files "
            "(e.g. roveagent.service) left over from older installs. Profile "
            "units (roveagent-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.roveagent/.env. "
            "Requires being logged in (roveagent setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.roveagent/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `roveagent proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
