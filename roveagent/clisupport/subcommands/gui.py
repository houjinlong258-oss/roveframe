"""``roveagent gui`` subcommand parser.
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the RoveAgent Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any roveagent CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--roveagent-root",
        help="Override the RoveAgent source root used by Desktop (sets ROVEAGENT_DESKTOP_ROVEAGENT_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets ROVEAGENT_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
    ...
    gui_parser.add_argument(
        "--identity",
        default="RoveAgent Local Signing",
        help="Certificate name to create/use for --setup-tcc-identity (default: RoveAgent Local Signing)",
    )
