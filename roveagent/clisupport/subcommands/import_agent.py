"""``roveagent import-agent`` subcommand parser.

Follows the ``roveagent claw`` pattern (see ``roveagent_cli/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``roveagent_cli/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into RoveAgent",
        description=(
            "One-command import of another coding agent's setup into RoveAgent. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their RoveAgent equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'roveagent setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing RoveAgent items on name conflicts (default: skip)",
    )
    parser.add_argument(
