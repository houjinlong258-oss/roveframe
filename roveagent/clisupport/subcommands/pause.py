"""``roveagent pause`` / ``roveagent resume`` — the global emergency stop.

``roveagent pause`` writes the ESTOP sentinel at ``$ROVEAGENT_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``roveagent resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    from roveagent.core.estop import engage, get_state, is_engaged

    reason = getattr(args, "reason", None)
    already = is_engaged()
    path = engage(reason=reason)
    state = get_state() or {}
    verb = "Still paused" if already else "RoveAgent paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `roveagent resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    from roveagent.core.estop import disengage, sentinel_path

    if disengage():
        print("▶️  RoveAgent resumed — dispatch picks up on the next tick.")
    else:
        print(f"RoveAgent is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`roveagent resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `roveagent pause`",
