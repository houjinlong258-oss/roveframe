"""``roveagent skills`` subcommand parser.

Extracted from ``roveagent_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_trust = skills_subparsers.add_parser(
        "trust",
        help="Trust a project so its repo-local skills (./.roveagent/skills, ./.agents/skills) load",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.roveagent/skills/.bundled_manifest) "
            "so future 'roveagent update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `roveagent update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `roveagent update` reports as user-modified and skips. "
            "Use `roveagent skills diff <name>` to see changes and `roveagent skills reset "
    ...
    skills_diff = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "running `roveagent skills reset`."
        ),
    )
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`roveagent update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `roveagent update`. Pass --sync to re-seed now."
        ),
    )
