"""``roveagent sync`` subcommand parser — Skill Sync.

Cloned from ``roveagent_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    roveagent sync status                 show gate/opt-in/head state
    roveagent sync pull                   pull and materialize opted-in skills
    roveagent sync push                   push opted-in skills
    roveagent sync now                    reconcile: pull then push
    roveagent sync enable <skill>         opt a skill into sync
    roveagent sync disable <skill>        opt a skill out of sync
    roveagent sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    roveagent sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  roveagent sync status            what is synced, and from where\n"
            "  roveagent sync enable my-skill   include a skill in your sync\n"
            "  roveagent sync now               pull, then push\n"
            "  roveagent sync propose my-skill  share a skill with your team\n"
        ),
