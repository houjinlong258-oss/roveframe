"""Per-terminal session breadcrumbs for ``roveagent -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$ROVEAGENT_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``roveagent -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_roveagent_home() / "terminal-sessions"
