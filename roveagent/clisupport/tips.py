...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or roveagent -c.",
    ...
    "Ctrl+Z suspends RoveAgent to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "roveagent -c resumes your most recent CLI session. roveagent -c \"project name\" resumes by title.",
    "roveagent -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "roveagent -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "roveagent chat -t web,terminal enables only specific toolsets for a focused session.",
    "roveagent chat -s github-pr-workflow preloads a skill at launch.",
    "roveagent chat -q \"query\" runs a single non-interactive query and exits.",
    "roveagent chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "roveagent chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "roveagent --yolo bypasses all dangerous command approval prompts for the entire session.",
    "roveagent chat --source telegram tags the session for filtering in roveagent sessions list.",
    "roveagent -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "roveagent doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "roveagent dump outputs a compact setup summary — great for bug reports.",
    "roveagent config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "roveagent config edit opens config.yaml in your default editor.",
    "roveagent config check scans for missing or stale configuration options.",
    "roveagent sessions browse opens an interactive session picker with search.",
    "roveagent sessions stats shows session counts by platform and database size.",
    "roveagent sessions prune --older-than 30 cleans up old sessions.",
    "roveagent skills search react --source skills-sh searches the skills.sh public directory.",
    "roveagent skills check scans installed hub skills for upstream updates.",
    "roveagent skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "roveagent skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "roveagent mcp add github --command npx adds MCP servers from the command line.",
    "roveagent mcp serve runs RoveAgent itself as an MCP server for other agents.",
    "roveagent auth add lets you add multiple API keys for credential pool rotation.",
    "roveagent completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "roveagent logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "roveagent backup creates a zip backup of your entire RoveAgent home directory.",
    "roveagent profile create coder creates an isolated profile that becomes its own command.",
    "roveagent profile create work --clone copies your current config and keys to a new profile.",
    "roveagent update syncs new bundled skills to ALL profiles automatically.",
    "roveagent gateway install sets up RoveAgent as a system service (systemd/launchd).",
    "roveagent memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "roveagent webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: roveagent tools disables unused tools, roveagent skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "roveagent models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as roveagent -w).",
    ...
    "ROVEAGENT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call RoveAgent tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'roveagent profile create coder' creates the 'coder' command.",
    "roveagent profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, RoveAgent prints a resume command with session ID and stats.",
    "roveagent sessions export backup.jsonl exports all sessions for backup or analysis.",
    "roveagent -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "roveagent skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .roveagent/plans/ without executing anything.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: roveagent cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "RoveAgent runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "roveagent gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to RoveAgent via a local macOS server.",
    ...
    "Context files (.roveagent.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "RoveAgent loads project context from .roveagent.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "roveagent mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "roveagent mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "roveagent mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — roveagent tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.roveagent/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "roveagent chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "roveagent plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "roveagent acp runs RoveAgent as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ROVEAGENT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "roveagent auth add nous or roveagent auth add openai-codex sets up OAuth-based providers.",
    ...
    "roveagent status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.roveagent/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make RoveAgent your own.",
    ...
    "Before a gateway session reset, RoveAgent auto-flushes important facts to memory in the background.",
    ...
    "roveagent.core.tool_use_enforcement steers models that describe actions instead of calling tools — auto for GPT/Codex.",
    "roveagent.core.restart_after_turn_timeout lets in-flight turns finish before /restart enters stop(); restart_drain_timeout is only the force-interrupt budget once stop() begins.",
    "roveagent.core.api_max_retries (default 3) controls how many times the agent retries a failed API call before surfacing the error — lower it for fast fallback.",
    ...
    "The skills audit log at ~/.roveagent/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope RoveAgent state via ROVEAGENT_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ROVEAGENT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ROVEAGENT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.roveagent/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'roveagent profile create ls' would be rejected.",
    "roveagent profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "roveagent claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ROVEAGENT_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via roveagent tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.roveagent/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.roveagent/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — RoveAgent auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of RoveAgent config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while RoveAgent is working.',
    ...
    '/reload reloads ~/.roveagent/.env into the running session — pick up new API keys without restarting.',
    ...
    "ROVEAGENT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.roveagent/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.roveagent/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'roveagent curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "roveagent curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'roveagent curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'roveagent auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'ROVEAGENT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ROVEAGENT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'roveagent dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'roveagent dashboard embeds the full RoveAgent TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.roveagent/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.roveagent/dashboard-plugins/ — no npm build required.',
    ...
    'ROVEAGENT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'ROVEAGENT_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ROVEAGENT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ROVEAGENT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.roveagent/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.roveagent/skills/ so drop-in skills appear without restarting the session.',
    ...
    'roveagent -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'roveagent chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'roveagent chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'roveagent chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "roveagent chat --source tool tags programmatic chats so they don't clutter roveagent sessions list.",
    'roveagent dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'roveagent sessions rename <ID> "new title" renames any past session; roveagent sessions delete <ID> removes one.',
    'roveagent import restores a full RoveAgent backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'roveagent fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'roveagent pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'roveagent setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'roveagent status --deep runs the full health sweep across every component; plain roveagent status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ROVEAGENT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ROVEAGENT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.roveagent/plugins/ — trust-gated by design.',
    "ROVEAGENT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ROVEAGENT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ROVEAGENT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ROVEAGENT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ROVEAGENT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ROVEAGENT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ROVEAGENT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ROVEAGENT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ROVEAGENT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ROVEAGENT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ROVEAGENT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes RoveAgent block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from roveagent sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.roveagent/dashboard-plugins/.',
]
...
# any project or none — RoveAgent is not a coding-only agent.
# ---------------------------------------------------------------------------
