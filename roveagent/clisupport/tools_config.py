"""
Unified tool configuration for RoveAgent Agent.

`roveagent tools` and `roveagent setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.roveagent/config.yaml under
...
from roveagent.clisupport.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from roveagent.clisupport.nous_account import format_nous_portal_entitlement_message
from roveagent.clisupport.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS,
    toolset_allowed_for_platform as _toolset_allowed_for_platform,
)
from roveagent.tools.tool_backend_helpers import NOUS_MANAGED_PROVIDER, fal_key_is_configured
from roveagent.utils import base_url_hostname, is_truthy_value
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``roveagent tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`roveagent_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from roveagent.clisupport.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ROVEAGENT_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `roveagent tools` → Video Generation, which walks
...
# `roveagent tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `roveagent tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from roveagent.clisupport.auth import _read_xai_oauth_tokens
        ...
        pass
    try:
        from roveagent.tools.xai_http import get_env_value as _xai_get_env_value
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``roveagent tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from roveagent.clisupport.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from roveagent.clisupport.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``roveagent tools`` checklist actually offers
    for ``platform``.
    ...
    so ``roveagent tools`` never claims to add or remove a toolset the user was
...
from roveagent.clisupport.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "RoveAgent routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "tag": "Zig headless browser spawned by RoveAgent, text-only (no screenshots)",
                "env_vars": [],
                "browser_provider": "local",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set ROVEAGENT_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ROVEAGENT_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ROVEAGENT_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ROVEAGENT_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ROVEAGENT_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ROVEAGENT_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `roveagent model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ROVEAGENT_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``ROVEAGENT_CUA_DRIVER_CMD`` is not obliged to answer
...
def _resolved_cua_driver_cmd() -> Optional[str]:
    ...
    from roveagent.tools.computer_use.cua_backend import resolve_cua_driver_cmd

    return resolve_cua_driver_cmd()


def _cua_driver_env() -> dict:
    """cua-driver child env with the RoveAgent telemetry policy applied.
    ...
    """
    try:
        from roveagent.tools.computer_use.cua_backend import cua_driver_child_env
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports RoveAgent' runtime contract."""
    import time

    from roveagent.tools.computer_use.cua_backend import cua_driver_runtime_contract_status
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $ROVEAGENT_HOME/bin is never on PATH, so a bare which()
    # misses the uv RoveAgent installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``roveagent update`` and
      by ``roveagent computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``roveagent update`` sets
    ...
    a further ~600s wait on Windows). ``roveagent computer-use install
    ...
    ``roveagent update`` already prints a contextual line before its update
    ...
    ``roveagent update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `roveagent update` calls this
    ...
    override = os.environ.get("ROVEAGENT_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    ROVEAGENT_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails RoveAgent' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # RoveAgent' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `roveagent update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but RoveAgent cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("ROVEAGENT_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "roveagent computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `roveagent computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `roveagent update` (require_confirmed_update=True) keeps the
    ...
    # `roveagent computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    confirmed_version = None
    if binary and not repair_existing:
        _state = None
        try:
            from roveagent.tools.computer_use.cua_backend import cua_driver_update_check
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: roveagent computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: roveagent computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``roveagent update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `roveagent update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``roveagent update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `roveagent update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `roveagent update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.roveagent/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / RoveAgent process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the RoveAgent-managed ``$ROVEAGENT_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary RoveAgent installs
    and updates rather than a user-level side install.
    """
    _print_info("    Ensuring browser-use CLI (managed install)...")
    try:
        from roveagent.tools.browser_use_cli import install_cli
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key == "lightpanda":
        ...
        if lightpanda_bin:
            ...
            if os.name == "nt":
                _print_info("    Lightpanda has no native Windows build; run RoveAgent under WSL2.")
        return
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/RoveAgent-managed node -> npx) rather than a bare
        # shutil.which — RoveAgent-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/roveagent-agent:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # RoveAgent-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — RoveAgent-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import roveagent.tools.browser_tool
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.roveagent/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.roveagent/config.yaml")
        ...
        # Run the full `roveagent auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.roveagent/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from roveagent.clisupport.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: roveagent auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `roveagent auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: roveagent auth spotify")
        except Exception as exc:
        ...
        try:
            from roveagent.clisupport.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: roveagent plugins enable observability/langfuse")
        _print_info("    Restart RoveAgent for tracing to take effect.")
        _print_info("    Verify: roveagent plugins list")
        ...
        try:
            ...
            _print_info("    Run later: roveagent auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `roveagent auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: roveagent auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: roveagent auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``roveagent tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``roveagent tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: roveagent tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``roveagent-discord``, which contains those
...
#:
#: Saving ``roveagent tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[roveagent-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``roveagent tools``
    ...
    """
    from roveagent.toolsets import resolve_toolset
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"roveagent-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (roveagent-acp, roveagent-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    from roveagent.toolsets import resolve_toolset, TOOLSETS
    ...
    # ``roveagent-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"roveagent-{platform}"
    ...
    # explicit config like ``[roveagent-cli, a2a]`` must survive filtering just
    ...
    # "roveagent-cli" (which include all _ROVEAGENT_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[roveagent-cli, spotify]`` after enabling Spotify via ``roveagent
        ...
        # (e.g. "roveagent-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``roveagent tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `roveagent tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"roveagent-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("roveagent-")}
    ...
    # shipped as a bundled plugin but user must opt in via `roveagent tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `roveagent tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by roveagent tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `roveagent config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `roveagent` instead of
    # `roveagent-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `roveagent update`/`roveagent doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `roveagent tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (roveagent-cli, roveagent-telegram, etc.)
    ...
    # Opening `roveagent tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key == "vision":
        try:
            from roveagent.core.auxiliary_client import resolve_vision_provider_client
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from roveagent.constants import roveagent_home_key

    scope = roveagent_home_key()

    try:
        # Trigger full tool discovery (imports all tool modules).
        from roveagent import model_tools as model_tools
...
def _plugin_video_gen_providers() -> list[dict]:
    ...
    """
    try:
        from roveagent.core.video_gen_registry import list_providers
        from roveagent.clisupport.plugins import _ensure_plugins_discovered
...
def _plugin_web_search_providers() -> list[dict]:
    ...
    """
    try:
        from roveagent.core.web_search_registry import list_providers as _list_web_providers
        from roveagent.clisupport.plugins import _ensure_plugins_discovered
...
def web_provider_capabilities(backend: str) -> list:
    ...
    """
    try:
        from roveagent.core.web_search_registry import get_provider
...
def _plugin_browser_providers() -> list[dict]:
    ...
    """
    try:
        from roveagent.core.browser_registry import list_providers as _list_browser_providers
        from roveagent.clisupport.plugins import _ensure_plugins_discovered
...
def _plugin_tts_providers() -> list[dict]:
    ...
    """
    try:
        from roveagent.core.tts_registry import _BUILTIN_NAMES, list_providers
        from roveagent.clisupport.plugins import _ensure_plugins_discovered
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `roveagent tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``roveagent tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``roveagent tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from roveagent.clisupport.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``roveagent tools post-setup`` process,
    ...
    bt = sys.modules.get("roveagent.tools.browser_tool")
    if bt is not None:
...
def _lightpanda_installed() -> bool:
    ...
    try:
        from roveagent.tools.browser_lightpanda import find_lightpanda_binary
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from roveagent.clisupport.nous_subscription import _has_agent_browser
...
def _web_tier_matches(provider: dict, config: dict) -> bool:
    ...
    try:
        from roveagent.core.web_search_provider import get_provider_env
...
def _is_provider_active(
    ...
    *,
    ...
    if provider.get("browser_backend"):
        ...
        try:
            from roveagent.tools.browser_use_cli import (
                _find_cli,
            ...
            if is_legacy_browser_use_cloud_config(browser_cfg or {}):
                return True
            try:
                from roveagent.tools.browser_camofox import is_camofox_mode
...
def _plugin_video_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from roveagent.core.video_gen_registry import get_provider
        from roveagent.clisupport.plugins import _ensure_plugins_discovered
...
# Kept in sync with the dashboard selects (roveagent_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `roveagent model`).
    if managed_feature:
        from roveagent.clisupport.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``roveagent model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``roveagent model`` aux-task
    ...
    """
    try:
        from roveagent.clisupport.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `roveagent model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `roveagent model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `roveagent tools` and `roveagent setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ RoveAgent Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://roveagent-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color(f"  Tool configuration saved to {display_roveagent_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'roveagent' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    try:
        from roveagent.tools.mcp_tool import probe_mcp_server_tools
    ...
    for server_name, tools in server_tools.items():
        ...
        try:
            from roveagent.tools.mcp_tool import matches_name_filter as _match_filter
