...
the tool registry or launching RoveAgent (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``roveagent-cli`` to the non-existent ``roveagent``. ``resolve_toolset('roveagent')``
...
from roveagent.clisupport.platforms import PLATFORMS
...
def _platform_default_toolset(platform: object) -> str:
    ...
    return info.default_toolset if info is not None else f"roveagent-{platform}"
...
def _platform_default_is_valid(
    ...
    # though their synthesized roveagent-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``roveagent-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``roveagent`` instead of ``roveagent-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `roveagent tools` to configure explicitly."
            )
            if platform_valid_count == 0:
                warnings.append(
                    ...
                    f"Run `roveagent tools` to reconfigure."
                )
            continue
        names = raw
        for name in names:
        ...
        if platform_valid_count == 0:
            ...
            warnings.append(
                ...
                f"tools on this platform. Run `roveagent tools` to reconfigure."
            )

    if valid_count == 0:
        warnings.append(
            "platform_toolsets resolves to zero valid toolsets — the agent will "
            "have no tools. Run `roveagent tools` to reconfigure."
        )
    return warnings
