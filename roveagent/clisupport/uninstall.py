"""
RoveAgent Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.roveagent/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove RoveAgent PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing roveagent-agent or roveagent PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# RoveAgent Agent" comment and following line
                if '# RoveAgent Agent' in line or '# roveagent-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('roveagent' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing roveagent
                if 'roveagent' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                from roveagent.utils import atomic_write_text

                # This is the user's own shell rc, not a RoveAgent-owned file, and
...
def remove_wrapper_script():
    """Remove the roveagent wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "roveagent",
        Path.home() / ".local" / "bin" / "roveagent-acp",
        Path.home() / ".local" / "bin" / "roveagent-agent",
        Path("/usr/local/bin/roveagent"),
        Path("/usr/local/bin/roveagent-acp"),
        Path("/usr/local/bin/roveagent-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains roveagent_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'roveagent_cli' in content or 'roveagent-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(roveagent_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``roveagent`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this RoveAgent home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (roveagent_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``roveagent gateway run`` processes
    ...
    try:
        from roveagent.clisupport.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from roveagent.clisupport.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ROVEAGENT_HOME`` and ``ROVEAGENT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\roveagent\git\cmd``, ``%LOCALAPPDATA%\roveagent\git\bin``,
#      ``%LOCALAPPDATA%\roveagent\git\usr\bin``, ``%LOCALAPPDATA%\roveagent\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\roveagent\git\`` and Node to
#      ``%LOCALAPPDATA%\roveagent\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``roveagent dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\roveagent\gateway-service\`` and sometimes
...
def _roveagent_path_markers(roveagent_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify RoveAgent-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the roveagent launchers and the managed uv) — only wanted when
    ...
    """
    root = str(roveagent_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare roveagent-agent install dir.
    markers = [root + "\\roveagent-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if ROVEAGENT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "roveagent".  We don't want to catch
    # unrelated entries like "croveagent-foo" or "ephermeral", so we look for
    # backslash-roveagent as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(roveagent_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip RoveAgent-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<roveagent_home>\\bin`` (the managed binary
    dir holding the roveagent launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _roveagent_path_markers(roveagent_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_roveagent_env_vars_windows() -> list[str]:
    """Delete ROVEAGENT_HOME and ROVEAGENT_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ROVEAGENT_HOME", "ROVEAGENT_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``roveagent`` launchers install.ps1 staged in the managed
    binary dir (the default RoveAgent root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``roveagent`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_roveagent_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from roveagent.clisupport._install_repair import _WINDOWS_BIN_LAUNCHERS
        from roveagent.constants import get_default_roveagent_root

        bin_dir = get_default_roveagent_root() / "bin"
    except Exception as e:
...
def _is_default_roveagent_home(roveagent_home: Path) -> bool:
    """Return True when ``roveagent_home`` points at the default (non-profile) root."""
    try:
        from roveagent.constants import get_default_roveagent_root
        return roveagent_home.resolve() == get_default_roveagent_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ROVEAGENT_HOME directory.

    We shell out to ``roveagent -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ROVEAGENT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m roveagent_cli.main` so we don't depend on a `roveagent`
    #    wrapper that may be half-removed mid-uninstall.
    roveagent_invocation = [_sys.executable, "-m", "roveagent.clisupport.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                roveagent_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's ROVEAGENT_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``roveagent uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$ROVEAGENT_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from roveagent.clisupport.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(roveagent_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ RoveAgent Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No RoveAgent Chat GUI installation was found.")
        print(f"  Checked: {roveagent_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The RoveAgent agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(roveagent_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The RoveAgent agent at {roveagent_home / 'roveagent-agent'}")
        print(f"  • Your config, sessions, and secrets under {roveagent_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The RoveAgent agent is still installed. Run 'roveagent' to use the CLI,")
    print("or 'roveagent uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.roveagent/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.roveagent/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie ROVEAGENT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_roveagent_home(roveagent_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.roveagent data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ RoveAgent Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {roveagent_home / 'config.yaml'}")
    print(f"  Secrets: {roveagent_home / '.env'}")
    print(f"  Data:    {roveagent_home / 'cron/'}, {roveagent_home / 'sessions/'}, {roveagent_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ROVEAGENT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL RoveAgent data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the RoveAgent code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, roveagent_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • RoveAgent PATH entries from shell configs / Windows User PATH")
    print("  • RoveAgent wrapper scripts and RoveAgent-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • RoveAgent config/data: {roveagent_home}")
        if _is_default_roveagent_home(roveagent_home):
        ...
        print(f"  • Keep RoveAgent config/data: {roveagent_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``roveagent`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$ROVEAGENT_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in roveagent_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\roveagent\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (roveagent\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_roveagent_home(roveagent_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(roveagent_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No RoveAgent-owned PATH entries in User environment")

        log_info("Removing ROVEAGENT_HOME / ROVEAGENT_GIT_BASH_PATH User env vars...")
        removed_env = remove_roveagent_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No RoveAgent-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing roveagent command...")
    ...
    #     would dangle — `roveagent` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows roveagent launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows roveagent launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this RoveAgent home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing RoveAgent-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(roveagent_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No RoveAgent-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(roveagent_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE ROVEAGENT_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from roveagent.clisupport.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(roveagent_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.roveagent/, just remove the roveagent-agent subdir
            if roveagent_home in project_root.parents or project_root.parent == roveagent_home:
    ...
    #     under ROVEAGENT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(roveagent_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(roveagent_home)
    ...
    # 5. Optionally remove ~/.roveagent/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ROVEAGENT_HOME dirs live under
        ...
        try:
            if roveagent_home.exists():
                shutil.rmtree(roveagent_home)
                log_success(f"Removed {roveagent_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {roveagent_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {roveagent_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {roveagent_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://roveagent-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://roveagent-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using RoveAgent Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m roveagent_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import roveagent_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``roveagent_constants`` + ``roveagent_cli.colors``
    (and lazily ``roveagent_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m roveagent_cli.uninstall")
    parser.add_argument(
        "--mode",
