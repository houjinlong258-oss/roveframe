"""RoveAgent update pipeline — extracted from ``roveagent_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``roveagent_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``roveagent_cli.main`` reference — so existing call sites and test monkeypatches
that target ``roveagent_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``roveagent_cli.main``.
...
Imports are one-way: ``roveagent_cli.main`` imports this module, never the reverse
...
from roveagent.constants import get_default_roveagent_root, venv_python_path

logger = logging.getLogger(__name__)


def _m():
    """Lazy ``roveagent_cli.main`` reference.

    Lets callers keep patching ``roveagent_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``roveagent_cli.main`` -> ``roveagent_cli.update_cmd`` stays one-way
    at import time.
    """
    from roveagent.clisupport import main
...
_UPDATE_RUNTIME_RELOAD_MODULES = (
    "roveagent_constants",
    "roveagent.tools.environments.local",
    "roveagent.tools.lazy_deps",
)
...
#: ``_purge_stale_roveagent_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        "roveagent_cli",
        "roveagent.clisupport.main",
        "roveagent.clisupport.update_cmd",
        "roveagent.clisupport.roveagent_logging",
    }
)


def _purge_stale_roveagent_modules() -> None:
    """Evict every cached RoveAgent module after the checkout changed in-place.

    ``roveagent update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from roveagent_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``roveagent_cli.gateway`` does
    ``from roveagent_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the RoveAgent
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale RoveAgent module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale RoveAgent modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``roveagent update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``roveagent update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``roveagent_cli.config_defaults``,
    ``roveagent_cli.config``, and ``roveagent_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``roveagent_cli._subprocess_compat`` and
    ``roveagent_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "roveagent.clisupport.config_defaults",
        "roveagent.clisupport.config",
        "roveagent.clisupport.config_migrations",
        "roveagent.clisupport._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from roveagent.clisupport.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from roveagent.clisupport.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``roveagent update`` historically
    ...
    caller): scope config reads/writes via the context-local ROVEAGENT_HOME
    ...
    try:
        ...
        from roveagent.clisupport.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_roveagent_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_roveagent_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'roveagent config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'roveagent config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'roveagent config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: roveagent config migrate")
        else:
            print()
            print("Skipped. Run 'roveagent config migrate' later to configure.")
    else:
    ...
    # context-local ROVEAGENT_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from roveagent.clisupport.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from roveagent.clisupport.backup import restore_cron_jobs_all_profiles
...
# Critical files that RoveAgent must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "roveagent_cli/main.py",
    "roveagent_cli/config.py",
    "roveagent_cli/__init__.py",
    "roveagent_cli/web_server.py",
    "cli.py",
    "roveagent.runtime.py",
    "roveagent.model_tools.py",
    "roveagent.toolsets.py",
    "roveagent.constants.py",
)
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``roveagent.exe`` has
    ...
    Skipping is safe because RoveAgent pins its editable finder to a *static*
...
def _validate_python_files_syntax(
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="roveagent-syntax-check-") as tmpdir:
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``roveagent`` startup; if any of them
...
def _critical_module_import_failures(
    ...
    'roveagent.tools.todo_tool'``. Every file is valid Python; the *combination* is not.
    ...
    ``_venv_core_imports_healthy``): ``roveagent update`` can be driven by a
    ...
    a later one.
    """
    from roveagent.constants import FIRST_PARTY_MODULE_ROOTS

    import secrets

    marker = f"__ROVEAGENT_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # The root set is injected from roveagent_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('roveagent_') or %r:\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``roveagent update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `roveagent update`.
    ...
    print("  Preview now:  roveagent curator run --dry-run")
    print("  Pause it:     roveagent curator pause")
    print(
        "  Docs:         https://roveagent-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `roveagent update`.
    ...
    try:
        ...
        return
    db_path = get_roveagent_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `roveagent sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    roveagent sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  roveagent sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``roveagent update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``roveagent update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: roveagent curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from roveagent_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    roveagent dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``roveagent_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.roveagent-update-staging"
    backup = f"{dst}.roveagent-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `roveagent update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``roveagent_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``roveagent update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && roveagent update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("ROVEAGENT_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== roveagent-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live RoveAgent launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["roveagent.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically RoveAgent")
    print("  Desktop, a gateway, or another roveagent REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `roveagent` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``roveagent.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.roveagent-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".roveagent-update-staging", ".roveagent-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `roveagent update`.")
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second RoveAgent process the updater's drain
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from roveagent.clisupport.backup import _foreign_db_holder_pids, verify_sqlite_integrity

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (roveagent gateway stop), "
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update RoveAgent Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `roveagent update "
            f"--branch {branch}`, or update against main with `roveagent update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = (
        f"https://github.com/NousResearch/roveagent-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="roveagent-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"roveagent-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for roveagent-agent itself
        ...
        # GitHub ZIPs extract to roveagent-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"roveagent-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # RoveAgent.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `roveagent update` to retry; if the agent won't start, "
            "reinstall from https://roveagent-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from roveagent.clisupport.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from roveagent.clisupport.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `roveagent update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: roveagent skills list-modified  "
    ...
    try:
        from roveagent.clisupport.model_catalog import seed_cache_from_checkout
    ...
    try:
        from roveagent.clisupport.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_roveagent_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_roveagent_home())
    ...
    try:
        from roveagent.clisupport.update_receipt import finalize_update_receipt
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "roveagent-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``roveagent update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the RoveAgent agent unexecutable.")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if RoveAgent behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes roveagent completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but RoveAgent couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but RoveAgent couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if RoveAgent behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but RoveAgent couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but RoveAgent couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/roveagent-agent.git",
    "git@github.com:NousResearch/roveagent-agent.git",
    "https://github.com/NousResearch/roveagent-agent",
    "git@github.com:NousResearch/roveagent-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/roveagent-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_roveagent_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_roveagent_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official RoveAgent repository.")
        print("  This means you may miss updates from NousResearch/roveagent-agent.")
        print()
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/NousResearch/roveagent-agent.git"
            )
            return False
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/roveagent-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/roveagent-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``roveagent update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from roveagent.constants import get_default_roveagent_root

    default_home = get_default_roveagent_root()
...
# ``fleet_restart_pending`` lives under ROVEAGENT_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """ROVEAGENT_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_roveagent_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from roveagent.clisupport.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `roveagent update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `roveagent update` or `roveagent gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every roveagent-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_roveagent_modules()
    except Exception:
        pass
    try:
        from roveagent.clisupport.gateway import (
            find_gateway_pids,
            is_macos,
            is_windows,
    ...
    try:
        ...
        if is_windows():
            try:
                from roveagent.clisupport import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``roveagent update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: roveagent gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "roveagent.exe"
    lines = ["✗ Another roveagent.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close RoveAgent Desktop, exit any open `roveagent` REPLs, and")
    lines.append("  stop the gateway (`roveagent gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `roveagent update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from roveagent.clisupport._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``roveagent update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a RoveAgent Desktop backend child, an unrelated ``roveagent`` REPL)
...
def _capture_active_lazy_features() -> list[str]:
    ...
    try:
        from roveagent.tools import lazy_deps
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``roveagent tools``."""
    try:
        from roveagent.clisupport import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active RoveAgent Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``roveagent tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("RoveAgent Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} RoveAgent Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``roveagent update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `roveagent update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `roveagent update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in RoveAgent' editable-install extras or
    ...
    try:
        from roveagent.clisupport.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from roveagent.clisupport.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$ROVEAGENT_HOME/bin/uv``, but on Termux the official
    ...
    """
    from roveagent.clisupport.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `roveagent update` is exactly the step
...
def _npm_lockfile_changed(roveagent_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `roveagent
    ...
    try:
        ...
        cache_file = roveagent_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(roveagent_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    roveagent update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `roveagent update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from roveagent.constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `roveagent update`.")
            failed = []
            if any(
    ...
    # RoveAgent profile using this checkout. Keep one per-checkout cache under the
    # shared RoveAgent root rather than rerunning npm once per named profile.
    shared_roveagent_root = get_default_roveagent_root()
    ...
    # `roveagent update` case. Synchronous and can block ~11s on a true cold
    # cache (~0.4s once warm) — print first so that doesn't look like a hang.
    print("→ Warming npx cache for agent-browser...")
    try:
        from roveagent.tools.browser_tool import warm_agent_browser_npx_cache
        warm_agent_browser_npx_cache()
    except Exception:
        pass

    if not _m()._npm_lockfile_changed(shared_roveagent_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `roveagent update`.")
        return list(labels)
    ...
    nixos_env = with_roveagent_node_path(_m()._nixos_build_env())
    ...
    # `roveagent update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_roveagent_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.roveagent/logs/update.log`` only, never the terminal.

    During ``roveagent update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``roveagent update --check``: fetch and report without installing.
    ...
    from roveagent.clisupport.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from roveagent.clisupport.banner import _github_compare_behind
            from roveagent.clisupport.config import recommended_update_command
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``roveagent update`` without requiring a reinstall.  The
    ...
    ``roveagent`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/roveagent.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v roveagent'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/roveagent, code at /usr/local/lib/roveagent-agent).
    fhs_link = Path("/usr/local/bin/roveagent")
    ...
    path_comment = (
        "# RoveAgent Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``roveagent-acp`` launcher next to the ``roveagent`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``roveagent update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``roveagent-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report RoveAgent as not installed even when it is.

    The shim simply delegates to the sibling ``roveagent`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``roveagent`` / ``roveagent-acp``
    launchers into the managed binary dir ``$RoveAgentHome\bin`` and puts THAT
    ...
    ``roveagent_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``roveagent-acp`` is already present next to the
    ``roveagent`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        roveagent_cmd = bin_dir / "roveagent"
        acp_cmd = bin_dir / "roveagent-acp"
        try:
            if not (roveagent_cmd.is_file() or roveagent_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of ROVEAGENT_HOME under
      ``backups/`` (restorable via ``roveagent import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from roveagent.clisupport.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from roveagent_constants import
        # get_roveagent_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from roveagent.clisupport.config import get_roveagent_home as _get_home
        ...
        try:
            from roveagent.clisupport.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from roveagent.clisupport.update_receipt import record_step
    ...
    try:
        from roveagent.clisupport.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from roveagent.clisupport.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_roveagent_home so the user sees ~/.roveagent/...
    try:
        from roveagent.constants import get_roveagent_home, display_roveagent_home

        home = get_roveagent_home()
        try:
            display_path = f"{display_roveagent_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  roveagent import {out_path}")
...
def _write_update_planned_stop_marker(profile_path: Path, pid: int) -> bool:
    ...
    try:
        ...
        from roveagent.gateway.status import _get_process_start_time
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``roveagent update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``roveagent update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run roveagent from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.roveagent-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".roveagent-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The roveagent.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m roveagent_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``roveagent
    ...
    skip.add(os.getpid())
    try:
        from roveagent.gateway.status import looks_like_gateway_command_line as _is_gw
    except Exception:
        _is_gw = None
    try:
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/RoveAgent candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "roveagent.exe"}
        ):
            continue
        ...
        # this venv's path, or a `-m roveagent_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "roveagent.clisupport.main" in cmdline_low:
            try:
        ...
        # the `-m roveagent_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``roveagent_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``roveagent.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``roveagent`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `roveagent` launch will complete it")
...
def _roveagent_holder_subcommand(cmdline: str) -> str | None:
    """The actual RoveAgent SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``roveagent_cli.main`` / ``roveagent(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("roveagent.clisupport.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("roveagent", "roveagent.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``roveagent dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other RoveAgent processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← RoveAgent backend (if the Desktop app is open, close it)",
        "dashboard": "  ← roveagent dashboard (stop it: roveagent dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _roveagent_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the RoveAgent desktop app / other RoveAgent terminals, then re-run:"
    )
    lines.append("    roveagent update")
    lines.append("  (or use `roveagent update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``roveagent update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from roveagent.clisupport._scan_venv_blockers import _is_pausable_gateway
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``roveagent update`` through its terminal tool.
    ...
    try:
        from roveagent.clisupport.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `roveagent update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    RoveAgent spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from roveagent.clisupport.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    try:
        ...
        if scripts_dir is not None:
            for name in ("roveagent.exe", "roveagent"):
                candidate = scripts_dir / name
                if candidate.is_file():
                    roveagent = str(candidate)
                    break
    except Exception:
        roveagent = None
    if roveagent is None:
        roveagent = "roveagent"
    for entry in entries:
        port = entry.get("port")
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (roveagent serve --host <ip> --port <port>)."
        )
    try:
        from roveagent.clisupport.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning roveagent-setup, and the update-in-progress marker
    ...
    the update with "RoveAgent is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a RoveAgent backend (``roveagent_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.roveagent-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "roveagent.clisupport.main" in argv_low and (
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # value round-trips through pid_is_roveagent at kill time. (Reading
...
def _ledger_reapable_backend_pids(
    ...
    (``roveagent_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of RoveAgent ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``roveagent.exe``) is open, NOTHING legitimate
    ...
    - Only RoveAgent backends (``roveagent_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``roveagent.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    ``.roveagent-runtime`` interpreter child alive and holding the install open
    (#70026). Best effort; never raises.
    """
    from roveagent.gateway.status import get_process_start_time
    from roveagent.clisupport._subprocess_compat import pid_is_roveagent, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_roveagent(
                pid,
                expected_start_time=expected_start_time,
            ):
                logger.debug(
                    "Skipping taskkill of non-RoveAgent or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``roveagent serve`` / ``roveagent dashboard`` argv.
    ...
    """
    if "roveagent.clisupport.main" not in (cmdline or "").lower():
        return False
    return _roveagent_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    roveagent.exe concurrent-instance guard does not see them. They still import
    ...
    try:
        from roveagent.gateway.status import get_process_start_time, terminate_pid
        from roveagent.clisupport.gateway import (
    ...
    for pid in running_pids:
        ...
        try:
            from roveagent.gateway.control_socket import pause_gateway_for_update
    ...
    print("→ Stopping Windows gateway process(es) before updating RoveAgent...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m roveagent_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: roveagent gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``roveagent gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from roveagent.clisupport import gateway_windows
        from roveagent.clisupport.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``roveagent-gateway*.service``/``roveagent-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("roveagent-serve")`` alone would also accept the
        # unrelated ``roveagent-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "roveagent-gateway.service"
            or unit.startswith("roveagent-gateway-")
            or unit == "roveagent-serve.service"
            or unit.startswith("roveagent-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``roveagent-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``roveagent-serve*`` units (#83438) don't, so sending them
    ...
    (``roveagent-gateway-helper`` is fine — profile units are
    ``roveagent-gateway-<profile>`` — but ``roveagent-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "roveagent-gateway" or svc_name.startswith("roveagent-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from roveagent.clisupport.gateway import is_macos
    ...
    if is_macos():
        ...
        print("    roveagent gateway status")
    ...
    print("    roveagent gateway status")
    if any(not name.startswith("ai.roveagent.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.roveagent.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: roveagent gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: roveagent gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.roveagent.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``roveagent_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``roveagent-gateway-foobar.service`` was already restarted.  These are
    ...
    if profile == "default":
        return name in {
            "roveagent-gateway",
            "ai.roveagent.gateway",
            "gateway",
            "gateway-default",
        }
    return name in {
        f"roveagent-gateway-{profile}",
        f"ai.roveagent.gateway-{profile}",
        f"gateway-{profile}",
    }
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``roveagent_cli.gateway`` in the failing
...
def _recover_gateway_restart_after_abort(
    ...
    ``roveagent update`` normally performs the fleet restart in the interpreter
    ...
    command = [
        sys.executable,
        "-m",
        "roveagent.clisupport.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["ROVEAGENT_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_ROVEAGENT_GATEWAY", "ROVEAGENT_GATEWAY", "ROVEAGENT_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``roveagent_cli.gateway`` from the freshly pulled checkout)
    ...
    print("    roveagent gateway restart")
    print("    roveagent gateway status")
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``roveagent update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``roveagent-setup.exe``) executes
    ``$ROVEAGENT_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_roveagent_home()) / "bootstrap-cache"
...
def _portable_git_candidates() -> list:
    ...
    The RoveAgent-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped ROVEAGENT_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``roveagent update`` must
    ...
    candidates = []
    try:
        for root in (get_default_roveagent_root(), Path(get_roveagent_home())):
            candidates.append(
...
def _locate_real_git() -> Optional[Path]:
    ...
    RoveAgent-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``roveagent update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``roveagent update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `roveagent desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "roveagent.clisupport.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the RoveAgent-managed Node on PATH: when
    # `roveagent update` runs inside the desktop updater chain (Desktop →
    # roveagent-setup → roveagent update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from roveagent.constants import with_roveagent_node_path

    build_env = with_roveagent_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `roveagent desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from roveagent.constants import display_roveagent_home as _dhh
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo roveagent`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``roveagent update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/roveagent`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave RoveAgent broken.")
    print("  This usually happens after running roveagent or pip with sudo. Offending paths:")
    for p, uid in foreign:
        print(f"    - {p} (owner uid {uid})")
    print("\n  Fix ownership, then re-run the update:")
    print(f"    sudo chown -R $(id -un): {project_root}")
    print("    roveagent update")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating RoveAgent Agent...")
    print()
    ...
    try:
        from roveagent.clisupport.update_receipt import begin_update_receipt
    ...
    # running RoveAgent runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another roveagent.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_roveagent_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `roveagent serve` backend) will keep .pyd
    ...
    # the roveagent.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned RoveAgent backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "RoveAgent backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .roveagent-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "RoveAgent is still
        ...
        if _venv_holders:
            ...
            # `roveagent serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving RoveAgent backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_roveagent_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} RoveAgent backend process(es) "
    ...
    # process by design (a CLI `roveagent update` IS the venv python), and an
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://roveagent-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by roveagent.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all RoveAgent windows/gateways and re-run: roveagent update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running RoveAgent gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in roveagent_cli/config.py bricked
        # every user who ran ``roveagent update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``roveagent
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``roveagent update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``roveagent update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``roveagent doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && roveagent update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``roveagent update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_roveagent_home added to roveagent_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``roveagent`` launch finishes the install
        ...
        # symbols in roveagent_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `roveagent` again "
        ...
        if not import_ok:
            ...
            print("    Run `roveagent update` again — if it persists, reinstall:")
            print("    https://roveagent-agent.nousresearch.com")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if RoveAgent re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.nousresearch.roveagent` (repeat "
        ...
        try:
            from roveagent.clisupport.macos_tcc_anchor import ensure_tcc_anchor
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_roveagent_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.roveagent/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit ROVEAGENT_HOME so
        # it is not affected by sync_skills()'s module-level ROVEAGENT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ROVEAGENT_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from roveagent.clisupport.profiles import backfill_profile_envs
        ...
        try:
            from roveagent.plugins.memory.honcho.cli import sync_honcho_profiles_quiet
        ...
        # check `roveagent curator status`. Self-stamps after printing so it
        ...
        # Self-heal the roveagent-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve RoveAgent on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("roveagent-acp launcher self-heal failed: %s", e)

        # Migrate the Windows roveagent launchers to the managed binary dir
        # (the default RoveAgent root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (roveagent-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `roveagent`
        ...
        # the managed clone under the default RoveAgent root).
        try:
            from roveagent.clisupport._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``roveagent update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from roveagent.clisupport.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `roveagent update`
                # must stay fast; `roveagent computer-use install --upgrade`
        ...
        # When running as ``roveagent update --gateway`` (spawned by the gateway's
        ...
        #
        # Purge stale cached RoveAgent modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... roveagent-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... roveagent-gateway*)
            ...
            try:
                from roveagent.clisupport.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all roveagent-gateway* units (default + profiles) plus
            # roveagent-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: roveagent gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # roveagent-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `roveagent update` restart it automatically, allow\n"
                        ...
                        # path in `roveagent gateway restart`
            ...
            # Restart EVERY ai.roveagent.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: roveagent gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: roveagent -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_roveagent_home() / ".update_exit_code"
                    try:
            ...
            try:
                from roveagent.clisupport.update_receipt import record_gateway_restart
        ...
        # Warn if legacy RoveAgent gateway unit files are still installed.
        # When both roveagent.service (from a pre-rename install) and the
        # current roveagent-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `roveagent update` surfaces the issue until the user migrates.
        try:
            from roveagent.clisupport.gateway import (
                has_legacy_roveagent_units,
                _find_legacy_roveagent_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_roveagent_units():
                print()
                print("⚠ Legacy RoveAgent gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_roveagent_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (roveagent.service) fight the current")
                print("  roveagent-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    roveagent gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes roveagent-serve*,
        ...
        print("  roveagent model              # Select provider and model")
        ...
        try:
            from roveagent.clisupport.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import roveagent.clisupport.update_receipt
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``roveagent update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from roveagent_cli.main import main; main()" update --yes'
                    )
            try:
                from roveagent.clisupport.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``roveagent_cli.gateway`` no longer imports,
