...
One read-only pass that answers, BEFORE any mutation: what RoveAgent runtimes
...
`detect_install_method`) — so `roveagent update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `roveagent update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `roveagent serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) RoveAgent runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "roveagent update"
...
def _detect_supervisor_for_pid(
    ...
    if pid in service_pids:
        try:
            from roveagent.clisupport.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"roveagent -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        ...
        try:
            from roveagent.clisupport.image_provenance import read_image_provenance
    ...
    try:
        from roveagent.clisupport.gateway import _get_service_pids
    ...
    try:
        from roveagent.clisupport.gateway import find_windows_gateway_services
    ...
    try:
        from roveagent.gateway.status import _pid_exists, read_runtime_status
        ...
        for profile, home in profile_homes:
            ...
            identity = None
            try:
                from roveagent.gateway.control_socket import identify_gateway
    ...
    try:
        from roveagent.clisupport.gateway import find_profile_gateway_processes
    ...
    # launched `roveagent serve --host <ip>` for a remote Desktop, or a
    # long-lived `roveagent dashboard`. Every serve/dashboard registers itself
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running RoveAgent services: none detected — code swap only.")
        return
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.profile in relaunched or r.profile in external:
                ...
                r.profile in unit or (r.profile == "default" and "roveagent-gateway" in unit)
                for unit in failed_set
            ):
                outcome = "failed"
            elif any(
                r.profile in svc or (r.profile == "default" and "roveagent-gateway" in svc)
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    print("      roveagent gateway restart                # active profile")
    print("      roveagent -p <profile> gateway restart   # named profile")
    return True
