"""Cross-process mutual exclusion for in-flight RoveAgent updates.

Three different surfaces can start an update of the same install tree:

* ``roveagent update`` from a terminal,
* the dashboard's Update button (``POST /api/roveagent/update`` →
  ``_spawn_roveagent_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``roveagent-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``roveagent update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ROVEAGENT_HOME>/.roveagent-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``roveagent update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("RoveAgent is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``roveagent-setup`` binary under ``~/.roveagent`` is only refreshed by a full
  installer run (``copy_self_to_roveagent_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("RoveAgent is still running") on every
  GUI update forever, with no RoveAgent process actually running.
"""
...
MARKER_NAME = ".roveagent-update-in-progress"

# Set by an orchestrating updater (the Tauri `roveagent-setup --update` flow) to
# its own pid before spawning `roveagent update` as a child stage. The parent
...
HANDOFF_PID_ENV = "ROVEAGENT_UPDATE_HANDOFF_PID"
...
# to show "RoveAgent is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* RoveAgent home (never the context-local profile override):
    the Rust updater resolves ``$ROVEAGENT_HOME`` or the platform default, and the
    ...
    return get_process_roveagent_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``roveagent update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``roveagent-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another RoveAgent update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `roveagent update` as a stage): we run under ITS claim
