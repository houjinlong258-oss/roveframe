...
1. **Update receipt** — a machine-readable JSON record of what one
   ``roveagent update`` run discovered, did, skipped (and why), written to
   ``<ROVEAGENT_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``roveagent_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``roveagent update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``roveagent update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_roveagent_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``roveagent update`` has many early-termination paths (Windows
...
def collect_fleet_versions(
    *, pre_restart_pids: Optional[list[int]] = None
    ...
    try:
        from roveagent.gateway.status import read_runtime_status, runtime_status_pid_is_live
