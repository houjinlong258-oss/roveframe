"""``roveagent verify`` — detect a project's run recipe and smoke-test it.
...
``.roveagent/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``roveagent verify`` satisfies
...
def run_verify_command(args) -> int:
    from roveagent.core.verify import (
        load_or_detect,
        manifest_path,
        run_verify,
        save_manifest,
...
def _merge_project_facts_commands(root: Path, recipe) -> None:
    ...
    """
    try:
        from roveagent.core.coding_context import detect_project_facts
...
def _record_evidence(root: Path, recipe, result, *, partial: bool) -> None:
    ...
    """
    try:
        from roveagent.core.verification_evidence import record_verify_run
