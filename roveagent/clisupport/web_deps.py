...
Why this exists
---------------
``roveagent_cli/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``roveagent_cli/web_routers/`` need those helpers, but
...
thin proxy that resolves ``roveagent_cli.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``roveagent_cli.web_server`` module (imported on demand)."""
    mod = sys.modules.get("roveagent.clisupport.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import roveagent.clisupport.web_server
