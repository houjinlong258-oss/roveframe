"""Pydantic request/response models for the RoveAgent dashboard web server.

Extracted verbatim from ``roveagent_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from roveagent_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``roveagent model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `roveagent import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`roveagent -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the ROVEAGENT_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under ROVEAGENT_HOME.
    output: str = ""
