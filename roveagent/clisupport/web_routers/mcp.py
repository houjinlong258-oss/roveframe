...
web_server - reached via the late-binding seam in :mod:`roveagent_cli.web_deps`
...
async def list_mcp_servers(profile: Optional[str] = None):
    from roveagent.clisupport.mcp_config import _get_mcp_servers
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from roveagent.clisupport.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from roveagent.clisupport.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the ROVEAGENT_HOME
...
async def auth_mcp_server(name: str, request: Request, profile: Optional[str] = None):
    ...
    from roveagent.clisupport.mcp_config import _get_mcp_servers
    from roveagent.tools.mcp_dashboard_oauth import DashboardOAuthFlow

    _require_token(request)
    _gc_mcp_oauth_flows()
    from roveagent.constants import get_roveagent_home

    process_home = str(get_roveagent_home().expanduser().resolve(strict=False))

    def _read():
        with _profile_scope(profile):
            return _get_mcp_servers(), str(get_roveagent_home().expanduser().resolve(strict=False))
    ...
    with _mcp_oauth_flows_lock:
        ...
        if any(
            ...
            and flow.roveagent_home == flow_home
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to RoveAgent and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to RoveAgent for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to RoveAgent.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `roveagent mcp catalog` / `roveagent mcp install` read.  ``profile`` scopes
    the installed/enabled annotations (the catalog itself is repo-shipped
    and identical for every profile).
    """
    try:
        from roveagent.clisupport import mcp_catalog
    except Exception as exc:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    from roveagent.clisupport.config import validate_env_var_name_for_write
    ...
    # The -p subprocess rebinds ROVEAGENT_HOME-derived paths in the child.
