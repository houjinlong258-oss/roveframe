...
from roveagent.state import is_malformed_db_error
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "`roveagent doctor` for diagnosis."
            ),
        ) from exc
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another RoveAgent surface. FTS can't find those unless the
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole RoveAgent backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `roveagent sessions stats`).
...
async def get_session_messages(
    ...
    from roveagent.core.compaction_display import project_compaction_message_for_display
