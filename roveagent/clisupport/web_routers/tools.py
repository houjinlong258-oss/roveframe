...
late-binding seam in :mod:`roveagent_cli.web_deps` (``late`` for callables,
...
from roveagent.clisupport.web_deps import late, LateState
from roveagent.clisupport.web_models import (
...
_log = logging.getLogger("roveagent.clisupport.web_server")
...
_spawn_roveagent_action = late("_spawn_roveagent_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from roveagent.clisupport.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `roveagent tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `roveagent computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``roveagent tools`` picker shows
    ...
    def _read():
        with _profile_scope(profile):
            ...
            if name == "web":
                ...
                try:
                    from roveagent.tools.web_tools import _get_extract_backend, _get_search_backend
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `roveagent tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``roveagent tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.roveagent/.env`` via ``save_env_value`` —
    the same store ``roveagent tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``roveagent tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``roveagent -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same ROVEAGENT_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``roveagent computer-use permissions grant`` as a background action.
