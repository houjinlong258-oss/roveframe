"""
RoveAgent Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m roveagent_cli.main web          # Start on http://127.0.0.1:9119
    python -m roveagent_cli.main web --port 8080
"""
...
from roveagent.clisupport._subprocess_compat import windows_detach_flags, windows_hide_flags
from roveagent.clisupport.install_identity import get_install_id as _shared_get_install_id
...
from roveagent.plugins.memory.config_schema import (
    ProviderConfigSchema,
    ProviderField,
    STORAGE_HONCHO_HOST_BLOCK,
    get_provider_config_schema,
)
from roveagent.gateway.status import (
    derive_gateway_busy,
...
from roveagent.utils import env_var_enabled
...
try:
    ...
    # running `roveagent dashboard` needs fastapi+uvicorn; lazy install keeps
    # them out of every other install path. After install, re-import.
    try:
        from roveagent.tools.lazy_deps import ensure as _lazy_ensure
...
WEB_DIST = Path(os.environ["ROVEAGENT_WEB_DIST"]) if "ROVEAGENT_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``roveagent gateway run`` — but the
    desktop app spawns a ``roveagent dashboard`` backend, not a gateway, so a cron
    ...
    """
    from roveagent.cron.scheduler_provider import InProcessCronScheduler, resolve_cron_scheduler
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from roveagent.clisupport.profiles import profiles_to_serve

            profile_homes = list(profiles_to_serve(multiplex=True))
            if len(profile_homes) > 1:
                start_kwargs["profile_homes"] = profile_homes
                from roveagent.logsetup import enable_profile_log_routing
...
def _warm_gateway_module() -> None:
    ...
    ``roveagent_cli.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "roveagent.clisupport.auth",
        "roveagent.clisupport.copilot_auth",
        "roveagent.clisupport.runtime_provider",
        ...
        "roveagent.clisupport.skin_engine",
        # model.options / picker context — parses provider catalogs and
        # the models.dev cache on first use.
        "roveagent.clisupport.inventory",
        "roveagent.clisupport.model_switch",
    ):
        try:
            __import__(mod)
        except Exception:
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from roveagent.clisupport.gateway import _get_restart_drain_timeout
        return _get_restart_drain_timeout()
    except ImportError:
        from roveagent.gateway.restart import DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `roveagent update` current before the
    ...
    """
    try:
        from roveagent.state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `roveagent update` kept 500ing every
    ...
    # Import roveagent_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `roveagent update` replaced the code
    ...
    from roveagent.gateway.code_skew import record_boot_fingerprint
    ...
    # Desktop-spawned backends (ROVEAGENT_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `roveagent
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ROVEAGENT_DESKTOP") == "1":
        ...
        # a healthy standalone gateway (launched via `roveagent gateway run`,
        ...
        try:
            from roveagent.clisupport.gateway import _reap_unsupervised_gateway_orphans
...
app = FastAPI(title="RoveAgent Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from roveagent.clisupport.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ROVEAGENT_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("ROVEAGENT_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-RoveAgent-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".roveagent-ssh-runtime-{nonce}")
...
#
# This list is defined in ``roveagent_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from roveagent.clisupport.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-RoveAgent-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already RoveAgent' canonical browser-facing URL behind a
    ...
    """
    from roveagent.clisupport.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``roveagent-0day``
...
def _desktop_loopback_auth_exempt(
    ...
    for EVERY ``roveagent serve`` on the machine — including the private loopback
    backends the Desktop app spawns for itself. Those backends authenticate
    with the per-spawn session token (injected via
    ``ROVEAGENT_DASHBOARD_SESSION_TOKEN`` for local spawns, ``--ssh-session-token
    ...
    Exemption requires ALL of: loopback bind, ``ROVEAGENT_DESKTOP=1`` (set by
    ...
    ``roveagent serve`` with ``ROVEAGENT_DESKTOP=1`` exported but no credential is
    NOT exempt.
    """
    ...
    if os.environ.get("ROVEAGENT_DESKTOP") != "1":
        return False
    return bool(
        os.environ.get("ROVEAGENT_DASHBOARD_SESSION_TOKEN")
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from roveagent.clisupport.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from roveagent.clisupport.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from roveagent.clisupport.dashboard_auth.token_auth import token_auth_middleware
...
def _memory_provider_options() -> List[str]:
    ...
    options = [""]
    try:
        from roveagent.plugins.memory import list_memory_provider_names
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `roveagent egress setup` "
            "and `roveagent egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates RoveAgent (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during roveagent update. "
    ...
    "roveagent.plugins.hook_callback_timeout": {
        "type": "number",
        "description": (
...
def _custom_provider_options(
    ...
    *kind* is ``"tts"`` or ``"stt"``. The result keeps the built-in display
    ...
    if kind == "tts":
        from roveagent.tools.tts_tool import BUILTIN_TTS_PROVIDERS as _runtime_builtins
    else:
        from roveagent.tools.transcription_tools import BUILTIN_STT_PROVIDERS as _runtime_builtins
    ...
    try:
        if kind == "tts":
            from roveagent.core.tts_registry import list_providers as _list_voice_providers
        else:
            from roveagent.core.transcription_registry import list_providers as _list_voice_providers
...
from roveagent.clisupport.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real RoveAgent provider slug — fine.
    ...
    1. Vendor-name → RoveAgent-provider mapping: when the provider string is
       not a known RoveAgent provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from roveagent.clisupport.config import get_compatible_custom_providers
    from roveagent.clisupport.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from roveagent.clisupport.model_normalize import normalize_model_for_provider
    from roveagent.clisupport.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known RoveAgent
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from roveagent.clisupport.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    RoveAgent process is using it.
    """
    from roveagent.state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "ROVEAGENT_DASHBOARD_FILES_ROOT"
...
# ROVEAGENT_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to ROVEAGENT_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical RoveAgent credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no ROVEAGENT_HOME resolution, so it blocks these trees wherever
...
def _default_roveagent_root_is_opt_data() -> bool:
    raw = os.environ.get("ROVEAGENT_HOME", "").strip()
    if not raw:
        return False
    try:
        from roveagent.constants import get_default_roveagent_root

        root = get_default_roveagent_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``roveagent update``.
    ...
    a container — e.g. the roveagent-webui image sharing the RoveAgent source tree),
    the dashboard's ``roveagent update`` button is the correct update path and
    ...
    """
    if _default_roveagent_root_is_opt_data():
        return True
    try:
        from roveagent.constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's RoveAgent root is actually /opt/data
    # (the container/hosted layout) or when ROVEAGENT_DASHBOARD_FILES_ROOT is set.
    if _default_roveagent_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs RoveAgent inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``roveagent:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.roveagent-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``roveagent_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from roveagent.clisupport import web_git as _web_git  # noqa: E402
...
from roveagent.clisupport.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from roveagent.clisupport.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _profile_platform_ports(profile_home: Path, runtime: Optional[dict]) -> Dict[str, int]:
    ...
    try:
        ...
        from roveagent.clisupport.config import read_user_config_raw
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from roveagent.clisupport.profiles import _check_gateway_running, profiles_to_serve
        from roveagent.gateway.status import read_runtime_status
...
# install, minted on first read and persisted under the ROOT RoveAgent home
# (get_default_roveagent_root()) — NOT the profile-scoped ROVEAGENT_HOME — so every
...
def _load_configured_gateway_platforms() -> set[str]:
    ...
    """
    from roveagent.gateway.config import load_gateway_config
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors roveagent_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_roveagent_home() at call time
    ...
    try:
        ...
        # ROVEAGENT_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of roveagent_cli.gateway blocks the
        ...
        # and which providers are registered so ``roveagent status`` and the
        ...
        auth_flows: list[str] = []
        try:
            from roveagent.clisupport.dashboard_auth import (
        ...
        try:
            from roveagent.clisupport.auth import get_nous_session_validity
        ...
        status = {
            ...
            "can_update_roveagent": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        try:
            from roveagent.gateway.readiness import _probe_state_db

            storage_check = await run_in_threadpool(_probe_state_db, get_roveagent_home())
        ...
        try:
            from roveagent.gateway.memory_status import collect_memory_status

            status["memory"] = await run_in_threadpool(
                collect_memory_status,
                profile_dir if profile_dir else get_roveagent_home(),
            )
        except Exception:
            status["memory"] = {"pressure": "unknown"}

        # Disk-usage rollup (NS-656, same lineage as OOF-2/OOF-107 fleet
        # disk-exhaustion incidents). One statvfs call on ROVEAGENT_HOME's
        ...
        try:
            from roveagent.gateway.disk_status import collect_disk_status
        ...
        try:
            from roveagent.state import SessionDB as _SDB
            from roveagent.constants import get_roveagent_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — RoveAgent Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the roveagent home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_roveagent_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def get_curator_status():
    try:
        from roveagent.core import curator
    except Exception as exc:
...
async def run_curator():
    ...
    try:
        proc = _spawn_roveagent_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_learning_graph(profile: Optional[str] = None):
    ...
    """
    def _run():
        from roveagent.core.learning_graph import build_learning_graph
...
async def get_learning_node(id: str, profile: Optional[str] = None):
    ...
    from roveagent.core.learning_mutations import node_detail
...
async def delete_learning_node(body: LearningNodeRef):
    ...
    from roveagent.core.learning_mutations import delete_node
...
async def update_learning_node(body: LearningNodeEdit):
    ...
    from roveagent.core.learning_mutations import edit_node
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from roveagent.clisupport.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from roveagent.clisupport.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_roveagent_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_roveagent_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_roveagent_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from roveagent.clisupport.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.roveagent/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_roveagent_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "roveagent-update": "roveagent-update.log",
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(
    r"^=== roveagent-update completed ([0-9a-f]{32}) ===$"
)
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('roveagent.clisupport.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_roveagent_action(
    subcommand: List[str],
    name: str,
    *,
    env_overrides: Optional[Dict[str, str]] = None,
) -> subprocess.Popen:
    """Spawn ``roveagent <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``roveagent_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "roveagent.clisupport.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _ROVEAGENT_GATEWAY=1. Inheriting it makes a spawned `roveagent gateway restart`
    ...
    action_env = {**os.environ, "ROVEAGENT_NONINTERACTIVE": "1"}
    action_env.pop("_ROVEAGENT_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("ROVEAGENT_ACTION_ID")
    if action_id:
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== roveagent update started "):
            last_start = index
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["roveagent", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``roveagent gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``roveagent gateway restart`` children race each other on the
    ...
    proc = _spawn_roveagent_action(subcommand, "gateway-restart")
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``roveagent gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``ROVEAGENT_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
    ...
    """
    from roveagent.gateway.drain_control import (
        clear_drain_request,
        drain_requested,
        write_drain_request,
    )
...
@app.post("/api/roveagent/update")
async def update_roveagent():
    """Kick off ``roveagent update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "RoveAgent updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("roveagent-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "roveagent-update",
    ...
    if refusal is not None:
        _record_completed_action("roveagent-update", refusal.message, exit_code=1)
    ...
    existing = _ACTION_PROCS.get("roveagent-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("roveagent-update")
        if action_id:
    ...
    try:
        proc = _spawn_roveagent_action(
            ["update"],
            "roveagent-update",
            env_overrides={"ROVEAGENT_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn roveagent update")
...
@app.get("/api/roveagent/update/check")
async def check_roveagent_update(force: bool = False):
    """Report whether a RoveAgent update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/roveagent/update`` actually runs ``roveagent update``.

    Returns:
        install_method: 'apt' | 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed RoveAgent version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "RoveAgent is managed by Termux APT; run `pkg upgrade roveagent-agent`."
        )
        return payload
    ...
    try:
        from roveagent.clisupport.banner import check_for_updates

        if force:
            try:
                (get_roveagent_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def transcribe_audio_upload(
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            prefix="roveagent-desktop-voice-",
            suffix=suffix,
            delete=False,
        ) as tmp:
        ...
        from roveagent.tools.voice_mode import transcribe_recording
...
async def get_client_voice_config(profile: Optional[str] = None):
    ...
    """
    from roveagent.tools.voice_client_config import resolve_client_voice_config
...
async def get_elevenlabs_voices(profile: Optional[str] = None):
    ...
    if not api_key:
        ...
        try:
            from roveagent.core.secret_scope import UnscopedSecretError, get_secret
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.roveagent/config.yaml`` under ``tts.``.
    """
    ...
    try:
        from roveagent.tools.tts_tool import text_to_speech_tool
...
def _split_text_for_speak_stream(text: str, cap: int) -> list:
    ...
    """
    from roveagent.tools.tts_streaming import SENTENCE_BOUNDARY_RE as _SENTENCE_BOUNDARY_RE
...
async def speak_stream_ws(ws: "WebSocket") -> None:
    ...
    def _resolve():
        from roveagent.tools.tts_streaming import resolve_streaming_provider
        from roveagent.tools.tts_tool import _get_provider, _load_tts_config, _resolve_max_text_length
    ...
    def _produce():
        from roveagent.tools.tts_streaming import SentenceChunker
        from roveagent.tools.tts_tool import _strip_markdown_for_tts
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "roveagent-update":
        ...
        if durable_update_action_id:
            marker = f"=== roveagent-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``roveagent
    ...
    """
    try:
        from roveagent.clisupport.update_receipt import read_latest_receipt
...
@app.get("/api/roveagent/update/receipt")
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `roveagent update` run recorded).",
        )
...
from roveagent.clisupport.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from roveagent.clisupport.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from roveagent.clisupport.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from roveagent.clisupport.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    RoveAgent supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_roveagent_home() / provider.name / "config.json"
...
def _honcho_resolvers():
    ...
    from roveagent.plugins.memory.honcho.client import _host_block, resolve_active_host, resolve_config_path
...
def _write_provider_flat(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    from roveagent.utils import atomic_json_write
...
def _write_provider_honcho(provider: ProviderConfigSchema, values: Dict[str, str]) -> None:
    ...
    from roveagent.plugins.memory.honcho.oauth import ACCESS_TOKEN_PREFIX, _config_refresh_lock
    from roveagent.utils import atomic_json_write
...
def _load_memory_provider(name: str):
    try:
        from roveagent.plugins.memory import load_memory_provider
...
def _memory_provider_manifest(name: str) -> Dict[str, Any]:
    try:
        from roveagent.plugins.memory import find_provider_dir
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/roveagent is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (ROVEAGENT_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
    ...
    try:
        from roveagent.tools.lazy_deps import install_specs
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    roveagent_home = get_roveagent_home()
    values: Dict[str, Any] = {}

    # Common native provider stores.
    for path in (
        roveagent_home / f"{name}.json",
        roveagent_home / name / "config.json",
    ):
    ...
    # Holographic stores under plugins.roveagent-memory-store.
    plugins_cfg = cfg.get("plugins") if isinstance(cfg, dict) else {}
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("roveagent-memory-store")
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            from roveagent.core.memory_provider import MemoryProvider as _BaseMemoryProvider
        except Exception:
            provider.save_config(values, str(get_roveagent_home()))
            return
...
def _discover_memory_provider_statuses() -> List[Dict[str, Any]]:
    ...
    try:
        from roveagent.plugins.memory import discover_memory_providers
...
async def get_egress_status():
    ...
    from roveagent.clisupport.proxy_cli import format_status_text
...
def get_model_info(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from roveagent.core.model_metadata import get_model_context_length
        ...
        caps = {}
        try:
            from roveagent.core.models_dev import get_model_capabilities
...
# in roveagent_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    boot.  When ``roveagent update`` (or a manual ``git pull``) replaces the
    ...
    positive).
    """
    from roveagent.gateway.code_skew import detect_code_skew
    ...
    return (
        ...
        f"(systemctl --user restart roveagent-dashboard, or roveagent dashboard --port <port>)"
    )
...
async def get_model_options(
    ...
    try:
        ...
        from roveagent.clisupport.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `roveagent model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from roveagent.clisupport.auth import get_provider_auth_state
    ...
    try:
        from roveagent.clisupport.inventory import build_models_payload, load_picker_context
        from roveagent.clisupport.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from roveagent.clisupport.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from roveagent.clisupport.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.roveagent/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from roveagent.clisupport.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (roveagent_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from roveagent.clisupport.nous_subscription import apply_nous_managed_defaults
                from roveagent.clisupport.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``roveagent model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from roveagent.clisupport.main import _auto_provider_name, _save_custom_provider
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different ROVEAGENT_HOME than this process's gateway sessions.
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_roveagent_home().resolve()
...
def _approval_mode_of(config: Dict[str, Any]) -> str:
    ...
    """
    from roveagent.tools.approval import _normalize_approval_mode
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``roveagent model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from roveagent.clisupport.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from roveagent.clisupport.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `roveagent model` provider — would be
        ...
        # Vertex — a `roveagent model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `roveagent model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from roveagent.clisupport.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level ROVEAGENT_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from roveagent.clisupport.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run RoveAgent from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect RoveAgent to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use RoveAgent from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect RoveAgent to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use RoveAgent in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use RoveAgent through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from RoveAgent via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to RoveAgent through an IMAP/SMTP mailbox.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect RoveAgent to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use RoveAgent inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect RoveAgent to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use RoveAgent through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect RoveAgent to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect RoveAgent to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and RoveAgent.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use RoveAgent from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with RoveAgent over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use RoveAgent through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to RoveAgent over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect RoveAgent to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose RoveAgent as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use RoveAgent via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the RoveAgent Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in roveagent gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in roveagent gateway setup",
...
def _messaging_platform_catalog() -> tuple[dict[str, Any], ...]:
    ...
    """
    from roveagent.gateway.config import Platform
...
# roveagent_cli/setup_hidden_env.py. Shared with the `roveagent setup gateway`
# wizard so the surfaces ask for the same things.
from roveagent.clisupport.setup_hidden_env import (  # noqa: E402
...
def _gateway_platform_config(platform_id: str):
    from roveagent.gateway.config import Platform, load_gateway_config
...
def _messaging_platform_payload(
    ...
    # ROVEAGENT_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from roveagent.constants import get_roveagent_dir

    return get_roveagent_dir("platforms/whatsapp/session", "whatsapp/session")
...
def _ensure_whatsapp_bridge_dependencies(bridge_dir: Path) -> None:
    ...
    from roveagent.utils import env_int
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.roveagent-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"RoveAgentDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "RoveAgent Agent").strip() or "RoveAgent Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, RoveAgent appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the ROVEAGENT_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    """
    from roveagent.gateway.config import PORT_BINDING_PLATFORM_VALUES, load_gateway_config
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized ROVEAGENT_HOME)
...
# the canonical ``roveagent auth add <provider>`` command so the dashboard can
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.roveagent/.anthropic_oauth.json`` — RoveAgent-managed terminal PKCE
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_roveagent_dotenv()``, so the same check covers them)
    ...
    try:
        from roveagent.core.anthropic_adapter import (
            read_roveagent_oauth_credentials,
            _get_roveagent_oauth_file,
        )
    except ImportError:
        read_roveagent_oauth_credentials = None  # type: ignore
        _get_roveagent_oauth_file = None  # type: ignore

    roveagent_creds = None
    if read_roveagent_oauth_credentials:
        try:
            roveagent_creds = read_roveagent_oauth_credentials()
        except Exception:
            roveagent_creds = None
    if roveagent_creds and roveagent_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "roveagent_pkce",
            "source_label": f"RoveAgent PKCE ({_get_roveagent_oauth_file() if _get_roveagent_oauth_file else None})",
            "token_preview": _truncate_token(roveagent_creds.get("accessToken")),
            "expires_at": roveagent_creds.get("expiresAt"),
            "has_refresh_token": bool(roveagent_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from roveagent.clisupport.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into RoveAgent even
    when they also have a separate RoveAgent-managed PKCE login.
    """
    try:
        from roveagent.core.anthropic_adapter import read_claude_code_credentials
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    RoveAgent never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "roveagent auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "roveagent auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "roveagent auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "roveagent auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "roveagent auth add xai-oauth",
        "docs_url": "https://roveagent-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # server). RoveAgent previously reimplemented that subscription-OAuth PKCE
    ...
    # credentials. Login still works via the terminal (`roveagent auth add
    # anthropic`, unaffected by this change) or a plain API key below.
    {
        "id": "anthropic",
        "name": "Anthropic API Key",
        "flow": "external",
        "cli_command": "roveagent auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from roveagent.clisupport import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside RoveAgent, so the disconnect
    ...
    user sees exactly what executes, and RoveAgent then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # catalog entry) but, unlike other external providers, RoveAgent still OWNS
    # the credential it can show here: the RoveAgent-managed PKCE file
    # (~/.roveagent/.anthropic_oauth.json) and its credential-pool entry, both
    # written by `roveagent auth add anthropic` in the terminal. Those are ours
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside RoveAgent — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``roveagent model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `roveagent model` universe (zero-edit for new plugins).
    try:
        from roveagent.clisupport.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"roveagent auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("roveagent_pkce", "claude_code", ...)
    ...
    sync with the `roveagent model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the RoveAgent-managed PKCE file and auth-store entry.
            ...
            if provider_id == "anthropic":
                cleared = False
                try:
                    from roveagent.core.anthropic_adapter import _get_roveagent_oauth_file
                    oauth_file = _get_roveagent_oauth_file()
                ...
                try:
                    from roveagent.clisupport.auth import clear_provider_auth
            ...
            try:
                from roveagent.clisupport.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
# points at `roveagent auth add anthropic` (terminal PKCE, unaffected) instead.
#
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ROVEAGENT_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from roveagent.clisupport.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from roveagent.clisupport.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``roveagent auth add minimax-oauth``.
    """
    from roveagent.clisupport.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `roveagent auth add xai-oauth`: first credential may become
            ...
            # prior ``roveagent auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``roveagent model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before RoveAgent "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from roveagent.clisupport.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `roveagent update` until the first message forced a writable open.
    """
    from roveagent.state.schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from roveagent.state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``roveagent serve`` — it runs neither the
    ...
    try:
        ...
        from roveagent.clisupport.config import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `roveagent sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_roveagent_home()
...
async def get_logs(
    ...
    from roveagent.clisupport.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_roveagent_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from roveagent.logsetup import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    """
    from roveagent.clisupport import profiles as profiles_mod
    try:
        return [
            {
                "name": name,
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (ROVEAGENT_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.roveagent``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    ROVEAGENT_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ROVEAGENT_HOME)."""
    from roveagent.clisupport import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["roveagent_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from roveagent.cron import jobs as cron_jobs
    from roveagent.constants import (
        reset_roveagent_home_override,
        set_roveagent_home_override,
    )

    token = set_roveagent_home_override(str(home))
    try:
        with cron_jobs.use_cron_store(home):
            if func_name == "create_job":
                from roveagent.cron.scheduler import create_job_with_scheduler_registration

                result = create_job_with_scheduler_registration(*args, **kwargs)
            else:
                result = getattr(cron_jobs, func_name)(*args, **kwargs)
    finally:
        reset_roveagent_home_override(token)
...
def _notify_cron_provider_for_profile(target_profile: Optional[str]) -> None:
    ...
    try:
        ...
        from roveagent.cron import jobs as cron_jobs
        from roveagent.cron.scheduler_provider import (
...
def _raise_if_cron_registration_error(e: Exception) -> None:
    ...
    """
    from roveagent.cron.scheduler import CronSchedulerRegistrationError

    if isinstance(e, CronSchedulerRegistrationError):
        raise HTTPException(status_code=424, detail=e.to_dict()) from e


from roveagent.clisupport.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from roveagent.clisupport.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/roveagent_cli/
        # test_config_read_guard.py). The ROVEAGENT_HOME override scopes
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``roveagent gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (roveagent_cli.mcp_config), so
# servers managed here show up under `roveagent mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from roveagent.clisupport.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from roveagent.clisupport.mcp_security import validate_mcp_server_entry
...
from roveagent.clisupport.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from roveagent.clisupport.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from roveagent.clisupport.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.roveagent_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        from roveagent.core.secret_scope import (
            build_profile_secret_scope,
            reset_secret_scope,
            set_secret_scope,
        )
        from roveagent.constants import reset_roveagent_home_override, set_roveagent_home_override
        from roveagent.tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from roveagent.tools.mcp_oauth import RoveAgentTokenStorage, force_interactive_oauth
        from roveagent.tools.mcp_oauth_manager import get_manager

        home_token = set_roveagent_home_override(flow.roveagent_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.roveagent_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = RoveAgentTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        roveagent_home=flow.roveagent_home,
                    )
                    ...
                    if flow.reconnect_live:
                        from roveagent.tools.mcp_tool import reconnect_mcp_server
            ...
            reset_roveagent_home_override(home_token)
    except Exception as exc:
        msg = str(exc)
        ...
        try:
            from roveagent.tools.mcp_oauth import humanize_oauth_registration_error
...
def _pairing_store(profile: Optional[str] = None):
    ...
    the ``await`` boundary.
    """
    from roveagent.gateway.pairing import PairingStore
...
#
# Wraps the same JSON store the CLI uses (roveagent_cli.webhook); the webhook
...
async def list_webhooks():
    import roveagent.clisupport.webhook
...
# spawn the real `roveagent gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_roveagent_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_roveagent_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    from roveagent.core.credential_pool import load_pool
    from roveagent.clisupport.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    from roveagent.core.credential_pool import (
        load_pool,
        PooledCredential,
        AUTH_TYPE_API_KEY,
        CUSTOM_POOL_PREFIX,
    ...
    try:
        ...
        # (via DELETE below or `roveagent auth remove`) can seed again.
        # Mirrors the `roveagent auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``roveagent auth remove`` uses: each source cleans up its
    ...
    """
    from roveagent.core.credential_pool import load_pool
    from roveagent.core.credential_sources import find_removal_step
    from roveagent.clisupport.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_roveagent_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_roveagent_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_roveagent_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"roveagent-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_roveagent_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_roveagent_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    """
    def _run():
        from roveagent.clisupport.config import load_config as _load_config
        from roveagent.core import shell_hooks

        try:
            from roveagent.clisupport.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <roveagent_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_roveagent_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_roveagent_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``roveagent`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_roveagent_action`` tracks one process/log per name, so a shared
...
from roveagent.clisupport.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from roveagent.clisupport.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _installed_hub_identifiers(profile: Optional[str] = None) -> dict:
    ...
    """
    try:
        from roveagent.tools.skills_hub import HubLockFile
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_roveagent_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "roveagent setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local ROVEAGENT_HOME override so the write lands in the target
    ...
    """
    from roveagent.constants import set_roveagent_home_override, reset_roveagent_home_override

    token = set_roveagent_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local ROVEAGENT_HOME override (same mechanism as
    ...
    """
    from roveagent.constants import set_roveagent_home_override, reset_roveagent_home_override
    from roveagent.clisupport.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the ROVEAGENT_HOME override. Returns the
    number of skills newly disabled.
    """
    from roveagent.constants import set_roveagent_home_override, reset_roveagent_home_override
    from roveagent.clisupport.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_roveagent_home()`` at call
       time — the context-local override from ``set_roveagent_home_override``
    ...
    retargeted to the *current* ``get_roveagent_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a ROVEAGENT_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    from roveagent.tools import skills_tool as _skills_tool
    from roveagent.tools import skill_manager_tool as _skill_mgr

    token = None
    if not requested or requested.lower() == "current":
        profile_dir = get_roveagent_home()
    else:
        profile_dir = _resolve_profile_dir(requested)
        token = set_roveagent_home_override(str(profile_dir))

    with _SKILLS_PROFILE_LOCK:
        old_home = _skills_tool.ROVEAGENT_HOME
        old_skills_dir = _skills_tool.SKILLS_DIR
        old_mgr_home = _skill_mgr.ROVEAGENT_HOME
        old_mgr_skills_dir = _skill_mgr.SKILLS_DIR
        _skills_tool.ROVEAGENT_HOME = profile_dir
        _skills_tool.SKILLS_DIR = profile_dir / "skills"
        _skill_mgr.ROVEAGENT_HOME = profile_dir
        _skill_mgr.SKILLS_DIR = profile_dir / "skills"
        try:
            yield profile_dir if token is not None else None
        finally:
            _skills_tool.ROVEAGENT_HOME = old_home
            _skills_tool.SKILLS_DIR = old_skills_dir
            _skill_mgr.ROVEAGENT_HOME = old_mgr_home
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_roveagent_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_roveagent_home()`` at call time
...
def _clear_skills_prompt_cache() -> None:
    ...
    """
    try:
        from roveagent.core.prompt_builder import clear_skills_system_prompt_cache
...
from roveagent.clisupport.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from roveagent.clisupport.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _plugin_terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from roveagent.core.terminal_env_registry import list_providers
...
def _probe_modal_backend() -> tuple:
    try:
        from roveagent.tools.tool_backend_helpers import has_direct_modal_credentials
...
def _probe_terminal_backend(name: str, terminal_cfg: dict) -> tuple:
    ...
    try:
        ...
        try:
            from roveagent.core.terminal_env_registry import get_provider
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not RoveAgent,
...
#
# The endpoint spawns the same ``roveagent --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from roveagent.clisupport.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from roveagent.clisupport.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from roveagent.clisupport.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
_GATEWAY_WS_PROTOCOL = "roveagent-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "roveagent-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from roveagent.clisupport.dashboard_auth.audit import AuditEvent, audit_log
        from roveagent.clisupport.dashboard_auth.ws_tickets import (
            TicketInvalid,
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._roveagent_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    Default: whatever ``roveagent --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ROVEAGENT_TUI_RESUME`` env var —
    matching what ``roveagent_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``ROVEAGENT_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``ROVEAGENT_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``ROVEAGENT_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``ROVEAGENT_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_roveagent_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``roveagent -p <name>`` performs. The in-process
    ``ROVEAGENT_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from roveagent.clisupport.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # RoveAgent TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope still overrides ROVEAGENT_HOME before config is
    # bridged into the child environment.
    from roveagent.tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["ROVEAGENT_HOME"] = str(profile_dir)
    try:
        from roveagent.clisupport.config import (
    ...
    env.setdefault("ROVEAGENT_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ROVEAGENT_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["ROVEAGENT_TUI_RESUME"] = resume

    if sidecar_url:
        env["ROVEAGENT_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile ROVEAGENT_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["ROVEAGENT_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``ROVEAGENT_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("ROVEAGENT_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from roveagent.clisupport.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="roveagent-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe RoveAgent Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full RoveAgent CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="roveagent-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from roveagent.clisupport.console_engine import RoveAgentConsoleEngine

        engine = RoveAgentConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. RoveAgent Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall RoveAgent inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("ROVEAGENT_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_roveagent_auth_identity", None),
        subprotocol=getattr(ws, "_roveagent_ws_subprotocol", None),
    )
...
# ROVEAGENT_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`roveagent_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from roveagent.clisupport.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default RoveAgent Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="roveagent-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/roveagent/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /roveagent`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ROVEAGENT_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `roveagent serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("ROVEAGENT_SERVE_HEADLESS") == "1"
    if _headless:
        _msg = (
            "Headless backend (roveagent serve): web UI disabled — use "
            "`roveagent dashboard` for the browser UI."
        )

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake (#94227): the Electron shell boots by
            # fetching `/` and extracting ``window.__ROVEAGENT_SESSION_TOKEN__``
            ...
            # `roveagent update` replaced the backend) had no way to adopt the
            ...
            if full_path == "" and not gated:
                ...
                return HTMLResponse(
                    ...
                    f"window.__ROVEAGENT_SESSION_TOKEN__={token_js};"
                    "window.__ROVEAGENT_AUTH_REQUIRED__=false;"
                    "</script></head><body>"
                    "Headless backend (roveagent serve): web UI disabled — use "
                    "`roveagent dashboard` for the browser UI."
                    "</body></html>",
    ...
    # (#82614): a long-lived `roveagent dashboard --skip-build` process that
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/roveagent``)
        ...
        ``__ROVEAGENT_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__ROVEAGENT_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ROVEAGENT_BASE_PATH__="{prefix}";'
                f"window.__ROVEAGENT_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__ROVEAGENT_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__ROVEAGENT_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__ROVEAGENT_BASE_PATH__="{prefix}";'
                f"window.__ROVEAGENT_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``ROVEAGENT_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default RoveAgent Teal canvas, then
    ...
    # under ``/roveagent`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the RoveAgent backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "RoveAgent Teal",         "description": "Classic dark teal — the canonical RoveAgent look"},
    {"name": "default-large", "label": "RoveAgent Teal (Large)", "description": "RoveAgent Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.roveagent/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.roveagent/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_roveagent_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``ROVEAGENT_HOME``.
    """
    themes_dir = get_process_roveagent_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.roveagent/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as roveagent_cli.plugins):
    1. User plugins:    ~/.roveagent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.roveagent/plugins/  (only if ROVEAGENT_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from roveagent.clisupport.plugins import get_bundled_plugins_dir
    ...
    # ROVEAGENT_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``ROVEAGENT_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the roveagent root (``~/.roveagent/plugins``). Scan the default
    # root as well (``get_default_roveagent_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``ROVEAGENT_HOME`` unchanged when it *is* the root), mirroring how
    # ``roveagent_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any)
    # authoritative over same-named root plugins.
    from roveagent.constants import get_default_roveagent_root

    user_plugin_roots = [get_process_roveagent_home() / "plugins"]
    root_plugins = get_default_roveagent_root() / "plugins"
    ...
    # ``roveagent_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ROVEAGENT_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".roveagent" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from roveagent.clisupport.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_roveagent_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
    ...
    for name, version, description, source, dir_str, key in _discover_all_plugins():
        ...
        if provides_tools:
            try:
                from roveagent.tools.registry import get_cached_check_fn_result, registry
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"roveagent auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from roveagent.clisupport.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from roveagent.clisupport.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from roveagent.clisupport.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from roveagent.clisupport.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.roveagent/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.roveagent/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"roveagent_dashboard_plugin_{plugin['name']}"
...
from roveagent.clisupport.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``ROVEAGENT_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("ROVEAGENT_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``ROVEAGENT_PARENT_PID`` is the Electron Desktop PID, not necessarily this
    Python process's immediate PPID. On Windows the venv ``roveagent.exe`` launcher
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``ROVEAGENT_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("ROVEAGENT_PARENT_PID")
    start_marker = os.environ.get("ROVEAGENT_PARENT_START_MARKER")
    nonce = os.environ.get("ROVEAGENT_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("ROVEAGENT_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
# spawn (and any script wrapping ``roveagent serve``) cannot tell "port occupied"
...
# ROVEAGENT_BACKEND_READY sentinel (which is NOT changed by any of this).
...
def _write_machine_sentinel_line(line: str) -> None:
    ...
    ROVEAGENT_DESKTOP_READY_FILE channel) for port discovery instead. Never
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'roveagent serve' / 'roveagent dashboard' backend or the RoveAgent gateway. "
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``ROVEAGENT_SERVE_HEADLESS``), so
    ...
    try:
        from roveagent.clisupport.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the roveagent-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing ROVEAGENT_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with RoveAgent Agent exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from roveagent.plugins.dashboard_auth import nous as _nous_plugin
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from roveagent.clisupport.dashboard_auth.prefix import (
                ...
                _local_only_hint = (
                    ...
                    "(and unset ROVEAGENT_DASHBOARD_PUBLIC_URL) to restore the "
            ...
            _fix_hint = (
                ...
                "roveagent.plugins.dashboard_auth.basic import hash_password; "
                "print(hash_password('your-password'))\")\n"
                "  • OAuth: run `roveagent dashboard register` (Nous Portal) or "
            ...
            # (#54489).
            try:
                from roveagent.clisupport.config import load_config as _load_cfg
                from roveagent.clisupport.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "roveagent.plugins.disabled but dashboard.basic_auth is "
                        "configured.\n"
                        "Remove 'basic' from plugins.disabled (or run "
                        "`roveagent plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # ROVEAGENT_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("ROVEAGENT_DESKTOP") == "1":
                try:
                    from roveagent.clisupport.dashboard_procs import (
            ...
            try:
                from roveagent.clisupport.process_identity import reap_orphaned_mcp_helpers
            ...
            # for standalone `roveagent serve` (no ROVEAGENT_PARENT_PID env).
            ...
            # is what lets `roveagent update` relaunch a manually-started serve
            # on its real endpoint instead of dropping it (#63206).
            try:
                from roveagent.clisupport.process_identity import (
            ...
            ready_token = "ROVEAGENT_BACKEND_READY" if headless else "ROVEAGENT_DASHBOARD_READY"
            ...
            if headless:
                ...
                print(f"  RoveAgent backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  RoveAgent Web UI → http://{host}:{actual_port}")
