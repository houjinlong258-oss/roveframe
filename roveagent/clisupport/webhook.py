"""roveagent webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    roveagent webhook subscribe <name> [options]
    roveagent webhook list
    roveagent webhook remove <name>
    roveagent webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.roveagent/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _roveagent_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: roveagent gateway run
"""
...
def webhook_command(args):
    """Entry point for 'roveagent webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: roveagent webhook {subscribe|list|remove|test}")
        print("Run 'roveagent webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (roveagent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: roveagent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from roveagent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (roveagent gateway run)")
