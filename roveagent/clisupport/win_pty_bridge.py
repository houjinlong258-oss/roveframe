"""Windows ConPTY bridge for the `roveagent dashboard` chat tab.

Drop-in counterpart to ``roveagent_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``roveagent_cli.web_server`` consumes: ``spawn``, ``read``,
