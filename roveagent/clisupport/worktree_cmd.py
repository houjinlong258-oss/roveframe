"""``roveagent worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``roveagent_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    roveagent worktree list             # audit: verdict + reason per tree
    roveagent worktree prune            # reap safe trees + merged branches
    roveagent worktree prune --dry-run  # show the plan, change nothing
    roveagent worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `roveagent worktree prune`."
        )
