...
Pure logic: walks a RoveAgent config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `roveagent doctor` and a future `roveagent migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a RoveAgent config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a RoveAgent config and return retirement issues.
    ...
    if isinstance(plugins, dict):
        ...
        if isinstance(image_gen, dict):
            ...
            if isinstance(ig_xai, dict):
                _check("roveagent.plugins.image_gen.xai.model", ig_xai.get("model"))
...
def apply_migration(
    ...
    # None``, so the next `roveagent migrate xai` reports nothing to migrate
