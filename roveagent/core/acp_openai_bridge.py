"""OpenAI-shape bridge shared by RoveAgent' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach RoveAgent as
...
the agent's *own* tool notifications. RoveAgent' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from RoveAgent' perspective).
...
of its own (Copilot) forwards everything RoveAgent offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only RoveAgent'
agent-level tools, because re-offering the overlapping ones makes RoveAgent re-run
...
class StreamChunks(list):
    ...
    RoveAgent reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``roveagent_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just RoveAgent'
