...
from roveagent.core.context_compressor import ContextCompressor
from roveagent.core.iteration_budget import IterationBudget
from roveagent.core.memory_manager import StreamingContextScrubber
...
from roveagent.core.process_bootstrap import _install_safe_stdio
from roveagent.core.subdirectory_hints import SubdirectoryHintTracker
from roveagent.core.think_scrubber import StreamingThinkScrubber
...
from roveagent.clisupport.timeouts import get_provider_request_timeout
from roveagent.constants import get_roveagent_home
from roveagent.utils import base_url_host_matches, is_truthy_value
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.roveagent/.env``. See NousResearch/roveagent-agent#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'roveagent memory status'. Note: "
        "systemd/gateway services do not inherit ~/.roveagent/.env automatically; set "
...
def _ra():
    ...
    """
    from roveagent import runtime as run_agent
    return run_agent
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from roveagent.clisupport.providers import ROVEAGENT_OVERLAYS, get_provider

        overlay = ROVEAGENT_OVERLAYS.get(provider)
    ...
    try:
        ...
        from roveagent.clisupport.models import normalize_provider as normalize_model_provider
        from roveagent.clisupport.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: roveagent config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$ROVEAGENT_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_roveagent_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$ROVEAGENT_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — RoveAgent never even initiates the compaction), so no
    ...
    RoveAgent compression entirely.
    """
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .roveagent.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ROVEAGENT_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.roveagent/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
        ...
        try:
            from roveagent.clisupport.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    if credential_pool is not None:
        try:
            from roveagent.core.credential_pool import credential_pool_matches_provider
    ...
    agent._cache_ttl = "5m"
    try:
        from roveagent.clisupport.config import load_config_readonly as _load_pc_cfg

        from roveagent.core.agent_runtime_helpers import cache_ttl_means_disabled
    ...
    # report cumulative micros spent.  Surfaced behind ROVEAGENT_DEV_CREDITS.
    agent._credits_state = None
    agent._credits_session_start_micros = None
    # Threshold-notice latch (L4): active sticky-notice keys + the crossing gates.
    from roveagent.core.credits_tracker import new_credits_latch
    ...
    # both live under ~/.roveagent/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from roveagent.logsetup import setup_logging, setup_verbose_logging
    setup_logging(roveagent_home=_ra()._roveagent_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that roveagent_logging.setup_logging() does not
        ...
        # handler level inside roveagent_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from roveagent.clisupport.auth import build_minimax_oauth_token_provider
            ...
            from roveagent.core.anthropic_adapter import _is_oauth_token as _is_oat
            ...
            if not agent.quiet_mode:
                ...
                from roveagent.core.azure_identity_adapter import is_token_provider
        ...
        try:
            from roveagent.clisupport.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                from roveagent.core.auxiliary_client import build_or_headers
                client_kwargs["default_headers"] = build_or_headers()
            elif base_url_host_matches(effective_base, "integrate.api.nvidia.com"):
                from roveagent.core.auxiliary_client import build_nvidia_nim_headers
                ...
                client_kwargs["default_headers"] = roveagent_xai_default_headers()
            elif "default_headers" not in client_kwargs:
                # Fall back to profile.default_headers for providers that
                # declare custom headers (e.g. Vercel AI Gateway attribution,
                # Kimi User-Agent on non-kimi.com endpoints).
                try:
                    from roveagent.providers import get_provider_profile as _gpf
            ...
            from roveagent.core.auxiliary_client import resolve_provider_client
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `roveagent model` to "
                        "select a provider, or run `roveagent setup` for first-time "
                        "configuration."
                    )
        ...
        try:
            from roveagent.core.bedrock_adapter import configure_bedrock_openai_client_kwargs
    ...
    # A multiplexed gateway may enter a different ROVEAGENT_HOME after
    ...
    try:
        from roveagent.tools.registry import registry as _snapshot_registry
    ...
    # worker (kanban_show tool is present iff ROVEAGENT_KANBAN_TASK is set).
    # Resolving the ~835-token block once here avoids re-running the
    # membership test + reference on every system-prompt rebuild
    # (init + each context compression).
    from roveagent.core.prompt_builder import KANBAN_GUIDANCE
    ...
    try:
        ...
        try:
            from roveagent.core.delegation_context import is_delegated_child_context

            delegated_child = is_delegated_child_context()
        except Exception:
            delegated_child = False
        if not delegated_child:
            os.environ["ROVEAGENT_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.roveagent/sessions/ alongside gateway sessions
    roveagent_home = get_roveagent_home()
    agent.logs_dir = roveagent_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.roveagent/sessions/session_{sid}.json)
    ...
    try:
        from roveagent.clisupport.config import load_config_readonly as _load_sess_cfg
    ...
    # `roveagent --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from roveagent.tools.approval import _YOLO_MODE_FROZEN
    ...
    try:
        from roveagent.clisupport.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical RoveAgent behavior) or left to LM Studio's
    ...
    if not skip_memory or _memory_toolset_requested:
        try:
            from roveagent.tools.memory_tool import (
            ...
            if agent._memory_enabled or agent._user_profile_enabled:
                from roveagent.tools.memory_tool import MemoryStore
    ...
    if not skip_memory:
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                from roveagent.core.memory_manager import MemoryManager as _MemoryManager
                from roveagent.plugins.memory import load_memory_provider as _load_mem
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "roveagent"
                    except Exception:
                        pass
    ...
    from roveagent.core.memory_manager import inject_memory_provider_tools as _inject_memory_provider_tools
    ...
    from roveagent.core.empty_response_guard import resolve_guard_settings
    (
    ...
    if agent._environment_probe:
        try:
            from roveagent.tools.env_probe import warm_environment_probe_async
    ...
    # Lets an enterprise admin append to or replace RoveAgent' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "roveagent", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, roveagent, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from roveagent.clisupport.auth import resolve_provider as resolve_auth_provider
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from roveagent.clisupport.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        try:
            from roveagent.plugins.context_engine import load_context_engine
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from roveagent.clisupport.plugins import get_plugin_context_engine
    ...
    if _selected_engine is not None:
        ...
        if _compressor_max_tokens is None:
            try:
                from roveagent.core.gemini_native_adapter import (
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by RoveAgent Agent.  Choose a model with at least "
    ...
    # Nous RoveAgent 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from roveagent.clisupport.model_switch import _check_roveagent_model_warning

            _roveagent_warn = _check_roveagent_model_warning(agent.model or "")
            if _roveagent_warn:
                _user_msg = (
                    "⚠ Nous Research RoveAgent 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_roveagent_warn)
        except Exception:
            pass
    ...
    if (
        ...
        from roveagent.core.memory_manager import normalize_tool_schema as _normalize_tool_schema
