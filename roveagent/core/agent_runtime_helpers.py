...
from roveagent.core.tool_dispatch_helpers import _trajectory_normalize_msg, make_tool_result_message
from roveagent.core.trajectory import convert_scratchpad_to_think
...
from roveagent.core.turn_context import drop_stale_api_content
from roveagent.utils import base_url_host_matches, base_url_hostname, env_var_enabled, atomic_json_write
    ...
    def _rotate_failed_credential(rotate_status: int):
        ...
        if effective_reason is not None:
            ...
            if effective_reason == FailoverReason.billing and billing_unverified:
                ...
                from roveagent.core.credential_pool import FAILURE_REASON_BILLING_UNVERIFIED
...
def restore_primary_runtime(agent) -> bool:
    ...
    try:
        ...
        from roveagent.core.chat_completion_helpers import _reset_stale_streak
        _reset_stale_streak(agent)

        # Undo the fallback's identity rewrite so the prompt is
        # byte-identical to the stored copy again (prefix cache match).
        from roveagent.core.chat_completion_helpers import rewrite_prompt_model_identity
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-RoveAgent-Session-Id header), and an
        ...
        if env_var_enabled("ROVEAGENT_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from roveagent.clisupport.config import load_config as _load_moa_cfg
    ...
    from roveagent.core.anthropic_adapter import _model_name_is_kimi_family
    is_kimi = (
    ...
    # that are canonicalized only after RoveAgent sends the request. Honor its
    ...
    if not _supports_anthropic_cache_markers:
        ...
        from roveagent.clisupport.providers import custom_provider_aliases
    ...
    if _supports_anthropic_cache_markers and (
        ...
        try:
            from roveagent.clisupport.config import get_custom_provider_model_capability
    ...
    if is_anthropic_wire and is_minimax_route:
        from roveagent.core.model_metadata import _model_name_suggests_minimax_m3
    ...
    from roveagent.core.prompt_caching import ALIBABA_FAMILY_PROVIDERS, is_qwen_model
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    from roveagent.core.auxiliary_client import _validate_base_url, _validate_proxy_env_urls
    ...
    # Delegate all rate-limit / 5xx retry to roveagent's outer conversation loop,
    ...
    if agent.provider == "opencode-free":
        from roveagent.clisupport.models import opencode_zen_free_headers
    ...
    # All primary construction and recovery paths must identify RoveAgent to the
    # official Codex endpoint, including snapshots with custom header overrides.
    from roveagent.core.codex_headers import apply_required_codex_headers
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    ``/fast``) are preserved.
    """
    from roveagent.core.agent_init import _custom_provider_extra_body_for_agent
    ...
    if custom_providers is None:
        try:
            from roveagent.clisupport.config import load_config, get_compatible_custom_providers
...
def switch_model(
    ...
    turn-scoped).
    """
    from roveagent.clisupport.providers import determine_api_mode
    ...
    from roveagent.clisupport.models import opencode_provider_family
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        if _sm_custom_providers is None:
            try:
                from roveagent.clisupport.config import get_compatible_custom_providers, load_config
    ...
    try:
        ...
        from roveagent.clisupport.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from roveagent.clisupport.middleware import apply_tool_request_middleware
    ...
    if block_message is not None:
        ...
        try:
            from roveagent.model_tools import _emit_post_tool_call_hook
    ...
    if function_name == "todo":
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.todo_tool import todo_tool as _todo_tool
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from roveagent.tools.session_search_tool import session_search as _session_search
            return _finish_agent_tool(
                _session_search(
        ...
        def _execute(next_args: dict) -> Any:
            ...
            from roveagent.tools.memory_tool import memory_tool as _memory_tool
            result = _memory_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.clarify_tool import clarify_tool as _clarify_tool
            return _finish_agent_tool(
                _clarify_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.read_terminal_tool import read_terminal_tool as _read_terminal_tool
            return _finish_agent_tool(
                _read_terminal_tool(
        ...
        def _execute(next_args: dict) -> Any:
            ...
            if (next_args.get("action") or "").strip() == "read":
                from roveagent.tools.read_preview_tool import read_preview_tool as _read_preview_tool
                return _finish_agent_tool(
                    _read_preview_tool(
            ...
            from roveagent.tools.preview_tool import _handle_preview
            return _finish_agent_tool(_handle_preview(next_args), next_args)
    elif function_name == "drive_preview":
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.drive_preview_tool import drive_preview_tool as _drive_preview_tool
            return _finish_agent_tool(
                _drive_preview_tool(
                    action=next_args.get("action", ""),
        ...
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.annotate_preview_tool import annotate_preview_tool as _annotate_preview_tool
            return _finish_agent_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.read_window_tool import read_window_below_tool as _read_window_below_tool
            return _finish_agent_tool(
        ...
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.tour_tool import tour_tool as _tour_tool
            return _finish_agent_tool(
                _tour_tool(
                    action=next_args.get("action", ""),
        ...
        def _execute(next_args: dict) -> Any:
            from roveagent.tools.setup_mcp_tool import setup_mcp_tool as _setup_mcp_tool
            return _finish_agent_tool(
                _setup_mcp_tool(
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: roveagent' own dispatch loop intentionally keeps an
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    """
    from roveagent.core.message_sanitization import apply_reasoning_content_policy
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    ...
    * Switching TO a strict provider that rejects the field (Mistral,
    ...
    removed.
    """
    from roveagent.core.message_sanitization import reapply_reasoning_echo
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    RoveAgent' keepalive client (#10324 / ``_build_keepalive_http_client``) and
