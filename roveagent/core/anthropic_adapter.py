"""Anthropic Messages API adapter for RoveAgent Agent.

Translates between RoveAgent's internal OpenAI-style message format and
...
from roveagent.utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
from roveagent.core.anthropic_endpoints import (  # noqa: F401
...
from roveagent.core.anthropic_message_convert import (  # noqa: F401
...
from roveagent.core.anthropic_credentials import (  # noqa: F401
    _OAUTH_CLIENT_ID,
    _OAUTH_REDIRECT_URI,
    _OAUTH_SCOPES,
    _OAUTH_TOKEN_URL,
    ...
    run_roveagent_oauth_login_pure,
...
# RoveAgent effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    from roveagent.core.azure_identity_adapter import build_bearer_http_client
    ...
    kwargs = {
        ...
        # Delegate retry to roveagent's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to roveagent's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + RoveAgentAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://roveagent-agent.nousresearch.com")
        headers.setdefault("X-Title", "RoveAgent Agent")
        headers.setdefault("User-Agent", f"RoveAgentAgent/{_ROVEAGENT_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (RoveAgent loads that into the process env from
    # ``~/.roveagent/.env``). The result is dual auth —
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("RoveAgent Agent", "Claude Code")
                text = text.replace("RoveAgent agent", "Claude Code")
                text = text.replace("roveagent-agent", "claude-code")
        ...
        #      a) bare RoveAgent-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that RoveAgent surfaces in its CLI. We
...
def _is_stream_unavailable_error(exc: Exception) -> bool:
    ...
    if "invokemodelwithresponsestream" in err_lower:
        from roveagent.core.bedrock_adapter import is_streaming_access_denied_error
