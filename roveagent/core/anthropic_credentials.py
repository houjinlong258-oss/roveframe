...
3. ``~/.roveagent/.anthropic_oauth.json`` (RoveAgent PKCE login)
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in roveagent_cli/runtime_provider.py.
    """
...
class CredentialPersistError(RuntimeError):
    ...
    ``~/.roveagent/.anthropic_oauth.json`` for ``roveagent_pkce``).
...
#     authority boundary of ``claude_code``/``roveagent_pkce`` is the shared
#     singleton file, which other RoveAgent processes/profiles read with fresh
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    ...
    return source_path.with_name(source_path.name + ".roveagent-spent-rotations.json")
...
def spent_rotation_source_path(source: Any) -> Optional[Path]:
    ...
    if source == "roveagent_pkce":
        return _get_roveagent_oauth_file()
    return None
        ...
        payload = json.dumps(
            {
                ...
                "comment": (
                    ...
                    "never durably committed. Written by RoveAgent so sibling "
...
def mark_rotation_consumed_uncommitted(
    ...
    *pre-rotation* pair is what lets later resolution steps recognise the stale
    ...
    """
    from roveagent.core.credential_persistence import fingerprint_secret_value
...
def claude_code_credentials_path() -> Path:
    ...
    This file is not profile-owned: every RoveAgent profile's credential pool
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    RoveAgent notices an expired token, Claude Code may have already rotated it.
    ...
    try:
        from roveagent.clisupport.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float

        refresh_timeout_seconds = env_float(
            "ROVEAGENT_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20
        )
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    RoveAgent historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``roveagent models``) never mutates ``~/.roveagent/auth.json``
    or makes a network refresh call. Refresh-on-expiry is owned by the API call
    path's pool recovery, not the resolver.
    """
    try:
        from roveagent.core.credential_pool import AUTH_TYPE_OAUTH, load_pool
    except Exception:
        return None
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by RoveAgent)
      ...
      5. Anthropic credential_pool OAuth entry (~/.roveagent/auth.json)
    ...
    # 1. RoveAgent-managed OAuth/setup token env var
    ...
    # 5. RoveAgent credential_pool OAuth entry.
...
# ── RoveAgent-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.roveagent/.anthropic_oauth.json (our own file).
...
def _get_roveagent_oauth_file() -> Path:
    return get_roveagent_home() / ".anthropic_oauth.json"
...
def run_roveagent_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run RoveAgent-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize RoveAgent with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from roveagent.clisupport.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_roveagent_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read RoveAgent-managed OAuth credentials from ~/.roveagent/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read RoveAgent OAuth credentials: %s", e)
    return None


def _write_roveagent_oauth_credentials(
    access_token: str,
    refresh_token: Optional[str],
    expires_at_ms: Optional[int],
) -> None:
    """Write refreshed roveagent_pkce tokens back to ~/.roveagent/.anthropic_oauth.json.

    Without this, a successful pool-level refresh of a ``roveagent_pkce``-sourced
    ...
    try:
        ...
        logger.error(
            "Failed to write refreshed RoveAgent OAuth credentials to %s: %s", oauth_file, e
        )
