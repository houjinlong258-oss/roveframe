...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.roveagent/auth.json active provider)
...
from roveagent.core.codex_headers import (
...
#       `patch("roveagent.core.auxiliary_client.OpenAI", ...)` — tests see the proxy,
...
from roveagent.constants import OPENROUTER_BASE_URL
from roveagent.utils import base_url_host_matches, base_url_hostname, env_float, is_truthy_value, model_forces_max_completion_tokens, normalize_proxy_env_vars
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``ROVEAGENT_CA_BUNDLE`` /
    ``SSL_CERT_FILE`` env conventions. Best-effort: any failure falls back to
    the httpx/certifi default (``True``).
    """
    try:
        from roveagent.core.ssl_verify import resolve_httpx_verify
    ...
    *,
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                "roveagent.core.process_bootstrap.build_keepalive_http_client is "
                "unavailable — mixed/stale install detected (#64333). Falling "
                "back to the SDK default HTTP client. Run `roveagent update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # RoveAgent owns auxiliary retry + provider/model fallback policy (the
    ...
    # RoveAgent sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let RoveAgent control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="roveagent-protected-aux-provider",
        daemon=True,
    ).start()
...
def _is_codex_gpt54_or_gpt55(model: Optional[str], provider: Optional[str] = None) -> bool:
    ...
    if prov != "openai-codex":
        return False
    bare = (model or "").strip().lower().rsplit("/", 1)[-1]
    from roveagent.core.model_metadata import is_codex_context_variant
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before RoveAgent triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from roveagent.clisupport.auth import resolve_api_key_provider_credentials
        from roveagent.clisupport.models import fetch_models_with_pricing
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://roveagent-agent.nousresearch.com",
    "X-Title": "RoveAgent Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``ROVEAGENT_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``ROVEAGENT_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("ROVEAGENT_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("ROVEAGENT_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "RoveAgentAgent",
}
...
# in lockstep with roveagent_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from roveagent.core.portal_tags import nous_portal_tags as _nous_portal_tags


def _nous_extra_body() -> dict:
    """Return a fresh Nous Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``roveagent_cli.__version__`` is
...
_AUTH_JSON_PATH = get_roveagent_home() / "auth.json"
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from roveagent.clisupport.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("ROVEAGENT_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        from roveagent.core.codex_responses_adapter import _chat_messages_to_responses_input
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the RoveAgent-side ``-900k`` large-context picker suffix —
        ...
        if isinstance(extra_body, dict):
            ...
            if isinstance(reasoning_cfg, dict):
                if reasoning_cfg.get("enabled") is False:
                    ...
                    from roveagent.core.reasoning_effort import (
                        clamp_effort,
        ...
        if tools:
            ...
            try:
                ...
                from roveagent.tools.schema_sanitizer import (
        ...
        try:
            from roveagent.core.transports.codex import (
        ...
        def _check_cancelled() -> None:
            ...
            try:
                from roveagent.tools.interrupt import is_interrupted
        ...
        try:
            ...
            from roveagent.core.codex_runtime import (
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        if not self._base_url:
            ...
            if candidate:
                try:
                    from roveagent.core.anthropic_adapter import _is_nous_portal_endpoint
    ...
    def create(self, **kwargs) -> Any:
        from roveagent.core.anthropic_adapter import build_anthropic_kwargs, create_anthropic_message
        ...
        if temperature is not None:
            from roveagent.core.anthropic_adapter import _forbids_sampling_params
        ...
        #   - ``_``-prefixed keys: private RoveAgent plumbing (_reasoning_config
...
class _BedrockCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from roveagent.core.bedrock_adapter import call_converse
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``roveagent_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    if _safe_isinstance(client_obj, CodexAuxiliaryClient):
        return client_obj
    try:
        from roveagent.core.gemini_native_adapter import GeminiNativeClient
        if _safe_isinstance(client_obj, GeminiNativeClient):
            return client_obj
    except ImportError:
        pass
    try:
        from roveagent.core.copilot_acp_client import CopilotACPClient
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.roveagent/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from roveagent.clisupport.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from roveagent.clisupport.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``roveagent auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``roveagent_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from RoveAgent auth store.
    ...
    try:
        from roveagent.clisupport.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from roveagent.clisupport.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from roveagent.clisupport.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if provider_id == "gemini":
                from roveagent.core.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                try:
                    from roveagent.providers import get_provider_profile as _gpf_aux
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from roveagent.providers import get_provider_profile as _gpf_aux2
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from roveagent.core.nous_rate_guard import nous_rate_limit_remaining
    ...
    if not _aux_probe_active():
        ...
        try:
            from roveagent.clisupport.models import get_nous_recommended_aux_model
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from roveagent.clisupport.config import load_config
        from roveagent.clisupport.moa_config import resolve_moa_preset
...
def _relay_sync_completion(
    ...
    *,
    ...
    from roveagent.core import relay_llm
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `roveagent setup` or `roveagent model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=roveagent_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`roveagent_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from roveagent.clisupport.runtime_provider import _resolve_azure_foundry_runtime
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    try:
        from roveagent.core.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
    except ImportError:
        return None, None
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second roveagent
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``roveagent config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from roveagent.clisupport.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from roveagent.core.anthropic_credentials import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_roveagent_fallback_destination", None)
...
def _replan_synchronous_cache_sections(
    ...
    *,
    ...
    from roveagent.core.agent_runtime_helpers import (
...
def _try_main_agent_model_fallback(
    ...
    from roveagent.core.backend_identity import (
        BackendIdentity,
        FailureScope,
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``roveagent_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._roveagent_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into RoveAgent' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `roveagent model` but the
    #    old OPENAI_BASE_URL lingers in ~/.roveagent/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: roveagent model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.roveagent/.env",
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_roveagent_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_roveagent_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from roveagent.core.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        async_kwargs["default_headers"] = roveagent_xai_default_headers()
    else:
        # Fall back to profile.default_headers for providers that declare
        # client-level headers on their ProviderProfile (e.g. attribution
        # User-Agent strings). Provider is inferred from the hostname.
        try:
            from roveagent.core.model_metadata import _infer_provider_from_url
            from roveagent.providers import get_provider_profile as _gpf_async
    ...
    # See _create_openai_client: disable SDK-internal retries so RoveAgent owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: roveagent auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: roveagent model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: roveagent model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "custom":
        ...
        if custom_base and custom_key:
            ...
            if base_url_host_matches(custom_base, "api.kimi.com"):
                ...
                try:
                    from roveagent.providers import get_provider_profile as _gpf_custom
    ...
    try:
        ...
        if custom_entry:
            ...
            if custom_key_cmd:
                from roveagent.core.command_token_source import build_command_token_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: roveagent doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("roveagent.clisupport.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from roveagent.clisupport.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(roveagent_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
            ...
            try:
                from roveagent.providers import get_provider_profile as _gpf_main
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from roveagent.core.vertex_adapter import get_vertex_config, has_vertex_credentials
        except ImportError:
        ...
        try:
            ...
            # Bedrock Mantle branch below and patch("roveagent.core.auxiliary_client.OpenAI")).
        ...
        if is_openai_bedrock_model(final_model):
            ...
            # patch("roveagent.core.auxiliary_client.OpenAI", ...).
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`roveagent_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one RoveAgent will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if not _is_gemini_native and _effective_base:
            try:
                from roveagent.core.gemini_native_adapter import is_native_gemini_base_url
    ...
    if _provider_for_portal in {"nous", "nous-portal", "nousresearch"}:
        ...
        if "session_id" not in merged_extra:
            try:
                from roveagent.core.portal_tags import get_conversation_context
    ...
    # Anthropic Messages adapters translate RoveAgent reasoning into native
...
def _validate_llm_response(
    ...
    from roveagent.core.aux_accounting import record_aux_usage
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional RoveAgent reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `roveagent model`."
                    )
