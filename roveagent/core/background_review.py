...
See the ``roveagent-agent-dev`` skill (``references/self-improvement-loop.md``)
...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches RoveAgent through a client
    shim, and a shim that cannot carry RoveAgent tool calls back turns the fork into
    ...
    exists so a shim that can't declares ``SUPPORTS_ROVEAGENT_TOOL_CALLS = False``
    ...
    for candidate in (client, type(client) if client is not None else None):
        if candidate is None:
            continue
        supported = getattr(candidate, "SUPPORTS_ROVEAGENT_TOOL_CALLS", None)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with RoveAgent, e.g. 'roveagent-agent').\n"
    "  • Hub-installed skills (installed via 'roveagent skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'roveagent curator pin'). You are an "
    ...
    "'roveagent curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'roveagent skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'roveagent curator pin'). Pin blocks "
    ...
    "reply and recommend 'roveagent curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("ROVEAGENT_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def build_cache_parity_fork(
    ...
    *,
    ...
    # scratch (fresh _roveagent_now() timestamp, fresh
...
def _run_review_in_thread(
    ...
    # Local import to avoid a hard circular dep at module load.
    from roveagent.runtime import AIAgent
    from roveagent.tools.terminal_tool import set_approval_callback as _set_approval_callback
    ...
    # An agent-as-provider whose client can't carry RoveAgent tool calls back would
    ...
    if not _parent_can_emit_tool_calls(agent) and not bool(
        ...
        logger.warning(
            "Background review skipped: provider %r cannot emit RoveAgent tool calls, "
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            try:
                from roveagent.tools.skill_manager_tool import _reset_background_review_read_marks
