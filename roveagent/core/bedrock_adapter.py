"""AWS Bedrock Converse API adapter for RoveAgent Agent.
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install RoveAgent with Bedrock support: pip install -e '.[bedrock]'"
        )
    # converse() / converse_stream() were added in boto3 1.34.59.
    # When RoveAgent is installed editable into system Python, the system boto3
...
def resolve_bedrock_runtime_region(config: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``roveagent_cli/runtime_provider.py``):
...
def probe_bedrock_context_length(model_id: str, region: str) -> Optional[int]:
    ...
    """
    try:
        from roveagent.core.model_metadata import parse_context_limit_from_error
