...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``ROVEAGENT_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``ROVEAGENT_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """Map ``ROVEAGENT_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    Mirrors ``ROVEAGENT_DEV_CREDITS_FIXTURE``; the usage *bar* still comes from
    ``ROVEAGENT_DEV_CREDITS_FIXTURE`` (set both to pair a bar with a billing state).
    """
    name = (os.getenv("ROVEAGENT_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return BillingState(logged_in=False, error=f"unknown ROVEAGENT_DEV_BILLING_FIXTURE: {name}")
