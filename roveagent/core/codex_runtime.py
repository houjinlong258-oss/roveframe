...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into RoveAgent accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    RoveAgent' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, RoveAgent should still count that turn
    ...
    from roveagent.core.usage_pricing import CanonicalUsage, estimate_usage_cost
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in RoveAgent state.

    The app-server owns the compacted thread context, so RoveAgent should not
    ...
    if not force:
        try:
            from roveagent.core.conversation_compression import COMPACTION_STATUS
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior RoveAgent deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → RoveAgent UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal RoveAgent tool loop. Without this bridge gateway
...
# Codex item types that map to a RoveAgent tool_call in the projector (and
...
# Internal MCP server that wraps RoveAgent' native tools for codex. When
# codex calls back through it, the inner dispatch runs in a SEPARATE
# roveagent-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.roveagent-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as RoveAgent tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "roveagent-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic RoveAgent tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (RoveAgent' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into RoveAgent' gateway UI callbacks.
    ...
    def _stable_call_id(item: dict, name: str) -> str:
        ...
        from roveagent.core.transports.codex_event_projector import _deterministic_call_id
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into RoveAgent'
    ...
    if getattr(agent, "compression_checkpoint_required", False) is True:
        from roveagent.core.conversation_compression import _checkpoint_blocked
    ...
    from roveagent.core.transports.codex_app_server_session import (
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to RoveAgent' standard prompt flow if a
        ...
        # explicitly opted out of RoveAgent approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / ROVEAGENT_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing RoveAgent UI. Defaults (manual/smart/unset) preserve the
        # current fail-closed behavior — this is a no-op for those users.
        auto_approve_requests = False
        try:
            from roveagent.tools.approval import is_approval_bypass_active
        ...
        # item/agentMessage/delta, ...) into RoveAgent' gateway UI callbacks
...
def _raise_stream_error(event: Any) -> None:
    ...
    """
    from roveagent.runtime import _StreamErrorEvent
...
def _is_plain_json_data(value: Any) -> bool:
    ...
    format.  RoveAgent assembles Codex payloads from JSON round-trips, so they
...
def _bypass_sdk_request_transform(stream_kwargs: dict) -> dict:
    ...
    the transform.  Set ROVEAGENT_CODEX_SDK_TRANSFORM=1 to restore the pre-fix
    behavior.
    """
    if os.environ.get("ROVEAGENT_CODEX_SDK_TRANSFORM", "").strip().lower() in {
        "1", "true", "yes", "on"
    }:
