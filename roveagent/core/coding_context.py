"""Coding-context awareness — base RoveAgent, every interactive surface.

When the user runs RoveAgent inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), RoveAgent shifts into a **coding posture**. This module is the
...
from roveagent.clisupport._subprocess_compat import bounded_git_probe

logger = logging.getLogger("roveagent.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "roveagent", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _ROVEAGENT_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``ROVEAGENT_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether RoveAgent should operate in its coding posture right now."""
