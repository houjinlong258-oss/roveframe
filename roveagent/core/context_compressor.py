...
from roveagent.core.context_engine import ContextEngine, sanitize_memory_context
from roveagent.core.error_classifier import FailoverReason, classify_api_error
from roveagent.core.message_sanitization import tool_result_id_variants
...
_SUMMARY_ROUTE_PIN: contextvars.ContextVar[Optional[Dict[str, Any]]] = (
    contextvars.ContextVar("roveagent_summary_route_pin", default=None)
)
...
# heuristics. See https://github.com/NousResearch/roveagent-agent/issues/38389
#
...
def _salvage_reduce_todo_snapshot(out: List[Dict[str, Any]]) -> None:
    ...
    the row entirely.
    """
    from roveagent.core.conversation_compression import _PRUNED_SKILL_RELOAD_NOTICE_HEADER
...
#   roveagent_cli/oneshot.py no-user callback.
...
# re-accesses the compacted region. RoveAgent already persists every
...
def _build_recovery_footer(session_id: str, region_len: int) -> str:
    ...
    RoveAgent persists every pre-compaction message in state.db; session_search
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the roveagent compressor emits).
    """
    if not messages:
        return messages
...
class ContextCompressor(ContextEngine):
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates so RoveAgent compresses before a provider rejects the
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _generate_summary(
        ...
        # user's configured timezone via roveagent_time.now(). The compaction summary
        ...
        try:
            ...
            _today_str = _roveagent_now().strftime("%Y-%m-%d")
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def _is_synthetic_compression_user_turn(cls, message: Any) -> bool:
        ...
        from roveagent.core.conversation_loop import (
    ...
    def _latest_user_task_snapshot(
        ...
        from roveagent.core.conversation_compression import _is_real_user_message
    ...
    def _tool_call_id_variants(tc) -> set:
        ...
        """
        from roveagent.core.message_sanitization import tool_call_id_variants
    ...
    def _sanitize_tool_pairs(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        """
        from roveagent.core.agent_runtime_helpers import _classify_tool_call_orphans
    ...
    def _stale_thinking_on_wire(self) -> bool:
        ...
        """
        try:
            from roveagent.core.message_sanitization import stale_thinking_reaches_wire
    ...
    def _micro_summarize_one(
        ...
        """
        from roveagent.core.auxiliary_client import call_llm, aux_interrupt_protection
    ...
    def compress(
        ...
        # ``roveagent kanban`` worker seeded with a single short
