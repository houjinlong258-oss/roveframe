...
from roveagent.core.model_metadata import estimate_tokens_rough
from roveagent.clisupport._subprocess_compat import IS_WINDOWS, windows_hide_flags
...
_SENSITIVE_ROVEAGENT_DIRS = (Path("skills") / ".hub",)
...
async def _default_url_fetcher(url: str) -> str:
    from roveagent.tools.web_tools import web_extract_tool
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    roveagent_home = get_roveagent_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(roveagent_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(roveagent_home / rel for rel in _SENSITIVE_ROVEAGENT_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal RoveAgent path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.roveagent/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal RoveAgent path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
