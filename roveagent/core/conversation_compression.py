...
from roveagent.core.auxiliary_client import AuxiliaryExplicitCancellation
from roveagent.core.context_engine import (
    automatic_compaction_status_message,
    sanitize_memory_context,
)
from roveagent.core.memory_provider import PRE_COMPRESS_CHECKPOINT_API_VERSION
from roveagent.core.model_metadata import (
    estimate_messages_tokens_rough,
    estimate_request_tokens_rough,
)
from roveagent.core.session_activity import ActivityProvenance, normalize_activity_provenance
...
def _refresh_agent_tool_definitions(agent) -> bool:
    ...
    a compaction.
    """
    from roveagent.tools.mcp_tool import refresh_agent_mcp_tools
...
def _cached_prompt_reflects_builtin_memory(agent: Any, cached_prompt: str) -> bool:
    ...
    if snapshot is None:
        return False
    try:
        from roveagent.tools.memory_tool import MEMORY_BLOCK_HEADERS
    except Exception:
        return False
...
# Mirror roveagent_cli.config.DEFAULT_CONFIG["compression"] keys of the same name.
...
def resolve_compression_fallback_route() -> Optional[dict]:
    ...
    for index, entry in enumerate(chain):
        ...
        try:
            ...
            api_key = None
        from roveagent.core.auxiliary_client import _coerce_positive_timeout
...
def _retry_compression_on_fallback_chain(
    *,
    ...
    try:
        from roveagent.core.context_compressor import pin_summary_route
...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``roveagent_state.SessionDB`` class (and its live instances) is old.
...
def _adopt_live_compression_child(
    ...
    try:
        ...
        os.environ["ROVEAGENT_SESSION_ID"] = child_session_id
    try:
...
def _direct_messages_for_pre_compress_memory(messages: Any) -> list[dict[str, Any]]:
    ...
    receive one normalized host contract instead of having to infer RoveAgent
    ...
    from roveagent.core.context_compressor import COMPRESSED_SUMMARY_METADATA_KEY
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `roveagent setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by RoveAgent "
        ...
        if aux_context < threshold:
            ...
            from roveagent.core.context_compressor import ContextCompressor as _CC
            ...
            if threshold_suggestion_viable:
                ...
                msg += (
                    ...
                    f"RoveAgent's small-context floor and output reservation "
...
def _strip_stale_todo_snapshot(content: Any) -> Any:
    ...
    """
    from roveagent.tools.todo_tool import TODO_INJECTION_HEADER
...
def compress_context(
    ...
    *,
    ...
    # RoveAgent' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|roveagent|off).
    # The memory-provider context handoff below is intentionally RoveAgent-only:
    ...
    try:
        ...
        if todo_snapshot:
            ...
            from roveagent.core.context_compressor import _append_text_to_content
        ...
        if agent._session_db:
            ...
            try:
                ...
                if _rough_out > _rough_in:
                    ...
                    from roveagent.core.context_compressor import salvage_grown_transcript
                ...
                if in_place:
                    ...
                    if callable(_parent_row_reader):
                        try:
                            from roveagent.state.common import is_automatic_end_reason
                    ...
                    # whose thread lacks the ROVEAGENT_HOME context.
                    try:
                    ...
                    agent._session_db.publish_compression_child(
                        ...
                        or os.environ.get("ROVEAGENT_SESSION_SOURCE", "cli"),
                        model=agent.model,
                    ...
                    try:
                        from roveagent.clisupport.goals import migrate_goal_to_session
                        migrate_goal_to_session(old_session_id, agent.session_id, reason="compression")
                    except Exception as _goal_err:
                        logger.debug("Could not migrate goal on compression: %s", _goal_err)
                    # Same boundary hazard for /heartbeat state — carry it too.
                    try:
                        from roveagent.clisupport.heartbeat import migrate_heartbeat_to_session
                    ...
                    # survive compression.
                    try:
                        from roveagent.clisupport.loops import migrate_loop_to_session
        ...
        # engines (e.g. roveagent-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See roveagent-lcm#68. Built-in ContextCompressor
        ...
        try:
            from roveagent.tools.file_tools import reset_file_dedup
        ...
        try:
            from roveagent.tools.skills_tool import reset_skill_view_dedup
...
def _record_codex_compaction_failure(agent: Any, error: str) -> None:
    ...
    """
    from roveagent.core.context_compressor import _SUMMARY_FAILURE_COOLDOWN_SECONDS
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    RoveAgent' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep RoveAgent' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "roveagent", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "roveagent":
        logger.info(
    ...
    # way the RoveAgent path below does. An active cooldown means a recent
...
def try_shrink_image_parts_in_messages(
    ...
    *,
    ...
    try:
        from roveagent.tools.vision_tools import _resize_image_for_vision
    except Exception as exc:
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="roveagent_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
