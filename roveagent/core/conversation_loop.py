...
from roveagent.core.codex_responses_adapter import _summarize_user_message_for_log
...
from roveagent.core.context_engine import automatic_compaction_status_message
from roveagent.core.display import KawaiiSpinner
from roveagent.core.error_classifier import FailoverReason, classify_api_error
from roveagent.core.message_metadata import append_message
from roveagent.core.turn_context import (
...
from roveagent.core.turn_retry_state import TurnRetryState
from roveagent.core.runtime_cwd import resolve_agent_cwd
from roveagent.core.message_sanitization import (
...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in roveagent_state.py — kept local
# to avoid importing roveagent_state at module load time (its module-level
# DEFAULT_DB_PATH = get_roveagent_home() / "state.db" breaks tests that
# monkeypatch get_roveagent_home to return a str).
...
from roveagent.core.prompt_caching import (
    build_prompt_cache_plan,
    effective_cache_ttl,
    strip_anthropic_cache_control,
    strip_anthropic_tool_cache_control,
)
from roveagent.core.provider_projection import splice_provider_projection
from roveagent.core.retry_utils import (
    adaptive_rate_limit_backoff,
    is_zai_coding_overload_error,
    jittered_backoff,
    zai_coding_overload_retry_ceiling,
)
from roveagent.core.repetition_guard import is_repetition_dominated
from roveagent.core.trajectory import has_incomplete_scratchpad
# Bind before the turn starts so a source-tree swap cannot load a skewed
# finalizer at turn end.
from roveagent.core.turn_finalizer import finalize_turn
from roveagent.core.usage_pricing import estimate_usage_cost, normalize_usage
from roveagent.core import empty_response_guard as _empty_guard
from roveagent.constants import PARTIAL_STREAM_STUB_ID
from roveagent.logsetup import set_session_context
from roveagent.tools.skill_provenance import set_current_write_origin
from roveagent.utils import base_url_host_matches, env_var_enabled
...
def _should_skip_model_call_for_reference_handoff(
    ...
    from roveagent.core.context_compressor import reference_handoff_would_drive_next_model_call
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.roveagent/prefill.json incident, 20/20 blocked with
    ...
    if messages and messages[-1].get("role") == "assistant":
        ...
        if not visible:
            ...
            # (#81841).
            from roveagent.core.agent_runtime_helpers import _INTERRUPTED_PLACEHOLDER
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for RoveAgent tool use: "
    ...
    return (
        ...
        f"context, but RoveAgent needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In RoveAgent config, set `model.ollama_num_ctx: 65536` "
...
def _billing_or_entitlement_message(
    *,
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `roveagent auth reset anthropic`. Until "
    ...
    try:
        from roveagent.core.billing_links import build_billing_block
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        ...
        _bot_stale = False
        try:
            from roveagent.tools.bot_mode_probe import (
                BOT_CHAT_TITLE,
            ...
            try:
                from roveagent.core.system_prompt import _agent_home
        ...
        from roveagent.core.system_prompt import restore_plugin_prompt_sections
        ...
        from roveagent.core.system_prompt import reconstruct_static_prefix
    ...
    try:
        from roveagent.core.credits_tracker import seed_credits_at_session_start
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only RoveAgent' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `roveagent fallback add`."
)
...
def _peel_moa_guidance(
    ...
    """
    from roveagent.core.moa_loop import peel_reference_guidance
...
def _redecorate_prompt_cache_for_provider(
    ...
    *,
    ...
    if agent._use_prompt_caching:
        ...
        from roveagent.core.prompt_caching import envelope_tool_part_cache_markers_supported
...
def _apply_context_engine_selection(
    ...
    *,
    ...
    try:
        from roveagent.core.context_engine import ContextEngine as _CE
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from roveagent.clisupport.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.roveagent/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see roveagent_state.classify_persistence_error).
    ...
    # all run inside Codex). Default RoveAgent path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if _pre_api_steer:
            ...
            for _si in range(len(messages) - 1, -1, -1):
                ...
                if isinstance(_sm, dict) and _sm.get("role") == "tool":
                    from roveagent.core.prompt_builder import format_steer_marker
        ...
        from roveagent.core.agent_runtime_helpers import repair_message_sequence_with_cursor
        ...
        # cache prefix.  The system prompt is reserved for RoveAgent internals.
        #
        # RoveAgent invariant: the system prompt is built ONCE per session
        ...
        if moa_config:
            try:
                from roveagent.core.message_content import flatten_message_text as _flatten_mt
                from roveagent.core.moa_loop import _preset_temperature, aggregate_moa_context
        ...
        from roveagent.core.turn_context import _agent_stale_thinking_on_wire
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for RoveAgent tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            if agent.provider == "nous":
                try:
                    from roveagent.core.nous_rate_guard import (
            ...
            try:
                ...
                try:
                    from roveagent.clisupport.middleware import apply_llm_request_middleware
                ...
                try:
                    from roveagent.clisupport.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("ROVEAGENT_DUMP_REQUESTS"):
                ...
                from roveagent.clisupport.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a RoveAgent/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if agent.provider == "nous":
                    try:
                        from roveagent.core.nous_rate_guard import clear_nous_rate_limit
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: roveagent auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `roveagent doctor` for credential-chain diagnostics, or")
                    ...
                    from roveagent.constants import display_roveagent_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for RoveAgent-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: roveagent config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: roveagent config set ANTHROPIC_API_KEY \"\"")
                ...
                # and omits the field, and RoveAgent' local compression takes
                ...
                if (
                    ...
                    from roveagent.core.native_compaction import is_native_compaction_rejection
                ...
                if (
                    ...
                    try:
                        from roveagent.tools.schema_sanitizer import strip_pattern_and_format
                ...
                if getattr(api_error, "status_code", None) == 404:
                    try:
                        from roveagent.clisupport.model_normalize import suggest_prefixed_model_id
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `roveagent model`."
                        )
                ...
                # providers (DeepSeek, Kimi, MiMo, RoveAgent).  A 429 can
                ...
                # which RoveAgent' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. RoveAgent' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`roveagent",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # RoveAgent may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See roveagent-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `roveagent auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `roveagent model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: roveagent portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: roveagent setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        roveagent fallback add   (interactive picker — same as `roveagent model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a RoveAgent/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    from roveagent.core.thinking_timeout_guidance import (
                        is_thinking_timeout,
                    )
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.roveagent/config.yaml` to extend the per-call "
                            f"timeout. (RoveAgent's built-in floor is 600s for "
        ...
        try:
            ...
            # with the iterations RoveAgent never saw. Appended before this turn's
                        ...
                        for _key in (
                            ...
                            if _key in interim_msg:
                                if _key == "codex_reasoning_items":
                                    ...
                                    from roveagent.core.native_compaction import (
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates RoveAgent mid-turn, resume logic still sees the
                    ...
                    from roveagent.state import classify_persistence_error
                ...
                # timeout (ROVEAGENT_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    from roveagent.core.verification_stop import (
                ...
                try:
                    from roveagent.core.verify_hooks import max_verify_nudges
                    from roveagent.clisupport.lifecycle import has_hook
                    from roveagent.clisupport.plugins import get_pre_verify_continue_message

                    if _edited and has_hook("pre_verify") and _attempt < max_verify_nudges():
                        # Posture is fixed for the session — resolve once + cache.
                        coding = getattr(agent, "_resolved_is_coding", None)
                        if coding is None:
                            from roveagent.core.coding_context import is_coding_context
                ...
                try:
                    from roveagent.core.kanban_stop import build_kanban_stop_nudge
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("ROVEAGENT_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
            ...
            if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
                ...
                final_response = (
                    ...
                    "resumed with: roveagent --resume <session-id>"
                )
