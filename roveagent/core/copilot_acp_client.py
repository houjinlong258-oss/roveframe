"""OpenAI-compatible shim that forwards RoveAgent requests to `copilot --acp`.

This adapter lets RoveAgent treat the GitHub Copilot ACP server as a chat-style
backend. Each request starts a short-lived ACP session, sends the formatted
conversation as a single prompt, collects text chunks, and converts the result
back into the minimal shape RoveAgent expects from an OpenAI client.
"""
...
from roveagent.core.acp_openai_bridge import (
...
from roveagent.core.file_safety import get_read_block_error, get_write_denied_error, is_write_approval_required
...
def _resolve_args() -> list[str]:
    raw = os.getenv("ROVEAGENT_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    ROVEAGENT_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set ROVEAGENT_HOME explicitly if they
...
def _build_subprocess_env() -> dict[str, str]:
    ...
    env = roveagent_subprocess_env(inherit_credentials=True)
...
def _format_messages_as_prompt(
    ...
    sections: list[str] = [
        "You are being used as the active ACP agent backend for RoveAgent.",
        "Use ACP capabilities to complete tasks.",
        "IMPORTANT: If you take an action with a tool, you MUST output tool calls using <tool_call>{...}</tool_call> blocks with JSON exactly in OpenAI function-call shape.",
        "If no tool is needed, answer normally.",
    ]
    if model:
        sections.append(f"RoveAgent requested model hint: {model}")

    # Copilot has no tools of its own that would collide with RoveAgent', so it
    ...
    def _run_prompt(self, prompt_text: str, *, timeout_seconds: float) -> tuple[str, str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"ROVEAGENT_COPILOT_ACP_COMMAND / ROVEAGENT_COPILOT_ACP_ARGS "
                f"to a working pair."
            )
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "RoveAgent ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "point RoveAgent at it explicitly:\n"
                        "  export ROVEAGENT_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
                        "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\n"
                        "directly with a Copilot subscription token) via `roveagent setup`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
        ...
        *,
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(
                message_id,
                -32601,
                f"ACP client method '{method}' is not supported by RoveAgent yet.",
            )
