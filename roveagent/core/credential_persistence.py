...
``auth.json``.  They intentionally have no dependency on ``roveagent_cli.auth`` so
...
# Sources RoveAgent owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and RoveAgent-owned OAuth/device-code state)
