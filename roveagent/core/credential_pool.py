...
from roveagent.clisupport.config import load_env
from roveagent.core.secret_scope import get_secret as _get_secret
from roveagent.core.credential_persistence import (
    fingerprint_secret_value,
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import roveagent.clisupport.auth
...
# un-throttled log fires on *every* selection. On Windows several RoveAgent
...
# readiness handshake ("Timed out connecting to RoveAgent backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``roveagent_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_credentials_file(self, entry: PooledCredential) -> PooledCredential:
        ...
        if self.provider != "anthropic" or entry.source != "claude_code":
            return entry
        try:
            from roveagent.core.anthropic_credentials import read_claude_code_credentials
    ...
    def _sync_anthropic_entry_from_pool_store(
        ...
        (``~/.roveagent/auth.json`` / profile equivalent), so it works for
        every *pool-owned* Anthropic source - ``roveagent_pkce`` and
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``roveagent model`` / ``roveagent auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another RoveAgent process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``roveagent`` invocation that defaults to the active provider
        (e.g. setup wizard, ``roveagent auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across RoveAgent
        ...
        # Previously "anthropic" was excluded from this tuple: two RoveAgent
        ...
        # "claude_code" (roveagent_pkce, dashboard-issued manual entries) —
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every RoveAgent profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every RoveAgent process) that might
        refresh a ``claude_code``-sourced Anthropic entry, not just callers
        sharing one profile's ``auth.json``.
        """
        from roveagent.core.anthropic_credentials import claude_code_credentials_path
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        ``roveagent_pkce`` sources the singleton file — not ``auth.json`` — is the
        ...
        try:
            from roveagent.core.anthropic_credentials import (
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ROVEAGENT_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ROVEAGENT_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ROVEAGENT_XAI_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "xai-oauth"
            else "ROVEAGENT_ANTHROPIC_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("roveagent_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if entry.source == "claude_code":
                    try:
                        from roveagent.core.anthropic_credentials import _write_claude_code_credentials
                ...
                # Same rationale for the singleton source roveagent_pkce:
                # _seed_from_singletons() reads ~/.roveagent/.anthropic_oauth.json
                ...
                # Do not use endswith here: manual:roveagent_pkce is already
                # pool-owned, and creating a singleton for it would introduce
                # a second authority for the same refresh-token family.
                elif entry.source == "roveagent_pkce":
                    try:
                        from roveagent.core.anthropic_credentials import _write_roveagent_oauth_credentials
                        _write_roveagent_oauth_credentials(
                            refreshed["access_token"],
                            refreshed["refresh_token"],
                            refreshed["expires_at_ms"],
                        )
                        ...
                        return self._fail_closed_unpersisted_rotation(
                            entry, wexc, store="~/.roveagent/.anthropic_oauth.json"
                        )
            elif self.provider == "openai-codex":
                # Adopt fresher tokens from auth.json before spending the
                # refresh_token — single-use tokens consumed by another RoveAgent
            ...
            if self.provider == "anthropic" and entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        from roveagent.core.anthropic_credentials import refresh_anthropic_oauth_pure
                        refreshed = refresh_anthropic_oauth_pure(
                            synced.refresh_token,
                            use_json=synced.source.endswith("roveagent_pkce"),
                        )
                ...
                # Backstop for non-claude_code sources (roveagent_pkce,
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another RoveAgent process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other RoveAgent profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `roveagent model` / `roveagent auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `roveagent model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``roveagent auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `roveagent auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `roveagent auth remove <provider> <N>` is stable across all source types.
    try:
        from roveagent.clisupport.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, RoveAgent PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `roveagent setup` ("Claude
        ...
        # Claude Code CLI, roveagent_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.roveagent/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `roveagent setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"roveagent_pkce", "claude_code"}
            ]
        ...
        for source_name, creds in (
            ("roveagent_pkce", read_roveagent_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `roveagent auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (roveagent auth remove copilot gh_cli) must not pay either on
        ...
        # RoveAgent auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from roveagent.clisupport.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.roveagent/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `roveagent auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `roveagent auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the RoveAgent auth store.  Without this gate the removal is instantly
        ...
        # RoveAgent owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `roveagent auth openai-codex`.
        ...
        # When the user logs in via ``roveagent model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``roveagent auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from roveagent.clisupport.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.roveagent/.env over os.environ — the user's config file is the
# authoritative source for RoveAgent credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_roveagent_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `roveagent auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.roveagent/.env.
    ...
    try:
        from roveagent.clisupport.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    if provider == "openrouter":
        # Prefer ~/.roveagent/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `roveagent auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and RoveAgent
        ...
        return (
            ...
            or entry.source == "roveagent_pkce"
        )
