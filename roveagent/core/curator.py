...
def _state_file() -> Path:
    return get_roveagent_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.roveagent/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `roveagent update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``roveagent curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``roveagent update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``roveagent curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `roveagent curator run --dry-run` to preview now"
            )
            save_state(state)
...
def _cron_referenced_skills() -> Set[str]:
    ...
    """
    try:
        from roveagent.cron.jobs import referenced_skill_names as _refs
        return _refs()
    except Exception as e:
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    """
    from roveagent.tools import skill_usage as _u
    ...
    for row in _u.curated_report():
        ...
        if anchor <= archive_cutoff and current != _u.STATE_ARCHIVED:
            ...
            try:
                from roveagent.tools.skill_ledger import reset_ledger_actor, set_ledger_actor
...
CURATOR_REVIEW_PROMPT = (
    "You are running as RoveAgent' background skill CURATOR. This is an "
    ...
    "into ~/.roveagent/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: roveagent-config-*, roveagent-dashboard-*, gateway-*, codex-*, "
    ...
    "`roveagent curator rollback` restores a hollow skill (issue #96962).\n\n"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.roveagent/logs/curator/``)
    ...
    authored skill data in ``~/.roveagent/skills/``.

    ``ensure_roveagent_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_roveagent_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every roveagent install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: roveagent curator status
        keep an umbrella stable: roveagent curator pin document-tools
    ...
    lines.append("full report: roveagent curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: roveagent curator pin {example}"
            )
    return "\n".join(lines)
    ...
    *,
    ...
    try:
        ...
        if consolidated_map or pruned_names:
            from roveagent.cron.jobs import rewrite_skill_refs as _rewrite_cron_refs
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.roveagent/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.roveagent/skills/.archive/` for "
            "safety and can be restored via `roveagent curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.roveagent/skills/.archive/`. "
            "Restore any via `roveagent curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `roveagent curator restore <name>`")
    lines.append("- All archives live under `~/.roveagent/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``roveagent curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    if dry_run:
        ...
        try:
            from roveagent.core import curator_backup
    ...
    # real pass out. We still record a summary so `roveagent curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `roveagent update` only if "
        ...
        # recorded in state so `roveagent curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``roveagent model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `roveagent model` → auxiliary
    ...
    try:
        ...
        review_agent = AIAgent(
            ...
            **_agent_kwargs,
            ...
            # followed snapshotted an already-stripped package and ``roveagent
