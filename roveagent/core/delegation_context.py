...
The parent RoveAgent process may itself be a Kanban dispatcher worker with
ROVEAGENT_KANBAN_* variables in process env. delegate_task children run inside the
...
# though the worker's ROVEAGENT_KANBAN_* vars are legitimately in os.environ (cron
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar(
    "roveagent_non_dispatcher_owned_context",
    default=False,
)

DELEGATED_CHILD_ENV_MARKER = "ROVEAGENT_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "ROVEAGENT_KANBAN_TASK",
    "ROVEAGENT_KANBAN_RUN_ID",
    "ROVEAGENT_KANBAN_WORKSPACE",
    "ROVEAGENT_KANBAN_WORKSPACES_ROOT",
    "ROVEAGENT_KANBAN_CLAIM_LOCK",
    "ROVEAGENT_KANBAN_BOARD",
    "ROVEAGENT_KANBAN_DB",
)
...
def delegated_child_context(session_id: str | None = None) -> Iterator[None]:
    ...
    try:
        ...
        from roveagent.gateway.session_context import scoped_current_session_id
...
def non_dispatcher_owned_context() -> Iterator[None]:
    ...
    own process, where ``ROVEAGENT_KANBAN_TASK`` is legitimately set.  Without this
    ...
    ``kanban_complete`` defaults ``task_id`` to ``$ROVEAGENT_KANBAN_TASK`` — letting
...
def is_dispatcher_owned_worker_context() -> bool:
    ...
    The single predicate every ``ROVEAGENT_KANBAN_*`` identity gate should use
...
def delegated_child_subprocess_env(
    ...
    parent dispatcher ``ROVEAGENT_KANBAN_*`` vars while losing the ContextVar in
