...
``roveagent pause`` writes a sentinel file at ``$ROVEAGENT_HOME/ESTOP``;
``roveagent resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "RoveAgent is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.roveagent/ESTOP``.
...
def _roveagent_home() -> Path:
    """Resolve the active ROVEAGENT_HOME (profile-aware) at call time."""
    try:
...
def _canonical_root() -> Path:
    """Fleet-wide RoveAgent root, even when this process is a profile gateway.

    Profile gateways launch with ROVEAGENT_HOME=~/.roveagent/profiles/<name>.
    ``roveagent pause`` from an operator seat writes ~/.roveagent/ESTOP. If we
    ...
    try:
        ...
        return Path(get_default_roveagent_root())
    except Exception:
        return Path(os.path.expanduser("~/.roveagent"))


def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `roveagent pause`."""
    return _roveagent_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    Engaged if ANY candidate sentinel exists: the process ROVEAGENT_HOME
    (profile-local) or the fleet canonical root (~/.roveagent). Fail SAFE on
...
def disengage() -> bool:
    ...
    ``roveagent resume`` from a profile gateway still clears an operator pause
    written at ~/.roveagent/ESTOP.
    """
    lifted = False
    for path in _candidate_sentinel_paths():
        try:
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ RoveAgent is paused ({reason}). New work is on hold; "
            "run `roveagent resume` to pick things back up."
        )
    return (
        "⏸️ RoveAgent is paused. New work is on hold; "
        "run `roveagent resume` to pick things back up."
    )
