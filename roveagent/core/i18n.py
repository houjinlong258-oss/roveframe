"""Lightweight internationalization (i18n) for RoveAgent static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by RoveAgent itself -- approval prompts, a handful of gateway slash
...
Usage::
    ...
    print(t("roveagent.gateway.draining", count=3))                  # {count} formatted
    print(t("approval.choose_long", lang="zh"))            # explicit override

Language resolution order:
    1. Explicit ``lang=`` argument passed to :func:`t`
    2. ``ROVEAGENT_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``ROVEAGENT_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("ROVEAGENT_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "ROVEAGENT_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a roveagent dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`roveagent_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("ROVEAGENT_LANGUAGE")
    if env_lang:
...
def t(key: str, lang: str | None = None, **format_kwargs: Any) -> str:
    ...
    **format_kwargs
        ``str.format`` substitution arguments (``t("roveagent.gateway.drain", count=3)``
