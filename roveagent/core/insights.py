"""
Session Insights Engine for RoveAgent Agent.

Analyzes historical session data from the SQLite state database to produce
comprehensive usage insights — token consumption, cost estimates, tool usage
patterns, activity trends, model/platform breakdowns, and session metrics.

Inspired by Claude Code's /insights command, adapted for RoveAgent Agent's
...
from roveagent.core.usage_pricing import (
    CanonicalUsage,
    estimate_usage_cost,
    format_cost_label,
...
class InsightsEngine:
    ...
    def __init__(self, db):
        ...
        Args:
            db: A SessionDB instance (from roveagent_state.py)
        """
        self.db = db
        self._conn = db._conn
    ...
    def _compute_overview(
        ...
        if models:
            ...
            # with the per-model table and stops `roveagent insights`
    ...
    def format_terminal(self, report: Dict) -> str:
        ...
        lines.append("")
        lines.append("  ╔══════════════════════════════════════════════════════════╗")
        lines.append("  ║                    📊 RoveAgent Insights                    ║")
    ...
    def format_gateway(self, report: Dict) -> str:
        ...
        lines.append(f"📊 **RoveAgent Insights** — Last {days} days\n")
