...
Grep recipe::
    ...
    tail -f ~/.roveagent/logs/agent.log | rg 'lsp\\['
"""
...
event_log = logging.getLogger("roveagent.lint.lsp")
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install via `roveagent lsp install <id>` or set lsp.servers.<id>.command)",
        )
    else:
