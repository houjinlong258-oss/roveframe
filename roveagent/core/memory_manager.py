...
from roveagent.core.memory_provider import MemoryProvider, PRE_COMPRESS_CHECKPOINT_API_VERSION
    ...
    if not memory_provider_tools_exposed(agent):
        ...
        if _providers:
            logger.info(
                ...
                "roveagent.core.disabled_toolsets) — provider tools and system-prompt "
...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        from roveagent.toolsets import _ROVEAGENT_CORE_TOOLS

        _core_tool_names = set(_ROVEAGENT_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, RoveAgent expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile ROVEAGENT_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped ROVEAGENT_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``roveagent_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_roveagent_home()`` themselves.
        """
        if "roveagent_home" not in kwargs:
            from roveagent.constants import get_roveagent_home
            kwargs["roveagent_home"] = str(get_roveagent_home())
        for provider in self._providers:
            try:
