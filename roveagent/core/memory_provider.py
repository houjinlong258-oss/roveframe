...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `roveagent backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - roveagent_home (str): The active ROVEAGENT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.roveagent``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "roveagent").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'roveagent memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], roveagent_home: str) -> None:
        ...
        Called by 'roveagent memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``roveagent_home`` is the active ROVEAGENT_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE ROVEAGENT_HOME.

        ``roveagent backup`` only walks ROVEAGENT_HOME, so any provider state kept
        ...
        of the archive, and ``roveagent import`` restores them to their original
