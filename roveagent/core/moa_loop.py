...
MoA-enabled; the normal RoveAgent agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from roveagent.clisupport.moa_config import coerce_privacy_filter
...
def _run_reference(
    ...
    *,
    ...
    """
    from roveagent.core.usage_pricing import CanonicalUsage, estimate_usage_cost, normalize_usage
    ...
    try:
        ...
        from roveagent.core.auxiliary_client import _normalize_aux_provider
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of RoveAgent boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model RoveAgent agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "RoveAgent agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal RoveAgent agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
class MoAChatCompletions:
    ...
    def consume_and_save_trace(
        ...
        if not pending or "aggregator_input_messages" not in pending:
            return
        try:
            from roveagent.core.moa_trace import save_moa_turn
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        from roveagent.clisupport.config import get_config_path, load_config
        ...
        if _refs_from_cache:
            ...
            try:
                from roveagent.core.moa_trace import slot_metrics
