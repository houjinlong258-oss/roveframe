...
from roveagent.utils import atomic_json_write, atomic_yaml_write, base_url_host_matches, base_url_hostname

from roveagent.constants import OPENROUTER_MODELS_URL
from roveagent.core.message_metadata import PERSISTENCE_ONLY_MESSAGE_FIELDS
...
# ``patch("roveagent.core.model_metadata.requests.get")`` in tests, which resolves the
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``ROVEAGENT_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    for env_var in ("ROVEAGENT_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
try:
    from roveagent.providers import list_providers as _list_providers
except Exception:
    def _list_providers():
        return []
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_roveagent_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from roveagent.constants import get_roveagent_home
    return get_roveagent_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run RoveAgent Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent RoveAgent from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for RoveAgent' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets RoveAgent minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from roveagent.constants import get_roveagent_home
    return get_roveagent_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # RoveAgent grow conversations past the runtime limit and Ollama
...
# who never asked for it. The variant suffix is a RoveAgent-side alias: it is
...
# RoveAgent-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a RoveAgent picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are RoveAgent picker aliases — the Codex catalog only
...
def get_model_context_length(
    ...
    if provider and model:
        try:
            from roveagent.core.models_dev import _override_context_window
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # entries are dropped — never a cached value above the table.
                ...
                try:
                    from roveagent.core.bedrock_adapter import get_bedrock_context_length
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from roveagent.clisupport.models import get_copilot_model_context
    ...
    if effective_provider:
        from roveagent.core.models_dev import lookup_models_dev_context
    ...
    # ``RoveAgent-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets RoveAgent sends to providers:
