...
Data resolution order:
  ...
  2. Disk cache (~/.roveagent/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: RoveAgent ↔ models.dev
# ---------------------------------------------------------------------------

# RoveAgent provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; RoveAgent' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → RoveAgent ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_roveagent_ids(mdev_id: str) -> List[str]:
    """Return the RoveAgent provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for roveagent_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(roveagent_id)
...
def _get_cache_path() -> Path:
    ...
    return get_roveagent_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from roveagent.constants import get_roveagent_home
    return get_roveagent_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``roveagent config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the RoveAgent provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by RoveAgent id.
    for roveagent_id in _models_dev_to_roveagent_ids(provider_key):
        if roveagent_id != provider_key:
            candidates.append(roveagent_id)
...
def _get_provider_models(
    ...
    """Resolve a RoveAgent provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``roveagent model``), which is an interactive user-facing flow
    where a fresh catalog is worth a short network wait.
    """
    from roveagent.clisupport.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal RoveAgent agent traffic.
...
def get_provider_info(
    ...
    Accepts either a RoveAgent provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve RoveAgent ID → models.dev ID
...
def get_model_info(
    ...
    Accepts RoveAgent or models.dev provider ID.  Tries exact match then
