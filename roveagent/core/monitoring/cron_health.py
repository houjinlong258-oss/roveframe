...
from roveagent.core.monitoring.events import CronExecutionEvent
from roveagent.core.monitoring.gateway_health import GatewayHealthSnapshot, GatewayMetric
...
def build_cron_health_snapshot() -> CronHealthSnapshot:
    metrics: list[GatewayMetric] = []
    for name, reader in (
        ("roveagent.cron.scheduler.heartbeat_age_seconds", get_ticker_heartbeat_age),
        ("roveagent.cron.scheduler.last_success_age_seconds", get_ticker_success_age),
    ):
        try:
            value = reader()
    ...
    try:
        ...
        metrics.append(GatewayMetric("roveagent.cron.jobs.enabled", len(enabled), {}))
        metrics.append(
            GatewayMetric(
    ...
    try:
        metrics.append(
            GatewayMetric("roveagent.cron.jobs.running", len(get_running_job_ids()), {})
        )
    except Exception:
