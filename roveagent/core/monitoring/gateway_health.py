...
from roveagent.core.monitoring.events import GatewayDiagnosticEvent, GatewayHealthEvent
...
def _allowed_logger(name: str) -> bool:
    return name == "gateway" or name.startswith("roveagent.gateway.")
...
def subsystem_for_logger(logger_name: str) -> str:
    if logger_name == "roveagent.gateway.relay" or logger_name.startswith("roveagent.gateway.relay."):
        return "platform.relay"
    if logger_name.startswith("roveagent.gateway.platforms."):
...
def _derive_busy(gateway_running: bool, gateway_state: Any, active_agents: Any) -> bool:
    try:
        from roveagent.gateway.status import derive_gateway_busy
...
def _derive_drainable(gateway_running: bool, gateway_state: Any) -> bool:
    try:
        from roveagent.gateway.status import derive_gateway_drainable
...
def _base_attrs(*, profile: str, install_id: str, version: str, supervision_mode: str) -> Dict[str, str]:
    ...
    return {
        ...
        "roveagent.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("roveagent.gateway.up", 1 if gateway_running else 0, base),
        _metric("roveagent.gateway.active_agents", active_agents, base),
        _metric("roveagent.gateway.busy", 1 if busy else 0, base),
        _metric("roveagent.gateway.drainable", 1 if drainable else 0, base),
        _metric("roveagent.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("roveagent.gateway.state", 1, base, **{"roveagent.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"roveagent.platform": str(platform), "roveagent.platform.state": state},
        ))
        metrics.append(_metric(
            "roveagent.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"roveagent.platform": str(platform), "roveagent.platform.state": state, "roveagent.error_code": error_code},
        ))
        if is_degraded:
    ...
    events.insert(0, GatewayHealthEvent(
        name="roveagent.gateway.health_snapshot",
    ...
    try:
        ...
        if old_gateway_state != new_gateway_state and new_gateway_state:
            out.append(GatewayHealthEvent(
                name="roveagent.gateway.lifecycle",
            ...
            if new_gateway_state == "startup_failed":
                out.append(GatewayDiagnosticEvent(
                    name="roveagent.gateway.startup_failed",
                    subsystem="gateway",
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            ...
            event = GatewayDiagnosticEvent(
                name=f"roveagent.gateway.log.{record.levelname.lower()}",
                subsystem=subsystem,
