...
configured under ``monitoring.export.otlp``. Lets an operator stream RoveAgent
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install roveagent-agent[otlp]``), imported lazily so the
...
def _require_sdk(*, auto_install: bool = True, prompt: bool = True):
    ...
    try:
        ...
        raise OTLPUnavailable(
            ...
            "    pip install 'roveagent-agent[otlp]'\n"
            f"(import error: {e})"
        )
...
def _resource_attributes(config: Dict[str, Any]) -> Dict[str, str]:
    ...
    from roveagent.core.monitoring.gateway_health_export import _runtime_resource_attributes
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"roveagent.event": kind or "unknown"}
    keep_by_kind = {
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'roveagent-agent[otlp]'")
        return None
    from roveagent.core.monitoring.emitter import get_emitter
