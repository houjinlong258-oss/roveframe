...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time RoveAgent starts and finds ``~/.openclaw/``.

    Points users at ``roveagent claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``roveagent claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to RoveAgent, run "
        "`roveagent claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `roveagent claw cleanup` (renames it to ~/.openclaw.pre-migration — "
