...
an 8-column × 9-row atlas; older RoveAgent/petdex sheets used an 8-row atlas.
RoveAgent infers the row taxonomy from the sheet and maps agent activity onto
...
- :mod:`agent.pet.store`     — install / list / resolve pets on disk
                               (profile-aware via ``get_roveagent_home()``).
...
from roveagent.core.pet.constants import (
    DEFAULT_SCALE,
    FRAME_H,
    FRAME_W,
    FRAMES_PER_STATE,
...
from roveagent.core.pet.state import derive_pet_state
