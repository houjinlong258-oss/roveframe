...
row taxonomy is inferred from the atlas shape so RoveAgent can render both legacy
...
# rows (1536x1872), while older RoveAgent/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are RoveAgent' activity state names. They are not always identical to the
...
# Legacy RoveAgent/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical RoveAgent activity names -> accepted row-name aliases in descending
