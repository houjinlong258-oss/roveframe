...
Plugins built on RoveAgent Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing RoveAgent
...
:class:`~roveagent_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors RoveAgent' main config
...
handles every provider, fallback chain, and per-task override RoveAgent
supports.
"""
...
def _resolve_task_ownership(plugin_id: str) -> tuple[frozenset, frozenset]:
    ...
    try:
        ...
        pass
    try:
        from roveagent.clisupport.main import _AUX_TASKS
...
def _resolve_attribution(
    *,
    ...
    if route_provider:
        ...
        try:
            from roveagent.core.auxiliary_client import _read_main_provider
    ...
    if isinstance(response_model, str) and response_model.strip():
        ...
        try:
            from roveagent.core.auxiliary_client import _read_main_model
...
class PluginLlm:
    ...
    Instances are constructed by :class:`roveagent_cli.plugins.PluginContext`
    ...
    async def _invoke_async(
        ...
        *,
        ...
        from roveagent.core.auxiliary_client import async_call_llm
        merged_extra = dict(extra_body or {})
        if profile_override:
            merged_extra.setdefault("metadata", {})["auth_profile"] = profile_override
        route_info: Optional[Dict[str, str]] = {} if task else None
