...
Every RoveAgent request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to RoveAgent Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=roveagent-agent",
        "client=roveagent-client-v<__version__>",
    ]

The version is sourced live from ``roveagent_cli.__version__`` so it auto-aligns
...
``roveagent_cli.__version__`` is the canonical source of truth.
"""
...
def _roveagent_version() -> str:
    """Return the current RoveAgent release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``roveagent_cli`` cannot be imported (should
...
def roveagent_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=roveagent-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=roveagent-client-v{_roveagent_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a RoveAgent session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    RoveAgent conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.roveagent/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific RoveAgent
    ...
    """
    tags = ["product=roveagent-agent", roveagent_client_tag()]
