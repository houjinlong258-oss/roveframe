...
from roveagent.tools.threat_patterns import scan_for_threats as _scan_for_threats
...
_ROVEAGENT_MD_NAMES = (".roveagent.md", "ROVEAGENT.md")


def _find_roveagent_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.roveagent.md`` or ``ROVEAGENT.md``.
    ...
    # could pick up a .roveagent.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _ROVEAGENT_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are RoveAgent Agent, built by Nous Research. Be direct: match the "
...
ROVEAGENT_AGENT_HELP_GUIDANCE = (
    ...
    # precedence. Injected only when skill_view exists AND the roveagent-agent
    # skill is actually installed (see system_prompt.py slot resolution).
    "You run on RoveAgent Agent (by Nous Research). When the user needs help with "
    "RoveAgent itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://roveagent-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. The `roveagent-agent` skill has the actual commands and proven "
    "workflows — load it with skill_view(name='roveagent-agent') before configuring, "
    "modifying, or troubleshooting RoveAgent so you don't guess or invent workarounds."
)
...
ROVEAGENT_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on RoveAgent Agent (by Nous Research). When the user needs help with "
    "RoveAgent itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://roveagent-agent.nousresearch.com/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.roveagent/kanban.db`. Your task id is in "
    "`$ROVEAGENT_KANBAN_TASK`; your workspace is `$ROVEAGENT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ROVEAGENT_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ROVEAGENT_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ROVEAGENT_KANBAN_BRANCH:-wt/$ROVEAGENT_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ROVEAGENT_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`roveagent profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `roveagent kanban <verb>` for board operations. Use "
...
#
# The roveagent-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to roveagent-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: RoveAgent appends their message to the "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of RoveAgent floating over another application, so the
    user is rarely asking about RoveAgent — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating RoveAgent "
        ...
        "rather than anything inside RoveAgent. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the RoveAgent terminal UI (TUI). Files: there is no "
    ...
    "desktop": (
        ...
        "You are chatting inside the RoveAgent desktop app, a graphical chat "
        ...
        "data-roveagent-send=\"prompt\" on any clickable element (or "
        "window.roveagent.send(\"prompt\")) sends that prompt as a hidden user "
...
# container / remote host rather than on the machine where RoveAgent itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host RoveAgent runs on.
    """
    ...
    try:
        ...
        from roveagent.tools.terminal_tool import _create_environment, _get_env_config  # type: ignore
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where RoveAgent itself is running. The host OS, home, and cwd "
                f"of the RoveAgent process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where RoveAgent "
    ...
    # Embedder-supplied environment description. Lets a host that wraps RoveAgent
    ...
    extra = (os.getenv("ROVEAGENT_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_roveagent_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ROVEAGENT_PLATFORM") or os.environ.get("ROVEAGENT_SESSION_PLATFORM")
    if platform:
        return platform

    session_context = sys.modules.get("roveagent.gateway.session_context")
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.roveagent/skills/`` directory.  External dirs
    ...
    # ROVEAGENT_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_roveagent_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.roveagent/skills, ./.agents/skills at
        ...
        from roveagent.core.skill_utils import get_project_skills_dirs
        ...
        if _home_token is not None:
            reset_roveagent_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if project_dirs:
        from roveagent.core.skill_utils import iter_project_skill_files
...
def load_soul_md(
    ...
    """Load SOUL.md from ROVEAGENT_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the ROVEAGENT_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from roveagent.clisupport.config import ensure_roveagent_home
        ensure_roveagent_home()
    except Exception as e:
        logger.debug("Could not ensure ROVEAGENT_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_roveagent_home()
...
def _load_roveagent_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.roveagent.md / ROVEAGENT.md — walk to git root."""
    roveagent_md_path = _find_roveagent_md(cwd_path)
    if not roveagent_md_path:
        return ""
    try:
        content = roveagent_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = roveagent_md_path.name
        try:
            rel = str(roveagent_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".roveagent.md", context_length=context_length,
            read_path=str(roveagent_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", roveagent_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .roveagent.md / ROVEAGENT.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ROVEAGENT_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the RoveAgent install/source
    ...
    # explicitly configured cwd is honored verbatim — the RoveAgent tree is a
    ...
    # their launch dir IS the user's shell cwd (developing RoveAgent in-tree).
    from roveagent.core.runtime_cwd import _is_install_tree
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the RoveAgent install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_roveagent_md(cwd_path, context_length)
    ...
    # SOUL.md from ROVEAGENT_HOME only — skip when already loaded as identity
    if not skip_soul:
