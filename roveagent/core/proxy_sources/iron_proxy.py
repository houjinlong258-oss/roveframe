...
* The ``iron-proxy`` binary is auto-installed into ``<roveagent_home>/bin/iron-proxy``
  on first use.  RoveAgent pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<roveagent_home>/proxy/ca.{crt,key}`` is generated on
  first ``roveagent egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<roveagent_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``roveagent egress start``), pidfile
  at ``<roveagent_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<roveagent_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``roveagent egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<roveagent_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "ROVEAGENT_IRON_PROXY_MGMT_KEY"
...
# credential (RoveAgent' auth.py keys Google on both GEMINI_API_KEY and
...
def _roveagent_bin_dir() -> Path:
    from roveagent.constants import get_roveagent_home

    return get_roveagent_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.roveagent/proxy/`` just to
    check whether a pidfile exists.
    """
    from roveagent.constants import get_roveagent_home

    return get_roveagent_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<roveagent_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _roveagent_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``roveagent proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _roveagent_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="roveagent-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "roveagent-agent"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``roveagent egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="roveagent-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
        ...
        subprocess.run(  # noqa: S603
            [
                ...
                "-key", str(tmp_key),
                "-sha256", "-days", "3650",
                "-subj", "/CN=roveagent iron-proxy CA",
...
def mint_proxy_token(prefix: str = "roveagent-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<roveagent_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``ROVEAGENT_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``roveagent egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="roveagent-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `roveagent egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `roveagent egress setup` and use "
            "`roveagent egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `roveagent egress setup`, "
            "then `roveagent egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``roveagent egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `roveagent egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<roveagent_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a RoveAgent dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``roveagent egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``roveagent egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_ROVEAGENT_IRON_PROXY_NONCE_ENV = "ROVEAGENT_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_ROVEAGENT_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `roveagent egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `roveagent egress setup` first."
        )
    ...
    env[_ROVEAGENT_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the RoveAgent
    # process.
    #
    ...
    # ``roveagent egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``roveagent egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `roveagent egress stop` if that proxy is stuck."
            )
...
def _build_proxy_subprocess_env(
    *,
    ...
    if refresh_from_bitwarden and bitwarden_config:
        try:
            from roveagent.core.secret_sources import bitwarden as bw
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`roveagent egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `roveagent secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
