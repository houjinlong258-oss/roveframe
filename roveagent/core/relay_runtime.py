"""Profile-scoped NeMo Relay runtimes owned by the RoveAgent agent core."""
...
SESSION_SCOPE = "roveagent.session"
TURN_SCOPE = "roveagent.turn"
LOGICAL_LLM_SCOPE = "roveagent.logical_llm_call"
RUNTIME_SCHEMA_KEY = "roveagent.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "roveagent.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "roveagent.relay.runtime_instance"
RELAY_PLUGINS_EXECUTION_CONSUMER = "roveagent.nemo_relay.plugins"
...
def _scope_op_executor():
    ...
    if _SCOPE_OP_EXECUTOR is None:
        with _SCOPE_OP_EXECUTOR_LOCK:
            if _SCOPE_OP_EXECUTOR is None:
                from roveagent.tools.daemon_pool import DaemonThreadPoolExecutor
...
def pop_relay_scope(
    ...
    *,
    ...
    'metadata'`` when RoveAgent finalization forwards runtime metadata. Filter to
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a RoveAgent session."""
    ...
    if _SEGMENTS_CONFIG is None:
        with _SEGMENTS_CONFIG_LOCK:
            if _SEGMENTS_CONFIG is None:
                ...
                max_turns = 0
                try:
                    from roveagent.gateway.run import _load_gateway_config  # late import
    ...
    def run_in_session(
        ...
        **kwargs: Any,
        ...
        with self._lock:
            ...
            if runtime is None:
                raise RuntimeError("RoveAgent Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            if self._active and not self._clear_active():
                logger.warning(
                    "RoveAgent Relay plugin cleanup is still pending; refusing to "
            ...
            try:
                ...
                logger.warning(
                    "RoveAgent could not determine whether a process-global Relay "
            ...
            if existing_report is not None:
                logger.warning(
                    ...
                    "outside RoveAgent native ownership; leaving it unchanged and "
                    "disabling RoveAgent-managed Relay middleware for this process"
                )
                return self._remember(
                    owner_id,
            ...
            try:
                ...
                if dynamic_plugins:
                    try:
                        ...
                        raise RuntimeError(
                            "RoveAgent Relay dynamic plugin activation failed"
                        ) from exc

                if self._activation is None:
                    # RoveAgent only enters Relay's initialization path after an
                ...
                logger.warning(
                    "RoveAgent Relay plugin initialization failed: %s",
                    exc,
                    exc_info=True,
                )
                return self._remember(
            ...
            logger.info(
                ...
                "hosted by this RoveAgent process."
            )
            return state
    ...
    def _clear_active(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "RoveAgent Relay plugin subscriber flush failed",
                exc_info=True,
            )
            return False
        try:
            ...
            logger.warning(
                "RoveAgent Relay plugin configuration cleanup failed",
                exc_info=True,
            )
            return False
        self._active = False
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a RoveAgent-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    ...
                    old_handle,
                    output={"roveagent.session.segment_reason": reason},
                    metadata={
                        RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                        RUNTIME_INSTANCE_KEY: self.runtime_id,
                    },
                ...
                logger.warning(
                    "RoveAgent Relay segment close failed (session=%s segment=%d); "
            ...
            scope_metadata = {
                RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                RUNTIME_INSTANCE_KEY: self.runtime_id,
                "roveagent.session.segment": session.segment,
                "roveagent.session.segment_reason": reason,
            }
            parent_handle = None
            if session.parent_session_id:
                with self._sessions_lock:
            ...
            try:
                ...
                logger.warning(
                    "RoveAgent Relay segment open failed (session=%s segment=%d); "
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active RoveAgent Relay session without creating one."""
        with self._sessions_lock:
            if self._closing:
                return None
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a RoveAgent session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("RoveAgent Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("RoveAgent Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("RoveAgent Relay runtime is shutting down")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the RoveAgent session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before RoveAgent authorizes a tool call."""
        ...
        def close_with_drain() -> None:
            ...
            for _ in range(drain_limit):
                ...
                try:
                    ...
                    logger.warning(
                        "RoveAgent Relay orphaned scope drain failed",
                        exc_info=True,
                    )
                    break
            ...
            if drained_holder["count"]:
                logger.warning(
                    "RoveAgent Relay drained %d orphaned scope(s) before closing %s",
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "RoveAgent Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        if has_active_operations:
            thread = threading.Thread(
                ...
                name=f"roveagent-nemo-relay-shutdown-{self.runtime_id[:8]}",
                daemon=True,
            )
            try:
                thread.start()
                ...
                with self._sessions_lock:
                    self._shutdown_started = False
                logger.warning(
                    "RoveAgent Relay deferred shutdown could not start",
                    exc_info=True,
                )
            return
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("RoveAgent Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("RoveAgent Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical RoveAgent profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "RoveAgent Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one RoveAgent turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "roveagent_relay_turn", default=None
)

# Depth of managed Relay callbacks executing on the current logical call path.
# Set >0 while the native Relay pipeline is mid-dispatch of a RoveAgent callback
...
_MANAGED_CALLBACK_DEPTH: contextvars.ContextVar[int] = contextvars.ContextVar(
    "roveagent_relay_managed_callback_depth", default=0
)
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for RoveAgent core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "RoveAgent Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"roveagent.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "RoveAgent Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("RoveAgent Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # RoveAgent turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent RoveAgent turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "RoveAgent Relay segment rotation failed", exc_info=True
                )
            try:
                turn.handle = lease.host.run_in_session(
                    ...
                    metadata={
                        ...
                        "roveagent.execution_surface": lease.platform or "unknown",
                    },
                    timeout=_SCOPE_OP_TIMEOUT,
                )
            except Exception:
                logger.warning("RoveAgent Relay turn initialization failed", exc_info=True)
            ...
            try:
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        ...
                        if failure:
                            logger.warning(
                                "RoveAgent Relay turn finalization failed: %s",
                                failure,
                            )
            finally:
                try:
                ...
                try:
                    ...
                    logger.warning(
                        "RoveAgent Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "RoveAgent Relay deferred session close failed", exc_info=True
            )
    ...
    def notify_session_compacted(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "RoveAgent Relay compaction notification failed", exc_info=True
            )
    ...
    def _finish_logical_calls(
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            logger.warning("RoveAgent Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0:
        ...
        # a RoveAgent tool). Nested managed execution here is structurally
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a RoveAgent session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("RoveAgent Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at RoveAgent's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by RoveAgent core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("RoveAgent Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared RoveAgent session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("RoveAgent Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("RoveAgent Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared RoveAgent session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("RoveAgent Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active RoveAgent profile."""
...
def _configured_plugin_inputs(
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError(
                "RoveAgent [[dynamic_plugins]] records are unsupported; use Relay "
        ...
        raise _RelayPluginConfigurationLoadError(
            "RoveAgent Relay plugin configuration could not be loaded from "
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="roveagent-nemo-relay-plugin-lifecycle",
        daemon=True,
    )
    thread.start()
    thread.join()
