...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ROVEAGENT_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # RoveAgent runtime / deployment
    "ROVEAGENT_HOME", "ROVEAGENT_PROFILE", "ROVEAGENT_GATEWAY_LOCK_DIR",
    "ROVEAGENT_MAX_ITERATIONS", "ROVEAGENT_MAX_TOKENS", "ROVEAGENT_API_TIMEOUT",
    "ROVEAGENT_REDACT_SECRETS", "ROVEAGENT_NOUS_TIMEOUT_SECONDS",
    "_ROVEAGENT_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ROVEAGENT_KANBAN_DB", "ROVEAGENT_KANBAN_WORKSPACES_ROOT", "ROVEAGENT_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ROVEAGENT_KANBAN_",
    "ROVEAGENT_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset RoveAgent writes
    ...
    ``roveagent_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical RoveAgent parser: save_env_value
    ...
    from roveagent.clisupport.config import _parse_env_value
...
def build_profile_secret_scope(roveagent_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(roveagent_home)
    secrets = load_env_file(home / ".env")

    try:
        from roveagent.clisupport.env_loader import get_secret_source_values
