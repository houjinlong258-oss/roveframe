...
cache problem must never block RoveAgent startup.
"""
...
from roveagent.core.secret_sources.base import (  # noqa: E402
    FetchResult,
    is_valid_env_name,
)
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the RoveAgent home used for cache paths.

    ``home_path`` is whatever ``load_roveagent_dotenv()`` already resolved;
    falling back to ``$ROVEAGENT_HOME`` / ``~/.roveagent`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from roveagent.constants import get_roveagent_home

        home_path = get_roveagent_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<roveagent_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ROVEAGENT_HOME is ever made traversable.
            try:
