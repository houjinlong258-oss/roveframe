...
``roveagent_cli.env_loader._apply_external_secret_sources()``.
...
from roveagent.core.secret_sources.base import (
...
def get_source(name: str, *, scope: Optional[str] = None) -> Optional[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        return _SCOPED_SOURCES.get(scope or roveagent_home_key(), {}).get(
            name
...
def list_sources(*, scope: Optional[str] = None) -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(scope or roveagent_home_key(), {}))
...
def list_plugin_sources() -> List[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        ...
        merged.update(_SCOPED_SOURCES.get(roveagent_home_key(), {}))
        return list(merged.values())
...
def _ensure_builtin_sources() -> None:
    ...
    with _REGISTRY_LOCK:
        ...
        try:
            from roveagent.core.secret_sources.bitwarden import BitwardenSource
        ...
        try:
            from roveagent.core.secret_sources.onepassword import OnePasswordSource
        ...
        try:
            from roveagent.core.secret_sources.command import CommandSource
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``roveagent`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's ROVEAGENT_HOME is ``~/.roveagent/profiles/<name>``; the
    default profile (``~/.roveagent``) returns "".
    """
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(
        secrets_cfg, scope=roveagent_home_key(home_path)
    )
    if not enabled:
        return report
