...
Storage
-------
Bundles live in ``~/.roveagent/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ROVEAGENT_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``roveagent bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ROVEAGENT_HOME.

    Honors ``ROVEAGENT_BUNDLES_DIR`` for tests; falls back to
    ``<ROVEAGENT_HOME>/skill-bundles``.
    """
    override = os.environ.get("ROVEAGENT_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_roveagent_home() / "skill-bundles"
...
def build_bundle_invocation_message(
    ...
    from roveagent.core.skill_commands import _load_skill_payload, _build_skill_message
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `roveagent bundles` CLI subcommand.
# ---------------------------------------------------------------------------
