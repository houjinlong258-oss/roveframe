...
from roveagent.core.skill_preprocessing import (
...
#
# When a user invokes a /skill (or /bundle), RoveAgent expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_roveagent_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``ROVEAGENT_PLATFORM`` env var and
    ``ROVEAGENT_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("ROVEAGENT_PLATFORM")
            or get_session_env("ROVEAGENT_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("ROVEAGENT_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective RoveAgent home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_roveagent_home_override``), but the
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        from roveagent.tools.skills_tool import SKILLS_DIR, skill_view
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.roveagent.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_roveagent_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.roveagent/skills/ and return a mapping of /command -> skill info.
    ...
    try:
        from roveagent.tools.skills_tool import SKILLS_DIR, _parse_frontmatter, skill_matches_platform, skill_matches_environment, _get_disabled_skill_names
        from roveagent.core.skill_utils import (
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core RoveAgent slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core RoveAgent command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's RoveAgent home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.roveagent/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``roveagent -s <skill>`` or
    a deployment's ``ROVEAGENT_TUI_SKILLS`` env var could force-load a skill an
