...
from roveagent.constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.roveagent/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    ROVEAGENT_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ROVEAGENT_KANBAN_TASK`` / ``ROVEAGENT_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ROVEAGENT_KANBAN_TASK") or os.getenv("ROVEAGENT_KANBAN_BOARD"):
        ...
        if _owns_dispatcher_task:
            ...
            try:
                from roveagent.tools.kanban_tools import _profile_has_kanban_toolset
        ...
        # The RoveAgent Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``roveagent_cli.config`` on the
...
# `roveagent-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting RoveAgent itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with RoveAgent, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"roveagent-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ROVEAGENT_PLATFORM`` or
            ``ROVEAGENT_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("ROVEAGENT_PLATFORM")
        or get_session_env("ROVEAGENT_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``roveagent config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``roveagent`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.roveagent/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``roveagent`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ROVEAGENT_HOME, not cwd
        if not p.is_absolute():
            p = (roveagent_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.roveagent/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.roveagent/skills`` at the git root) are
...
#   <root>/.roveagent/skills/   — RoveAgent-native location
...
# a one-line "run `roveagent skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when ROVEAGENT_HOME itself lives inside a git checkout:
    ``<root>/.roveagent/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``roveagent skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`roveagent skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under ROVEAGENT_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_roveagent_home() / "cache" / "project_skill_scans"
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    if cached is not None:
        return cached
    try:
        from roveagent.tools.skills_guard import scan_skill_cached
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.roveagent/skills/`` (including via symlinks) or configured
    ...
    # ~/.roveagent/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: RoveAgent can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    roveagent = metadata.get("roveagent") or {}
    if not isinstance(roveagent, dict):
        roveagent = {}
    return {
        "fallback_for_toolsets": roveagent.get("fallback_for_toolsets", []),
        "requires_toolsets": roveagent.get("requires_toolsets", []),
        "fallback_for_tools": roveagent.get("fallback_for_tools", []),
        "requires_tools": roveagent.get("requires_tools", []),
        ...
        "session_platforms": roveagent.get("session_platforms", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    roveagent = metadata.get("roveagent")
    if not isinstance(roveagent, dict):
        return []
    raw = roveagent.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes RoveAgent metadata, VCS, virtualenv/dependency, cache, and skill
