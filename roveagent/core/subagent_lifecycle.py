"""Public, plugin-safe lifecycle API for delegated RoveAgent subagents.
...
from roveagent.tools.daemon_pool import DaemonThreadPoolExecutor as _DaemonExecutor

_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="roveagent-lifecycle")
_SECRET = secrets.token_bytes(32)
_ACTIVE_PARENT_AGENT: contextvars.ContextVar[Any] = contextvars.ContextVar(
    "roveagent_subagent_lifecycle_parent", default=None
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active RoveAgent parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("RoveAgent failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _run(self, record: _Record, goal: str, parent: Any) -> None:
        ...
        try:
            from roveagent.tools.delegate_tool import _run_child_lifecycle
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because RoveAgent delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. RoveAgent always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
