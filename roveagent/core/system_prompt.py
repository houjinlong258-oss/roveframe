...
upstream prefix cache warm.  See ``roveagent-agent-dev``'s
...
from roveagent.core.prompt_builder import (
...
from roveagent.core.runtime_cwd import resolve_context_cwd
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- roveagent-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _ra():
    ...
    ``patch("roveagent.runtime.load_soul_md", ...)``; if we imported them
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ROVEAGENT_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ROVEAGENT_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the ROVEAGENT_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from roveagent.clisupport.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from roveagent.clisupport.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound ROVEAGENT_HOME ContextVar override wins. Surfaces that multiplex
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  RoveAgent never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ROVEAGENT_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the ROVEAGENT_HOME ContextVar
    ...
    # Pointer to the docs (and, when it exists, the roveagent-agent skill) for
    # user questions about RoveAgent itself. The skill_view() pointer is a
    # dangling reference in two cases — no skill tools in the toolset
    # (Blank Slate) OR the roveagent-agent skill not installed — so the
    ...
    stable_parts.append(ROVEAGENT_AGENT_HELP_GUIDANCE_NO_SKILLS)
    ...
    # ROVEAGENT_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if agent.valid_tool_names:
        ...
        if _exec_inject:
            from roveagent.core.prompt_builder import execution_guidance_text
    ...
    if has_skills_tools:
        ...
        try:
            from roveagent.core.coding_context import coding_compact_skill_categories
    ...
    # the roveagent-agent skill actually present in the index (gating on the
    # rendered index line keeps this a pure string check — no second
    # filesystem scan, and it inherits the index cache's stability).
    if _has_skill_view and "- roveagent-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = ROVEAGENT_AGENT_HELP_GUIDANCE
    ...
    # Coding posture (base RoveAgent, any interactive coding surface in a code
    ...
    if agent.valid_tool_names:
        try:
            from roveagent.core.coding_context import coding_system_prompt_parts
    ...
    if getattr(agent, "_environment_probe", True):
        try:
            from roveagent.tools.env_probe import get_environment_probe_line
    ...
    # `roveagent -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['roveagent-bots']).
    ...
    # Active-profile hint — names the RoveAgent profile the agent is running
    # under so it doesn't conflate ~/.roveagent/skills/ (default profile) with
    # ~/.roveagent/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient ROVEAGENT_HOME: on a build thread that lost the ContextVar this
    ...
    try:
        if _agent_home_path is not None:
            ...
            from roveagent.core.file_safety import _resolve_active_profile_name
    ...
    # from get_default_roveagent_root(): get_roveagent_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_roveagent_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_roveagent_root())
    else:
        _home_str = _root_str = str(get_roveagent_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active RoveAgent profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_roveagent_root()), which in ambient
        # profile mode is NOT get_roveagent_home().
        profile_home = _home_str
        default_root = get_default_roveagent_root()
        post_workspace_parts.append(
            f"Active RoveAgent profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing RoveAgent). Every other surface (desktop chat panel,
        ...
        if getattr(agent, "_context_cwd_is_launch_artifact", False):
            ...
            # then reject RoveAgent's bundled contributor AGENTS.md (#97448).
            context_cwd = None
    ...
    if agent._memory_manager:
        try:
            from roveagent.core.memory_manager import memory_provider_tools_exposed as _mem_exposed
        except Exception:
            _mem_exposed = None
    ...
    from roveagent.timeutils import get_timezone as _roveagent_tz, now as _roveagent_now
    now = _roveagent_now()
