...
Providers live in ``~/.roveagent/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.roveagent/cache`` files land
  inside the backend (e.g. ``"~/.roveagent"`` for home-synced backends,
  ``"/root/.roveagent"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_roveagent_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced RoveAgent cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``roveagent setup`` after this backend is selected.
