...
from roveagent.core.context_compressor import LEGACY_SUMMARY_PREFIX
...
# RoveAgent' own machine-authored openers. A compaction handoff or a resumed
...
def _auto_title_enabled() -> bool:
    ...
    try:
        ...
        # from agent code paths where a module-level roveagent_cli import risks
...
def auto_title_session(
    ...
    post-``roveagent update`` stale-module window, where this function's lazy
    ...
    try:
        ...
        logger.warning(
            ...
            "restart the running RoveAgent process): %s",
            e,
        )
...
def _auto_title_session(
    ...
    from roveagent.core.aux_accounting import set_accounting_context
    from roveagent.core.portal_tags import set_conversation_context
...
def _is_real_user_turn(message: Any) -> bool:
    ...
    RoveAgent persists a lot of machinery under ``role="user"`` — compaction
