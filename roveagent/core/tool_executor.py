...
from roveagent.core.message_sanitization import coalesce_tool_call_id
...
from roveagent.tools.tool_result_storage import (
    maybe_persist_tool_result,
    enforce_turn_budget,
    extract_persisted_path,
)
from roveagent.tools.budget_config import BudgetConfig, DEFAULT_BUDGET, budget_for_context_window
...
def _ensure_file_checkpoint(
    ...
    # which can differ from the RoveAgent process cwd (notably in Docker).  Resolve
    # through that same path pipeline before asking the checkpoint manager to
    # discover the project root.
    from roveagent.tools.file_tools import _resolve_path_for_task
...
def _authorization_gate_lock_timeout() -> float:
    ...
    """
    try:
        from roveagent.tools.approval import human_wait_ceiling
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``ROVEAGENT_CONCURRENT_TOOL_TIMEOUT_S`` env var
    ...
    return resolve_timeout(
        "roveagent.tools.concurrent_batch",
        default=_DEFAULT_CONCURRENT_TOOL_TIMEOUT_S,
        env_var="ROVEAGENT_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current RoveAgent process before the normal turn-end persistence path runs.
...
def _tool_search_scoped_names(agent) -> frozenset:
    ...
    try:
        ...
        from roveagent.tools import tool_search as _ts
...
class _ConcurrentToolAuthorizationGate:
    ...
    def _human_wait_seconds(self) -> float:
        try:
            from roveagent.tools.approval import human_wait_seconds
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before RoveAgent policy and dispatch exactly once."""
    from roveagent.core import relay_tools
    from roveagent.clisupport.middleware import (
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
                    "RoveAgent tool execution callback invoked more than once"
                )
    ...
    def _roveagent_pipeline(relay_args: dict[str, Any]) -> Any:
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``ROVEAGENT_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
    ...
    return resolve_timeout(
        "roveagent.tools.sequential_call",
...
def _begin_tool_execution(
    ...
    *,
    ...
    try:
        from roveagent.tools.environments.base import set_activity_callback
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo":
            ...
            def _execute(next_args: dict) -> Any:
                from roveagent.tools.bot_mode_dm import message_agent_tool as _message_agent_tool
            ...
            try:
                def _execute(next_args: dict) -> Any:
                    from roveagent.model_tools import suppress_post_tool_call_hook
