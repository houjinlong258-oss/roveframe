"""Upload a RoveAgent session transcript to Hugging Face as an agent trace.

RoveAgent stores sessions in its own SQLite store (``roveagent_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``roveagent trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through RoveAgent' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "roveagent-traces"
_ROVEAGENT_VERSION = "roveagent-agent"
...
# ---------------------------------------------------------------------------
# Conversion: RoveAgent OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses RoveAgent' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render RoveAgent conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.roveagent/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `roveagent trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/roveagent-traces`` dataset. Returns a
