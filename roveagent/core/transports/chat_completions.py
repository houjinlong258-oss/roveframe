...
from roveagent.core.moonshot_schema import is_moonshot_model, sanitize_moonshot_tools
from roveagent.core.prompt_builder import DEVELOPER_ROLE_MODELS
from roveagent.core.transports.base import ProviderTransport
from roveagent.core.transports.types import NormalizedResponse, ToolCall, Usage
...
_XAI_TOOL_SEARCH_ALIAS = "roveagent_tool_search"
...
def _rename_tool_search_bridge_for_xai(
    ...
    occupies ``roveagent_tool_search``, the bridge takes a ``_2``/``_3``
...
def _add_prompt_cache_key(
    ...
    *,
    ...
    from roveagent.core.transports.codex import _bounded_prompt_cache_key
    ...
    from roveagent.core.transports.codex import _cache_scope_from_session_id, _content_cache_key
...
def _reasoning_config_for_model(model: str, reasoning_config: dict | None) -> dict | None:
    ...
    RoveAgent' internal effort set extends the wire vocabulary with ``ultra``
...
def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate RoveAgent/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # Gemini 2.5 accepts thinkingBudget; don't guess a budget from RoveAgent'
    ...
    # is stricter (low/high). Clamp RoveAgent' wider effort set to what each
...
def _raise_gemini_thinking_max_tokens(
    ...
    global RoveAgent cap of 4096 is enough for visible text, but Ultra/high
    ...
    if not thinking_config:
        return requested
    from roveagent.core.gemini_native_adapter import _effective_gemini_max_output_tokens
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        - RoveAgent-internal scaffolding markers — any top-level message key
        ...
        for msg_idx, msg in enumerate(messages):
            ...
            # Drop all RoveAgent-internal scaffolding markers (``_``-prefixed).
    ...
    def _build_kwargs_from_profile(self, profile, model, sanitized, tools, params):
        ...
        """
        from roveagent.providers.base import OMIT_TEMPERATURE
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if message_tool_calls:
            ...
            for tc in message_tool_calls:
                ...
                # preserve an explicit blank name for RoveAgent's recovery path.
                ...
                # ``roveagent_tool_search`` dispatches as itself when no alias
