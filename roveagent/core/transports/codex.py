...
from roveagent.core.transports.base import ProviderTransport
from roveagent.core.transports.types import NormalizedResponse, ToolCall
...
# Wire-name used when RoveAgent keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through RoveAgent's configured provider
# (Firecrawl / Exa / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "roveagent_web_search"
...
# rename on the wire (roveagent_<name>), map back in normalize_response so
# RoveAgent dispatch is unaffected.
...
# RoveAgent's progressive-disclosure bridge registers exactly that literal
...
_RESERVED_TOOL_ALIAS_PREFIX = "roveagent_"
...
# named ``roveagent_tool_search`` is never silently rewritten to ``tool_search``
# unless THIS request actually emitted that alias (#95003 review contract).
_LEGACY_ALIAS_FALLBACK = {
    **_RESERVED_ALIAS_TO_NAME,
    "roveagent_web_search": "web_search",
}
...
def _alias_reserved_tools(
    ...
    a legitimate user/plugin/MCP tool already named ``roveagent_<x>`` is
...
def _xai_prefers_native_web_search() -> bool:
    ...
    """
    try:
        from roveagent.core.web_search_registry import get_active_search_provider

        provider = get_active_search_provider()
        if provider is not None:
            return getattr(provider, "name", None) == "xai"

        from roveagent.tools.web_tools import _get_search_backend
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def _resolve_issuer_kind(self, params: Dict[str, Any]) -> str:
        ...
        from roveagent.core.codex_responses_adapter import _classify_responses_issuer
    ...
    def convert_tools(self, tools: List[Dict[str, Any]]) -> Any:
        ...
        from roveagent.core.codex_responses_adapter import _responses_tools
    ...
    def build_kwargs(
        ...
        **params,
        ...
        from roveagent.runtime import DEFAULT_AGENT_IDENTITY
        ...
        # xAI server-side web search vs RoveAgent web providers.
        #
        ...
        #    keep RoveAgent dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``roveagent_web_search`` so Grok cannot hijack the name. The alias
        ...
        kwargs = {
            # ``-900k`` large-context picker variants are RoveAgent-side aliases
        ...
        if reasoning_enabled and is_xai_responses:
            from roveagent.core.model_metadata import grok_supports_reasoning_effort
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo THIS request's wire aliases before RoveAgent dispatch.
                ...
                # ``roveagent_tool_search`` etc. is dispatched as itself when
    ...
    def preflight_kwargs(
        ...
        *,
        ...
        """
        from roveagent.core.codex_responses_adapter import _preflight_codex_api_kwargs
...
from roveagent.core.transports import register_transport  # noqa: E402
