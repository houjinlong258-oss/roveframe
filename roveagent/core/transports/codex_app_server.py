...
projection into RoveAgent' display, approval bridging, transcript projection into
...
"codex_app_server"`. RoveAgent' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 RoveAgent secret — gateway
        ...
        spawn_env = roveagent_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("ROVEAGENT_KANBAN_TASK"):
            kanban_db = spawn_env.get("ROVEAGENT_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "ROVEAGENT_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ROVEAGENT_HOME", os.path.expanduser("~/.roveagent")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "roveagent",
        client_title: str = "RoveAgent Agent",
