...
Owns one Codex thread per RoveAgent session. Drives `turn/start`, consumes
...
from roveagent.core.codex_responses_adapter import _format_responses_error
from roveagent.core.redact import redact_sensitive_text
from roveagent.core.transports.codex_app_server import (
    CodexAppServerClient,
    CodexAppServerError,
)
from roveagent.core.transports.codex_event_projector import CodexEventProjector
...
# RoveAgent' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ROVEAGENT_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse RoveAgent/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per RoveAgent session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ROVEAGENT_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ROVEAGENT_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="roveagent",
            client_title="RoveAgent Agent",
            client_version=_get_roveagent_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but RoveAgent' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into RoveAgent' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own roveagent-tools callback we
            # auto-accept — the user already approved RoveAgent' tools
            ...
            if server_name == "roveagent-tools":
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO RoveAgent
        approval-mode/timeout logic. The RoveAgent-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; RoveAgent approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map RoveAgent approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    RoveAgent returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the RoveAgent approval mode/timeout resolution
    ...
    # "prompt expired"; the RoveAgent-side messaging already distinguishes them.
    return "decline"
...
def _get_roveagent_version() -> str:
    """Best-effort RoveAgent version string for codex's userAgent line."""
    try:
