"""Projects codex app-server events into RoveAgent' messages list.

The translator that lets RoveAgent' memory/skill review keep working under the
...
RoveAgent' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape RoveAgent
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way RoveAgent' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but RoveAgent stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way RoveAgent already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — RoveAgent' messages store text only.
        text_parts: list[str] = []
