...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into RoveAgent with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.roveagent/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of RoveAgent
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``roveagent tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``roveagent setup`` for
