...
from roveagent.core.memory_manager import build_memory_context_block
...
def _maybe_title_session_at_turn_start(agent: Any, messages: List[Any]) -> None:
    ...
    try:
        ...
        from roveagent.core.title_generator import maybe_auto_title
...
def reanchor_current_turn_user_idx(messages: List[Any], user_message: Any) -> int:
    ...
    """
    from roveagent.core.context_compressor import user_originated_turn_view
...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``roveagent logs``.
    ...
    try:
        from roveagent.core.auxiliary_client import set_runtime_main
    ...
    try:
        if not getattr(agent, "_skip_mcp_refresh", False):
            ...
            import sys as _sys
            if "roveagent.tools.mcp_tool" in _sys.modules:
                from roveagent.tools.mcp_tool import has_registered_mcp_tools, refresh_agent_mcp_tools
    ...
    from roveagent.core.agent_runtime_helpers import note_turn_start
    ...
    if reaction_callback is not None:
        try:
            from roveagent.core.reactions import detect_reaction
    ...
    try:
        from roveagent.tools.bot_mode_dm import ensure_message_agent_tool
    ...
    if (
        ...
        # RoveAgent only initiates compaction in "roveagent" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping RoveAgent preflight compression for codex app-server "
                "(mode=%s); RoveAgent will not start thread compaction here.",
    ...
    try:
        ...
        try:
            from roveagent.tools.hook_output_spill import (
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by RoveAgent (via _emit_status), so it always
