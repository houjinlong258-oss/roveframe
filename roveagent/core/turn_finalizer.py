...
decomposition campaign (``~/.roveagent/plans/god-file-decomposition.md``, Phase 1
...
keep the exact logger name (``"roveagent.core.conversation_loop"``).
"""
...
def finalize_turn(
    ...
    *,
    ...
    """
    from roveagent.core.conversation_loop import logger
    ...
    if iteration_limit_fallback:
        ...
        _kanban_task = os.environ.get("ROVEAGENT_KANBAN_TASK")
        if _kanban_task:
    ...
    try:
        ...
        if interrupted:
            from roveagent.core.message_sanitization import close_interrupted_tool_sequence
    ...
    try:
        from roveagent.core.conversation_loop import _notify_context_engine_turn_complete
    ...
    result = {
        ...
        # billing audits by callers like `roveagent -z --usage-file`.
        "service_tier": (
    ...
    if failed and str(_turn_exit_reason) == "session_persistence_failed":
        result["error"] = final_response or (
            ...
            "health (`roveagent doctor`), then send your message again"
        )
