...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("roveagent-verify-", "roveagent-ad-hoc-")
...
def _db_path() -> Path:
    return get_roveagent_home() / "verification_evidence.db"
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "roveagent verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``roveagent verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``roveagent verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="roveagent verify",
        kind="verify",
