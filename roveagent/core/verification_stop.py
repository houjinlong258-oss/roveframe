...
def _session_is_messaging_surface() -> bool:
    ...
    """
    try:
        from roveagent.gateway.session_context import session_is_messaging_surface
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``ROVEAGENT_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("ROVEAGENT_VERIFY_ON_STOP")
    if env is not None:
...
def _verification_snapshot(
    *,
    ...
    try:
        ...
        from roveagent.core.verification_evidence import verification_status
    except Exception:
        return None
...
def _workspace_has_runnable_recipe(root: Any) -> bool:
    """Whether the workspace has a runtime verify recipe ``roveagent verify`` can run.

    True when a saved ``.roveagent/environment.json`` manifest exists, or when
    ...
    try:
        ...
        from roveagent.core.verify.environment import manifest_path

        if manifest_path(root_path).is_file():
            return True
        from roveagent.core.verify.recipes import detect_recipe
...
def build_verify_on_stop_nudge(
    *,
    ...
    try:
        from roveagent.core.verify_hooks import coding_verify_guidance
    ...
    if verify_commands:
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction += (
                ...
                "start + readiness), prefer `roveagent verify --json` — a passing "
        ...
        if _workspace_has_runnable_recipe(facts.get("root")):
            command_instruction = (
                ...
                "project has a runnable verification recipe. Run `roveagent verify "
            ...
            command_instruction = (
                ...
                "`tempfile` path with a `roveagent-verify-` filename prefix, run it "
