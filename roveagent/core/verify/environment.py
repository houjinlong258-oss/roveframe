...
The manifest lives at ``<project>/.roveagent/environment.json`` and is the
...
from roveagent.core.verify.recipes import Recipe, detect_recipe

MANIFEST_VERSION = 1
_MANIFEST_RELPATH = Path(".roveagent") / "environment.json"
...
def load_manifest(root: Path) -> Recipe | None:
    ...
    fresh detection instead of breaking ``roveagent verify``.
    """
