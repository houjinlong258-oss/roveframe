"""Vertex AI (Google Cloud) adapter for RoveAgent Agent.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows RoveAgent to use Gemini models via Google Cloud with
...
from roveagent.core.secret_scope import get_secret as _get_secret, is_multiplex_active
...
# who installed plain `roveagent-agent` and only later selected a Gemini model.
try:
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed RoveAgent at Vertex.
    ...
    RoveAgent up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only RoveAgent-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the RoveAgent-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
