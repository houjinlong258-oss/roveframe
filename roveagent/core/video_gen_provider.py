...
as ``kind: backend``) or ``~/.roveagent/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``roveagent tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$ROVEAGENT_HOME/cache/videos/``, creating parents as needed."""
    from roveagent.constants import get_roveagent_home

    path = get_roveagent_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$ROVEAGENT_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$ROVEAGENT_HOME/cache/videos/``.
