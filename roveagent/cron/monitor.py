...
def _snapshot_path(job_id: str):
    from roveagent.cron.jobs import _job_output_dir
...
def _fetch_monitor_url(url: str) -> tuple[bool, str]:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "roveagent-cron-monitor"})
...
def _run_monitor_source(job: dict) -> tuple[bool, str]:
    ...
    if monitor_script:
        ...
        from roveagent.cron.scheduler import _run_job_script
...
def _persist_monitor_state(job_id: str, new_hash: str, output: str) -> None:
    from roveagent.cron.jobs import _roveagent_now, update_job
        ...
        update_job(
            job_id,
            {
                "monitor_state": {
                    ...
                    "last_changed_at": _roveagent_now().isoformat(),
                }
            },
        )
    except Exception as exc:
