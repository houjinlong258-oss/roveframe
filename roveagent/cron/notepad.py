...
Write path is the CLI (``roveagent cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_roveagent_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`roveagent cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `roveagent cron notepad {job_id} delete "
