...
Uses a file-based lock (~/.roveagent/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `roveagent update` reloads
# the module) fail with ModuleNotFoundError for roveagent_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from roveagent.constants import get_roveagent_home
from roveagent.clisupport._subprocess_compat import windows_hide_flags
...
from roveagent.clisupport.fallback_config import get_fallback_chain
from roveagent.timeutils import now as _roveagent_now
from roveagent.core.interrupt_compat import request_hard_interrupt
from roveagent.core.delegation_context import (
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `roveagent fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `roveagent cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    (``roveagent cron status`` must not show green while jobs only fire from the
...
def _should_yield_tick_to_fresh_gateway() -> tuple[str, str] | None:
    ...
    if skew is None:
        return None
    try:
        from roveagent.gateway import status as _gateway_status
    except Exception:
        return None
    try:
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running RoveAgent, pin it explicitly: "
                f"`roveagent cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `roveagent update`, manual
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`roveagent gateway restart` to fix it."
            )
...
def _upsert_incident_for_failure(
    ...
    """
    try:
        from roveagent.cron.incidents import get_incident, upsert_incident
...
def _mark_incident_alerted(incident_id: Optional[str]) -> None:
    ...
    if not incident_id:
        return
    try:
        from roveagent.cron.incidents import set_incident_state
...
def _resolve_cron_disabled_toolsets(cfg: dict) -> list[str]:
    ...
    from roveagent.core.skill_utils import parse_config_string_list
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy roveagent_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from roveagent.clisupport.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``roveagent tools`` config for the ``cron`` platform.
    ...
    try:
        from roveagent.clisupport.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    """
    from roveagent.constants import parse_reasoning_effort, resolve_reasoning_config
...
from roveagent.cron.jobs import (
    _ensure_cron_dir,
    advance_next_runs,
    claim_dispatch,
    claim_job_for_fire,
...
from roveagent.cron.executions import create_execution, finish_execution, mark_execution_running
...
def _is_cron_silence_response(text: str) -> bool:
    ...
    """
    from roveagent.gateway.response_filters import is_autonomous_silence_response
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``ROVEAGENT_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("ROVEAGENT_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid ROVEAGENT_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _cron_interval_minutes(expr: str) -> Optional[float]:
    ...
    result = None
    try:
        from roveagent.cron.jobs import _ensure_croniter

        if _ensure_croniter():
            from roveagent.cron.jobs import croniter as _croniter
...
def _job_interval_minutes(job: dict) -> Optional[float]:
    ...
    try:
        ...
        if isinstance(schedule, str) and schedule.strip():
            from roveagent.cron.jobs import parse_schedule
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _roveagent_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_roveagent_home() / "cron" / "inflight_forced_releases.jsonl"
...
def sweep_stale_inflight(due_jobs: Optional[list] = None) -> list:
    ...
    from roveagent.cron.executions import _TERMINAL_STATES as _terminal_states
    ...
    if _ledger_candidates:
        try:
            from roveagent.cron.executions import latest_executions as _latest_execs
    ...
    def _row_belongs_to_claim(row: dict, claim_started: float) -> bool:
        ...
        if not claimed_at:
            return False
        try:
            from roveagent.cron.jobs import _ensure_aware as _ensure_aware_ts
    ...
    for job_id, age, allowance, fut, _reason in stale:
        ...
        logger.warning(
            "roveagent.cron.inflight.forced_release event=forced_release reason=%s job='%s' "
        ...
        if isinstance(repeat, dict) and repeat.get("times") is not None:
            logger.warning(
                "roveagent.cron.inflight.forced_release.job_untouched job='%s' id=%s — "
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_roveagent_home())
...
def _cron_inactivity_seconds() -> float:
    """Parse ROVEAGENT_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    """
    raw = os.getenv("ROVEAGENT_CRON_TIMEOUT", "").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
    except (ValueError, TypeError):
        logger.warning("Invalid ROVEAGENT_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_roveagent_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_roveagent_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.roveagent/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``roveagent update`` / ``roveagent gateway stop`` / systemd restart, or an
    ...
    """
    from roveagent.tools.interpreter_shutdown import interpreter_shutting_down

    return interpreter_shutting_down(exc)


# Backward-compatible module override used by tests and emergency monkeypatches.
_roveagent_home: Path | None = None


def _get_roveagent_home() -> Path:
    """Resolve RoveAgent home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active ROVEAGENT_HOME at call time
    ...
    """
    return _roveagent_home or get_roveagent_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    roveagent_home = _get_roveagent_home()
    lock_dir = roveagent_home / "cron"
...
def _maybe_mirror_cron_delivery(
    ...
    *,
    ...
    if not text:
        return
    try:
        from roveagent.gateway.mirror import mirror_to_session
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"RoveAgent — {task_name}"
    try:
        from roveagent.core.async_utils import safe_schedule_threadsafe
...
def _seed_cron_thread_session(
    ...
    try:
        ...
        from roveagent.gateway.session import SessionSource
...
def _get_config_home_channel(platform_name: str):
    ...
    """
    try:
        from roveagent.gateway.config import load_gateway_config, Platform
...
def _env_home_target_chat_id(platform_name: str) -> str:
    ...
    if not env_var:
        return ""
    try:
        from roveagent.core.secret_scope import get_secret
    except Exception:
...
def _relay_fronted_delivery_platforms(connected: set) -> set:
    ...
    if "relay" not in connected:
        return set()
    try:
        from roveagent.gateway.relay import relay_fronted_platforms
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from roveagent.clisupport.profiles import list_profile_names
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if ":" in deliver_value:
        ...
        from roveagent.tools.send_message_tool import (
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``roveagent [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's ROVEAGENT_HOME) or a validated local profile name.  Returns None
    ...
    roveagent_bin = _shutil.which("roveagent")
    if roveagent_bin:
        argv = [roveagent_bin]
    else:
        try:
            ...
            if _ilu.find_spec("roveagent_cli") is not None:
                argv = [sys.executable, "-m", "roveagent.clisupport.main"]
            else:
                return "bot-chat delivery failed: roveagent CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: roveagent CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover ROVEAGENT_HOME
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="roveagent-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the ROVEAGENT_HOME
    ...
    ``~/.roveagent/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits ROVEAGENT_HOME, no name needed.
        return {"platform": BOT_CHAT_PLATFORM, "chat_id": "", "thread_id": None}
    try:
        from roveagent.clisupport.profiles import normalize_profile_name, profile_exists
...
def _send_media_via_adapter(
    ...
    from roveagent.gateway.platforms.base import BasePlatformAdapter, should_send_media_as_audio
    ...
    for raw_path, _v in requested:
        try:
            from roveagent.gateway.platforms.base import validate_media_delivery_path
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # ROVEAGENT_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    from roveagent.tools.send_message_tool import _send_to_platform
    from roveagent.gateway.config import load_gateway_config, Platform
    ...
    from roveagent.gateway.platforms.base import BasePlatformAdapter
    ...
    # at boot; a standalone process (manual `roveagent cron run` from the CLI,
    ...
    from roveagent.gateway.media_policy import apply_media_policy_env
    ...
    for target in targets:
        ...
        from roveagent.gateway.delivery import resolve_delivery_transport
        ...
        if transport is not None:
            ...
            # (`roveagent cron run`) has no live relay adapter, so surface the
        ...
        if live_adapter_ready:
            ...
            from roveagent.gateway.delivery import (
                DeliveryRouter,
                DeliveryTarget,
                _looks_like_int,
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ROVEAGENT_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ROVEAGENT_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _get_media_send_timeout() -> int:
    ...
    ROVEAGENT_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("ROVEAGENT_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid ROVEAGENT_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _get_session_db_timeout() -> float:
    ...
    Mirrors the ``script_timeout_seconds`` resolution pattern: the
    ROVEAGENT_CRON_SESSION_DB_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("ROVEAGENT_CRON_SESSION_DB_TIMEOUT", "").strip()
    if env_value:
        try:
            return float(env_value)
        except (ValueError, TypeError):
            logger.warning(
                "Invalid ROVEAGENT_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _terminate_cron_script_tree(proc: subprocess.Popen) -> None:
    ...
    try:
        ...
        from roveagent.core.deadline import kill_process_tree
    except Exception:
        logger.warning(
            "roveagent.core.deadline.kill_process_tree unavailable; "
            "falling back to process-group termination",
            exc_info=True,
        )
        _terminate_cron_script_process(proc)
...
def _run_job_script(
    ...
    Scripts must reside within ROVEAGENT_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other RoveAgent-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ROVEAGENT_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_roveagent_home() / "scripts"
    ...
    # escape — scripts MUST reside within ROVEAGENT_HOME/scripts/.
    try:
...
def _build_job_prompt(
    ...
    if context_from:
        from roveagent.cron.jobs import get_cron_output_dir
    ...
    from roveagent.cron import notepad as cron_notepad
    ...
    from roveagent.tools.skills_tool import skill_view
    from roveagent.tools.skill_usage import bump_use
    from roveagent.core.skill_bundles import build_bundle_invocation_message, resolve_bundle_command_key
    from roveagent.core.skill_utils import normalize_skill_lookup_name
    ...
    stable_prefix = None
    if prompt:
        from roveagent.core.skill_commands import append_user_instruction
    ...
    if stable_prefix and len(assembled) > len(stable_prefix) and assembled.startswith(stable_prefix):
        ...
        # caching.
        from roveagent.core.prompt_cache_boundary import register_stable_prefix
...
def _scan_assembled_cron_prompt(
    ...
    *,
    ...
    """
    from roveagent.tools.cronjob_tools import _scan_cron_prompt, _scan_cron_skill_assembled
...
def _guard_job_credential_exfil(job: dict) -> None:
    ...
    """
    try:
        from roveagent.tools.cronjob_tools import _validate_cron_base_url
...
def _block_and_pause_job(
    ...
    """
    from roveagent.cron.jobs import pause_job
    ...
    now_iso = _roveagent_now().strftime("%Y-%m-%d %H:%M:%S")
    doc = (
        f"# Cron Job: {job_name}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {now_iso}\n"
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("ROVEAGENT_MODEL") or ""

    from roveagent.clisupport.auth import AuthError

    try:
        from roveagent.clisupport.runtime_provider import resolve_runtime_provider
        ...
        return (
            ...
            "Set the provider API key in .env (or `roveagent setup`), or pin a "
            "working provider via `roveagent cron edit "
...
def _delivery_platform_routed_from_primary_gateway(platform_name: str) -> bool:
    ...
    """
    try:
        from roveagent.constants import get_default_roveagent_root, get_roveagent_home

        primary_home = get_default_roveagent_root()
        current_home = Path(get_roveagent_home())
        if (
        ...
        from roveagent.gateway.profile_routing import parse_profile_routes
        from roveagent.clisupport.profiles import profile_matches_home
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            ...
            return (
                ...
                "`roveagent setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    if job.get("no_agent"):
        ...
        # "no delivery target resolved". load_roveagent_dotenv does not override
        ...
        try:
            ...
            load_roveagent_dotenv(roveagent_home=_get_roveagent_home())
        except Exception:
            logger.debug(
        ...
        if not str(script_path or "").strip():
            from roveagent.cron.jobs import NO_AGENT_WITHOUT_SCRIPT_ERROR
    ...
    # ---------------------------------------------------------------
    from roveagent.cron.jobs import EMPTY_PAYLOAD_ERROR, job_payload_is_empty
    ...
    # ---------------------------------------------------------------
    from roveagent.cron.monitor import check_monitor, job_has_monitor

    _monitor_context: Optional[str] = None
    if job_has_monitor(job):
        _mon = check_monitor(job)
        _mon_now = _roveagent_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_roveagent_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_roveagent_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    from roveagent.gateway.session_context import set_session_vars, clear_session_vars, _VAR_MAP

    # Cron execution is an internal scheduler context, not a live inbound
    # gateway message. Do not seed ROVEAGENT_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ROVEAGENT_SESSION_PLATFORM
    #     and ROVEAGENT_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ROVEAGENT_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ROVEAGENT_SESSION_PLATFORM == "telegram".
    ...
    #     ROVEAGENT_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ROVEAGENT_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ROVEAGENT_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ROVEAGENT_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # ROVEAGENT_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ ROVEAGENT_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "ROVEAGENT_CRON_AUTO_DELIVER_PLATFORM",
        "ROVEAGENT_CRON_AUTO_DELIVER_CHAT_ID",
        "ROVEAGENT_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    from roveagent.tools.terminal_tool import clear_session_cwd as _clear_tool_session_cwd
    from roveagent.tools.terminal_tool import record_session_cwd as _record_tool_session_cwd
    if _job_workdir:
        _record_tool_session_cwd(_cron_task_id, _job_workdir)
    _cron_session_var = _VAR_MAP["ROVEAGENT_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `roveagent chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with ROVEAGENT_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $ROVEAGENT_KANBAN_TASK -- letting
        ...
        # load_roveagent_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this ROVEAGENT_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_roveagent_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from roveagent.clisupport.env_loader import (
        ...
        if delivery_target:
            _VAR_MAP["ROVEAGENT_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ROVEAGENT_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ROVEAGENT_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > ROVEAGENT_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``roveagent cron edit --model``
        ...
        try:
            ...
            _cfg_path = str(_get_roveagent_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"ROVEAGENT_MODEL={os.getenv('ROVEAGENT_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`roveagent cron edit {job_id} --model <name>` or set a "
                "default with `roveagent model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from roveagent.constants import apply_ipv4_preference
        ...
        prefill_file = (
            os.getenv("ROVEAGENT_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_roveagent_home() / pfpath
            if pfpath.exists():
                try:
        ...
        from roveagent.clisupport.config import resolve_turn_limit as _resolve_turn_limit
        ...
        from roveagent.clisupport.runtime_provider import (
        ...
        try:
            if _cron_preflight_enabled(_cfg):
                ...
                if not _pf_reason and job.get("preflight_alerted"):
                    ...
                    try:
                        from roveagent.cron.jobs import clear_preflight_alerted
        ...
        if _pf_reason:
            ...
            try:
                from roveagent.cron.jobs import mark_preflight_alerted
        ...
        try:
            # Do not inject ROVEAGENT_INFERENCE_PROVIDER here. resolve_runtime_provider()
            ...
            for entry in fb_list:
                ...
                if not fb_provider or not fb_model:
                    continue
                try:
                    from roveagent.clisupport.fallback_config import resolve_entry_api_key
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running RoveAgent "
                        "pin it explicitly: "
                        f"`roveagent cron edit {job_id} --provider <provider> "
                ...
                try:
                    from roveagent.cron.jobs import mark_drift_alerted
        ...
        #
        # Bounded with its own timeout (separate from ROVEAGENT_CRON_TIMEOUT,
        ...
        agent = AIAgent(
            ...
            # ROVEAGENT_HOME. When a workdir is configured, also inject project
        ...
        # override via ROVEAGENT_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if not isinstance(result, dict):
            raise RuntimeError(
                f"roveagent.core.run_conversation returned {type(result).__name__} instead of dict: {result!r}"
            )
        ...
        if final_response:
            from roveagent.gateway.media_repair import (
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from roveagent.state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
**Job ID:** {job_id}
**Run Time:** {_roveagent_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_roveagent_now().strftime('%b %d %H:%M')}"
            ...
            # roveagent_state's session_lifecycle_statuses docstring — keep the
...
def _teardown_cron_agent(
    ...
    def _cleanup_agent() -> None:
        ...
        try:
            from roveagent.core.auxiliary_client import cleanup_stale_async_clients
...
def run_one_job(
    ...
    *,
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`roveagent cron run --prompt` /
    ...
    profile_home = _get_roveagent_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_roveagent_home())
        )
        ...
        if interrupted:
            if delivery_error:
                ...
                # early.
                try:
                    from roveagent.cron.jobs import update_job
        ...
        if delivery_error:
            ...
            # exact signature via `roveagent cron incidents ack`.
...
def _notify_provider_jobs_changed() -> None:
    ...
    """
    try:
        from roveagent.cron.scheduler_provider import resolve_cron_scheduler
...
def create_job_with_scheduler_registration(**kwargs) -> dict:
    ...
    from roveagent.cron.jobs import create_job
    from roveagent.cron.scheduler_provider import resolve_cron_scheduler
...
# `roveagent -w` launches, so on gateway-driven boxes (where sessions arrive via
...
def _worktree_maintenance_repos() -> List[str]:
    ...
    Candidates: the roveagent install checkout itself (where ``roveagent -w``
    ...
    # The roveagent source checkout (editable/git installs). Wheel installs have
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    ``roveagent -w`` startup path uses — dirty/unpushed/live-locked trees are
...
def tick(
    ...
    *,
    ...
    # ``git pull``, interrupted ``roveagent update``) serves MIXED sys.modules:
    ...
    try:
        # Global emergency stop (`roveagent pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `roveagent resume`. Logged once per
        # engagement (not every tick) by check_paused.
        try:
            from roveagent.core.estop import check_paused as _estop_check_paused
        ...
        # scheduler STARTUP. A one-shot `roveagent cron run` that claimed a job
        ...
        if (
            ...
            try:
                from roveagent.cron.executions import recover_interrupted_executions
        ...
        # boxes never hit the `roveagent -w` startup pruner, so this is the only
        ...
        if _running_job_ids:
            ...
            try:
                ...
                if not _inflight_ids <= _due_ids:
                    from roveagent.cron.jobs import load_jobs as _load_all_jobs
        ...
        if not due_jobs:
            ...
            if verbose:
                logger.info("%s - No jobs due", _roveagent_now().strftime('%H:%M:%S'))
            try:
                from roveagent.tools.mcp_tool import _kill_orphaned_mcp_children
        ...
        if verbose:
            logger.info("%s - %s job(s) due", _roveagent_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ROVEAGENT_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ROVEAGENT_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ROVEAGENT_CRON_MAX_PARALLEL value; defaulting to unbounded")
