...
def _note_tick_failure(exc: BaseException, consecutive_failures: int) -> int:
    ...
    """
    from roveagent.cron.scheduler import _is_fd_exhaustion, _reclaim_fds_best_effort
...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    the ticker runs — via ``roveagent profile delete``, the desktop's DELETE
    ...
    and in ``roveagent profile list`` (#47368). Filtering on directory existence
...
class CronScheduler(ABC):
    ...
    def claim_fire(self, job_id: str, *, force: bool = False) -> dict | None:
        ...
        """
        from roveagent.cron.executions import create_execution, finish_execution
        from roveagent.cron.jobs import claim_job_for_fire
    ...
    def fire_claimed(
        ...
        *,
        ...
        """
        from roveagent.cron.scheduler import run_one_job
...
def fire_overdue_jobs(
    ...
    *,
    ...
    logger = logging.getLogger("roveagent.cron.scheduler_provider")
    ...
    from roveagent.cron.jobs import _ensure_aware, _roveagent_now, is_job_runnable, load_jobs
    ...
    for job in load_jobs():
        ...
        if str(schedule.get("kind") or "") == "once":
            from roveagent.cron.jobs import ONESHOT_GRACE_SECONDS
...
def resolve_cron_scheduler() -> "CronScheduler":
    ...
    try:
        from roveagent.plugins.cron_providers import load_cron_scheduler
        provider = load_cron_scheduler(name)
        if provider is None:
            logger.warning("roveagent.cron.provider '%s' not found; using built-in ticker", name)
            return InProcessCronScheduler()
        if not provider.is_available():
            logger.warning("roveagent.cron.provider '%s' not available; using built-in ticker", name)
            return InProcessCronScheduler()
...
def scheduler_for_profile_mode(
    ...
    logging.getLogger("roveagent.cron.scheduler_provider").warning(
        "roveagent.cron.provider '%s' does not support multiplex_profiles; using built-in ticker",
        provider.name,
    )
    return InProcessCronScheduler()
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        import logging
        from roveagent.cron.scheduler import CronTickYielded
        from roveagent.cron.scheduler import tick as cron_tick
        ...
        # only the process-global ROVEAGENT_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `roveagent cron
        ...
        # Heartbeat once before the first sleep so `roveagent cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `roveagent cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_roveagent_home_override()`` + ``use_cron_store()``
        ...
        for entry in _existing_profile_homes(profile_homes):
            home = entry[1] if isinstance(entry, tuple) else entry
            home_token = set_roveagent_home_override(str(home))
            try:
            ...
            for entry in _existing_profile_homes(profile_homes):
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `roveagent cron status` can show WHY ticks fail
                        # (#68483).
                        if _home_ok:
                            clear_ticker_error()
