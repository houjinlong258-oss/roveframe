...
window before ``roveagent update``'s graceful restart fires), a first-time lazy
...
def _fingerprint() -> str | None:
    ...
    ``roveagent_cli.main`` is always already imported in a gateway process (it's
    the entry point), so this import is free and avoids duplicating the
    worktree-aware ref resolution.
    """
    try:
        from roveagent.clisupport.main import _read_git_revision_fingerprint
