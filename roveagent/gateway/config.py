...
from roveagent.core.secret_scope import current_secret_scope, get_secret as _get_secret
from roveagent.gateway.shutdown_watchdog import (
...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from roveagent.clisupport.profiles import normalize_profile_name, validate_profile_name
...
def coerce_systemd_watchdog_seconds(
    value: Any, key: str = "roveagent.gateway.systemd_watchdog_seconds"
) -> int:
...
# dashboard's pre-write mutation validation (roveagent_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "RoveAgent is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_roveagent_home() / "sessions")
    ...
    # gateway behaves exactly as before — single ROVEAGENT_HOME, no profile stamping.
    ...
    # advertise a route. ROVEAGENT_ROOM_LINK_URL remains the operator override.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if "systemd_watchdog_seconds" in data:
            ...
            systemd_watchdog_key = "roveagent.gateway.systemd_watchdog_seconds"
        ...
        if multiplex_profiles is None and isinstance(nested_gateway, dict):
            ...
            # ``roveagent config set gateway.multiplex_profiles true``.
        ...
        if "max_concurrent_sessions" in data:
            ...
            max_concurrent_key = "roveagent.gateway.max_concurrent_sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.roveagent/config.yaml (primary user-facing config)
    3. ~/.roveagent/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_roveagent_home()
    gw_data: dict = {}

    # Legacy fallback: gateway.json provides the base layer.
    # config.yaml keys always win when both specify the same setting.
    gateway_json_path = _home / "roveagent.gateway.json"
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through roveagent_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `roveagent config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `roveagent config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``roveagent config set gateway.streaming.*``
            ...
            try:
                ...
                from roveagent.gateway.platform_registry import platform_registry as _pr
            except Exception as e:
