...
(the updater, `roveagent serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, roveagent_home, code_sha/code_version
...
- POSIX: Unix domain socket at ``$ROVEAGENT_HOME/gateway.sock``. When the home
  path is too long for ``sun_path`` (~104 bytes on macOS/BSD), the socket is
  bound in the system temp dir and a pointer file
  ``$ROVEAGENT_HOME/gateway.sock.path`` records the real location; clients
  follow the pointer transparently.
- Windows: named pipe ``\\\\.\\pipe\\roveagent-gateway-<home-hash>`` served via
...
Consumers (``roveagent update --plan`` inventory, the post-update fleet version
...
_SOCKET_FILENAME = "roveagent.gateway.sock"
_POINTER_FILENAME = "roveagent.gateway.sock.path"
...
def windows_pipe_name(home: Path) -> str:
    """Per-ROVEAGENT_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\roveagent-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"roveagent-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.roveagent")
        or env.get("LAUNCHD_SOCKET")
    ):
        return "launchd"
    if env.get("ROVEAGENT_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "roveagent_home": record.get("roveagent_home"),
        "profile": _profile_label_for_home(record.get("roveagent_home") or ""),
...
class GatewayControlServer:
    ...
    its ROVEAGENT_HOME), ``stop()`` on shutdown. All failures are non-fatal —
    ...
    def __init__(
        ...
        *,
        ...
        if home is None:
            from roveagent.gateway.status import _get_process_roveagent_home
