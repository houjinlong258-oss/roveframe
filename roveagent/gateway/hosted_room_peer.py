...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    return hmac.new(
        material,
        b"roveagent-hosted-room-installation-grant-v1",
        hashlib.sha256,
    ).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local ROVEAGENT_HOME override. The process
        # environment retains the installation root and is the authority here.
        root = os.environ.get("ROVEAGENT_HOME") or get_roveagent_home()
...
def derive_room_grant_secret(api_key: str) -> bytes:
    ...
    return hmac.new(
        ...
        b"roveagent-hosted-room-grant-v1",
        hashlib.sha256,
    ).digest()
...
def catalog_mapping(
    *,
    ...
    persistent_process = bool(persistent_process and os.getenv("ROVEAGENT_DESKTOP") != "1")
    checked_policy = RoomExecutionPolicy.from_mapping(
        execution_policy
        or execution_policy_mapping(
            target_profile=(
                str(target_profile or "").strip()
                or (os.getenv("ROVEAGENT_PROFILE") or "default").strip()
                or "default"
            )
        )
    )
...
def _room_link_url_from_config(home: str) -> str | None:
    ...
    if str(get_roveagent_home()) == home:
...
def _configured_room_link_url() -> str | None:
    ...
    override = os.getenv("ROVEAGENT_ROOM_LINK_URL")
