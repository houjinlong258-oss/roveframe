...
# "roveagent.gateway.run") so extracted log records keep their original logger name.
...
def _kanban_dispatch_allowed() -> bool:
    """Return False while the global emergency stop (`roveagent pause`) is engaged.
    ...
    """
    try:
        from roveagent.core.estop import check_paused
    except ImportError:
        return True
...
def _acquire_singleton_lock(lock_path) -> "tuple[Optional[object], str]":
    ...
    """
    try:
        from roveagent.gateway.status import _try_acquire_file_lock  # deferred; same package
    except ImportError:
...
def _release_singleton_lock(handle) -> None:
    ...
    if handle is None:
        return
    try:
        from roveagent.gateway.status import _release_file_lock
...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                ...
                def _collect():
                    ...
                    # ROVEAGENT_KANBAN_DB pins the board path; without this guard
                ...
                for d in deliveries:
                    ...
                    for ev in d["events"]:
                        ...
                        from roveagent.gateway.wake import adapter_supports_push
                        ...
                        from roveagent.gateway.wake import adapter_supports_push as _adapter_push_ok
                        ...
                        if _wake_kinds:
                            ...
                            _parts = []
                            if "completed" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.completed"))
                            if "gave_up" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.gave_up"))
                            if "crashed" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.crashed"))
                            if "timed_out" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.timed_out"))
                            if "blocked" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.blocked"))
                            if "review_requested" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.review_requested"))
                            if "changes_requested" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.changes_requested"))
                            if "block_loop_detected" in _wake_kinds: _parts.append(t("roveagent.gateway.kanban.wake.block_loop_detected"))
                            _status = t("roveagent.gateway.kanban.wake.status_joiner").join(_parts) or t("roveagent.gateway.kanban.wake.status_default")
                            _synth = t(
                                "roveagent.gateway.kanban.wake.message",
                                task_id=sub["task_id"],
                                status=_status,
                                title=_title,
                                assignee=_assignee,
                            ...
                            if wake_handoff:
                                _synth += "\n" + t(
                                    "roveagent.gateway.kanban.wake.handoff",
                                    summary=wake_handoff,
                                )
                            if wake_review_detail:
                                _synth += "\n" + t(
                                    "roveagent.gateway.kanban.wake.review_detail",
                                    reason=wake_review_detail,
                                )
                            _synth += "\n\n" + t(
                                "roveagent.gateway.kanban.wake.guidance"
                            )
                        ...
                        if not _is_push_adapter and _wake_kinds and _session_key:
                            ...
                            from roveagent.gateway.wake import deliver_wake
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `roveagent kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours ROVEAGENT_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        try:
            ...
            return
        env_override = os.environ.get("ROVEAGENT_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via ROVEAGENT_KANBAN_DISPATCH_IN_GATEWAY env")
            return
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `roveagent kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real RoveAgent profile
            ...
            PATH, missing venv, credential loss for a real RoveAgent profile).
            """
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from roveagent.clisupport import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("ROVEAGENT_KANBAN_BOARD")
                try:
                    os.environ["ROVEAGENT_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("ROVEAGENT_KANBAN_BOARD", None)
                    else:
                        os.environ["ROVEAGENT_KANBAN_BOARD"] = prev_env
            return successes
        ...
        while self._running:
            ...
            try:
                # Global emergency stop (`roveagent pause`): skip auto-decompose
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`roveagent kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
