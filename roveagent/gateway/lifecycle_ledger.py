...
``<ROVEAGENT_HOME>/state/gateway.lifecycle.json``:
...
_LIFECYCLE_RELATIVE = ("state", "roveagent.gateway.lifecycle.json")
...
def _process_roveagent_home() -> Path:
    """ROVEAGENT_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<ROVEAGENT_HOME>/state/gateway.lifecycle.json``."""
...
def check_state_db_integrity(home: Optional[Path] = None) -> str:
    ...
    ``_enforce_macos_synchronous_full`` in :mod:`roveagent_state` — macOS
        ...
        if evidence is not None:
            ...
            if verdict not in ("ok", "absent"):
                logger.error(
                    ...
                    "repaired. Run `roveagent doctor`.",
                    verdict,
                )
            record = {
                "ts": datetime.now(timezone.utc).isoformat(),
                "tag": "roveagent.gateway.previous_unclean_exit",
                "pid": os.getpid(),
                **evidence,
            }
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``roveagent_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
