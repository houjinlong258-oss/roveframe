...
def _read_heartbeat(home: Optional[Path]) -> Optional[Dict[str, Any]]:
    try:
        from roveagent.gateway.lifecycle_ledger import _read_json
...
def _read_sentinel(home: Optional[Path]) -> Optional[Dict[str, Any]]:
    try:
        from roveagent.gateway.lifecycle_ledger import (
            _read_json,
...
def collect_memory_status(
    ...
    *,
    ...
    ``home`` scopes the read to a profile's ROVEAGENT_HOME (the status
