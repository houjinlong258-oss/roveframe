...
from roveagent.gateway.whatsapp_identity import (
    expand_whatsapp_aliases,
    normalize_whatsapp_identifier,
)
from roveagent.constants import (
    get_default_roveagent_root,
    get_roveagent_dir,
    get_roveagent_home,
)
...
# path eagerly freezes it to whatever ROVEAGENT_HOME/profile context existed
# at that exact import moment for the rest of the process's lifetime --
# even if a context-local override (see roveagent_constants.set_roveagent_home_override)
# is established afterward. A freshly-started, short-lived process (e.g. the
# ``roveagent pairing`` CLI) re-imports this module later with the final
...
# working (NousResearch/roveagent-agent#93449).
#
...
# isolation (e.g. ``patch("roveagent.gateway.pairing.PAIRING_DIR", tmp_path)``); that
...
def _default_pairing_dir() -> Path:
    ...
    if PAIRING_DIR is not None:
        return PAIRING_DIR
    return get_roveagent_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``roveagent_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from roveagent.clisupport.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{ROVEAGENT_HOME}/pairing`` while newer code/docs may
    write ``{ROVEAGENT_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own ROVEAGENT_HOME using the same legacy/consolidated layout rules
    as ``roveagent -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current ROVEAGENT_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp ROVEAGENT_HOME
        ...
        if profile:
            ...
            # ``roveagent -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `roveagent` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> roveagent pairing approve ...` as root, "
                    "re-run with `docker exec -u roveagent <container> ...` and "
                    "chown the existing file to the roveagent user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``roveagent
