...
class PlatformEntry:
    ...
    # status displays (``roveagent setup``, ``roveagent status``, the dashboard
    ...
    # Env vars this platform needs (for ``roveagent setup`` display).
    ...
    # built-ins).  Used by ``roveagent gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ROVEAGENT_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `roveagent`
        # invocation -- including plain `roveagent chat`, which never touches any
        ...
        # `roveagent setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``roveagent setup``/``gateway status``, channel
