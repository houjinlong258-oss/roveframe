...
There are two ways to add a platform to the RoveAgent gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.roveagent/plugins/` (or under `plugins/platforms/`
...
**zero changes to core RoveAgent code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `roveagent gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `roveagent_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the RoveAgent core.
...
```python
    ...
    from roveagent.gateway.platforms.your_platform import YourAdapter, check_your_requirements
...
```python
"roveagent-your-platform": {
    "description": "Your Platform bot toolset",
    "roveagent.tools": _ROVEAGENT_CORE_TOOLS,
    "includes": []
},
```

And add it to the `roveagent-gateway` composite:

```python
"roveagent-gateway": {
    "includes": [..., "roveagent-your-platform"]
}
```
...
## 12. Status Display (`roveagent_cli/status.py`)
...
## 13. Gateway Setup Wizard (`roveagent_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ roveagent_cli/ toolsets.py \
  --include="*.py" -l | sort -u
