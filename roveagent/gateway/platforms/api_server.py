...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-RoveAgent-Session-Id header; opt-in long-term memory scoping via X-RoveAgent-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-RoveAgent-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists roveagent-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible RoveAgent sessions
- POST /api/sessions               — create an empty RoveAgent session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to roveagent-agent
...
_BROWSER_CONTROL_WS_PROTOCOL = "roveagent-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "roveagent-browser-control-ticket."
...
from roveagent.gateway.config import Platform, PlatformConfig
from roveagent.gateway.platforms import api_server_room_dispatch as _room_dispatch
from roveagent.gateway.platforms import api_server_room_grants as _room_grants
from roveagent.gateway.platforms import api_server_runs as _api_runs
from roveagent.gateway.platforms.base import (
...
from roveagent.gateway.platforms.api_server_run_idempotency import RunIdempotencyStore
...
from roveagent.gateway.readiness import collect_runtime_readiness
from roveagent.gateway.browser_control_artifacts import (
    ArtifactError,
    ArtifactRateLimiter,
    ArtifactStore,
    ArtifactTooLarge,
...
from roveagent.gateway.browser_control_broker import (
...
from roveagent.core.secret_scope import UnscopedSecretError as _UnscopedSecretError
from roveagent.core.secret_scope import get_secret as _scoped_get_secret
...
def _roveagent_version() -> str:
    """Return the canonical RoveAgent Agent version string.

    ``roveagent_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``roveagent_agent-*.dist-info`` after a source update until the environment is
    reinstalled. Never raises — a version probe must not be able to break the
    health endpoint.
    """
    try:
        from roveagent.clisupport import __version__
    ...
    try:
        ...
        return version("roveagent-agent")
    except Exception:
        return "dev"
...
# config.yaml accept (roveagent_constants.VALID_REASONING_EFFORTS); wire-level
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from roveagent.clisupport.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("ROVEAGENT_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``roveagent-agent``)
    ...
    ``direct_model_requests`` config value here, while RoveAgent-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously RoveAgent-aware and is
    always honored.
    """
...
def _is_compressed_summary_message(message: Any) -> bool:
    ...
    if not isinstance(message, dict):
        return False
    from roveagent.core.context_compressor import is_compaction_summary_message
...
def _project_client_message(message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    from roveagent.core.compaction_display import (
...
def _reap_disconnected_agent_processes(
    ...
    from roveagent.gateway.run import _reap_gateway_turn_processes
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_roveagent_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted ROVEAGENT_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # roveagent_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same RoveAgent session (and therefore the same Docker container sandbox
...
try:
    ...
    from roveagent.cron.scheduler import (
...
def _notify_cron_provider_jobs_changed() -> None:
    ...
    try:
        from roveagent.cron.scheduler import _notify_provider_jobs_changed
...
try:
    from roveagent.tools.cronjob_tools import _scan_cron_prompt as _scan_cron_prompt
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through roveagent-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the RoveAgent-native session-chat
    ...
    def _gateway_is_draining() -> bool:
        ...
        try:
            from roveagent.gateway.run import _gateway_runner_ref
    ...
    def _readiness_work_counts(self) -> tuple[int, int, int]:
        ...
        try:
            ...
            pass
        try:
            from roveagent.tools.async_delegation import active_count
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from roveagent.clisupport.config import cfg_get, load_config
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "roveagent-agent"

        Delegates the tiered fallthrough to
        :func:`roveagent_cli.model_switch.resolve_effective_model` (the shared
        override > mid-tier > default precedence owner).
        """
        from roveagent.clisupport.model_switch import resolve_effective_model
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "roveagent-agent")
    ...
    def _expected_api_key(self) -> str:
        ...
        try:
            ...
            from roveagent.clisupport.auth import has_usable_secret
    ...
    def _get_platform_callback_adapter(
        ...
        try:
            from roveagent.gateway.config import Platform as _Platform
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `roveagent peer dm mini/researcher` answered by the mini's default
    ...
    def _profile_scope(profile: Optional[str]):
        ...
        if not profile:
            try:
                from roveagent.core.secret_scope import is_multiplex_active

                if is_multiplex_active():
                    from roveagent.gateway.run import _profile_runtime_scope
                    from roveagent.constants import get_roveagent_home

                    return _profile_runtime_scope(get_roveagent_home())
            except Exception:
                pass
            return nullcontext()
        from roveagent.gateway.run import _profile_runtime_scope
        from roveagent.clisupport.profiles import get_profile_dir
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-RoveAgent-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-RoveAgent-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-RoveAgent-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-RoveAgent-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-RoveAgent-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``roveagent sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_roveagent_home()``, so each profile gets its own DB —
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_roveagent_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``roveagent-agent``) means "use
        ...
        400s ("roveagent-agent is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "roveagent-agent") is
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "roveagent_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "roveagent_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the roveagent-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-RoveAgent-Session-Key``).  Unlike ``session_id``
        ...
        def _resolve_provider_runtime(
            ...
            *,
            ...
            try:
                ...
                try:
                    from roveagent.gateway.run import _resolve_runtime_agent_kwargs_for_provider
        ...
        # roveagent_cli.model_switch.resolve_effective_model (session /model
        ...
        # (e.g. user ran `roveagent auth add openai-codex` without `roveagent model`),
        ...
        if not model and runtime_kwargs.get("provider"):
            try:
                from roveagent.clisupport.models import get_default_model_for_provider
        ...
        if room_dispatch is not None:
            from roveagent.gateway.hosted_room_execution_policy import RoomExecutionPolicy
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "roveagent-agent", "version": _roveagent_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        from roveagent.gateway.run import _resolve_gateway_model
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "roveagent-agent",
            "version": _roveagent_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list roveagent-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return RoveAgent provider/model inventory.
        ...
        RoveAgent provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every RoveAgent version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "roveagent.api_server.capabilities",
            "platform": "roveagent-agent",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side RoveAgent AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-RoveAgent-Session-Id",
                "session_key_header": "X-RoveAgent-Session-Key",
    ...
    def _browser_control_enabled(self) -> bool:
        ...
        """
        try:
            from roveagent.gateway.browser_control_broker import browser_control_enabled as _flag
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<ROVEAGENT_HOME>/plugin-data/.../artifacts``-style controlled root),
        ...
        try:
            ...
            # under the RoveAgent home.
            try:
                from roveagent.state import get_roveagent_home

                root = Path(get_roveagent_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_skills(self, request: "web.Request") -> "web.Response":
        ...
        try:
            from roveagent.tools.skills_tool import _find_all_skills, _sort_skills
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted RoveAgent sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        # Exact-title lookup, used by `roveagent peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `roveagent peer dm` resolution and mint
                # transient sessions — same accident the tui_gateway lookups
                # heal. Resurrect and re-list; deliberate archives stay put.
                try:
                    from roveagent.tools.bot_mode_probe import BOT_CHAT_TITLE
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty RoveAgent session row.
        ...
        from roveagent.gateway.session import _is_path_unsafe
        ...
        # (e.g. "provider::roveagent-agent") are split, and the virtual model
        # alias (self._model_name, e.g. "roveagent-agent") is nulled out
        # there — a bare "roveagent-agent" is not a model_routes alias, so a
        ...
        return web.json_response({"object": "roveagent.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "roveagent.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "roveagent.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "roveagent.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-RoveAgent-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-RoveAgent-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "roveagent.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-RoveAgent-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-RoveAgent-Session-Key"] = gateway_session_key
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "roveagent.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-RoveAgent-Session-Key.  This
        # is independent of X-RoveAgent-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-RoveAgent-Session-Id.
        ...
        provided_session_id = request.headers.get("X-RoveAgent-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-RoveAgent-Session-Id rejected: "
            ...
            # conversation map to the same RoveAgent session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``roveagent.tool.progress`` with ``status: running``.
                ...
                from roveagent.core.display import build_tool_preview, get_tool_emoji
        ...
        response_headers = {
            "X-RoveAgent-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-RoveAgent-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["roveagent"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-RoveAgent-Completed"] = "false"
            response_headers["X-RoveAgent-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + RoveAgent-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-RoveAgent-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-RoveAgent-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-RoveAgent-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-RoveAgent-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: roveagent.tool.progress`` SSE event so
                frontends can display them without storing the markers in
                conversation history.  See #6972 for the original event,
                #16588 for the ``toolCallId``/``status`` lifecycle fields.
                """
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="roveagent.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-RoveAgent-Session-Id header.  Without this,
        ...
        response_headers = {"X-RoveAgent-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-RoveAgent-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        # `roveagent cron run` / cronjob(action='run', prompt=...) — same length
    ...
    async def _handle_cron_fire(self, request: "web.Request") -> "web.Response":
        ...
        """
        from roveagent.clisupport.config import cfg_get, load_config
        from roveagent.plugins.cron_providers.chronos.verify import get_fire_verifier
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # RoveAgent agent; they are replayed for structured tool
    ...
    def _bind_api_server_session(
        *,
        ...
        """
        from roveagent.gateway.session_context import set_session_vars
    ...
    async def _run_agent(
        ...
        *requested_runtime* / *route_source* / *confirmed_runtime_lock*
        ...
        def _run():
            from roveagent.gateway.session_context import clear_session_vars
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-RoveAgent-Session-Id header) can track compression-
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_roveagent_api_runtime", {}) or {})
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local RoveAgent-Relay
            ...
            # that is the exact surface the roveagent-0day campaign abused to write
            # ~/.roveagent/config.yaml and plant persistence. Sandboxing (Docker /
