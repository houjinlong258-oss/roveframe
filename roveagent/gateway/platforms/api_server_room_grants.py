...
def _room_grant_token(request: "web.Request") -> str:
    ...
    if not separator or scheme.lower() != "roveagentroom":
        return ""
    return token.strip()


def _room_grant_secret(self) -> bytes:
    from roveagent.gateway.hosted_room_peer import gateway_room_grant_secret
...
def _room_grant_claims(
    ...
    *,
    ...
    from roveagent.gateway.hosted_room_peer import decode_room_grant
    ...
    from roveagent.gateway import hosted_rooms
...
async def _handle_room_member_invitation(
    ...
    *,
    ...
    try:
        from roveagent.gateway import hosted_rooms
        from roveagent.gateway.hosted_room_peer import (
            PROTOCOL_VERSION as ROOM_LINK_PROTOCOL_VERSION,
            catalog_mapping,
            decode_room_grant,
            issue_room_grant,
        )
        from roveagent.gateway.hosted_room_execution_policy import execution_policy_mapping
    ...
    return web.json_response(
        {
            "object": "roveagent.room_member.invitation",
            "grant": token,
            "target_profile": profile,
            "catalog": catalog,
...
async def _handle_room_member_capabilities(
    ...
    *,
    ...
    return web.json_response(
        {
            "object": "roveagent.room_member.capabilities",
...
async def _handle_room_member_grant_refresh(
    ...
    *,
    ...
    return web.json_response(
        {
            "object": "roveagent.room_member.grant",
            "grant": token,
...
async def _handle_room_member_grant_revoke(
    ...
    *,
    ...
    return web.json_response(
        {
            "object": "roveagent.room_member.grant.revocation",
            "revoked": True,
        }
    )
