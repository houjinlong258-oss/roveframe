...
logger = logging.getLogger("roveagent.gateway.platforms.api_server")
_ROOM_RETENTION_REQUEST_KEY = (
    RequestKey("roveagent.room_run_retention_until", float)
    if RequestKey is not None
    else "roveagent.room_run_retention_until"
)
...
def _remember_room_retention(request: "web.Request", claims: dict[str, Any]) -> None:
    ...
    try:
        ...
        setattr(request, "_roveagent_room_run_retention_until", value)
...
def _room_retention_until(request: "web.Request") -> float:
    try:
        ...
        value = getattr(request, "_roveagent_room_run_retention_until", 0)
...
def _check_run_auth(
    ...
    *,
    ...
    try:
        ...
        from roveagent.gateway.platforms.api_server_room_grants import (
...
def _durable_run_status(
    ...
    if owner_pid > 0:
        try:
            from roveagent.gateway.status import _pid_exists, get_process_start_time
...
async def _handle_runs(
    ...
    *,
    ...
    async def _run_and_close():
        try:
            ...
            def _approval_notify(approval_data: Dict[str, Any]) -> None:
                ...
                if "command" in event:
                    from roveagent.gateway.run import _redact_approval_command
            ...
            try:
                from roveagent.tools.approval import unregister_gateway_notify
    ...
    response_headers = (
        {"X-RoveAgent-Session-Key": gateway_session_key} if gateway_session_key else {}
    )
    return web.json_response(
...
async def _handle_run_approval(
    ...
    *,
    ...
    return web.json_response({
        "object": "roveagent.run.approval_response",
        "run_id": run_id,
        "choice": choice,
        **({"request_id": request_id} if request_id else {}),
        "resolved": resolved,
...
async def _handle_steer_run(
    ...
    *,
    ...
    return web.json_response({"object": "roveagent.run.steer", "run_id": run_id, "accepted": True})
