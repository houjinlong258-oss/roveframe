...
from roveagent.utils import normalize_proxy_url
...
# Audio file extensions RoveAgent recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to RoveAgent but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through RoveAgent' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. RoveAgent-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_roveagent_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``ROVEAGENT_SESSION_PLATFORM`` contextvar — that contextvar is
    ...
    """
    from roveagent.tools.tts_tool import OPUS_VOICE_PLATFORMS
...
from roveagent.gateway.session import SessionSource, build_session_key
from roveagent.constants import get_default_roveagent_root, get_roveagent_dir, get_roveagent_home

if TYPE_CHECKING:
    from roveagent.core.display import ToolPreview
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.roveagent/.env manually."
)
...
async def _ssrf_redirect_guard(response):
    ...
    """
    from roveagent.tools.url_safety import is_safe_url, redirect_target_from_response
...
IMAGE_CACHE_DIR = get_roveagent_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_roveagent_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_roveagent_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from roveagent.clisupport.config import load_config_readonly as _load_config
...
async def cache_image_from_url(url: str, ext: str = ".jpg", retries: int = 2) -> str:
    ...
    """
    from roveagent.tools.url_safety import create_ssrf_safe_async_client, is_safe_url
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; RoveAgentAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_roveagent_dir("cache/audio", "audio_cache")
...
def _sniff_audio_ext(data: bytes, fallback_ext: str) -> str:
    ...
    """
    from roveagent.tools.audio_container import sniff_audio_ext
...
VIDEO_CACHE_DIR = get_roveagent_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_roveagent_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_roveagent_dir("cache/screenshots", "browser_screenshots")
...
_ROVEAGENT_ROOT = get_default_roveagent_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "ROVEAGENT_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "ROVEAGENT_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "ROVEAGENT_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "ROVEAGENT_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _ROVEAGENT_HOME / "image_cache",
    _ROVEAGENT_HOME / "audio_cache",
    _ROVEAGENT_HOME / "video_cache",
    _ROVEAGENT_HOME / "document_cache",
    _ROVEAGENT_HOME / "browser_screenshots",
    ...
    _ROVEAGENT_HOME / "cache" / "images",
    _ROVEAGENT_HOME / "cache" / "audio",
    _ROVEAGENT_HOME / "cache" / "videos",
    _ROVEAGENT_HOME / "cache" / "documents",
    _ROVEAGENT_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared RoveAgent root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* ROVEAGENT_HOME's cache, so a gateway running at
    the root (e.g. ``ROVEAGENT_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when ROVEAGENT_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _ROVEAGENT_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("ROVEAGENT_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _ROVEAGENT_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active RoveAgent profile and shared RoveAgent root both contain control
    ...
    # ROVEAGENT_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.roveagent
    ...
    for roveagent_root in (_ROVEAGENT_HOME, _ROVEAGENT_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(roveagent_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and RoveAgent
    secrets (``~/.roveagent/.env``, ``auth.json``) are *separate, more-specific*
...
def _docker_sandbox_dir_candidates(session_key: str = "") -> List[str]:
    ...
    candidates: List[str] = []
    try:
        from roveagent.tools.environments.path_utils import sanitize_task_id_for_path
    except Exception:
        return ["default"]
...
def _default_docker_workspace_host_roots(session_key: str = "") -> List[Path]:
    ...
    try:
        from roveagent.tools.environments.base import get_sandbox_dir
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted RoveAgent cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.roveagent/...``
    ...
    if os.getenv("TERMINAL_ENV", "").strip().lower() != "docker":
        return []
    try:
        from roveagent.tools.credential_files import get_cache_directory_mounts
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted RoveAgent cache dirs (``/root/.roveagent/...``), the session's
    ...
    # In-process gateways (Desktop backend, `roveagent serve`) may not have
    ...
    try:
        from roveagent.tools.terminal_tool import _ensure_terminal_env_bridged
    ...
    # are longer prefixes, so /root/.roveagent/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.roveagent/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.roveagent"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    ``ROVEAGENT_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    RoveAgent-managed cache, under an operator-allowlisted root
    (``ROVEAGENT_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the RoveAgent root (~/.roveagent/.env,
    ...
    # ``MEDIA:~/.roveagent/google_token.json``) remain rejected.
...
def cache_media_bytes(
    ...
    *,
    ...
    """
    from roveagent.tools.credential_files import to_agent_visible_cache_path
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?roveagent\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+roveagent[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``ROVEAGENT_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # RoveAgent commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-ROVEAGENT_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("ROVEAGENT_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "ROVEAGENT_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "ROVEAGENT_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: RoveAgent sends
    ...
    def render_message_event(self, event: Any, sink: Any) -> None:
        ...
        """
        from roveagent.gateway.stream_events import MessageChunk, MessageStop, Commentary
    ...
    def format_tool_event(self, event: Any, *, mode: str = "all",
        ...
        """
        from roveagent.gateway.stream_events import ToolCallChunk
        if not isinstance(event, ToolCallChunk):
            return None

        from roveagent.core.display import get_tool_emoji
        ...
        if preview:
            from roveagent.core.display import prepare_tool_preview
    ...
    def _write_runtime_status_safe(self, context: str, **kwargs) -> None:
        ...
        """
        try:
            from roveagent.gateway.status import write_runtime_status
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-ROVEAGENT_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(roveagent --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    def _release_platform_lock(self) -> None:
        ...
        if not identity:
            return
        from roveagent.gateway.status import release_scoped_lock
    ...
    def _wire_plugin_handlers(self, native: Any = None) -> None:
        ...
        try:
            from roveagent.clisupport.plugins import get_plugin_manager
    ...
    def _history_media_paths_for_session(self, session_key: str) -> Optional[set]:
        ...
        from roveagent.gateway.run import _collect_history_media_paths
    ...
    async def send_clarify(
        ...
        if choices:
            ...
            _is_multi = False
            try:
                from roveagent.tools import clarify_gateway as _cg
                with _cg._lock:
            ...
            from roveagent.tools.clarify_gateway import mark_awaiting_text
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the RoveAgent home layout.
        """
    ...
    def prepare_tts_text(self, text: str) -> str:
        ...
        """
        try:
            from roveagent.tools.tts_text_normalize import prepare_spoken_text
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the RoveAgent home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.roveagent/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from roveagent.clisupport.commands import (
            ...
            if not cmd and event.allow_gateway_control:
                try:
                    from roveagent.tools import clarify_gateway as _clarify_mod
                    _has_text_clarify = (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          ROVEAGENT_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          ROVEAGENT_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          ROVEAGENT_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("ROVEAGENT_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("ROVEAGENT_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("ROVEAGENT_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_roveagent_run_generation", None),
                            event=event,
                        )):
                    try:
                        from roveagent.tools.tts_tool import text_to_speech_tool, check_tts_requirements
                        if check_tts_requirements():
                            ...
                            # ROVEAGENT_SESSION_PLATFORM contextvar the tool would
                ...
                if text_content and not _tts_caption_delivered:
                    ...
                    if not is_ephemeral_response and not str(
                        ...
                        try:
                            from roveagent.gateway.delivery_ledger import (
            ...
            # not at the top of this task.  _roveagent_run_generation is set on
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        try:
            from roveagent.gateway.shutdown_flush import flush_pending_to_file
    ...
    def build_source(
        ...
        per-profile ROVEAGENT_HOME isolation.
        """
        ...
        if runner is not None:
            from roveagent.gateway.profile_routing import ProfileRouteRejected
