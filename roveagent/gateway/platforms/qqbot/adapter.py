...
from roveagent.gateway.platforms.media_cache import ext_for_mime
...
from roveagent.gateway.platforms.qqbot.constants import (
    API_BASE,
    TOKEN_URL,
    GATEWAY_URL_PATH,
    DEFAULT_API_TIMEOUT,
...
from roveagent.gateway.platforms.qqbot.utils import (
    coerce_list as _coerce_list_impl,
    build_user_agent,
)
from roveagent.gateway.platforms.qqbot.chunked_upload import (
    ChunkedUploader,
    UploadDailyLimitExceededError,
    UploadFileTooLargeError,
)
from roveagent.gateway.platforms.qqbot.keyboards import (
    ApprovalRequest,
    InlineKeyboard,
    InteractionEvent,
...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.roveagent/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. RoveAgent upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "roveagent-agent",
                    "$device": "roveagent-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.roveagent/.update_response`` for the
          detached ``roveagent update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``roveagent update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `roveagent setup gateway` / `roveagent gateway`)
    ...
    async def send_approval_request(
        ...
        """
        from roveagent.gateway.platforms.qqbot.keyboards import build_approval_text
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``roveagent update --gateway`` watcher. Button
        ...
        ``~/.roveagent/.update_response`` so the detached update process
        can read it.
        """
