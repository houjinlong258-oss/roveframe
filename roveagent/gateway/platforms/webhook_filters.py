...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.roveagent to the active profile home."""
    ...
    if raw.startswith("~/.roveagent/"):
        return roveagent_home / raw.removeprefix("~/.roveagent/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return roveagent_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ROVEAGENT_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from roveagent.constants import get_roveagent_home

    scripts_root = (get_roveagent_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.roveagent" or raw_text.startswith("~/.roveagent/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__roveagent_ignore__") is True
        ):
            return False, None
