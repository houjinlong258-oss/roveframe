...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *RoveAgent Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to ROVEAGENT_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/roveagent), this function
    mirrors the bridge source to a writable ROVEAGENT_HOME location and returns that
    ...
    # Try ROVEAGENT_HOME location first
    roveagent_home = get_roveagent_home()
    roveagent_home_bridge = roveagent_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to ROVEAGENT_HOME if needed
    if roveagent_home_bridge.exists():
        return roveagent_home_bridge

    # Mirror the bridge source to ROVEAGENT_HOME
    try:
        roveagent_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            roveagent_home_bridge,
            dirs_exist_ok=False,
        )
        return roveagent_home_bridge
    except Exception:
