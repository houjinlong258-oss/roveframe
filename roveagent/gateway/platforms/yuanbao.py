...
from roveagent.gateway.platforms import helpers as _mdchunk
from roveagent.gateway.platforms.helpers import MessageDeduplicator
from roveagent.gateway.platforms.yuanbao_media import (
...
from roveagent.gateway.platforms.yuanbao_proto import (
    CMD_TYPE,
    _fields_to_dict,
    _get_string,
    _get_varint,
...
_APP_VERSION = _ROVEAGENT_VERSION
_BOT_VERSION = _ROVEAGENT_VERSION
_YUANBAO_INSTANCE_ID = str(ROVEAGENT_INSTANCE_ID)  # single source: yuanbao_proto.ROVEAGENT_INSTANCE_ID
        ...
        if not adapter._auto_sethome_done and adapter._sender_may_designate_home(ctx):
            ...
            if _should_set:
                try:
                    ...
                    from roveagent.clisupport.config import atomic_config_write, read_user_config_raw
...
class StickerHandler(MediaSendHandler):
    ...
    def build_msg_body(self, upload_result, **kwargs):
        from roveagent.gateway.platforms.yuanbao_sticker import (
            get_sticker_by_name,
            get_random_sticker,
            build_face_msg_body,
...
class GroupQueryService:
    ...
    async def query_group_info_raw(self, group_code: str) -> Optional[dict]:
        ...
        from roveagent.gateway.platforms.yuanbao_proto import decode_conn_msg as _decode
