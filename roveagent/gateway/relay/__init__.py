"""Relay/connector support package for the RoveAgent gateway.
...
def relay_url() -> Optional[str]:
    ...
    if url:
        return url.rstrip("/")
    try:
        from roveagent.gateway.run import _load_gateway_config  # late import to avoid cycle
...
def _relay_bot_ids_map() -> dict:
    ...
    try:
        ...
        logging.getLogger("roveagent.gateway.relay").warning(
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``roveagent gateway enroll`` writes them to
    ``~/.roveagent/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``ROVEAGENT_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``roveagent gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from roveagent.clisupport.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**RoveAgent Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``roveagent gateway enroll`` CLI. Three modes, in precedence order:
    ...
    if not token_url:
        ...
        from roveagent.clisupport.auth import resolve_nous_access_token
    ...
    if not client_id or not client_secret:
        ...
        raise RuntimeError(
            f"roveagent.gateway.idp.token_url is configured with a partial client credential "
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.roveagent/.env``.
    ...
    sets neither ``ROVEAGENT_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``roveagent gateway enroll``: has a PINNED
    ...
    logger = logging.getLogger("roveagent.gateway.relay")
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'roveagent'}"
...
def send_relay_policy() -> bool:
    ...
    try:
        from roveagent.gateway.relay.auth import make_upgrade_token
...
def register_relay_adapter(force: bool = False, url: Optional[str] = None) -> bool:
    ...
    from roveagent.gateway.platform_registry import PlatformEntry, platform_registry
    from roveagent.gateway.relay.adapter import RelayAdapter
    from roveagent.gateway.relay.descriptor import CONTRACT_VERSION, CapabilityDescriptor
    ...
    def _factory(config):
        ...
        transport = None
        if resolved_url:
            from roveagent.gateway.relay.ws_transport import WebSocketRelayTransport
