...
from roveagent.gateway.platforms.base import BasePlatformAdapter, MessageEvent, SendResult
from roveagent.gateway.relay.descriptor import CapabilityDescriptor
from roveagent.gateway.relay.media import RelayMediaClient
from roveagent.gateway.relay.transport import RelayTransport
...
class RelayAdapter(BasePlatformAdapter):
    ...
    def _relay_slack_extra(self) -> Dict[str, Any]:
        ...
        Enterprise knob shape (RoveAgent-config directed, relay-namespaced):
    ...
    def _slack_unfurl_hints(self, platform: Optional[str]) -> Optional[Dict[str, bool]]:
        ...
        for knob in ("unfurl_links", "unfurl_media"):
            ...
            if val is None:
                continue
            # Railway / `roveagent config set` write YAML strings ("true"/"false").
        ...
        try:
            ...
            if platform == "discord":
                ...
                if event is not None:
                    self._capture_scope(event)
                    # Phase 3: a component press carrying a RoveAgent prompt token
    ...
    def _discord_interaction_to_event(self, forward):
        ...
        from roveagent.gateway.platforms.base import MessageType
        ...
        if itype == 3:
            # Phase 3: a component press whose custom_id is a RoveAgent prompt
    ...
    def _decode_prompt_token(token: str):
        ...
        — and is not — a RoveAgent prompt answer.
        """
        import re
    ...
    async def disconnect(self) -> None:
        ...
        from roveagent.gateway.relay.ws_transport import _env_disconnect_budget_s
    ...
    def _resolve_reply_to_for_send(
        ...
        Native Slack RoveAgent already suppresses this synthetic DM thread anchor:
    ...
    def _get_media_client(self) -> Optional[RelayMediaClient]:
        ...
        try:
            from roveagent.gateway.relay import relay_connection_auth, relay_url
            from roveagent.gateway.relay.media import media_base_url
    ...
    async def _send_prompt(
        ...
        *,
        ...
        # RoveAgent suppresses this synthetic DM thread anchor; drop it here for the
        ...
        * **Our own expired/unknown prompt.** Answer with a short expiry notice
        ...
        try:
            if kind == "exec_approval":
                ...
                from roveagent.tools import slash_confirm as slash_confirm_mod
                ...
                from roveagent.tools.clarify_gateway import (
                    mark_awaiting_text,
    ...
    async def on_processing_complete(self, event, outcome) -> None:
        ...
        from roveagent.gateway.platforms.base import ProcessingOutcome
