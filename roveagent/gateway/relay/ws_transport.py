...
from roveagent.gateway.platforms.base import MessageEvent, MessageType
from roveagent.gateway.session import SessionSource
from roveagent.gateway.relay.descriptor import CapabilityDescriptor
from roveagent.gateway.relay.transport import InboundHandler
...
def _normalize_slack_parent_command(
    ...
    """Mirror native Slack ``/roveagent`` routing for authenticated relay text."""
    stripped = text.strip()
    parent_parts = stripped.split(maxsplit=1)
    if not parent_parts or parent_parts[0] != "/roveagent":
...
def _event_from_wire(raw: Dict[str, Any]) -> MessageEvent:
    ...
    source = SessionSource(
        ...
        # The ROVEAGENT profile this event is routed to (multiplex mode). The
    ...
    if platform_enum == Platform.SLACK:
        ...
        # relay, bypassing RoveAgent' native Slack command callback. Normalize at
        # the wire boundary so adapter-level active-session gates see the real
        # gateway command rather than the legacy `roveagent` parent name.
...
class WebSocketRelayTransport:
    ...
    async def _dial_and_start(self) -> None:
        ...
        for platform, bot_id in self._identities:
            ...
            if platform == "discord":
                try:
                    from roveagent.gateway.relay.command_manifest import build_relay_command_manifest
