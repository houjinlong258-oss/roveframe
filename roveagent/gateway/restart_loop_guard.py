...
Defenses 1 and 2 (the ``_ROVEAGENT_GATEWAY`` guard on ``roveagent gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.roveagent.gateway")``,
...
State lives in ``<ROVEAGENT_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_roveagent_home() / "gateway" / "restart_loop.json"
