...
# ``ROVEAGENT_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``ROVEAGENT_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("ROVEAGENT_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in roveagent_state, request-dump
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `roveagent tools` (opt-in, default
    ...
    """
    try:
        from roveagent.tools.mcp_tool import get_registered_mcp_server_names
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `roveagent tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `roveagent tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_roveagent_home()}/cron/output/)"
    )
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier RoveAgent session "
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # ``get_roveagent_home()`` for the turn (its docstring lists "sessions"
        ...
        from roveagent.gateway.session_db_recovery import RecoverableHandleCache
    ...
    def _open_session_db_for_active_scope(self):
        ...
        time, and that helper follows the context-local ROVEAGENT_HOME override
        ...
        if _db:
            ...
            if callable(loader):
                try:
                    ...
                    logger.warning(
                        "roveagent.gateway.session: state.db routing load failed: %s", e
                    )
        ...
        if sessions_file.exists():
            try:
                ...
                if imported and db_had_entries:
                    logger.info(
                        "roveagent.gateway.session: imported %d legacy sessions.json "
    ...
    def _prune_stale_sessions_locked(self) -> None:
        ...
        try:
                ...
                if row is not None and row.get("end_reason") is not None:
                    ...
                    if entry.origin is not None:
                        try:
                            ...
                            logger.debug(
                                "roveagent.gateway.session: recovery lookup failed for stale "
                    ...
                    if recovered_entry is not None and recovered_entry.session_id != entry.session_id:
                        logger.warning(
                            "roveagent.gateway.session: repointing stale sessions.json entry "
                    ...
                    if recovered_entry is not None:
                        logger.info(
                            "roveagent.gateway.session: reopened ended session %s for "
                    ...
                    logger.warning(
                        "roveagent.gateway.session: pruning stale sessions.json entry "
            ...
            logger.warning(
                "roveagent.gateway.session: stale-entry pruning skipped due to DB error: %s",
                exc,
            )
            return
    ...
    def _reconcile_recovered_routing_locked(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "roveagent.gateway.session: recovered state.db routing load failed: %s", exc
            )
            return
        ...
        with save_lock:
            ...
            if _db:
                ...
                if callable(replacer):
                    try:
                        ...
                        logger.warning(
                            "roveagent.gateway.session: state.db routing save failed: %s", exc
                        )
            if getattr(self, "_write_sessions_json", True) or not db_saved:
                try:
                    ...
                    logger.warning(
                        "roveagent.gateway.session: sessions.json mirror save failed "
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.roveagent/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.roveagent/state.db "
                "and are shown by `roveagent sessions list` and `/sessions`. "
        ...
        *,
        ...
        if callable(saver):
            ...
            try:
                ...
                logger.warning(
                    "roveagent.gateway.session: single-entry routing save failed for %r "
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                if _is_stale and entry.session_id == _stale_session_id:
                    ...
                    # a fresh session.
                    logger.warning(
                        "roveagent.gateway.session: routing key %r -> %s is ended in "
    ...
    def _append_to_transcript_serialized(
        ...
        with self._transcript_retry_lock:
            ...
            if len(pending) > self._MAX_PENDING_PER_SESSION:
                ...
                spool_path = None
                try:
                    from roveagent.gateway.shutdown_flush import (
        ...
        while True:
            try:
                ...
                from roveagent.state import CompressionSessionClosedError
    ...
    def _drain_spooled_drops(self, session_id: str) -> None:
        ...
        if not spooled_sessions or session_id not in spooled_sessions:
            return
        try:
            from roveagent.gateway.shutdown_flush import drain_transcript_spool
