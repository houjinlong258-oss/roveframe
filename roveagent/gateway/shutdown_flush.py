...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<roveagent_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active ROVEAGENT_HOME."""
    from roveagent.constants import get_roveagent_home

    flush_dir = get_roveagent_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<roveagent_home>/pending_messages/``), so a runtime cap rotation no
