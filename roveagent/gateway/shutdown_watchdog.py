...
2. An event-loop heartbeat file at ``<ROVEAGENT_HOME>/state/gateway.heartbeat`` so
...
from roveagent.gateway.restart import GATEWAY_SERVICE_RESTART_EXIT_CODE
...
_HEARTBEAT_RELATIVE = ("state", "roveagent.gateway.heartbeat")
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        while not stop_event.wait(timeout=interval):
            ...
            try:
                from roveagent.gateway.lifecycle_ledger import mark_exited
...
def _process_roveagent_home() -> Path:
    """ROVEAGENT_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<ROVEAGENT_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_roveagent_home()
...
def get_loop_tick_socket_path(
    ...
    ``<ROVEAGENT_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
    ...
    return base.joinpath(
        "state", f"roveagent.gateway.loop-tick.{int(pid if pid is not None else os.getpid())}.sock"
    )
...
def arm_shutdown_watchdog(
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            from roveagent.gateway.status import remove_pid_file, release_gateway_runtime_lock
            remove_pid_file()
            release_gateway_runtime_lock()
        except Exception:
            pass
        try:
            from roveagent.logsetup import drain_log_queue
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``roveagent_cli.gateway.probe_gateway_loop_liveness`` for the
        ...
        if os.name == "posix":
            try:
                for _stale in tick_socket_path.parent.glob(
                    "roveagent.gateway.loop-tick.*.sock"
                ):
