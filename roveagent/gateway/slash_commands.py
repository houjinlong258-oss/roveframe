...
Module-level run.py helpers a handler needs (``_roveagent_home``,
...
from roveagent.core.account_usage import fetch_account_usage, render_account_usage_lines
from roveagent.core.i18n import t
from roveagent.core.turn_context import extract_api_content_sidecar
from roveagent.gateway.config import HomeChannel, Platform, PlatformConfig, persist_home_channel
from roveagent.gateway.platforms.base import EphemeralReply, MessageEvent, MessageType
from roveagent.gateway.session import (
    AsyncSessionStore,
    SessionSource,
    build_session_key,
    is_shared_multi_user_session,
)
from roveagent.clisupport.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from roveagent.utils import (
    atomic_json_write,
    base_url_host_matches,
    is_truthy_value,
)

logger = logging.getLogger("roveagent.gateway.run")
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        "roveagent.gateway.model.error_prefix",
        error=(
            ...
            f"crash — restart the gateway to load the new code: roveagent gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach RoveAgent commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            from roveagent.tools.async_delegation import interrupt_for_session
        ...
        try:
            from roveagent.tools.env_passthrough import clear_env_passthrough
            clear_env_passthrough()
        except Exception:
            pass

        try:
            from roveagent.tools.credential_files import clear_credential_files
        ...
        if new_entry:
            header = await asyncio.to_thread(self._telegram_topic_new_header, source) or t("roveagent.gateway.reset.header_default")
        else:
            # No existing session, just create one
            new_entry = await self.async_session_store.get_or_create_session(source, force_new=True)
            header = await asyncio.to_thread(self._telegram_topic_new_header, source) or t("roveagent.gateway.reset.header_new")
        ...
        if _title_arg and self._session_db and new_entry:
            ...
            try:
                ...
                sanitized = None
                _title_note = t("roveagent.gateway.reset.title_rejected", error=str(e))
            if sanitized:
                try:
                    await self._session_db.set_session_title(new_entry.session_id, sanitized)
                    header = t("roveagent.gateway.reset.header_titled", title=sanitized)
                except ValueError as e:
                    _title_note = t("roveagent.gateway.reset.title_error_untitled", error=str(e))
                except Exception:
                    pass
            elif not _title_note:
                # sanitize_title returned empty (whitespace-only / unprintable)
                _title_note = t("roveagent.gateway.reset.title_empty_untitled")
        ...
        try:
            from roveagent.clisupport.lifecycle import invoke_hook as _invoke_hook
        ...
        try:
            from roveagent.clisupport.tips import get_random_tip
            _tip_line = t("roveagent.gateway.reset.tip", tip=get_random_tip())
        except Exception:
            _tip_line = ""
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        from roveagent.clisupport.slash_exec import CommandContext, execute_command
        ...
        if multiplexed:
            ...
            try:
                from roveagent.gateway.run import _profile_runtime_scope

                profile_home = self._resolve_profile_home_for_source(source)
                with _profile_runtime_scope(profile_home):
                    display = display_roveagent_home()
            except Exception:
                display = display_roveagent_home()
        ...
        lines = [
            t("roveagent.gateway.profile.header", profile=reply.data["profile"]),
            t("roveagent.gateway.profile.home", home=reply.data["home"]),
        ]
    ...
    async def _handle_whoami_command(self, event: MessageEvent) -> str:
        ...
        """
        from roveagent.gateway.slash_access import policy_for_source as _policy_for_source
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from roveagent.clisupport.kanban import run_slash
        ...
        try:
            ...
            return t("roveagent.gateway.kanban.error_prefix", error=exc)
            ...
            if m:
                ...
                try:
                    ...
                    if platform_str and chat_id:
                        def _sub():
                            from roveagent.clisupport import kanban_db as _kb
                        ...
                        output = (
                            output.rstrip()
                            + "\n"
                            + t("roveagent.gateway.kanban.subscribed_suffix", task_id=task_id)
                        )
        ...
        if len(output) > 3800:
            output = output[:3800] + "\n" + t("roveagent.gateway.kanban.truncated_suffix")
        return output or t("roveagent.gateway.kanban.no_output")

    async def _handle_status_command(self, event: MessageEvent) -> str:
        """Handle /status command."""
        from roveagent.gateway.run import _AGENT_PENDING_SENTINEL, _load_gateway_config, _resolve_gateway_model
        ...
        if model_name:
            if provider_name:
                model_line = t("roveagent.gateway.status.model_provider", model=model_name, provider=provider_name)
            else:
                model_line = t("roveagent.gateway.status.model", model=model_name)

        context_line = ""
        if context_total:
            pct = min(100, round((context_used / context_total) * 100)) if context_total else 0
            context_line = t(
                "roveagent.gateway.status.context",
                used=f"{context_used:,}",
                total=f"{context_total:,}",
                pct=f"{pct}",
            )
        elif context_used:
            context_line = t("roveagent.gateway.status.context_used", used=f"{context_used:,}")

        lines = [
            t("roveagent.gateway.status.header"),
            "",
            t("roveagent.gateway.status.session_id", session_id=session_entry.session_id),
        ]
        if title:
            lines.append(t("roveagent.gateway.status.title", title=title))
        lines.extend([
            t("roveagent.gateway.status.created", timestamp=session_entry.created_at.strftime('%Y-%m-%d %H:%M')),
            t("roveagent.gateway.status.last_activity", timestamp=session_entry.updated_at.strftime('%Y-%m-%d %H:%M')),
        ])
        if model_line:
        ...
        lines.extend([
            t("roveagent.gateway.status.tokens", tokens=f"{db_total_tokens:,}"),
            t("roveagent.gateway.status.agent_running", state=t("roveagent.gateway.status.state_yes") if is_running else t("roveagent.gateway.status.state_no")),
        ])
        if queue_depth:
            lines.append(t("roveagent.gateway.status.queued", count=queue_depth))
        if source.platform == Platform.MATRIX:
            ...
            lines.extend([
                "",
                t("roveagent.gateway.status.matrix_scope_header"),
                t("roveagent.gateway.status.matrix_scope_room", room=source.chat_name or source.chat_id),
                t("roveagent.gateway.status.matrix_scope_room_id", room_id=source.chat_id),
                t("roveagent.gateway.status.matrix_scope_thread", thread_id=thread),
                t("roveagent.gateway.status.matrix_scope_mode", scope=scope),
                t(
                    "roveagent.gateway.status.matrix_scope_key",
                    session_key=self._redact_matrix_session_key(session_key),
                ),
            ])
        lines.extend([
            "",
            t("roveagent.gateway.status.platforms", platforms=', '.join(connected_platforms)),
        ])
    ...
    async def _handle_context_command(self, event: MessageEvent) -> str:
        ...
        """
        from roveagent.gateway.run import _AGENT_PENDING_SENTINEL
        ...
        if not context_length:
            try:
                from roveagent.gateway.run import (
        ...
        if used > 0 and context_length > 0:
            ...
            lines = [
                t("roveagent.gateway.context.header"),
                "",
                t("roveagent.gateway.context.model", model=model_name or "?"),
                t("roveagent.gateway.context.window", total=f"{context_length:,}"),
                t(
                    "roveagent.gateway.context.in_use",
                    used=f"{used:,}",
                    total=f"{context_length:,}",
                    pct=f"{pct:.0f}",
                ),
                t("roveagent.gateway.context.bar", bar=bar),
                t("roveagent.gateway.context.headroom", headroom=f"{headroom:,}"),
            ]
            ...
            if ctx is not None:
                ...
                if threshold > 0:
                    if used >= threshold:
                        lines.append(
                            t(
                                "roveagent.gateway.context.over_threshold",
                        ...
                        lines.append(
                            t(
                                "roveagent.gateway.context.threshold",
                                threshold=f"{threshold:,}",
                                threshold_pct=f"{threshold_pct:.0f}",
                ...
                lines.append(t("roveagent.gateway.context.compressions", count=compressions))
                if compressions:
                    savings = getattr(ctx, "_last_compression_savings_pct", None)
                    if savings is not None:
                        lines.append(
                            t("roveagent.gateway.context.last_savings", savings=f"{savings:.0f}")
                        )
                ...
                lines.append("")
                lines.append(
                    t("roveagent.gateway.context.totals_header", calls=api_calls)
                )
                lines.append(
                    t(
                        "roveagent.gateway.context.totals_line",
                        input=f"{input_tokens:,}",
                        output=f"{output_tokens:,}",
                        reasoning=f"{reasoning_tokens:,}",
                    )
                )
                lines.append(t("roveagent.gateway.context.total_billed", total=f"{total_tokens:,}"))
                lines.append(t("roveagent.gateway.context.throughput_note"))
            else:
                lines.append("")
                lines.append(t("roveagent.gateway.context.detail_after_first"))
        ...
        if history:
            from roveagent.core.model_metadata import estimate_messages_tokens_rough
            ...
            return "\n".join(
                [
                    ...
                    "",
                    t(
                        "roveagent.gateway.context.estimated",
                        count=f"{approx:,}",
                        messages=len(msgs),
                    ),
                    t("roveagent.gateway.context.detail_after_first"),
                ]
            )
        return t("roveagent.gateway.context.no_data")
    ...
    def _resume_caller_is_admin(self, source: SessionSource) -> bool:
        ...
        """
        try:
            from roveagent.gateway.slash_access import policy_for_source
    ...
    async def _handle_agents_command(self, event: MessageEvent) -> str:
        ...
        from roveagent.gateway.run import _AGENT_PENDING_SENTINEL
        from roveagent.tools.process_registry import format_uptime_short, process_registry
            ...
            agent_rows.append(
                {
                    "session_key": session_key,
                    "elapsed": elapsed,
                    "state": t("roveagent.gateway.agents.state_starting") if is_pending else t("roveagent.gateway.agents.state_running"),
        ...
        lines = [
            t("roveagent.gateway.agents.header"),
            "",
            t("roveagent.gateway.agents.active_agents", count=len(agent_rows)),
        ]

        if agent_rows:
            for idx, row in enumerate(agent_rows[:12], 1):
                current = t("roveagent.gateway.agents.this_chat") if row["session_key"] == current_session_key else ""
            ...
            if len(agent_rows) > 12:
                lines.append(t("roveagent.gateway.agents.more", count=len(agent_rows) - 12))

        lines.extend(
            [
                "",
                t("roveagent.gateway.agents.running_processes", count=len(running_processes)),
            ]
        )
        if running_processes:
            ...
            if len(running_processes) > 12:
                lines.append(t("roveagent.gateway.agents.more", count=len(running_processes) - 12))

        lines.extend(
            [
                "",
                t("roveagent.gateway.agents.async_jobs", count=len(background_tasks)),
            ]
        )
        ...
        try:
            from roveagent.tools.async_delegation import list_async_delegations
            delegations = [
        ...
        if delegations:
            lines.extend(
                [
                    "",
                    t(
                        "roveagent.gateway.agents.background_delegations",
            ...
            if len(delegations) > 12:
                lines.append(
                    t("roveagent.gateway.agents.more", count=len(delegations) - 12)
                )
        ...
        if (
            ...
            lines.append("")
            lines.append(t("roveagent.gateway.agents.none"))
    ...
    async def _handle_stop_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        """
        from roveagent.gateway.run import _AGENT_PENDING_SENTINEL, _INTERRUPT_REASON_STOP
        source = event.source
        session_entry = await self.async_session_store.get_or_create_session(source)
        session_key = session_entry.session_key
        ...
        if agent is _AGENT_PENDING_SENTINEL:
            ...
            return EphemeralReply(t("roveagent.gateway.stop.stopped_pending"))
        if agent:
            ...
            return EphemeralReply(t("roveagent.gateway.stop.stopped"))
        ...
        return t("roveagent.gateway.stop.no_active")
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`roveagent gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from roveagent.gateway.run import _roveagent_home
        ...
        if self._restart_requested or self._draining:
            ...
            if count:
                return t("roveagent.gateway.draining", count=count)
            return EphemeralReply(t("roveagent.gateway.restart.in_progress"))
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _roveagent_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _roveagent_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
        ...
        if active_agents:
            return t("roveagent.gateway.draining", count=active_agents)
        return EphemeralReply(t("roveagent.gateway.restart.restarting"))

    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running RoveAgent Agent version."""
        from roveagent.clisupport.slash_exec import CommandContext, execute_command
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from roveagent.gateway.run import _telegramize_command_mentions
        from roveagent.clisupport.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from roveagent.gateway.run import _roveagent_home, _load_gateway_config
        from roveagent.clisupport.model_switch import (
            switch_model as _switch_model, parse_model_switch_args,
            resolve_persist_behavior,
            list_authenticated_providers,
            list_picker_providers,
        )
        from roveagent.clisupport.providers import get_label
        ...
        # via the shared single-owner parser (roveagent_cli.model_switch).
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _roveagent_home) / "config.yaml"
        try:
        ...
        if not model_input and not explicit_provider:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        if not result.success:
                            return t("roveagent.gateway.model.error_prefix", error=result.error_message)

                        try:
                            from roveagent.clisupport.context_switch_guard import (
                        ...
                        from roveagent.clisupport.model_switch import format_model_for_display
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from roveagent.clisupport.route_identity import should_clear_context_pin_async
                                ...
                                from roveagent.clisupport.config import save_config
                        ...
                        lines = [t("roveagent.gateway.model.switched", model=format_model_for_display(result.new_model))]
                        lines.append(t("roveagent.gateway.model.provider_label", provider=plabel))
                        mi = result.model_info
                        from roveagent.clisupport.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
                        ...
                        if ctx:
                            lines.append(t("roveagent.gateway.model.context_label", tokens=f"{ctx:,}"))
                        if mi:
                            if mi.max_output:
                                lines.append(t("roveagent.gateway.model.max_output_label", tokens=f"{mi.max_output:,}"))
                            lines.append(t("roveagent.gateway.model.capabilities_label", capabilities=mi.format_capabilities()))
                        if result.warning_message:
                            lines.append(t("roveagent.gateway.model.warning_prefix", warning=result.warning_message))
                        if persist_global:
                            lines.append(t("roveagent.gateway.model.saved_global"))
                        else:
                            lines.append(t("roveagent.gateway.model.session_only_hint"))
                        return "\n".join(lines)
            ...
            lines = [t("roveagent.gateway.model.current_label", model=current_model or "unknown", provider=provider_label), ""]
            ...
            try:
                ...
                for p in providers:
                    tag = t("roveagent.gateway.model.current_tag") if p["is_current"] else ""
                    lines.append(f"**{p['name']}** `--provider {p['slug']}`{tag}:")
                    if p["models"]:
                        model_strs = ", ".join(f"`{m}`" for m in p["models"])
                        extra = t("roveagent.gateway.model.more_models_suffix", count=p["total_models"] - len(p["models"])) if p["total_models"] > len(p["models"]) else ""
            ...
            lines.append(t("roveagent.gateway.model.usage_switch_model"))
            lines.append(t("roveagent.gateway.model.usage_switch_provider"))
            lines.append(t("roveagent.gateway.model.usage_persist"))
            return "\n".join(lines)
        ...
        async def _finish_switch() -> str:
            ...
            lines = [t("roveagent.gateway.model.switched", model=format_model_for_display(result.new_model))]
            lines.append(t("roveagent.gateway.model.provider_label", provider=provider_label))
            ...
            if cache_enabled:
                lines.append(t("roveagent.gateway.model.prompt_caching_enabled"))
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — RoveAgent default runtime
        ...
        from roveagent.clisupport import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        All resolution/persistence goes through roveagent_cli.personality —
        the single owner of personality state on every surface.
        """
        from roveagent.gateway.run import _load_gateway_config
        from roveagent.clisupport.personality import (
        ...
        if not args:
            ...
            lines = [t("roveagent.gateway.personality.header")]
            lines.append(t("roveagent.gateway.personality.none_option"))
            for name, prompt in personalities.items():
                marker = " ✓" if name == current else ""
                lines.append(
                    t(
                        "roveagent.gateway.personality.item",
                        name=f"{name}{marker}",
                        preview=describe_personality(prompt),
                    )
                )
            lines.append(t("roveagent.gateway.personality.usage"))
            return "\n".join(lines)
        ...
        try:
            ...
            return t("roveagent.gateway.personality.unknown", name=args.lower(), available=available)

        # Persist the selection only — roveagent_cli.personality never writes
        # agent.system_prompt (user-owned manual overlay).
        if not persist_personality(name):
            return t("roveagent.gateway.personality.save_failed", error="config write failed")

        if not name:
            self._ephemeral_system_prompt = prompt_text(
                cfg_get(config, "agent", "system_prompt", default="")
            )
            return t("roveagent.gateway.personality.cleared")

        # Update in-memory so it takes effect on the very next message.
        self._ephemeral_system_prompt = new_prompt
        return t("roveagent.gateway.personality.set_to", name=name)
    ...
    async def _handle_retry_command(self, event: MessageEvent) -> str:
        ...
        from roveagent.core.context_compressor import (
        ...
        if last_user_idx is None:
            return t("roveagent.gateway.retry.no_previous")
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if mgr is None:
            return t("roveagent.gateway.goal.unavailable")
        ...
        if lower == "pause":
            ...
            if state is None:
                return t("roveagent.gateway.goal.no_goal_set")
            try:
            ...
            return t("roveagent.gateway.goal.paused", goal=state.goal)

        if lower == "resume":
            state = mgr.resume()
            if state is None:
                return t("roveagent.gateway.goal.no_resume")
            ...
            return t("roveagent.gateway.goal.resumed", goal=state.goal)
        ...
        if lower in {"clear", "stop", "done"}:
            ...
            return t("roveagent.gateway.goal_cleared") if had else t("roveagent.gateway.no_active_goal")
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from roveagent.clisupport.goals import draft_contract
            ...
            from roveagent.clisupport.goals import parse_contract
        ...
        try:
            ...
            return t("roveagent.gateway.goal.invalid", error=str(exc))
        ...
        base = t("roveagent.gateway.goal.set", budget=state.max_turns, goal=state.goal)
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from roveagent.clisupport.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `roveagent cron` for durable schedules."
        )
    ...
    async def _handle_review_command(self, event: "MessageEvent") -> str:
        ...
        from roveagent.tools.approval import (
        ...
        def _dispatch():
            ...
            try:
                from roveagent.core.review_engine import start_review
        ...
        from roveagent.core.review_engine import format_dispatch_note
    ...
    async def _get_loop_manager_for_event(self, event: "MessageEvent"):
        ...
        """
        try:
            from roveagent.clisupport.loops import LoopManager
        except Exception as exc:
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from roveagent.clisupport.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        if raw_args:
            try:
                ...
                return t("roveagent.gateway.undo.invalid_count", arg=raw_args.split()[0])
            if n < 1:
                n = 1
        ...
        if result is None:
            return t("roveagent.gateway.undo.nothing")
        ...
        return t(
            "roveagent.gateway.undo.removed",
    ...
    async def _handle_set_home_command(self, event: MessageEvent) -> str:
        ...
        from roveagent.gateway.run import _home_target_env_var, _home_thread_env_var
        source = event.source
        platform_name = source.platform.value if source.platform else "unknown"
        chat_id = source.chat_id
        chat_name = source.chat_name or chat_id
        if source.platform is None:
            return t("roveagent.gateway.set_home.save_failed", error="Missing logical platform")
        ...
        if via_relay:
            ...
            if (
                ...
                return t(
                    "roveagent.gateway.set_home.save_failed",
        ...
        try:
            ...
            return t("roveagent.gateway.set_home.save_failed", error=e)
        ...
        return t("roveagent.gateway.set_home.success", name=chat_name, chat_id=chat_id)
    ...
    async def _handle_voice_command(self, event: MessageEvent) -> str:
        ...
        if args in {"on", "enable"}:
            ...
            return t("roveagent.gateway.voice.enabled_voice_only")
        elif args in {"off", "disable"}:
            self._voice_mode[voice_key] = "off"
            self._save_voice_modes()
            if adapter:
                self._set_adapter_auto_tts_disabled(adapter, chat_id, disabled=True)
            return t("roveagent.gateway.voice.disabled_text")
        elif args == "tts":
            self._voice_mode[voice_key] = "all"
            self._save_voice_modes()
            if adapter:
                self._set_adapter_auto_tts_enabled(adapter, chat_id, enabled=True)
            return t("roveagent.gateway.voice.tts_enabled")
            ...
            labels = {
                "off": t("roveagent.gateway.voice.label_off"),
                "voice_only": t("roveagent.gateway.voice.label_voice_only"),
                "all": t("roveagent.gateway.voice.label_all"),
            }
            ...
            if guild_id and hasattr(adapter, "get_voice_channel_info"):
                ...
                if info:
                    lines = [
                        t("roveagent.gateway.voice.status_mode", label=labels.get(mode, mode)),
                        t("roveagent.gateway.voice.status_channel", channel=info['channel_name']),
                        t("roveagent.gateway.voice.status_participants", count=info['member_count']),
                    ]
                    for m in info["members"]:
                        status = t("roveagent.gateway.voice.speaking") if m.get("is_speaking") else ""
                        lines.append(t("roveagent.gateway.voice.status_member", name=m['display_name'], status=status))
                    return "\n".join(lines)
            return t("roveagent.gateway.voice.status_mode", label=labels.get(mode, mode))
        else:
            ...
            if current == "off":
                ...
                if adapter:
                    self._set_adapter_auto_tts_enabled(adapter, chat_id, enabled=True)
                toggle_line = t("roveagent.gateway.voice.enabled_short")
            else:
                self._voice_mode[voice_key] = "off"
                self._save_voice_modes()
                if adapter:
                    self._set_adapter_auto_tts_disabled(adapter, chat_id, disabled=True)
                toggle_line = t("roveagent.gateway.voice.disabled_short")
            ...
            channels = (
                t("roveagent.gateway.voice.help_channels") if supports_voice_channels else ""
            )
            return t("roveagent.gateway.voice.help", toggle=toggle_line, channels=channels)

    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        """Handle /rollback command — list or restore filesystem checkpoints."""
        from roveagent.gateway.run import _checkpoint_agent_kwargs, _load_gateway_config
        from roveagent.tools.checkpoint_manager import CheckpointManager, format_checkpoint_list

        cp_kwargs = _checkpoint_agent_kwargs(_load_gateway_config())

        if not cp_kwargs["checkpoints_enabled"]:
            return t("roveagent.gateway.rollback.not_enabled")
        ...
        checkpoints = mgr.list_checkpoints(cwd)
        if not checkpoints:
            return t("roveagent.gateway.rollback.none_found", cwd=cwd)
        ...
        try:
            ...
            if 0 <= idx < len(checkpoints):
                ...
                return t("roveagent.gateway.rollback.invalid_number", max=len(checkpoints))
        except ValueError:
            target_hash = arg
        ...
        if result["success"]:
            msg = t(
                "roveagent.gateway.rollback.restored",
            ...
            if skipped:
                ...
                msg += "\n" + t(
                    "roveagent.gateway.rollback.kept_user_edits",
                    files=shown + more,
                )
            ...
            if oversize:
                ...
                msg += "\n" + t(
                    "roveagent.gateway.rollback.kept_oversize",
                    files=shown + more,
                )
            ...
            if failed:
                ...
                msg += "\n" + t(
                    "roveagent.gateway.rollback.failed_deletes",
                    files=shown + more,
                )
            return msg
        return t("roveagent.gateway.rollback.restore_failed", error=result["error"])
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        RoveAgent itself changed. ``--stat`` limits output to the summary.
        ...
        from roveagent.tools.working_diff import collect_working_diff

        result = await asyncio.to_thread(collect_working_diff, cwd, mode)
        if not result.get("success"):
            return t("roveagent.gateway.diff.failed",
        ...
        if result.get("empty") or (not stat and not diff and not untracked):
            return t("roveagent.gateway.diff.no_changes")
    ...
    async def _gateway_session_diff(self, cwd: str, stat_only: bool) -> str:
        ...
        from roveagent.gateway.run import _checkpoint_agent_kwargs, _load_gateway_config
        from roveagent.tools.checkpoint_manager import CheckpointManager

        cp_kwargs = _checkpoint_agent_kwargs(_load_gateway_config())
        if not cp_kwargs["checkpoints_enabled"]:
            return t("roveagent.gateway.diff.not_enabled")
    ...
    async def _handle_background_command(self, event: MessageEvent) -> str:
        ...
        if not prompt:
            return t("roveagent.gateway.background.usage")
        ...
        return t("roveagent.gateway.background.started", preview=preview, task_id=task_id)
    ...
    async def _handle_btw_command(self, event: MessageEvent) -> str:
        ...
        if not question:
            return t("roveagent.gateway.btw.usage")

        source = event.source
        session_entry = await self.async_session_store.get_or_create_session(source)
        history = await self.async_session_store.load_transcript(session_entry.session_id)
        if not history:
            return t("roveagent.gateway.btw.no_history")
        ...
        if not runtime_kwargs.get("api_key"):
            return t("roveagent.gateway.btw.no_provider")
        ...
        async def _run_side_question() -> None:
            from roveagent.core.side_question import answer_side_question
            try:
                ...
                if adapter is not None:
                    await adapter.send(
                        source.chat_id,
                        t("roveagent.gateway.btw.failed", preview=preview, error=str(e)),
            ...
            if adapter is not None:
                await adapter.send(
                    source.chat_id,
                    t("roveagent.gateway.btw.answer", preview=preview, answer=answer or ""),
                    metadata=_thread_metadata,
                )
        ...
        return t("roveagent.gateway.btw.started", preview=preview)
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        from roveagent.gateway.run import _roveagent_home
        from roveagent.clisupport.config import read_user_config_raw
        config_path = _roveagent_home / "config.yaml"
        try:
    ...
    def _apply_reasoning_selection(
        ...
        """
        from roveagent.constants import parse_reasoning_effort
        ...
        if value in {"show", "on"}:
            ...
            return t("roveagent.gateway.reasoning.display_set_on", platform=platform_key)
        if value in {"hide", "off"}:
            self._show_reasoning = False
            self._save_gateway_config_key(
                f"display.platforms.{platform_key}.show_reasoning", False
            )
            return t("roveagent.gateway.reasoning.display_set_off", platform=platform_key)

        if value == "reset":
            if persist_global:
                return t("roveagent.gateway.reasoning.reset_global_unsupported")
            self._set_session_reasoning_override(session_key, None)
            self._reasoning_config = self._load_reasoning_config()
            self._evict_cached_agent(session_key)
            return t("roveagent.gateway.reasoning.reset_done")

        parsed = parse_reasoning_effort(value)
        if parsed is None:
            return t("roveagent.gateway.reasoning.unknown_arg", arg=value)

        self._reasoning_config = parsed
        if persist_global:
            if self._save_gateway_config_key("roveagent.core.reasoning_effort", value):
                self._set_session_reasoning_override(session_key, None)
                self._evict_cached_agent(session_key)
                return t("roveagent.gateway.reasoning.set_global", effort=value)
            self._set_session_reasoning_override(session_key, parsed)
            self._evict_cached_agent(session_key)
            return t("roveagent.gateway.reasoning.set_global_save_failed", effort=value)

        self._set_session_reasoning_override(session_key, parsed)
        self._evict_cached_agent(session_key)
        return t("roveagent.gateway.reasoning.set_session", effort=value)

    def _reasoning_picker_choices(self, current_effort: str) -> list:
        """Build the choice list for the interactive /reasoning picker."""
        from roveagent.constants import VALID_REASONING_EFFORTS

        choices = [
            {
                "value": "none",
                "label": t("roveagent.gateway.reasoning.choice_none"),
        ...
        choices.extend(
            [
                {"value": "reset", "label": t("roveagent.gateway.reasoning.choice_reset"), "is_current": False},
                {"value": "show", "label": t("roveagent.gateway.reasoning.choice_show"), "is_current": False},
                {"value": "hide", "label": t("roveagent.gateway.reasoning.choice_hide"), "is_current": False},
            ]
        )
        return choices
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from roveagent.gateway.run import _platform_config_key
        ...
        if not raw_args:
            ...
            if rc is None:
                level = t("roveagent.gateway.reasoning.level_default")
                current_effort = "medium"
            elif rc.get("enabled") is False:
                level = t("roveagent.gateway.reasoning.level_disabled")
                current_effort = "none"
            else:
                level = rc.get("effort", "medium")
                current_effort = level
            display_state = (
                t("roveagent.gateway.reasoning.display_on")
                if self._show_reasoning
                else t("roveagent.gateway.reasoning.display_off")
            )
            has_session_override = session_key in (getattr(self, "_session_reasoning_overrides", {}) or {})
            scope = (
                t("roveagent.gateway.reasoning.scope_session")
                if has_session_override
                else t("roveagent.gateway.reasoning.scope_global")
            )
            ...
            picker_sent = await self._try_send_choice_picker(
                ...
                session_key,
                title=t(
                    "roveagent.gateway.reasoning.picker_title",
                    level=level,
                    scope=scope,
            ...
            return t(
                "roveagent.gateway.reasoning.status",
                level=level,
                scope=scope,
                display=display_state,
            )
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from roveagent.gateway.run import _roveagent_home
        from roveagent.clisupport.write_approval_commands import handle_pending_subcommand
        from roveagent.tools import write_approval as wa
        from roveagent.tools.memory_tool import load_on_disk_store
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.roveagent/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``roveagent skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from roveagent.gateway.run import _roveagent_home
        from roveagent.clisupport.write_approval_commands import handle_pending_subcommand
        from roveagent.tools import write_approval as wa
        ...
        # the real review surface. (Note: `roveagent skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.roveagent/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from roveagent.gateway.run import _load_gateway_config, _resolve_gateway_model
        from roveagent.clisupport.models import model_supports_fast_mode
        ...
        if not model_supports_fast_mode(model):
            return t("roveagent.gateway.fast.not_supported")
        ...
        def _apply_fast_selection(value: str, persist: bool = False) -> str:
            ...
            if value in {"fast", "on"}:
                tier = "priority"
                saved_value = "fast"
                label = t("roveagent.gateway.fast.label_fast")
            elif value in {"normal", "off"}:
                tier = None
                saved_value = "normal"
                label = t("roveagent.gateway.fast.label_normal")
            else:
                return t("roveagent.gateway.fast.unknown_arg", arg=value)
            self._service_tier = tier
            if persist:
                if self._save_gateway_config_key("roveagent.core.service_tier", saved_value):
                    # Global write supersedes any session override.
                    self._set_session_service_tier_override(
                        session_key, None, clear=True
                    )
                    self._evict_cached_agent(session_key)
                    return t("roveagent.gateway.fast.saved", label=label)
                ...
                self._evict_cached_agent(session_key)
                return t("roveagent.gateway.fast.session_only", label=label)
            self._set_session_service_tier_override(session_key, tier)
            self._evict_cached_agent(session_key)
            return t("roveagent.gateway.fast.session_only", label=label)
        ...
        if not args or args == "status":
            ...
            status = t("roveagent.gateway.fast.status_fast") if is_fast else t("roveagent.gateway.fast.status_normal")
            ...
            picker_sent = await self._try_send_choice_picker(
                event,
                session_key,
                title=t("roveagent.gateway.fast.picker_title", mode=status),
                choices=[
                    {
                        "value": "fast",
                        "label": t("roveagent.gateway.fast.choice_fast"),
                        "is_current": is_fast,
                    },
                    {
                        "value": "normal",
                        "label": t("roveagent.gateway.fast.choice_normal"),
            ...
            return t("roveagent.gateway.fast.status", mode=status)
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from roveagent.gateway.slash_access import policy_for_source
        from roveagent.clisupport.approval_mode import run_approval_mode_command
    ...
    async def _handle_yolo_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        if current:
            ...
            return EphemeralReply(t("roveagent.gateway.yolo.disabled"))
        else:
            enable_session_yolo(session_key)
            return EphemeralReply(t("roveagent.gateway.yolo.enabled"))
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from roveagent.gateway.run import _roveagent_home, _load_gateway_config, _platform_config_key
        ...
        if not gate_enabled:
            return t("roveagent.gateway.verbose.not_enabled")

        # --- cycle mode (per-platform) ----------------------------------------
        cycle = ["off", "new", "all", "verbose", "log"]
        descriptions = {
            "off": t("roveagent.gateway.verbose.mode_off"),
            "new": t("roveagent.gateway.verbose.mode_new"),
            "all": t("roveagent.gateway.verbose.mode_all"),
            "verbose": t("roveagent.gateway.verbose.mode_verbose"),
            "log": t("roveagent.gateway.verbose.mode_log"),
        }

        # Read current effective mode for this platform via the resolver
        from roveagent.gateway.display_config import resolve_display_setting
        ...
        try:
            ...
            return (
                ...
                + t("roveagent.gateway.verbose.saved_suffix", platform=platform_key)
            )
        except Exception as e:
            logger.warning("Failed to save tool_progress mode: %s", e)
            return f"{descriptions[new_mode]}\n" + t("roveagent.gateway.verbose.save_failed", error=e)

    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while RoveAgent is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if profile_name:
                from roveagent.gateway.run import _load_gateway_runtime_config
            ...
            if arg == "queue":
                behavior = "Messages will be queued for the next turn while RoveAgent is busy."
            elif arg == "steer":
                behavior = "Messages will be steered into the current run (after the next tool call)."
            else:
                behavior = "Messages will interrupt the current run while RoveAgent is busy."
            return EphemeralReply(
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from roveagent.gateway.run import _roveagent_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
        from roveagent.gateway.runtime_footer import resolve_footer_config
        ...
        try:
            ...
            return t("roveagent.gateway.config_read_failed", error=e)

        effective = resolve_footer_config(user_config, platform_key)

        if arg in {"status", "?"}:
            state = t("roveagent.gateway.footer.state_on") if effective["enabled"] else t("roveagent.gateway.footer.state_off")
            fields = ", ".join(effective.get("fields") or [])
            return t(
                "roveagent.gateway.footer.status",
                state=state,
                fields=fields,
        ...
        if arg in {"on", "enable", "true", "1"}:
            ...
            return t("roveagent.gateway.footer.usage")
        ...
        try:
            ...
            return t("roveagent.gateway.config_save_failed", error=e)

        state = t("roveagent.gateway.footer.state_on") if new_state else t("roveagent.gateway.footer.state_off")
        example = ""
        if new_state:
            # Show a preview using current agent state if available.
            from roveagent.gateway.runtime_footer import format_runtime_footer
            ...
            if preview:
                example = t("roveagent.gateway.footer.example_line", preview=preview)
        return t("roveagent.gateway.footer.saved", state=state, example=example)
    ...
    async def _compress_codex_app_server_session(
        ...
        try:
            ...
            return t("roveagent.gateway.compress.failed", error=exc)
    ...
    async def _handle_compress_command_inner(self, event: MessageEvent) -> str:
        ...
        if not history or len(history) < 4:
            return t("roveagent.gateway.compress.not_enough")

        # Parse args: either a focus topic (full compress) or the
        # boundary-aware "here [N]" form (partial compress).
        from roveagent.clisupport.partial_compress import (
        ...
        from roveagent.core.conversation_compression import (
        ...
        if _aggressive:
            ...
            _agg_note = t("roveagent.gateway.compress.aggressive_unsupported")
            if not _preview:
                return _agg_note

        if _preview:
            # Report what WOULD be compressed — no agent, no writes.
            from roveagent.core.model_metadata import estimate_request_tokens_rough
            _pv_msgs = [
        ...
        try:
            ...
            from roveagent.core.manual_compression_feedback import summarize_manual_compression
            from roveagent.core.model_metadata import estimate_request_tokens_rough
            ...
            if not runtime_kwargs.get("api_key"):
                return t("roveagent.gateway.compress.no_provider")
            ...
            from roveagent.utils import is_truthy_value as _is_truthy
            ...
            try:
                ...
                if not compressor.has_content_to_compress(head):
                    return t("roveagent.gateway.compress.nothing_to_do")
                ...
                if _lock_skipped is True or isinstance(_lock_skipped, str):
                    from roveagent.core.manual_compression_feedback import (
                ...
                if _summary_err:
                    from roveagent.core.redact import redact_sensitive_text
            ...
            if focus_topic:
                lines.append(t("roveagent.gateway.compress.focus_line", topic=focus_topic))
            ...
            if _summary_aborted:
                lines.append(
                    t(
                        "roveagent.gateway.compress.aborted",
                ...
                lines.append(
                    t(
                        "roveagent.gateway.compress.aux_failed",
            ...
            return t("roveagent.gateway.compress.failed", error=e)

    async def _handle_topic_command(self, event: MessageEvent, args: str = "") -> str:
        """Handle /topic for Telegram DM user-managed topic sessions."""
        source = event.source
        if source.platform != Platform.TELEGRAM or source.chat_type != "dm":
            return t("roveagent.gateway.topic.not_telegram_dm")
        if not self._session_db:
            from roveagent.state import format_session_db_unavailable
            return format_session_db_unavailable(prefix=t("roveagent.gateway.shared.session_db_unavailable_prefix"))
        ...
        if callable(auth_fn):
            try:
                if not auth_fn(source):
                    return t("roveagent.gateway.topic.unauthorized")
            except Exception:
        ...
        if args:
            if not source.thread_id:
                return t("roveagent.gateway.topic.restore_needs_topic")
        ...
        if capabilities.get("checked"):
            if capabilities.get("has_topics_enabled") is False:
                ...
                return t("roveagent.gateway.topic.topics_disabled")
            if capabilities.get("allows_users_to_create_topics") is False:
                if self._should_send_telegram_capability_hint(source):
                    await self._send_telegram_topic_setup_image(source)
                return t("roveagent.gateway.topic.topics_user_disallowed")
        ...
        try:
            ...
            return t("roveagent.gateway.topic.enable_failed", error=exc)
        ...
        if source.thread_id:
            ...
            if binding:
                ...
                try:
                    ...
                    title = None
                session_label = title or t("roveagent.gateway.topic.untitled_session")
                return t(
                    "roveagent.gateway.topic.bound_status",
                    label=session_label,
                    session_id=session_id,
                )
            return t("roveagent.gateway.topic.thread_ready")
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from roveagent.clisupport.session_export import (
            SAVE_USAGE,
        ...
        if redact:
            from roveagent.clisupport.session_export_md import redact_session_data

            export_data = redact_session_data(export_data)

        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="roveagent_save_")
    ...
    async def _handle_title_command(self, event: MessageEvent) -> str:
        ...
        if title_arg:
            ...
            try:
                ...
                return t("roveagent.gateway.shared.warn_passthrough", error=e)
            if not sanitized:
                return t("roveagent.gateway.title.empty_after_clean")
            # Set the title
            try:
                if await self._session_db.set_session_title(session_id, sanitized):
                    ...
                    return t("roveagent.gateway.title.set_to", title=sanitized)
                else:
                    return t("roveagent.gateway.title.not_found")
            except ValueError as e:
                return t("roveagent.gateway.shared.warn_passthrough", error=e)
        else:
            ...
            title = await self._session_db.get_session_title(session_id)
            if title:
                return t("roveagent.gateway.title.current_with_title", session_id=session_id, title=title)
            else:
                return t("roveagent.gateway.title.current_no_title", session_id=session_id)
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            return t("roveagent.gateway.resume.parse_error", error=exc)
            ...
            try:
                ...
                if not titled:
                    if source.platform == Platform.MATRIX and not allow_all:
                        return t("roveagent.gateway.resume.matrix_no_named_sessions")
                    return t("roveagent.gateway.resume.no_named_sessions")
                lines = [t("roveagent.gateway.resume.list_header")]
                for idx, s in enumerate(titled[:10], start=1):
                    ...
                    preview_part = t("roveagent.gateway.resume.list_preview_suffix", preview=preview) if preview else ""
                    lines.append(t("roveagent.gateway.resume.list_item_numbered", index=idx, title=title, preview_part=preview_part))
                lines.append(t("roveagent.gateway.resume.list_footer_numbered"))
                return "\n".join(lines)
            except Exception as e:
                logger.debug("Failed to list titled sessions: %s", e)
                return t("roveagent.gateway.resume.list_failed", error=e)
        ...
        if name.isdigit():
            ...
            if index < 1 or index > len(titled):
                return t("roveagent.gateway.resume.out_of_range", index=index)
        ...
        if not target_id:
            return t("roveagent.gateway.resume.not_found", name=name)
        ...
        if source.platform == Platform.MATRIX:
            ...
            if not self._same_matrix_room(source, target_origin) and not allow_cross_room:
                if target_origin is None:
                    return t("roveagent.gateway.resume.matrix_blocked_no_origin", name=name)
                return t(
                    "roveagent.gateway.resume.matrix_blocked_other_room",
            ...
            return t("roveagent.gateway.resume.blocked_not_owner", name=name)

        # Check if already on that session
        current_entry = await self.async_session_store.get_or_create_session(source)
        if current_entry.session_id == target_id:
            return t("roveagent.gateway.resume.already_on", name=name)
        ...
        if not new_entry:
            return t("roveagent.gateway.resume.switch_failed")
        ...
        if source.platform == Platform.MATRIX and allow_cross_room:
            return t(
                "roveagent.gateway.resume.matrix_cross_room_success",
                title=title,
                room=source.chat_name or source.chat_id,
                msg_part=msg_part,
            )
        if not msg_count:
            return t("roveagent.gateway.resume.resumed_no_count", title=title)
        if msg_count == 1:
            return t("roveagent.gateway.resume.resumed_one", title=title, count=msg_count)
        return t("roveagent.gateway.resume.resumed_many", title=title, count=msg_count)
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from roveagent.clisupport.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        if not history:
            return t("roveagent.gateway.branch.no_conversation")
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (roveagent_state.py:1994-2009) that searches by the complete
            ...
            return t("roveagent.gateway.branch.create_failed", error=e)
        ...
        if not new_entry:
            return t("roveagent.gateway.branch.switch_failed")
        ...
        key = "roveagent.gateway.branch.branched_one" if msg_count == 1 else "roveagent.gateway.branch.branched_many"
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        ...
        """
        from roveagent.core.account_usage import build_credits_view
        ...
        if view is None or not view.logged_in:
            return t("roveagent.gateway.credits.not_logged_in")
    ...
    def _context_breakdown_block(self, agent, source, expanded: bool) -> list[str]:
        ...
        """
        try:
            from roveagent.core.context_breakdown import (
    ...
    def _context_breakdown_lines(self, agent, source) -> list[str]:
        ...
        """
        try:
            from roveagent.core.context_breakdown import compute_session_context_breakdown
            ...
            out = [t("roveagent.gateway.usage.breakdown_header")]
            for cat in categories:
                tokens = int(cat.get("tokens") or 0)
                if tokens <= 0:
                    continue
                cat_id = str(cat.get("id") or "")
                label = t(f"roveagent.gateway.usage.breakdown_cat_{cat_id}")
                ...
                out.append(
                    t("roveagent.gateway.usage.breakdown_line", label=label, count=f"{tokens:,}", pct=pct)
                )
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        if args and not wants_reset:
            return t("roveagent.gateway.usage.unknown_subcommand", args=raw_args)
        ...
        if wants_reset:
            ...
            if normalized_provider != "openai-codex":
                return t("roveagent.gateway.usage.reset_wrong_provider")
            force = "--force" in args[1:]
            from roveagent.core.account_usage import redeem_codex_reset_credit
        ...
        try:
            from roveagent.core.account_usage import nous_credits_lines
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            if rl_state and rl_state.has_data:
                from roveagent.core.rate_limit_tracker import format_rate_limit_compact
                lines.append(t("roveagent.gateway.usage.rate_limits", state=format_rate_limit_compact(rl_state)))
                lines.append("")
            ...
            lines.append(t("roveagent.gateway.usage.header_session"))
            lines.append(t("roveagent.gateway.usage.label_model", model=agent.model))
            lines.append(t("roveagent.gateway.usage.label_input_tokens", count=f"{input_tokens:,}"))
            lines.append(t("roveagent.gateway.usage.label_output_tokens", count=f"{output_tokens:,}"))
            lines.append(t("roveagent.gateway.usage.label_total", count=f"{agent.session_total_tokens:,}"))
            lines.append(t("roveagent.gateway.usage.label_api_calls", count=agent.session_api_calls))
            ...
            if _lpt:
                ...
                lines.append(t("roveagent.gateway.usage.label_context", used=f"{_lpt:,}", total=f"{ctx.context_length:,}", pct=f"{pct:.0f}"))
            if ctx.compression_count:
                lines.append(t("roveagent.gateway.usage.label_compressions", count=ctx.compression_count))
        ...
        if history:
            ...
            approx = estimate_messages_tokens_rough(msgs)
            lines = [
                t("roveagent.gateway.usage.header_session_info"),
                t("roveagent.gateway.usage.label_messages", count=len(msgs)),
                t("roveagent.gateway.usage.label_estimated_context", count=f"{approx:,}"),
                t("roveagent.gateway.usage.detailed_after_first"),
            ]
            if account_lines:
                lines.append("")
                lines.extend(account_lines)
        ...
        if account_lines or credits_lines:
            ...
            return "\n".join(parts)
        return t("roveagent.gateway.usage.no_data")
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
            ...
            while i < len(parts):
                if parts[i] == "--days" and i + 1 < len(parts):
                    try:
                        ...
                        return t("roveagent.gateway.insights.invalid_days", value=parts[i + 1])
                    i += 2
        ...
        try:
            ...
            return t("roveagent.gateway.insights.error", error=e)
    ...
    async def _handle_reload_mcp_command(self, event: MessageEvent) -> Optional[str]:
        ...
        async def _on_confirm(choice: str) -> Optional[str]:
            if choice == "cancel":
                return t("roveagent.gateway.reload_mcp.cancelled")
            if choice == "always":
            ...
            if choice == "always":
                return f"{result}\n\n" + t("roveagent.gateway.reload_mcp.always_followup")
            return result

        prompt_message = t("roveagent.gateway.reload_mcp.confirm_prompt")
        return await self._request_slash_confirm(
            event=event,
            command="reload-mcp",
            title="/reload-mcp",
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        loop = asyncio.get_running_loop()
        try:
            from roveagent.core.skill_commands import reload_skills
            ...
            lines = [t("roveagent.gateway.reload_skills.header")]
            if not added and not removed:
                lines.append(t("roveagent.gateway.reload_skills.no_new"))
                lines.append(t("roveagent.gateway.reload_skills.total", count=total))
                return "\n".join(lines)
            ...
            def _fmt_line(item: dict) -> str:
                ...
                if desc:
                    return t("roveagent.gateway.reload_skills.item_with_desc", name=nm, desc=desc)
                return t("roveagent.gateway.reload_skills.item_no_desc", name=nm)

            if added:
                lines.append(t("roveagent.gateway.reload_skills.added_header"))
                for item in added:
                    lines.append(_fmt_line(item))
            if removed:
                lines.append(t("roveagent.gateway.reload_skills.removed_header"))
                for item in removed:
                    lines.append(_fmt_line(item))
            lines.append(t("roveagent.gateway.reload_skills.total", count=total))
            ...
            return t("roveagent.gateway.reload_skills.failed", error=e)
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `roveagent bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                ...
                return t("roveagent.gateway.approval_expired")
            return t("roveagent.gateway.approve.no_pending")
        ...
        confirmation_text = t(f"roveagent.gateway.approve.{choice}_{plural}", count=count)
    ...
    async def _handle_deny_command(self, event: MessageEvent) -> str:
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                self._pending_approvals.pop(session_key)
                return t("roveagent.gateway.deny.stale")
            return t("roveagent.gateway.deny.no_pending")
        ...
        if reason:
            if count > 1:
                confirmation_text = t("roveagent.gateway.deny.denied_reason_plural", count=count, reason=reason)
            else:
                confirmation_text = t("roveagent.gateway.deny.denied_reason_singular", reason=reason)
        elif count > 1:
            confirmation_text = t("roveagent.gateway.deny.denied_plural", count=count)
        else:
            confirmation_text = t("roveagent.gateway.deny.denied_singular")
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``roveagent debug share`` from the CLI.
        """
        import asyncio
        from roveagent.clisupport.debug import (
        ...
        def _collect_and_upload():
            ...
            try:
                ...
                return t("roveagent.gateway.debug.upload_failed", error=exc)

            # Schedule auto-deletion after 6 hours
            _schedule_auto_delete(list(urls.values()))

            lines = [_GATEWAY_PRIVACY_NOTICE, "", t("roveagent.gateway.debug.header"), ""]
            label_width = max(len(k) for k in urls)
            for label, url in urls.items():
                lines.append(f"`{label:<{label_width}}`  {url}")

            lines.append("")
            lines.append(t("roveagent.gateway.debug.auto_delete"))
            lines.append(t("roveagent.gateway.debug.full_logs_hint"))
            lines.append(t("roveagent.gateway.debug.share_hint"))
            return "\n".join(lines)

        return await loop.run_in_executor(None, _collect_and_upload)

    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update RoveAgent Agent to the latest version.

        Spawns ``roveagent update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``roveagent update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from roveagent.gateway.run import _roveagent_home, _resolve_roveagent_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from roveagent.clisupport.config import is_managed, format_managed_message
        ...
        if platform not in _allowed:
            try:
                ...
                if not entry or not entry.allow_update_command:
                    return t("roveagent.gateway.update.platform_not_messaging")
            except Exception:
                return t("roveagent.gateway.update.platform_not_messaging")

        if is_managed():
            return f"✗ {format_managed_message('update RoveAgent Agent')}"

        project_root = Path(__file__).parent.parent.resolve()
        git_dir = project_root / '.git'

        if not git_dir.exists():
            return t("roveagent.gateway.update.not_git_repo")

        roveagent_cmd = _resolve_roveagent_bin()
        if not roveagent_cmd:
            return t("roveagent.gateway.update.roveagent_cmd_not_found")

        pending_path = _roveagent_home / ".update_pending.json"
        output_path = _roveagent_home / ".update_output.txt"
        exit_code_path = _roveagent_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `roveagent update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `roveagent update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through roveagent_cmd (venv\Scripts\roveagent.exe): the shim
                ...
                roveagent_cmd_str = " ".join(shlex.quote(part) for part in roveagent_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {roveagent_cmd_str} update --gateway"
            ...
            exit_code_path.unlink(missing_ok=True)
            return t("roveagent.gateway.update.start_failed", error=e)

        self._schedule_update_notification_watch()
        return t("roveagent.gateway.update.starting")
