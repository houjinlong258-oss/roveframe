"""Centralized logging setup for RoveAgent Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.roveagent/logs/`` (profile-aware via ``get_roveagent_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``roveagent_cli.web_server``,
    ``roveagent_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in RoveAgent (TUI, gateway, ``hy_memory`` server, MCP
...
from roveagent.constants import get_config_path, get_roveagent_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_roveagent_session_injector", False):
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.roveagent_home = str(get_roveagent_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.roveagent_home = ""  # type: ignore[attr-defined]
        return record

    _session_record_factory._roveagent_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``roveagent logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``roveagent logs --component gateway``.
    "gateway": ("gateway", "roveagent_plugins", "roveagent.plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("roveagent_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "roveagent.clisupport.web_server",
        "roveagent.clisupport.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the RoveAgent logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    roveagent_home
        Override for the RoveAgent home directory.  Falls back to
        ``get_roveagent_home()`` (profile-aware).
    log_level
    ...
    home = roveagent_home or get_roveagent_home()
    from roveagent.constants import mkdir_under_roveagent_home
    log_dir = mkdir_under_roveagent_home(home / "logs")
    ...
    from roveagent.core.redact import RedactingFormatter
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_roveagent_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._roveagent_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the roveagent group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from roveagent.clisupport.config import is_managed
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their RoveAgent home.
    ...
    def __init__(
        ...
        *,
        ...
        self._roveagent_routed_log_path = Path(self.baseFilename)
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "roveagent_home", "")
        try:
    ...
    def _handler_for_home(self, home: Path) -> _ManagedRotatingFileHandler:
        with self._profile_handlers_lock:
            ...
            mkdir_under_roveagent_home(path.parent)
...
# the module header): when several RoveAgent processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._roveagent_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="roveagent-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_roveagent_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_roveagent_routed_log_path", None) == resolved:
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse roveagent_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when roveagent_cli.config isn't importable (bare
        # roveagent_logging consumers).
        try:
            from roveagent.clisupport.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
            from roveagent.utils import fast_safe_load
