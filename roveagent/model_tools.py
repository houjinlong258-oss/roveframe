...
from roveagent.tools.registry import (
...
from roveagent.toolsets import resolve_toolset, validate_toolset
...
def _is_dispatcher_owned_worker() -> bool:
    """False when ROVEAGENT_KANBAN_* is present but this execution does not own it
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_roveagent_home() under the active profile.
...
#   - cli.py, roveagent_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("ROVEAGENT_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by ROVEAGENT_KANBAN_TASK and
        ...
        from roveagent.toolsets import get_all_toolsets
        for ts_name in get_all_toolsets():
            tools_to_include.update(resolve_toolset(ts_name))

    # Always apply disabled toolsets as a subtraction step at the end.
    # This ensures that even if a composite toolset (like roveagent-cli)
    # is enabled, any tools belonging to a disabled toolset are strictly
    # stripped out. See issue #17309.
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                from roveagent.toolsets import bundle_non_core_tools, get_toolset
                if toolset_name.startswith("roveagent-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (roveagent-*) include _ROVEAGENT_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("roveagent-")
                            and toolset_name not in _WARNED_DISABLED_BUNDLES):
                        _WARNED_DISABLED_BUNDLES.add(toolset_name)
                        logger.info(
                            "roveagent.core.disabled_toolsets contains platform-bundle "
    ...
    if "execute_code" in available_tool_names:
        from roveagent.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    for discord_tool_name in _discord_schema_fns:
        if discord_tool_name in available_tool_names:
            try:
                from roveagent.tools import discord_tool as _dt
    ...
    try:
        from roveagent.tools.schema_sanitizer import sanitize_tool_schemas
    ...
    # window). Core RoveAgent tools (toolsets._ROVEAGENT_CORE_TOOLS) are NEVER
    ...
    try:
        from roveagent.tools.tool_search import assemble_tool_defs, load_config as _load_ts_config
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from roveagent.clisupport.config import load_config as _load
        cfg = _load() or {}
        ...
        if config_ctx is None and base_url:
            try:
                from roveagent.core.model_metadata import get_cached_context_length
...
from roveagent.tools.registry import _MAX_TOOL_ERROR_CHARS as _TOOL_ERROR_MAX_LEN
...
def coerce_tool_args(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        from roveagent.tools.schema_sanitizer import unrename_tool_args
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to roveagent-agent's coercion layer.
...
def _tool_result_observer_fields(
    ...
    try:
        ...
        pass
    try:
        from roveagent.core.display import _detect_tool_failure
...
def _emit_post_tool_call_hook(
    *,
    ...
    if _post_tool_call_hook_suppressed.get():
        return
    try:
        from roveagent.clisupport.lifecycle import has_hook, invoke_hook
...
def handle_function_call(
    ...
    _ts_mod = None
    try:
        from roveagent.tools import tool_search as _ts_mod  # noqa: F401
    except Exception:
        _ts_mod = None
    ...
    try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from roveagent.tools.file_tools import notify_other_tool_call
