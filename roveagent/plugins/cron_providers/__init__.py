...
1. Bundled providers: ``plugins/cron_providers/<name>/`` (shipped with roveagent-agent)
...
Usage:
    from roveagent.plugins.cron_providers import discover_cron_schedulers, load_cron_scheduler
...
_USER_NAMESPACE = "_roveagent_user_cron"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_roveagent_user_cron.<name>``, a dotted
    ...
    ``ModuleNotFoundError: No module named '_roveagent_user_cron'`` — the same
...
def load_cron_scheduler(name: str) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    (``$ROVEAGENT_HOME/plugins/<name>/``) directories. Bundled takes precedence
    on name collisions.
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["CronScheduler"]:  # noqa: F821
    ...
    module_name = f"roveagent.plugins.cron_providers.{name}" if _is_bundled else f"{_USER_NAMESPACE}.{name}"
    ...
    if cached is not None and getattr(cached, "__file__", None):
        ...
        for parent in ("plugins", "roveagent.plugins.cron_providers"):
    ...
    from roveagent.cron.scheduler_provider import CronScheduler
