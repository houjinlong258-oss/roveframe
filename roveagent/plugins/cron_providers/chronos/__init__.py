...
Chronos (the Greek god of time, alongside RoveAgent) is the first non-default
...
logger = logging.getLogger("roveagent.cron.chronos")
    ...
    def reconcile(self) -> None:
        ...
        for job_id, fire_at in desired.items():
            if observed.get(job_id) != fire_at:
                ...
                from roveagent.cron.jobs import get_job
                job = get_job(job_id)
                if job:
                    try:
                        self._arm_one_shot(job)
