...
mirroring the Nous provider's precedence convention:
  ...
  Environment overrides::
      ...
      ROVEAGENT_DASHBOARD_BASIC_AUTH_PASSWORD_HASH   # preferred
      ROVEAGENT_DASHBOARD_BASIC_AUTH_PASSWORD        # plaintext fallback
      ROVEAGENT_DASHBOARD_BASIC_AUTH_SECRET
      ROVEAGENT_DASHBOARD_BASIC_AUTH_TTL_SECONDS
...
def _resolve_secret(cfg_section: dict) -> bytes:
    ...
    """
    raw = _resolve(
        "ROVEAGENT_DASHBOARD_BASIC_AUTH_SECRET", cfg_section, "secret"
    )
    if not raw:
        logger.info(
            ...
            "secret (or ROVEAGENT_DASHBOARD_BASIC_AUTH_SECRET) for stable "
            "sessions."
        )
...
def register(ctx) -> None:
    ...
    username = _resolve(
        "ROVEAGENT_DASHBOARD_BASIC_AUTH_USERNAME", section, "username"
    )
    password_hash = _resolve(
        "ROVEAGENT_DASHBOARD_BASIC_AUTH_PASSWORD_HASH", section, "password_hash"
    )
    plaintext = _resolve(
        "ROVEAGENT_DASHBOARD_BASIC_AUTH_PASSWORD", section, "password"
    )
    ttl_raw = _resolve(
        "ROVEAGENT_DASHBOARD_BASIC_AUTH_TTL_SECONDS", section, "session_ttl_seconds"
    )

    if not username:
        LAST_SKIP_REASON = (
            "dashboard.basic_auth.username is not set (and "
            "ROVEAGENT_DASHBOARD_BASIC_AUTH_USERNAME is empty). Set a username "
    ...
    if not password_hash and not plaintext:
        LAST_SKIP_REASON = (
            ...
            "roveagent.plugins.dashboard_auth.basic.hash_password)."
        )
        logger.warning("dashboard-auth-basic: %s", LAST_SKIP_REASON)
        return

    # Precedence (env-wins convention): a password supplied via the
    # ROVEAGENT_DASHBOARD_BASIC_AUTH_PASSWORD env var overrides a config.yaml
    ...
    plaintext_from_env = os.environ.get(
        "ROVEAGENT_DASHBOARD_BASIC_AUTH_PASSWORD", ""
    ).strip()
