...
for-secrets-only rule), provisioned by NAS at deploy time (Phase 3):
    ...
    ROVEAGENT_DASHBOARD_DRAIN_SECRET   # the per-agent shared secret (>=43 url-safe-b64 chars)
...
When ``ROVEAGENT_DASHBOARD_DRAIN_SECRET`` is unset, the plugin is a no-op (records
...
def register(ctx) -> None:
    ...
    No-op (records a skip reason) when ``ROVEAGENT_DASHBOARD_DRAIN_SECRET`` is
    ...
    secret = os.environ.get("ROVEAGENT_DASHBOARD_DRAIN_SECRET", "").strip()
    if not secret:
        LAST_SKIP_REASON = (
            "ROVEAGENT_DASHBOARD_DRAIN_SECRET is not set. Set a per-agent "
    ...
    if reason is not None:
        LAST_SKIP_REASON = (
            f"ROVEAGENT_DASHBOARD_DRAIN_SECRET rejected — {reason}. "
    ...
    try:
        from roveagent.clisupport.dashboard_auth.token_auth import register_token_route
