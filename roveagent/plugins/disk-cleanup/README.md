...
Auto-tracks and cleans up ephemeral files created during RoveAgent Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ROVEAGENT_HOME` and `/tmp/roveagent-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ROVEAGENT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ROVEAGENT_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `ROVEAGENT_HOME` or `/tmp/roveagent-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ROVEAGENT_HOME/disk-cleanup/` is itself excluded
- `$ROVEAGENT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
