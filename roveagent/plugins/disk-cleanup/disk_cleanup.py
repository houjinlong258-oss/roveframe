"""disk_cleanup — ephemeral file cleanup for RoveAgent Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ROVEAGENT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ROVEAGENT_HOME and /tmp/roveagent-*
Never touches: ~/.roveagent/logs/ or any system directory.
"""
...
try:
    ...
    def get_roveagent_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ROVEAGENT_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".roveagent").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ROVEAGENT_HOME/logs/``."""
    return get_roveagent_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ROVEAGENT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ROVEAGENT_HOME or ``/tmp/roveagent-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    roveagent_home = get_roveagent_home()
    try:
        path.resolve().relative_to(roveagent_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/roveagent-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("roveagent-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "roveagent-agent", "backups", "profiles", ".worktrees",
...
# Paths under $ROVEAGENT_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ROVEAGENT_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        roveagent_home = get_roveagent_home()
        for parent in ("cron", "cronjobs"):
            base = roveagent_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ROVEAGENT_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ROVEAGENT_HOME, but never recurse into known
    # durable state trees.  Some installs place the RoveAgent checkout, venv,
    # and desktop build under ROVEAGENT_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    roveagent_home = get_roveagent_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in roveagent_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    roveagent_home = get_roveagent_home()
    try:
        rel = path.resolve().relative_to(roveagent_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "roveagent-agent",
        ...
        # Path isn't under ROVEAGENT_HOME (e.g. /tmp/roveagent-*) — fall through.
        pass
