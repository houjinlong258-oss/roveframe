...
Let the roveagent agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where roveagent runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `roveagent meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `roveagent meet` CLI |
...
| `cli.py` | `roveagent meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ROVEAGENT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `roveagent meet node {run,list,approve,remove,status,ping}` |
...
roveagent meet install                                      # pip + Chromium
roveagent meet setup                                        # preflight
roveagent meet auth                                         # optional
roveagent meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
roveagent meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.roveagent/.env
roveagent meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
roveagent meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
roveagent meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.roveagent/.env
roveagent meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, roveagent will **not** switch your system audio input automatically — the
...
roveagent plugins enable google_meet
roveagent meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
roveagent meet node ping my-mac
