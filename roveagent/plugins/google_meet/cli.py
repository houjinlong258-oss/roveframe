...
Wires ``roveagent meet <subcommand>``:
...
from roveagent.plugins.google_meet import process_manager as pm
from roveagent.plugins.google_meet.meet_bot import _is_safe_meet_url


def _auth_state_path() -> Path:
    return Path(get_roveagent_home()) / "workspace" / "meetings" / "auth.json"
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``roveagent meet`` argparse tree.
    ...
    join_p.add_argument("--guest-name", default="RoveAgent Agent")
    ...
    try:
        from roveagent.plugins.google_meet.node.cli import register_cli as _register_node_cli
        ...
        def _node_unavailable(args):
            print(f"roveagent meet node: module unavailable ({e})")
            return 1
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: roveagent meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
    if sub == "setup":
        return _cmd_setup()
    if sub == "install":
    ...
    if sub == "node":
        ...
        if fn is None or fn is meet_command:
            print("usage: roveagent meet node {run,list,approve,remove,status,ping}")
            return 2
        return fn(args)
...
def _cmd_setup() -> int:
    ...
    print(
        "  google auth    : "
        + (f"ok ({auth_path})" if auth_ok else "not saved — run: roveagent meet auth")
    )
    ...
    if all_ok:
        print(
            ...
            "roveagent meet join https://meet.google.com/abc-defg-hij"
        )
    else:
    ...
    if realtime:
        ...
        if system == "Linux":
            ...
            print(
                ...
                "  roveagent will not switch your default input for you."
            )
    else:
        print("\n[3/3] skipped (pass --realtime to install audio tooling too)")

    print("\ndone. verify with: roveagent meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: roveagent meet join <url>")
    return 0
...
def _cmd_join(
    ...
    *,
    ...
    if node:
        ...
        try:
            from roveagent.plugins.google_meet.node.registry import NodeRegistry
            from roveagent.plugins.google_meet.node.client import NodeClient
...
def _cmd_stop() -> int:
    res = pm.stop(reason="roveagent meet stop")
    print(json.dumps(res, indent=2))
    return 0 if res.get("ok") else 1


if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="roveagent meet")
    register_cli(parser)
