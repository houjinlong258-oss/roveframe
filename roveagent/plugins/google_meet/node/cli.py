"""`roveagent meet node ...` subcommand tree.

Wired into the existing ``roveagent meet`` parser by the plugin's top-level
...
from roveagent.plugins.google_meet.node.server import NodeServer


def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Add ``run / list / approve / remove / status / ping`` subparsers.

    *subparser* is the ``roveagent meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="roveagent-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``roveagent meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             roveagent meet node approve <name> ws://<host>:{args.port} {token}")
        try:
