...
Launched by ``roveagent meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$ROVEAGENT_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``roveagent meet node approve <name> <url> <token>``.
...
from roveagent.plugins.google_meet.node import protocol as _proto


def _default_token_path() -> Path:
    return Path(get_roveagent_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "roveagent-meet-node",
    ) -> None:
        self.host = host
        self.port = port
