...
session-scoped state file under ``$ROVEAGENT_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ROVEAGENT_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_roveagent_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    guest_name: str = "RoveAgent Agent",
    ...
    If a bot is already running for this roveagent install, leave it first —
    we enforce single-active-meeting semantics.

    Returns a dict summarizing the started bot.
    """
    from roveagent.plugins.google_meet.meet_bot import _is_safe_meet_url, _meeting_id_from_url
    ...
    env = os.environ.copy()
    env["ROVEAGENT_MEET_URL"] = url
    env["ROVEAGENT_MEET_OUT_DIR"] = str(out)
    env["ROVEAGENT_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ROVEAGENT_MEET_HEADED"] = "1"
    if auth_state:
        env["ROVEAGENT_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ROVEAGENT_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ROVEAGENT_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ROVEAGENT_MEET_MODE"] = mode
    if realtime_model:
        env["ROVEAGENT_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ROVEAGENT_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ROVEAGENT_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    ...
    # meet_bot checks ROVEAGENT_MEET_REALTIME_KEY before OPENAI_API_KEY.
    if not realtime_api_key:
        try:
            from roveagent.core.secret_scope import get_secret

            realtime_api_key = (
                get_secret("ROVEAGENT_MEET_REALTIME_KEY")
                or get_secret("OPENAI_API_KEY")
            )
        except ImportError:  # pragma: no cover — secret_scope is in-repo
            pass
    if realtime_api_key:
        env["ROVEAGENT_MEET_REALTIME_KEY"] = realtime_api_key
    ...
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "roveagent.plugins.google_meet.meet_bot"],
