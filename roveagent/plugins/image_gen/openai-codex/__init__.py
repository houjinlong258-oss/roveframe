...
Output is saved as PNG under ``$ROVEAGENT_HOME/cache/images/``. Source images for
...
def _read_codex_access_token() -> Optional[str]:
    ...
    """
    try:
        from roveagent.core.auxiliary_client import _read_codex_access_token as _reader
...
def _sniff_image_mime(raw: bytes) -> Optional[str]:
    ...
    """
    from roveagent.core.image_routing import _sniff_mime_from_bytes
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `roveagent auth codex` (or `roveagent setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`roveagent auth codex` (or `roveagent setup` → Codex) to sign in."
                ),
