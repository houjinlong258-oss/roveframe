...
three virtual model IDs so the ``roveagent tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `roveagent tools` → Image "
                    "Generation → OpenAI to configure, or `roveagent setup` "
                    "to add the key."
                ),
