...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with roveagent-agent)
2. User-installed providers: ``$ROVEAGENT_HOME/plugins/<name>/``
3. Project-local providers: ``./.roveagent/plugins/<name>/``, opt-in via
   ``ROVEAGENT_ENABLE_PROJECT_PLUGINS``
4. Pip-installed providers: ``roveagent_agent.memory_providers`` entry points
...
ENTRY_POINTS_GROUP = "roveagent_agent.memory_providers"
...
_USER_NAMESPACE = "_roveagent_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_roveagent_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_roveagent_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ROVEAGENT_HOME/plugins/`` or None if unavailable."""
    try:
        from roveagent.constants import get_roveagent_home
        d = get_roveagent_home() / "plugins"
...
def _get_project_plugins_dir() -> Optional[Path]:
    """Return ``./.roveagent/plugins/`` or None if unavailable or not opted in.

    Gated on ``ROVEAGENT_ENABLE_PROJECT_PLUGINS`` exactly as the general
    ...
    """
    try:
        from roveagent.clisupport.plugins import _env_enabled

        if not _env_enabled("ROVEAGENT_ENABLE_PROJECT_PLUGINS"):
            return None
        d = Path.cwd() / ".roveagent" / "plugins"
        return d if d.is_dir() else None
    except Exception:
        return None
...
def _iter_provider_dirs() -> List[Tuple[str, Path]]:
    ...
    # 2. User-installed providers ($ROVEAGENT_HOME/plugins/<name>/)
    # 3. Project-local providers (./.roveagent/plugins/<name>/), opt-in
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    ``roveagent <provider>`` subcommands — working, but a second-class citizen next
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from roveagent.clisupport.plugins import resolve_module_origin
...
def load_memory_provider(
    ...
    *,
    ...
    (``$ROVEAGENT_HOME/plugins/<name>/``), and pip entry-point providers.
...
def _load_provider_from_dir(
    ...
    *,
    ...
    module_name = f"roveagent.plugins.memory.{name}" if _is_bundled else f"{_USER_NAMESPACE}.{name}"
    ...
    if cached is not None and getattr(cached, "__file__", None):
        ...
        for parent in ("plugins", "roveagent.plugins.memory"):
...
class _ProviderCollector:
    ...
    def register_skill(self, *args, **kwargs):
        ...
        provider — ``roveagent memory status``, the setup picker — leaves no
    ...
    def _plugin_context(self):
        ...
        plugin manager, which discovery touches on every roveagent startup.
        """
        if self._context is None:
            from roveagent.clisupport.plugins import PluginContext, PluginManifest, get_plugin_manager
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"roveagent.plugins.memory.{active_provider}.cli" if _is_bundled else f"{_USER_NAMESPACE}.{active_provider}.cli"
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _roveagent_user_memory.<name>.cli, usually
