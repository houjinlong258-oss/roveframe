...
```bash
roveagent memory setup    # select "byterover"
```

Or manually:
```bash
roveagent config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.roveagent/.env
```
...
Working directory: `$ROVEAGENT_HOME/byterover/` (profile-scoped).
