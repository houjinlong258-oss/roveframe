...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: roveagent)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: roveagent)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $ROVEAGENT_HOME/hindsight/config.json (profile-scoped), falling back to
...
from roveagent.core.memory_provider import MemoryProvider, RecallStatus
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "roveagent").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so RoveAgent can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``roveagent memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'roveagent memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $ROVEAGENT_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_roveagent_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "roveagent": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "roveagent"),
...
def _event_timestamp() -> str:
    """Return the configured RoveAgent event time with an explicit UTC offset."""
    event_time = _roveagent_now()
    # roveagent_time.now() guarantees an aware datetime. Keep this fallback so a
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this RoveAgent config."""
    profile = config.get("profile", "roveagent")
    return str(profile or "roveagent")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the RoveAgent .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active RoveAgent profile name (from agent_identity)
      {workspace} — RoveAgent workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``roveagent-{user}`` with no user becomes ``roveagent``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between RoveAgent Agent and the User"
    ...
    def save_config(self, values, roveagent_home):
        """Write config to $ROVEAGENT_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(roveagent_home) / "hindsight"
    ...
    def post_setup(self, roveagent_home: str, config: dict) -> None:
        ...
        from roveagent.clisupport.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(roveagent_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "roveagent")
        ...
        self.save_config(provider_config, roveagent_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(roveagent_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(roveagent_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a RoveAgent starter template (best-effort)."""
        from roveagent.clisupport.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "roveagent")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "roveagent"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: roveagent-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between RoveAgent Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "roveagent"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "roveagent"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/roveagent (NS-605).
                from roveagent.tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "roveagent", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "roveagent")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between RoveAgent Agent and the User")
        ...
        # RoveAgent emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run RoveAgent as a non-root user, or switch "
                    "to cloud / local_external mode via 'roveagent memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # RoveAgent get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See NousResearch/roveagent-agent#5820.
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See roveagent-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When RoveAgent created/used that client on the shared async loop,
