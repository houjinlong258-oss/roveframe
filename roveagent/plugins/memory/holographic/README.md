...
```bash
roveagent memory setup    # select "holographic"
```

Or manually:
```bash
roveagent config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.roveagent-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ROVEAGENT_HOME/memory_store.db` | SQLite database path |
