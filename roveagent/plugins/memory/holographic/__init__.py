"""roveagent-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $ROVEAGENT_HOME/config.yaml (profile-scoped):
  plugins:
    roveagent-memory-store:
      db_path: $ROVEAGENT_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "roveagent-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, roveagent_home):
        """Write config to config.yaml under plugins.roveagent-memory-store."""
        from pathlib import Path
        config_path = Path(roveagent_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["roveagent-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_roveagent_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _roveagent_home = str(get_roveagent_home())
        _default_db = _roveagent_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $ROVEAGENT_HOME in user-supplied paths so config values like
        # "$ROVEAGENT_HOME/memory_store.db" or "~/.roveagent/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$ROVEAGENT_HOME", _roveagent_home)
            db_path = db_path.replace("${ROVEAGENT_HOME}", _roveagent_home)
