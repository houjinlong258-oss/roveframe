...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/roveagent>
...
```bash
roveagent memory setup honcho   # configure Honcho directly (works on a fresh install)
roveagent memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
roveagent config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.roveagent/.env
```

> `roveagent honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `roveagent memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `roveagent model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ROVEAGENT_HOME/honcho.json` | Profile-local (isolated RoveAgent instances) |
| 2 | `~/.roveagent/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active RoveAgent profile: `roveagent` (default) or `roveagent_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `roveagent honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `recallMode` | string | `"hybrid"` | `"hybrid"` (auto-inject + tools), `"context"` (auto-inject only, tools hidden), `"roveagent.tools"` (tools only, no injection). Legacy `"auto"` → `"hybrid"` |
...
| 4 | `per-session` strategy | RoveAgent session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`roveagent-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`roveagent`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-roveagent-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening roveagent in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — RoveAgent session ID (timestamp + hex). Every `roveagent` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple RoveAgent profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`roveagent`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active RoveAgent profile: `roveagent` (default) or `roveagent_<profile>` (e.g. `roveagent -p coder` -> host key `roveagent_coder`). Older `roveagent.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ROVEAGENT_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `roveagent-agent`) |
...
|---------|-------------|
| `roveagent memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `roveagent honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `roveagent memory setup`) |
| `roveagent honcho status` | Show resolved config for active profile |
| `roveagent honcho enable` / `disable` | Toggle Honcho for active profile |
| `roveagent honcho mode <mode>` | Change recall or observation mode |
| `roveagent honcho peer --user <name>` | Update user peer name |
| `roveagent honcho peer --ai <name>` | Update AI peer name |
| `roveagent honcho tokens --context <N>` | Set context token budget |
| `roveagent honcho tokens --dialectic <N>` | Set dialectic max chars |
| `roveagent honcho map <name>` | Map current directory to a session name |
| `roveagent honcho sync` | Create host blocks for all RoveAgent profiles |
