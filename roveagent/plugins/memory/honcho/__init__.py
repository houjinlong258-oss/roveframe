...
Config: Uses the existing Honcho config chain:
  1. $ROVEAGENT_HOME/honcho.json (profile-scoped)
...
from roveagent.core.memory_provider import TRIVIAL_PROMPT_RE, MemoryProvider, is_trivial_prompt
from roveagent.plugins.memory.honcho.client import spawn_context_thread
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, roveagent_home):
        """Write config to $ROVEAGENT_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(roveagent_home) / "honcho.json"
        existing = {}
    ...
    def post_setup(self, roveagent_home: str, config: dict) -> None:
        ...
        import types
        from roveagent.plugins.memory.honcho.cli import cmd_setup
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps RoveAgent CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "roveagent-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        from roveagent.plugins.memory.honcho.client import get_honcho_client
        from roveagent.plugins.memory.honcho.session import HonchoSessionManager
        ...
        # Skip under per-session strategy: every RoveAgent run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_roveagent_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "roveagent-default",
    ...
    def _pop_auth_notice(self) -> str:
        ...
        return (
            ...
            "'roveagent honcho setup' to re-authenticate will restore it."
        )
...
def register(ctx) -> None:
    ...
    from roveagent.plugins.memory.query_rewrite import rewrite_memory_query
