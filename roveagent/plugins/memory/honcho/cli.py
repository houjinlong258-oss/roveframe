...
Handles: roveagent honcho setup | status | sessions | map | peer
"""
...
from roveagent.plugins.memory.honcho.client import _host_block, profile_host_key, resolve_active_host, resolve_config_path, HOST
from roveagent.clisupport.config import cfg_get
...
def _ensure_peer_exists(host_key: str | None = None) -> bool:
    ...
    """
    try:
        from roveagent.plugins.memory.honcho.client import HonchoClientConfig, get_honcho_client
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "roveagent" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all RoveAgent profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'roveagent honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'roveagent honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `roveagent update` -- no output, no exceptions.
    """
    try:
        from roveagent.clisupport.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current RoveAgent profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $ROVEAGENT_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_roveagent_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        roveagent_host.get("pinUserPeer"),
        roveagent_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in roveagent_host:
        aliases_src = roveagent_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in roveagent_host:
        prefix_src = roveagent_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(roveagent_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom roveagent_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    roveagent_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        roveagent_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(roveagent_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = roveagent_host.get("userPeerAliases")
    prefix = roveagent_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(roveagent_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(roveagent_host)
    roveagent_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        roveagent_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        roveagent_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/roveagent (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives RoveAgent persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = roveagent_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            roveagent_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        from roveagent.plugins.memory.honcho.oauth import OAuthCredential
        existing_oauth = OAuthCredential.from_host_block(roveagent_host)
        ...
        if use_device:
            from roveagent.plugins.memory.honcho.oauth_flow import (
                AccessDenied,
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'roveagent honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'roveagent honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'roveagent honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(roveagent_host, cred)
            ...
            from roveagent.plugins.memory.honcho.oauth_flow import authorize_via_loopback
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'roveagent honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = roveagent_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        roveagent_host["peerName"] = new_peer

    current_ai = roveagent_host.get("aiPeer") or cfg.get("aiPeer", "roveagent")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        roveagent_host["aiPeer"] = new_ai

    current_workspace = roveagent_host.get("workspace") or cfg.get("workspace", "roveagent")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        roveagent_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the RoveAgent GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, roveagent_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the RoveAgent gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = roveagent_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(roveagent_host)
            roveagent_host["pinUserPeer"] = False
            if prior_aliases:
                roveagent_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                roveagent_host, current_prefix, prefix_from_root,
            ...
            if merged:
                roveagent_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                roveagent_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                roveagent_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = roveagent_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        roveagent_host["observationMode"] = new_obs
    else:
        roveagent_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(roveagent_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        roveagent_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        roveagent_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = roveagent_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        roveagent_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = roveagent_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        roveagent_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                roveagent_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(roveagent_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            roveagent_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        roveagent_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        roveagent_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        roveagent_host["dialecticReasoningLevel"] = new_reasoning
    else:
        roveagent_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = roveagent_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        roveagent_host["sessionStrategy"] = new_strat

    roveagent_host["enabled"] = True
    roveagent_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        roveagent_config = load_config()
        roveagent_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(roveagent_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: roveagent config set memory.provider honcho")

    # --- Test connection ---
    print("  Testing connection... ", end="", flush=True)
    try:
        from roveagent.plugins.memory.honcho.client import HonchoClientConfig, get_honcho_client, reset_honcho_client
        reset_honcho_client()
    ...
    print("\n  Other commands:")
    print("    roveagent honcho status     -- show full config")
    print("    roveagent honcho mode       -- change recall/observation mode")
    print("    roveagent honcho tokens     -- tune context and dialectic budgets")
    print("    roveagent honcho peer       -- update peer names")
    print("    roveagent honcho map <name> -- map this directory to a session name\n")


def _device_login_available() -> bool:
    """Whether the resolved host offers the RFC 8628 device grant. Fails closed."""
    try:
        from roveagent.plugins.memory.honcho.oauth_flow import resolve_endpoints, supports_device_login
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if roveagent_cli internals move."""
    try:
        from roveagent.clisupport.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(roveagent_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    roveagent_host["apiKey"] = cred.access_token
    roveagent_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        roveagent_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active RoveAgent profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("roveagent.work") stay readable per the README's back-compat
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: roveagent honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'roveagent honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'roveagent honcho setup' to configure.\n")
...
def _show_peer_cards(hcfg, client) -> None:
    ...
    """
    try:
        from roveagent.plugins.memory.honcho.session import HonchoSessionManager
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: roveagent honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        roveagent = hosts.get(_host_key(), {})
        user = roveagent.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = roveagent.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = roveagent.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = roveagent.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    RoveAgent' identity in Honcho. Seed with 'roveagent honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: roveagent honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: roveagent honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = roveagent.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = roveagent.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = roveagent.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. RoveAgent asks Honcho's AI peer a question")
        ...
        print("\n  Set with: roveagent honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'roveagent honcho setup' first.\n")
        return
    ...
    if show:
        from roveagent.plugins.memory.honcho.session import HonchoAuthError
        try:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'roveagent honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    roveagent honcho identity --show        — show both peer representations")
        print("    roveagent honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → RoveAgent + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → RoveAgent\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives RoveAgent persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  roveagent honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'roveagent honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'roveagent honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  roveagent honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'roveagent'")
        print("  with Honcho configured and no prior session history.")
        print("  (RoveAgent calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    roveagent honcho migrate  — this step handles it interactively")
        if has_key:
            answer = _prompt("  Upload user memory files to Honcho now?", default="y")
            if answer.lower() in {"y", "yes"}:
                try:
                    from roveagent.plugins.memory.honcho.client import (
                        HonchoClientConfig,
                        get_honcho_client,
                        reset_honcho_client,
                    )
            ...
            print("  Run 'roveagent honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In RoveAgent, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'roveagent honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    roveagent honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  roveagent honcho identity <path/to/SOUL.md>")
    ...
    print("    RoveAgent:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    RoveAgent:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    RoveAgent:   Honcho observes every message and updates representations")
    ...
    print("    RoveAgent:   per-session by default — each run gets its own session")
    print("              Map a custom name:  roveagent honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. roveagent honcho setup              — configure API key (required)")
        print("  2. roveagent honcho migrate            — re-run this walkthrough")
    else:
        print("  1. roveagent honcho status             — verify Honcho connection")
        print("  2. roveagent                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. roveagent honcho identity --show    — verify AI peer representation")
        print("  4. roveagent honcho tokens             — tune context and dialectic budgets")
        print("  5. roveagent honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'roveagent memory setup'...\n")
        from roveagent.clisupport.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``roveagent honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``roveagent honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to roveagent memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to RoveAgent Honcho",
    )
