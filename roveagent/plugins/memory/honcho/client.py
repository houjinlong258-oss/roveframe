...
Resolution order for config file:
  1. $ROVEAGENT_HOME/honcho.json  (instance-local, enables isolated RoveAgent instances)
...
from roveagent.clisupport.profiles import _get_default_roveagent_home
from roveagent.plugins.plugin_utils import SingletonSlot
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a RoveAgent profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active RoveAgent profile.

    Resolution order:
      1. ROVEAGENT_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``roveagent_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"roveagent"`` (default profile)
    """
    explicit = os.environ.get("ROVEAGENT_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named RoveAgent
    # profile share that same host.  Keep named profiles isolated; only the
    # default RoveAgent profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ROVEAGENT_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.roveagent/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_roveagent_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_roveagent_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.roveagent
    ...
    # resolve_config_path()/get_roveagent_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        Resolution: $ROVEAGENT_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active RoveAgent profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.roveagent block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the RoveAgent-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — RoveAgent session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. RoveAgent session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_roveagent_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_roveagent_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'roveagent honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.roveagent_home) if config.roveagent_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/roveagent_home) identity reappears with a
...
def _apply_fresh_oauth_token(config: HonchoClientConfig) -> None:
    ...
    """
    try:
        from roveagent.plugins.memory.honcho import oauth
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'roveagent honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            from roveagent.tools.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `roveagent honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                roveagent_cfg = load_config()
                honcho_cfg = roveagent_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
