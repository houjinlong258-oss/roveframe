...
``roveagent://`` handler. Endpoints are env-overridable with local-dev defaults
...
from roveagent.plugins.memory.honcho.client import resolve_active_host, resolve_config_path

logger = logging.getLogger(__name__)

# The loopback redirect registered for the RoveAgent OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "roveagent-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``roveagent-desktop`` / ``roveagent-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to RoveAgent.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local ROVEAGENT_HOME override can't reach.
