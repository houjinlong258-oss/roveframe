...
from roveagent.plugins.memory.honcho.client import get_honcho_client, spawn_context_thread
from roveagent.plugins.memory.honcho.oauth import redact_tokens as _redact_tokens
...
def _auth_error_message(exc: BaseException) -> str:
    return (
        ...
        "Re-authenticate with 'roveagent honcho setup'."
    )
...
class HonchoSessionManager:
    ...
    Runs alongside roveagent' existing SQLite state and file-based memory,
    ...
    def _force_reauth(self) -> bool:
        ...
        try:
            from roveagent.plugins.memory.honcho import oauth
            from roveagent.plugins.memory.honcho.client import reset_honcho_client
    ...
    def get_or_create(self, key: str) -> HonchoSession:
        ...
        assistant_peer_id = self._sanitize_id(
            self._config.ai_peer if self._config else "roveagent-assistant"
        )
    ...
    def dialectic_query(
        ...
        try:
            ...
            # Only automatic injection uses the RoveAgent-side character cap.
            if (
                apply_injection_cap
                and result
    ...
    def get_prefetch_context(self, session_key: str, user_message: str | None = None) -> dict[str, str]:
        ...
        # Also fetch AI peer's own representation so RoveAgent knows itself.
        try:
    ...
    def migrate_memory_files(self, session_key: str, memory_dir: str) -> bool:
        ...
        Args:
            ...
            memory_dir: Path to the memories directory (~/.roveagent/memories/).
