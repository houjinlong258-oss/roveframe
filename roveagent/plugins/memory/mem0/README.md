...
```bash
roveagent memory setup    # select "mem0"
```

Or manually:
```bash
roveagent config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.roveagent/.env
```

## Config

Behavioral settings live in `$ROVEAGENT_HOME/mem0.json` (set them via `roveagent memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.roveagent/.env`.
...
| `user_id` | `roveagent-user` | User identifier on Mem0 |
| `agent_id` | `roveagent` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   roveagent memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   roveagent memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.roveagent/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.roveagent/.env
   ```
   or in `$ROVEAGENT_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh RoveAgent session and call `mem0_search` — it connects to your server.
...
```bash
roveagent memory setup mem0 --mode oss \
...
```bash
roveagent memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ROVEAGENT_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.roveagent/mem0_qdrant"}}
  }
}
```
...
```bash
roveagent memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
roveagent memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$ROVEAGENT_HOME/mem0.json`).
