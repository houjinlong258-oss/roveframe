...
_DIRECT_OPENAI_PROVIDER = "roveagent_openai"
_DIRECT_OPENAI_CLASS_PATH = "roveagent.plugins.memory.mem0._openai_llm.DirectOpenAILLM"


def _register_direct_openai_provider() -> None:
    """Register RoveAgent' OpenAI-only Mem0 LLM provider once per factory."""
    ...
    if not isinstance(provider_map, dict) or not callable(register_provider):
        raise RuntimeError(
            ...
            "for the RoveAgent OpenAI OSS backend"
        )
