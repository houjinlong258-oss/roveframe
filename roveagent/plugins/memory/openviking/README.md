...
- OpenViking server running and reachable from RoveAgent

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
RoveAgent can identify older servers that expose the legacy status-only health
...
Then configure RoveAgent:

```bash
roveagent memory setup    # select "openviking"
```

The setup can link to an existing `~/.openviking/ovcli.conf`, copy its current
connection values into RoveAgent, or create a minimal `ovcli.conf` when one does
not exist.

Or manually:

```bash
roveagent config set memory.provider openviking
```

Add the connection settings to the active profile's `.env` file. For the
default profile that is `~/.roveagent/.env`; for a named profile use
`~/.roveagent/profiles/<profile>/.env`.
...
OpenViking's server config is separate from RoveAgent:
...
RoveAgent-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, RoveAgent lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
RoveAgent sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
RoveAgent also sends `User-Agent: openviking-memory-roveagent/<version>` on
OpenViking requests. This standard harness identifier contains the RoveAgent
...
does not ask for a peer ID. Without a configured peer, RoveAgent sends neither
...
order remains environment, linked OpenViking config, then RoveAgent YAML. To use
...
on the old implicit `roveagent` peer now use user memory for new writes. Without
...
Set `agent: roveagent` to restore peer-scoped writes. Memories written at user
...
current user). RoveAgent caches a confirmed user only for the active connection.
If the probe fails, RoveAgent uses the configured user, or `default`, for that
...
RoveAgent built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| RoveAgent action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because RoveAgent
...
file URIs, such as
`viking://user/default/peers/roveagent/memories/preferences/mem_abc123.md` (any
