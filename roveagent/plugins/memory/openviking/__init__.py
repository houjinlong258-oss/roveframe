...
from roveagent.core.skill_commands import extract_user_instruction_from_skill_message
from roveagent.clisupport import __version__ as _ROVEAGENT_VERSION
from roveagent.tools.registry import tool_error
from roveagent.utils import atomic_json_write, env_var_enabled
...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-roveagent/{_ROVEAGENT_VERSION}"
...
_SYNC_TRACE_ENV = "ROVEAGENT_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip RoveAgent slash-skill scaffolding before sending content to OpenViking.
...
def _zip_directory(dir_path: Path) -> Path:
    ...
    from roveagent.core.file_safety import raise_if_read_blocked
...
def _openviking_endpoint_is_always_blocked(candidate: str) -> bool:
    ...
    """
    from roveagent.tools.url_safety import is_always_blocked_url
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach RoveAgent to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; RoveAgent refused the connection."
        ) from exc
...
def _load_roveagent_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ROVEAGENT_HOME", "")).expanduser() if os.environ.get("ROVEAGENT_HOME") else Path.home() / ".roveagent"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. RoveAgent did not start "
    ...
    try:
        ...
        # The RoveAgent Desktop backend can include the RoveAgent venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the RoveAgent venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the RoveAgent venv,
        # aborting `roveagent update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; RoveAgent will retry on a later access or when "
        "the config changes."
    )
...
def _save_roveagent_only_config(
    *,
    config: dict,
    provider_config: dict,
    env_path: Path,
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new RoveAgent session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in RoveAgent only", "write values only to RoveAgent .env"),
    ...
    _save_roveagent_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to RoveAgent .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See roveagent-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_roveagent_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], roveagent_home: str) -> None:
    ...
    def post_setup(self, roveagent_home: str, config: dict) -> None:
        ...
        from roveagent.clisupport.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        roveagent_home_path = Path(roveagent_home)
        env_path = roveagent_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_roveagent_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        roveagent_home = str(kwargs.get("roveagent_home") or "").strip()
        if not roveagent_home:
            try:
                from roveagent.constants import get_roveagent_home
                roveagent_home = str(get_roveagent_home())
            except Exception:
                roveagent_home = str(Path.home() / ".roveagent")
        self._roveagent_home = roveagent_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "RoveAgent will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.roveagent/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts roveagent (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; RoveAgent will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._roveagent_home:
            return None
        return Path(self._roveagent_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._roveagent_home:
            return None
        return Path(self._roveagent_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_roveagent_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of RoveAgent' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert RoveAgent canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See roveagent-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/roveagent/.overview.md).
