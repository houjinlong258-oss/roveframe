...
Config (env vars or roveagent config.yaml under retaindb:):
...
class _Client:
    ...
    def _headers(self, path: str) -> dict:
        ...
        h = {
            ...
            "x-sdk-runtime": "roveagent-plugin",
        }
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "roveagent-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "roveagent-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > config.yaml project > roveagent-<profile> > "default"
        ...
        if explicit:
            ...
            roveagent_home = str(kwargs.get("roveagent_home", ""))
            profile_name = os.path.basename(roveagent_home) if roveagent_home else ""
            project = f"roveagent-{profile_name}" if (profile_name and profile_name not in {"", ".roveagent"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "roveagent") or "roveagent"

        from roveagent.constants import get_roveagent_home
        roveagent_home_path = get_roveagent_home()
        db_path = roveagent_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = roveagent_home_path / "SOUL.md"
        if soul_path.exists():
