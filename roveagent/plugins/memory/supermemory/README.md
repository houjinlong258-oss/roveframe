...
- Hosted: API key from [app.supermemory.ai/integrations?connect=roveagent](http://app.supermemory.ai/integrations?connect=roveagent)
...
```bash
roveagent memory setup    # select "supermemory"
```

Or manually:

```bash
roveagent config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.roveagent/.env
```
...
Before running `roveagent memory setup`, add the local endpoint to
`$ROVEAGENT_HOME/supermemory.json`:
...
Then run `roveagent memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ROVEAGENT_HOME/supermemory.json`
...
| `container_tag` | `roveagent` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `roveagent-{identity}` → `roveagent-coder`). |
...
`https://api.supermemory.ai`. RoveAgent resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: roveagent`, and document writes stamp
`metadata.sm_source: roveagent`. This is a **functional routing key, not telemetry**:
it groups RoveAgent-written memories into a dedicated "RoveAgent" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per RoveAgent profile:

```json
{
  "container_tag": "roveagent-{identity}"
}
```

For a profile named `coder`, this resolves to `roveagent-coder`. The default profile resolves to `roveagent-default`. Without `{identity}`, all profiles share the same container.
