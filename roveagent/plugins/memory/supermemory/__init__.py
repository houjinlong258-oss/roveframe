...
from roveagent.core.secret_scope import get_secret, is_multiplex_active
from roveagent.tools.registry import tool_error

logger = logging.getLogger(__name__)

_DEFAULT_CONTAINER_TAG = "roveagent"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=roveagent"
...
def _load_supermemory_config(roveagent_home: str) -> dict:
    config = _default_config()
    config_path = Path(roveagent_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, roveagent_home: str) -> None:
    config_path = Path(roveagent_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "roveagent"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes RoveAgent writes into the "RoveAgent" Space in the Supermemory
        ...
        merged = {"sm_source": "roveagent", **(metadata or {})}
...
def _resolve_container_tag_for_setup(roveagent_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(roveagent_home)
...
def _probe_supermemory_connection(api_key: str, roveagent_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(roveagent_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(roveagent_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `roveagent memory setup`.
        # All other options are documented for $ROVEAGENT_HOME/supermemory.json
    ...
    def save_config(self, values, roveagent_home):
        ...
        _save_supermemory_config(sanitized, roveagent_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, roveagent_home)
    ...
    def post_setup(self, roveagent_home: str, config: dict) -> None:
        ...
        from roveagent.clisupport.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, roveagent_home=roveagent_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from roveagent.constants import get_roveagent_home
        self._roveagent_home = kwargs.get("roveagent_home") or str(get_roveagent_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._roveagent_home)
