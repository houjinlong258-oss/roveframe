"""Meta Model API (Muse Spark) provider plugin for RoveAgent Agent.
...
Bundled from https://github.com/albertodepaola/roveagent-meta-provider. RoveAgent'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to roveagent' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map RoveAgent' reasoning_config to a Meta-safe ``reasoning_effort`` value.
    ...
    from roveagent.core.reasoning_effort import META_AI_EFFORTS, clamp_effort
...
meta_ai = MetaAIProfile(
    ...
    # in roveagent_cli/providers.py and the retention hint in
