"""Ramp Router (router.com) provider plugin for RoveAgent Agent.
...
* **Responses API is the native wire.** Router serves ``GET /v1/models``
  ...
  in ``roveagent_cli/providers.py`` keep every path on the native wire —
...
``roveagent_cli/models.py``: cache-only lookups on the per-request hot path
...
from roveagent.providers.base import ProviderProfile, _profile_user_agent
...
def _resolve_api_key() -> str:
    ...
    raw environment when the roveagent_cli helper is unavailable (e.g. stripped
    test environments).
    """
    resolvers = []
    try:
...
def _parse_efforts(items: Any) -> Optional[dict[str, list[str]]]:
    ...
    if not isinstance(items, list):
        return None
    try:
        from roveagent.core.reasoning_effort import EFFORT_LADDER
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_roveagent_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
...
def _warm_efforts_async() -> None:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        # Match the canonical caps warmer (roveagent_cli/models.py): a mid-suite
...
router = RouterProfile(
    ...
    base_url=_base_url(),
    auth_type="api_key",
    # Identify RoveAgent traffic to the gateway (Router attributes coding-agent
