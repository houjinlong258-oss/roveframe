...
This plugin ships bundled with RoveAgent but is **opt-in** — it only loads when
...
roveagent tools  # → Langfuse Observability

# Manual
pip install langfuse
roveagent plugins enable observability/langfuse
```

## Required credentials

Set these in `~/.roveagent/.env` (or via `roveagent tools`):

```bash
ROVEAGENT_LANGFUSE_PUBLIC_KEY=pk-lf-...
ROVEAGENT_LANGFUSE_SECRET_KEY=sk-lf-...
ROVEAGENT_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
```bash
roveagent plugins list                 # observability/langfuse should show "enabled"
roveagent chat -q "hello"              # then check Langfuse for a "RoveAgent turn" trace
```

Generation observations include the RoveAgent system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `ROVEAGENT_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
ROVEAGENT_LANGFUSE_ENV=production       # environment tag
ROVEAGENT_LANGFUSE_RELEASE=v1.0.0       # release tag
ROVEAGENT_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
ROVEAGENT_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
ROVEAGENT_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
ROVEAGENT_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`ROVEAGENT_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
roveagent plugins disable observability/langfuse
```
