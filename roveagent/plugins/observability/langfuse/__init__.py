"""langfuse — RoveAgent plugin for Langfuse observability.

Traces RoveAgent conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the RoveAgent plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``roveagent plugins enable
observability/langfuse`` or ``roveagent tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``roveagent tools`` or ~/.roveagent/.env):
  ROVEAGENT_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  ROVEAGENT_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  ROVEAGENT_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  ROVEAGENT_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  ROVEAGENT_LANGFUSE_RELEASE     - release/version tag
  ROVEAGENT_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  ROVEAGENT_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  ROVEAGENT_LANGFUSE_CAPTURE     - content capture mode (default: "sanitized")
  ...
  ROVEAGENT_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
_LANGFUSE_KEY_PREFIXES: Dict[str, str] = {
    "ROVEAGENT_LANGFUSE_PUBLIC_KEY": "pk-lf-",
    "ROVEAGENT_LANGFUSE_SECRET_KEY": "sk-lf-",
}
...
def _debug_enabled() -> bool:
    return _env_bool("ROVEAGENT_LANGFUSE_DEBUG")
...
def _capture_mode() -> str:
    ...
    value = _env("ROVEAGENT_LANGFUSE_CAPTURE").lower()
    if not value:
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid ROVEAGENT_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the RoveAgent plugin system —
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `roveagent tools` and configure Langfuse "
                "Observability to reinstall it."
            )
            _LANGFUSE_CLIENT = _INIT_FAILED
            return None

        public_key = _env("ROVEAGENT_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
        secret_key = _env("ROVEAGENT_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
        ...
        placeholder_issues = [
            msg
            for msg in (
                _validate_langfuse_key("ROVEAGENT_LANGFUSE_PUBLIC_KEY", public_key),
                _validate_langfuse_key("ROVEAGENT_LANGFUSE_SECRET_KEY", secret_key),
            )
            if msg
        ]
        if placeholder_issues:
            logger.warning(
                ...
                "or unset ROVEAGENT_LANGFUSE_PUBLIC_KEY / ROVEAGENT_LANGFUSE_SECRET_KEY to "
        ...
        base_url = _env("ROVEAGENT_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
        environment = _env("ROVEAGENT_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
        release = _env("ROVEAGENT_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
        sample_rate = _env("ROVEAGENT_LANGFUSE_SAMPLE_RATE")
        ...
        if sample_rate:
            try:
                ...
                logger.warning("Invalid ROVEAGENT_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
        ...
        # short-lived-process gap (kanban workers / roveagent chat -q / cron): exit
...
def _trace_key(
    ...
    *,
    ...
    Older RoveAgent paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("ROVEAGENT_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical RoveAgent usage into Langfuse usage and cost maps."""
    ...
    try:
        from roveagent.core.usage_pricing import estimate_usage_cost, resolve_billing_route
    ...
    # RoveAgent estimate rather than summing components because it also includes
    ...
    try:
        ...
        from roveagent.core.usage_pricing import get_pricing_entry
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="RoveAgent turn",
                tags=["roveagent", "langfuse"],
            ):
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``roveagent chat -q`` one-shots, cron jobs — can exit while
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older RoveAgent branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current RoveAgent also has a turn-scoped
    ...
    # messages is a list only for legacy RoveAgent branches that fired
    # pre_llm_call with API messages directly. Current RoveAgent fires
...
def on_post_llm_call(*, task_id: str = "", session_id: str = "", provider: str = "", base_url: str = "",
    ...
    if getattr(response, "usage", None) is not None:
        ...
        try:
            from roveagent.core.usage_pricing import CanonicalUsage
...
def register(ctx) -> None:
    ...
    # RoveAgent versions.  pre_api_request / post_api_request fire per API
