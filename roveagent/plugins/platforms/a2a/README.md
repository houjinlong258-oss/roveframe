# A2A — Agent-to-Agent protocol for RoveAgent
...
peer (another RoveAgent, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
roveagent gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, RoveAgent serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.roveagent/a2a_audit.jsonl`.
- Conversations persist to `~/.roveagent/a2a_conversations/` — they survive context
