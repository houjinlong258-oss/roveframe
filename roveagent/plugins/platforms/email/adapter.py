"""
Email platform adapter for the RoveAgent gateway.

Allows users to interact with RoveAgent by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _roveagent_version = "0"
        imap.xatom(
            "ID",
            f'("name" "roveagent-agent" "version" "{_roveagent_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `roveagent config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `roveagent gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "RoveAgent Agent")
        ...
        msg_id = f"<roveagent-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# roveagent_cli/gateway.py, and the _send_email dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        try:
            from roveagent.tools.send_message_tool import _error as _e
