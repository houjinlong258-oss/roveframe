...
In single-bot mode (what RoveAgent currently supports), open_id works as a
...
from roveagent.gateway.status import acquire_scoped_lock, release_scoped_lock
...
def _strip_markdown_to_plain_text(text: str) -> str:
    ...
    horizontal rules, \\r\\n normalisation).
    """
    from roveagent.gateway.platforms.helpers import strip_markdown
    plain = text.replace("\r\n", "\n")
...
def feishu_deps_present() -> bool:
    ...
    if FEISHU_AVAILABLE:
        return True
    try:
        from roveagent.tools.lazy_deps import is_available
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_roveagent_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                env_int("ROVEAGENT_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE),
            ),
            text_batch_delay_seconds=env_float(
                "ROVEAGENT_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS
            ),
            text_batch_split_delay_seconds=env_float(
                "ROVEAGENT_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0
            ),
            text_batch_max_messages=max(
                1,
                env_int("ROVEAGENT_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES),
            ),
            text_batch_max_chars=max(
                1,
                env_int("ROVEAGENT_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS),
            ),
            media_batch_delay_seconds=env_float(
                "ROVEAGENT_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS
            ),
            webhook_host=str(
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10,
                    thread_name_prefix="roveagent-feishu-sdk",
                )
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local RoveAgent gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``roveagent_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"roveagent_action": action_name, "approval_id": approval_id},
                }
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "roveagent_update_prompt_action": answer,
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_roveagent_home() / ".update_response"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that RoveAgent does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_drive_comment_event(self, data: Any) -> None:
        ...
        """
        from roveagent.plugins.platforms.feishu.feishu_comment import handle_drive_comment_event
    ...
    def _on_meeting_invited_event(self, data: Any) -> None:
        ...
        from roveagent.plugins.platforms.feishu.feishu_meeting_invite import handle_meeting_invited_event
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        roveagent_action = action_value.get("roveagent_action") if isinstance(action_value, dict) else None
        update_prompt_action = (
            action_value.get("roveagent_update_prompt_action")
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("roveagent_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        answer = str(action_value.get("roveagent_update_prompt_action", "") or "").strip().lower()
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by RoveAgent webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto RoveAgent' SessionSource fields.
    ...
    async def _connect_websocket(self) -> None:
        ...
        self._ws_client = FeishuWSClient(
            ...
            # See https://github.com/NousResearch/roveagent-agent/issues/50656
...
# Called by `roveagent gateway setup` via _setup_feishu() in roveagent_cli/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=roveagent&tp=roveagent"
    else:
        qr_url += "?from=roveagent&tp=roveagent"
    return {
...
# dict in roveagent_cli/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not await asyncio.to_thread(_load_lark_oapi):
        return {"error": "Feishu dependencies not installed. Run `roveagent setup` to install Feishu support."}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in roveagent_cli/gateway.py and the static
    ...
    from roveagent.clisupport.setup import prompt_choice
    ...
    if access_idx == 0:
        ...
        print_info("Unknown users can request access; approve with `roveagent pairing approve`.")
    elif access_idx == 1:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `roveagent setup` to install Feishu support.",
