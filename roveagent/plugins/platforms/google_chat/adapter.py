...
# ``model_tools`` import time, so ``roveagent status``, ``roveagent chat``, etc.
...
# ``roveagent_plugins.platforms__google_chat.adapter`` once the plugin
...
logger = logging.getLogger("roveagent.gateway.platforms.google_chat")
...
#     fresh "RoveAgent is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a roveagent MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "roveagent-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ROVEAGENT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from roveagent.constants import get_roveagent_home as _get_roveagent_home
            _roveagent_home = _get_roveagent_home()
        except (ModuleNotFoundError, ImportError):
            _roveagent_home = _Path.home() / ".roveagent"
        self._thread_count_store = _ThreadCountStore(
            _roveagent_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "RoveAgent is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ROVEAGENT_HOME", str(_Path.home() / ".roveagent"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ROVEAGENT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a roveagent MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'RoveAgent is thinking…' marker message.
        ...
        "RoveAgent is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "RoveAgent is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "RoveAgent is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "RoveAgent is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``roveagent setup``.

    The setup wizard at ``roveagent_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.roveagent/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. roveagent-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.roveagent/.env")
    print_info("Restart the gateway: roveagent gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``roveagent cron`` running as a
    separate process from ``roveagent gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the RoveAgent plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `roveagent setup` to install Google Chat support.",
        ...
        # the "RoveAgent is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'RoveAgent is thinking…' marker message "
