...
- Per-user tokens (keyed by sender email):
    ``${ROVEAGENT_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${ROVEAGENT_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${ROVEAGENT_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${ROVEAGENT_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${ROVEAGENT_HOME}/google_chat_user_client_secret.json``
"""
...
logger = logging.getLogger("roveagent.gateway.platforms.google_chat_user_oauth")

# Use the project's ROVEAGENT_HOME helper so the token follows the user's
# profile (e.g. tests can override via ROVEAGENT_HOME=/tmp/...).
try:
    from roveagent.constants import display_roveagent_home, get_roveagent_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where roveagent_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _roveagent_home.py shim).
...
def _roveagent_home() -> Path:
    """Resolve ROVEAGENT_HOME at call time (NOT module import).

    Tests and ``ROVEAGENT_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.roveagent/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _roveagent_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _roveagent_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _roveagent_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _roveagent_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _roveagent_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Run `roveagent setup` to install Google Chat support."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("Run `roveagent setup` to repair the managed installation, then retry.")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into ROVEAGENT_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_roveagent_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_roveagent_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for RoveAgent (native attachment delivery)"
    )
