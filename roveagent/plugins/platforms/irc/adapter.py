"""
IRC Platform Adapter for RoveAgent Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the RoveAgent agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "roveagent-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :RoveAgent Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :RoveAgent Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: roveagent_, roveagent_1, roveagent_2...
...
def check_requirements() -> bool:
    ...
    # roveagent setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `roveagent gateway setup` flow for the IRC platform.
    ...
    print_info("Connect RoveAgent to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. roveagent-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #roveagent — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.roveagent/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "roveagent-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "roveagent-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :RoveAgent Agent (cron)")
