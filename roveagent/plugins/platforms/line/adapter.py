"""
LINE Messaging API platform adapter for RoveAgent Agent.
...
to RoveAgent Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ROVEAGENT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            roveagent_home = Path(get_roveagent_home()).resolve()
        except Exception:
            roveagent_home = Path.home().joinpath(".roveagent").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``roveagent status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``roveagent status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``roveagent setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.roveagent/.env`` via ``roveagent_cli.config``.
    """
    print()
    ...
    try:
        from roveagent.clisupport.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("roveagent.clisupport.config not available; set LINE_* vars manually in ~/.roveagent/.env")
        return
