...
from roveagent.gateway.platforms.helpers import ThreadParticipationTracker
...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable RoveAgent command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    RoveAgent accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal RoveAgent ``/command`` text."""
...
# Uses get_roveagent_home() so each profile gets its own Matrix store.
from roveagent.constants import get_roveagent_dir as _get_roveagent_dir

_STORE_DIR = _get_roveagent_dir("platforms/matrix/store", "matrix/store")
...
def ensure_matrix_deps() -> bool:
    ...
    try:
        from roveagent.tools.lazy_deps import feature_missing, ensure_and_bind
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches RoveAgent
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("ROVEAGENT_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("ROVEAGENT_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "roveagent"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "RoveAgent Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in roveagent_cli/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"roveagent_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces roveagent_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        try:
            from roveagent.tools.lazy_deps import ensure as _lazy_ensure, feature_missing
        ...
        print_info("📬 Home Room: where RoveAgent delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via roveagent_cli.gateway.get_env_value so setup-status
