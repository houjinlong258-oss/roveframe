"""ntfy platform adapter (RoveAgent plugin).
...
a RoveAgent dependency.

This adapter ships as a RoveAgent platform plugin under
``plugins/platforms/ntfy/``. The RoveAgent plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "roveagent-in"              # subscribe topic (incoming)
          publish_topic: "roveagent-out"     # optional — defaults to topic
...
_ECHO_TAG = "roveagent-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``roveagent cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a RoveAgent dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `roveagent gateway status` without
