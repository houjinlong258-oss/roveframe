"""
``roveagent photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other RoveAgent
...
# Writable sidecar runtime dir (mirrors to ROVEAGENT_HOME on immutable
...
# wiring imports this module for `roveagent --help`).
...
# when called later (gateway start, roveagent status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `roveagent photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'RoveAgent Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``roveagent photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "RoveAgent Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.roveagent/.env,
    ...
    try:
        from roveagent.clisupport.config import write_platform_config_field
    ...
    print("  Start the gateway:  roveagent gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `roveagent photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`roveagent photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.roveagent/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `roveagent photon telemetry on` / `roveagent photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.roveagent/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  roveagent gateway restart")
    return 0
...
#
# `roveagent gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``roveagent photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `roveagent gateway setup` wizard."""
