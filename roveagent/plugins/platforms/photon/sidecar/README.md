...
Small Node helper that bridges RoveAgent Agent to Photon's Spectrum SDK
(`spectrum-ts`).  RoveAgent is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-RoveAgent-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and RoveAgent can receive inbound messages
...
The RoveAgent plugin's `roveagent photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. RoveAgent is Python, so replies go through this sidecar
