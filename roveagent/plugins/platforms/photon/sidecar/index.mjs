// RoveAgent Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> RoveAgent): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (RoveAgent -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-RoveAgent-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `roveagent photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so RoveAgent can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "roveagent-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply RoveAgent'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local RoveAgent allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of RoveAgent, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-roveagent-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when RoveAgent supplied them so a known-good
