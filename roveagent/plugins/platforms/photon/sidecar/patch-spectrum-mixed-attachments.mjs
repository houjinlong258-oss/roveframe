...
// `message.content.text` before RoveAgent can see it. We rewrite the two inbound
...
const MARKER = "RoveAgent patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // RoveAgent to patch; keep the legacy v8 path below for older pinned installs.
    if (
