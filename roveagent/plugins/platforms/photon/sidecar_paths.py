...
under an immutable ``/opt/roveagent`` (read-only for the roveagent user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$ROVEAGENT_HOME/photon/sidecar`` (the durable data volume,
...
``cli.py`` (``roveagent photon ...``) use it.
"""
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".roveagent-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_roveagent_home() / "photon" / "sidecar"
    try:
