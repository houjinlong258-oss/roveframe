"""SimpleX Chat platform adapter (RoveAgent plugin).
...
This adapter ships as a RoveAgent platform plugin under
``plugins/platforms/simplex/``. The RoveAgent plugin loader scans the
...
Optional environment variables:
    ...
    ROVEAGENT_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``roveagent setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside RoveAgent — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ROVEAGENT_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``roveagent setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.roveagent/.env`` via
    ``roveagent_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "roveagent.clisupport.config not available; set SIMPLEX_* vars manually in "
            "~/.roveagent/.env"
        )
        return
