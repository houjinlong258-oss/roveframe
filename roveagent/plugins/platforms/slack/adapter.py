...
from roveagent.gateway.platforms.helpers import MessageDeduplicator
...
# identify RoveAgentAgent traffic — matching other RoveAgent outbound surfaces
# that already set ``RoveAgentAgent/<version>`` for platform-partner attribution.
try:
    from roveagent.clisupport import __version__ as _ROVEAGENT_VERSION
except Exception:
    _ROVEAGENT_VERSION = "unknown"
_ROVEAGENT_SLACK_USER_AGENT_PREFIX = f"RoveAgentAgent/{_ROVEAGENT_VERSION}"
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``roveagent config set`` and Railway persist YAML
...
def check_slack_requirements() -> bool:
    ...
    from roveagent.tools.lazy_deps import ensure_and_bind
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from roveagent.clisupport.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # After the label, so that ``<@U…|roveagent>`` is stripped like ``<@U…>``.
    if bot_uid:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `roveagent slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `roveagent gateway setup` "
                "or in the active profile's ~/.roveagent/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `roveagent gateway setup` "
                "or add it to your active profile's ~/.roveagent/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `roveagent gateway setup` "
                "or in the active profile's ~/.roveagent/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `roveagent gateway setup` "
                "or add it to your active profile's ~/.roveagent/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_roveagent_home() / "slack_tokens.json"
        if tokens_file.exists():
            try:
                # Warn if the token file is world- or group-readable — it
                # contains plaintext bot tokens for all saved workspaces.
                from roveagent.utils import warn_if_credential_file_broadly_readable
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_ROVEAGENT_SLACK_USER_AGENT_PREFIX,
            )
            self._app = AsyncApp(
                token=primary_token,
            ...
            # RoveAgent has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by RoveAgent can "
                    "be removed from the Slack app manifest via "
                    "`roveagent slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /roveagent prefix). A single regex
            ...
            # manifest (see `roveagent slack manifest`). In Socket Mode, Slack
            ...
            from roveagent.clisupport.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/roveagent$")

            @self._app.command(_slash_pattern)
            async def handle_roveagent_command(ack, command):
            ...
            for _action_id in (
                "roveagent_approve_once",
                "roveagent_approve_session",
                "roveagent_approve_always",
                "roveagent_deny",
            ):
            ...
            for _action_id in (
                "roveagent_confirm_once",
                "roveagent_confirm_always",
                "roveagent_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("roveagent_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^roveagent_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("roveagent_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'roveagent slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: RoveAgent handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    async def send_native_task_card_progress(
        ...
        *,
        title: str = "RoveAgent is working",
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own RoveAgent session — matching the per-thread behavior channels already
    ...
    async def send_multiple_images(
        ...
        try:
            ...
            from roveagent.gateway.platforms.base import _ssrf_redirect_guard
    ...
    async def send_image(
        ...
        try:
            async def _ssrf_redirect_guard(response):
                ...
                from roveagent.tools.url_safety import redirect_target_from_response
    ...
    async def _handle_app_context_changed(
        ...
        # RoveAgent owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_roveagent_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_roveagent_no_thread_response"] = True
    ...
    async def _handle_slack_message_impl(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_roveagent_no_thread_response"):
        ...
        force_process = bool(event.get("_roveagent_force_process"))
        ...
        # the shape produced by peer RoveAgent agents in Socket Mode on some
        ...
        try:
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "roveagent_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "roveagent_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "roveagent_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "roveagent_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "roveagent_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "roveagent_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``roveagent_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``roveagent_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"roveagent_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "roveagent_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "roveagent_confirm_once": "once",
            "roveagent_confirm_always": "always",
            "roveagent_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "roveagent_approve_once": "once",
            "roveagent_approve_session": "session",
            "roveagent_approve_always": "always",
            "roveagent_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "roveagent_clarify_other" or token == "other":
    ...
    async def _format_thread_context(
        ...
        *,
        ...
        from roveagent.gateway.session import neutralize_untrusted_inline_text
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/roveagent <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/roveagent
        ...
        if slash_name in {"roveagent", ""}:
            # Legacy /roveagent <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from roveagent.clisupport.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/roveagent   ``).
        ...
        # "Running /cmd…" ack shown by handle_roveagent_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/roveagent <question>" must produce public replies so
    ...
    async def _download_slack_file(
        ...
        import httpx
        from roveagent.gateway.platforms.base import _ssrf_redirect_guard, safe_url_for_log
        from roveagent.tools.url_safety import create_ssrf_safe_async_client, is_safe_url
        ...
        async with create_ssrf_safe_async_client(
            ...
            for attempt in range(3):
                try:
                    ...
                    if audio:
                        from roveagent.gateway.platforms.base import cache_audio_from_bytes

                        return cache_audio_from_bytes(response.content, ext)
                    else:
                        from roveagent.gateway.platforms.base import cache_image_from_bytes
...
# ``roveagent_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _resolve_slack_user_dm(token: str, user_id: str) -> Optional[str]:
    ...
    try:
        ...
        return None
    try:
        from roveagent.gateway.platforms.base import proxy_kwargs_for_aiohttp
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_roveagent_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``roveagent_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ROVEAGENT_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from roveagent.clisupport.slack_cli import _build_full_manifest
            from roveagent.constants import get_roveagent_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="RoveAgent",
                bot_description="Your RoveAgent agent on Slack",
            )
            target = Path(get_roveagent_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `roveagent slack manifest --write` anytime to refresh after "
                "RoveAgent adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `roveagent update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://roveagent-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``roveagent_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `roveagent setup` to install Slack support.",
        # Interactive setup wizard — replaces roveagent_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in roveagent_cli/gateway.py.
