"""
Microsoft Teams platform adapter for RoveAgent Agent.
...
Requires:
    ...
    manually: ``<roveagent-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
class TeamsSummaryWriter:
    ...
    def _build_graph_client(self, config: dict[str, Any]) -> Any:
        ...
        from roveagent.tools.microsoft_graph_auth import MicrosoftGraphTokenProvider
        from roveagent.tools.microsoft_graph_client import MicrosoftGraphClient
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    RoveAgent owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "RoveAgent"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        from roveagent.gateway.platforms.base import _ssrf_redirect_guard, _read_httpx_body_with_limit

        if not is_safe_url(url):
            raise ValueError("Blocked unsafe attachment URL (SSRF protection)")

        headers = {"User-Agent": "Mozilla/5.0 (compatible; RoveAgentAgent/1.0)"}
    ...
    async def _on_card_action(
        ...
        from roveagent.tools.approval import resolve_gateway_approval, has_blocking_approval

        action = ctx.activity.value.action
        data = action.data or {}
        roveagent_action = data.get("roveagent_action", "")
        session_key = data.get("session_key", "")

        if not roveagent_action or not session_key:
        ...
        choice = choice_map.get(roveagent_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="roveagent_approve",
            data={**btn_data_base, "roveagent_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="roveagent_approve",
                data={**btn_data_base, "roveagent_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="roveagent_approve",
                    data={**btn_data_base, "roveagent_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="roveagent_approve",
            data={**btn_data_base, "roveagent_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"RoveAgent\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.roveagent/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual RoveAgent
    venv in every layout and sidesteps Ubuntu 24.04's PEP 668 failure that
    a bare ``pip install`` hint invites.
    """
    try:
        from roveagent.tools.lazy_deps import feature_install_command
