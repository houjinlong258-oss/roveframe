...
``<roveagent home>/plugins/<name>/``. That tree is the plugin *install* dir —
``roveagent plugins remove`` deletes it and ``roveagent plugins update`` git-pulls
...
``<roveagent home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in RoveAgent.

Usage::

    from roveagent.plugins.plugin_storage import plugin_data_dir, plugin_db
...
# Mirrors the plugin-name shape `roveagent plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<roveagent home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`roveagent_constants.get_roveagent_home` on every call. Don't cache the
    result across profile switches.
    """
    from roveagent.constants import get_roveagent_home

    root = get_roveagent_home() / "plugin-data" / _validate_name(name)
