...
``_check_spotify_available()`` — when the user has not run ``roveagent auth
spotify``, the tools remain registered (so they appear in ``roveagent
...
The Spotify auth flow (``roveagent auth spotify``), CLI plumbing, and docs
are unchanged. This move is purely structural.
"""

from __future__ import annotations

from roveagent.plugins.spotify.tools import (
