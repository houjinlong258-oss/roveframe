...
def _load_fal_client() -> Any:
    ...
    with _fal_client_lock:
        if _fal_client is not None:  # re-check inside the lock
            return _fal_client
        from roveagent.tools.fal_common import import_fal_client
...
def _get_managed_fal_video_client(managed_gateway):
    ...
    from roveagent.tools.fal_common import _ManagedFalSyncClient
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        from roveagent.tools.fal_common import _extract_http_status
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `roveagent tools` → Video Generation."
            ) from exc
        raise


def _check_fal_video_available() -> bool:
    """True if the FAL video backend selected via `roveagent tools` (or, on a
        ...
        if not _check_fal_video_available():
            ...
            return error_response(
                error=(
                    ...
                    "(run `roveagent tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`roveagent setup`) for managed gateway access."
                ),
                error_type="auth_required",
                provider="fal",
                prompt=prompt,
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `roveagent tools` → Video Generation."
                    ),
