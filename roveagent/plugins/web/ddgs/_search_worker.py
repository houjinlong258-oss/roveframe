...
Optional test hooks (only when ``ROVEAGENT_DDGS_ALLOW_TEST_HOOKS=1``)::
...
def main() -> int:
    ...
    if hook:
        if os.environ.get("ROVEAGENT_DDGS_ALLOW_TEST_HOOKS") != "1":
            _write_envelope(
    ...
    try:
        ...
        from roveagent.plugins.web.ddgs.provider import _run_ddgs_search
