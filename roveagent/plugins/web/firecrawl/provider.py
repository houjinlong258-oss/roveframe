...
#
# Tests that do ``patch("roveagent.tools.web_tools.Firecrawl", ...)`` continue to
...
def _use_keyless_ring() -> bool:
    ...
    import roveagent.tools.web_tools
    from roveagent import tools as tools
    from roveagent.tools.tool_backend_helpers import NOUS_MANAGED_PROVIDER, read_selection
    ...
    try:
        ...
        pass
    from roveagent.plugins.web.keyless_mcp import use_keyless
...
def _is_tool_gateway_ready() -> bool:
    ...
    that ``patch("roveagent.tools.web_tools._peek_nous_access_token", ...)`` see
...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `roveagent tools`
    ...
    existing tests and the ``roveagent tools`` setup flow.
    """
...
class FirecrawlWebSearchProvider(WebSearchProvider):
    ...
    def is_keyless_available(self) -> bool:
        ...
        """
        from roveagent.plugins.web.keyless_mcp import keyless_enabled, provider_tier
