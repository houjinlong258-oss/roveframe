...
``{title, url, description, position}`` rows every other RoveAgent web
provider produces.
...
prefers RoveAgent-managed xAI Grok OAuth (via ``roveagent auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.roveagent/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``roveagent tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `roveagent auth` to sign in with "
