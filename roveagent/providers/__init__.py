...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with roveagent-agent)
2. User plugins: ``$ROVEAGENT_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``roveagent_agent.plugins``
...
from roveagent.providers.base import OMIT_TEMPERATURE, ProviderProfile  # noqa: F401
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ROVEAGENT_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ROVEAGENT_HOME/plugins/model-providers/`` if it exists."""
    try:
        from roveagent.constants import get_roveagent_home

        d = get_roveagent_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ROVEAGENT_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"roveagent.plugins.model_providers.{safe_name}"
    else:
        module_name = f"_roveagent_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``roveagent_agent.plugins``
    entry-point group so they self-register.

    A distribution ships::

        [project.entry-points."roveagent_agent.plugins"]
        acme-inference = "acme_roveagent_plugin:register"
    ...
    * **Provider targets only.** The ``roveagent_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ROVEAGENT_HOME``) keep their
    ...
    try:
        from roveagent.clisupport.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ROVEAGENT_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``roveagent_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ROVEAGENT_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with roveagent-agent.
    ...
    # 2. User plugins — under $ROVEAGENT_HOME/plugins/model-providers/<name>/.
    ...
    try:
        ...
        from roveagent import providers as providers

        for _importer, modname, _ispkg in pkgutil.iter_modules(_pkg.__path__):
            if modname.startswith("_") or modname == "base":
                continue
            try:
                importlib.import_module(f"roveagent.providers.{modname}")
