...
class SelfHealingEngine:
    ...
    def sandbox_test(self, case_id: str,
        ...
        try:
            proc = subprocess.run(test_command, shell=True, cwd=self.repo,
    ...
    def deploy(self, case_id: str) -> HealingCase:
        ...
        msg = f"fix(self-healing): {case.error_signature} [case {case.case_id}]"
        ...
        proc = subprocess.run(
            f'git add "{rel}" && git commit -m "{msg}"',
            shell=True, cwd=self.repo, capture_output=True, text=True)
        if proc.returncode != 0:
            case.mark("failed", (proc.stdout + proc.stderr)[-500:])
            self._save(case)
            raise RuntimeError(f"git commit failed: {(proc.stdout + proc.stderr)[-300:]}")
        rev = subprocess.run("git rev-parse HEAD", shell=True, cwd=self.repo,
    ...
    def rollback(self, case_id: str, agent: str = "devops") -> HealingCase:
        ...
        if d.request:
            self.permissions.approve(d.request.request_id)  # 回滚走快速通道记录
        proc = subprocess.run(f"git revert --no-edit {case.commit_hash}",
                              shell=True, cwd=self.repo, capture_output=True, text=True)
        if proc.returncode != 0:
