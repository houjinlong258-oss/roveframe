...
# IMPORTANT: roveagent_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See roveagent_bootstrap.py for full rationale.
try:
    from roveagent import bootstrap as roveagent_bootstrap
except ModuleNotFoundError:
    # Graceful fallback when roveagent_bootstrap isn't registered in the venv
    # yet — happens during partial ``roveagent update`` where git-reset landed
...
#   (b) `patch("roveagent.runtime.OpenAI", ...)` test patterns used by ~28 test files.
#
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``roveagent -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        from roveagent.gateway.session_context import get_session_env

        source = get_session_env("ROVEAGENT_SESSION_SOURCE", "")
    except Exception:
        source = os.environ.get("ROVEAGENT_SESSION_SOURCE", "")
...
# `OpenAI` is re-exported here so `patch("roveagent.runtime.OpenAI", ...)` in tests works.
# The other `# noqa: F401` re-exports below cover names accessed via
# `mock.patch("roveagent.runtime.<X>")`, `from run_agent import <X>` in production
# siblings, or the `_ra().<X>` indirection in agent/system_prompt.py — none
# of which ruff's in-module usage scan can see.
from roveagent.core.process_bootstrap import (
    OpenAI,  # noqa: F401  # re-exported for tests that mock.patch("roveagent.runtime.OpenAI")
    _SafeWriter,  # noqa: F401  # re-exported for tests that `from run_agent import _SafeWriter`
    _get_proxy_for_base_url,
)
from roveagent.core.iteration_budget import IterationBudget
from roveagent.core.interrupt_compat import request_hard_interrupt


from roveagent.clisupport.env_loader import load_roveagent_dotenv
from roveagent.clisupport.timeouts import (
    get_provider_request_timeout,
    get_provider_stale_timeout,
)

_roveagent_home = get_roveagent_home()
_project_env = Path(__file__).parent / '.env'
_loaded_env_paths = load_roveagent_dotenv(roveagent_home=_roveagent_home, project_env=_project_env)
...
from roveagent.model_tools import (
    get_tool_definitions,  # noqa: F401  # re-exported for tests that mock.patch("roveagent.runtime.get_tool_definitions")
    get_toolset_for_tool,
    handle_function_call,  # noqa: F401  # re-exported for tests that mock.patch("roveagent.runtime.handle_function_call")
    check_toolset_requirements,  # noqa: F401  # re-exported for tests that mock.patch("roveagent.runtime.check_toolset_requirements")
)
from roveagent.tools.terminal_tool import cleanup_vm, get_active_env
from roveagent.tools.interrupt import set_interrupt as _set_interrupt
from roveagent.tools.browser_tool import cleanup_browser


# Agent internals extracted to agent/ package for modularity
from roveagent.core.memory_manager import sanitize_context
from roveagent.core.memory_provider import is_trivial_prompt
from roveagent.core.error_classifier import FailoverReason
from roveagent.core.redact import redact_sensitive_text
from roveagent.core.message_content import flatten_message_text
from roveagent.core.session_activity import ActivityProvenance
from roveagent.core.model_metadata import (
    estimate_request_tokens_rough,  # noqa: F401  # re-exported for tests that mock.patch("roveagent.runtime.estimate_request_tokens_rough")
    is_local_endpoint,
)
from roveagent.core.usage_pricing import normalize_usage
# Re-exported for tests that monkeypatch these symbols on run_agent.
from roveagent.core.context_compressor import (  # noqa: F401
    COMPRESSED_SUMMARY_METADATA_KEY,
    ContextCompressor,
    user_originated_turn_view,
)
from roveagent.core.retry_utils import jittered_backoff  # noqa: F401
from roveagent.core.prompt_builder import (  # noqa: F401  # re-exported via _ra() / mock.patch("roveagent.runtime.<name>") / from run_agent import <name>
...
from roveagent.core.process_bootstrap import _get_proxy_from_env  # noqa: F401
from roveagent.core.message_sanitization import (  # noqa: F401
    _SURROGATE_RE,
...
from roveagent.core.codex_responses_adapter import (
...
from roveagent.core.tool_guardrails import (
    ToolGuardrailDecision,
    append_toolguard_guidance,
    toolguard_synthetic_result,
)
from roveagent.core.tool_result_classification import (
    FILE_MUTATING_TOOL_NAMES as _FILE_MUTATING_TOOLS,
    file_mutation_result_landed,
)
from roveagent.core.trajectory import (
    convert_scratchpad_to_think,
    save_trajectory as _save_trajectory_to_file,
)
from roveagent.core.tool_dispatch_helpers import (
...
from roveagent.utils import atomic_json_write, base_url_host_matches, base_url_hostname, env_float, is_truthy_value, model_forces_max_completion_tokens
...
# (roveagent_state._rows_to_conversation), so any code that mutates a loaded or
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"RoveAgentAgent/{_ROVEAGENT_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-RoveAgent-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.roveagent/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[roveagent-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        from roveagent.core.agent_init import init_agent
        init_agent(
            self,
            base_url=base_url,
            api_key=api_key,
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # `roveagent --resume`. set_session_yolo() no-ops on a missing row, so
            # this is the only chance to record a pre-first-turn toggle.
            _init_model_config = self._session_init_model_config
            try:
                from roveagent.tools.approval import is_session_yolo_enabled
    ...
    def _ensure_lmstudio_runtime_loaded(
        ...
        from roveagent.clisupport.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(
        ...
        from roveagent.core.agent_runtime_helpers import switch_model
        return switch_model(
            self,
            new_model,
            new_provider,
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``roveagent chat -q``. In that mode,
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``roveagent chat -Q``) always wins: those flows neutralize the rendering
    ...
    from roveagent.core.stream_diag import STREAM_DIAG_HEADERS as _STREAM_DIAG_HEADERS  # noqa: E402

    @staticmethod
    def _stream_diag_init() -> Dict[str, Any]:
        """Forwarder — see ``agent.stream_diag.stream_diag_init``."""
        from roveagent.core.stream_diag import stream_diag_init
    ...
    def _stream_diag_capture_response(
        ...
        from roveagent.core.stream_diag import stream_diag_capture_response
    ...
    def _flatten_exception_chain(error: BaseException) -> str:
        ...
        from roveagent.core.stream_diag import flatten_exception_chain
    ...
    def _log_stream_retry(
        ...
        *,
        ...
        from roveagent.core.stream_diag import log_stream_retry
        log_stream_retry(
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        from roveagent.core.stream_diag import emit_stream_drop
        emit_stream_drop(
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        from roveagent.core.conversation_compression import check_compression_model_feasibility
    ...
    def _replay_compression_warning(self) -> None:
        ...
        from roveagent.core.conversation_compression import replay_compression_warning
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ROVEAGENT_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ROVEAGENT_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("ROVEAGENT_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ROVEAGENT_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ROVEAGENT_API_CALL_STALE_TIMEOUT")
        ...
        # local NIM endpoint.
        from roveagent.core.reasoning_timeouts import get_reasoning_stale_timeout_floor
    ...
    def _compute_non_stream_stale_timeout(self, api_payload: Any) -> float:
        ...
        from roveagent.core.chat_completion_helpers import estimate_request_context_tokens
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``ROVEAGENT_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("ROVEAGENT_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        roveagent-agent #21444 for the symptom history.  The upstream backend
        ...
        if self.api_mode != "codex_responses":
            return None
        from roveagent.core.codex_responses_adapter import classify_responses_route
        ...
        return (
            ...
            "See roveagent-agent#21444 for symptom history."
        )
    ...
    def _anthropic_prompt_cache_policy(
        ...
        *,
        ...
        from roveagent.core.agent_runtime_helpers import anthropic_prompt_cache_policy
    ...
    def _direct_native_anthropic_tool_cache_capability(
        ...
        *,
        ...
        from roveagent.core.agent_runtime_helpers import _direct_native_anthropic_tool_cache_capability
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from roveagent.clisupport.models import _should_use_copilot_responses_api
    ...
    def _strip_think_blocks(self, content: str) -> str:
        ...
        from roveagent.core.agent_runtime_helpers import strip_think_blocks
    ...
    def _looks_like_codex_intermediate_ack(
        ...
        from roveagent.core.agent_runtime_helpers import looks_like_codex_intermediate_ack
    ...
    def _extract_reasoning(self, assistant_message) -> Optional[str]:
        ...
        from roveagent.core.agent_runtime_helpers import extract_reasoning
    ...
    def _cleanup_task_resources(self, task_id: str) -> None:
        ...
        from roveagent.core.chat_completion_helpers import cleanup_task_resources
        return cleanup_task_resources(self, task_id)

    # ------------------------------------------------------------------
    # Background memory/skill review — prompts live in agent.background_review
    # ------------------------------------------------------------------
    from roveagent.core.background_review import (
    ...
    def _summarize_background_review_actions(
        ...
        from roveagent.core.background_review import summarize_background_review_actions
    ...
    def _spawn_background_review(
        ...
        task_cfg = None
        if focus is None:
            from roveagent.core.background_review import load_background_review_settings
        ...
        from roveagent.tools.thread_context import propagate_context_to_thread
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        from roveagent.core.background_review import build_memory_write_metadata
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        from roveagent.core.agent_runtime_helpers import note_turn_persisted
    ...
    def _repair_message_sequence(self, messages: List[Dict]) -> int:
        ...
        from roveagent.core.agent_runtime_helpers import repair_message_sequence
    ...
    def _flush_messages_to_session_db_unlocked(
        ...
        try:
            ...
            if _batch_rows:
                ...
                from roveagent.core.transcript_repair import sync_flushed_message_markers
    ...
    def _format_tools_for_system_message(self) -> str:
        ...
        from roveagent.core.system_prompt import format_tools_for_system_message
    ...
    def _convert_to_trajectory_format(self, messages: List[Dict[str, Any]], user_query: str, completed: bool) -> List[Dict[str, Any]]:
        ...
        from roveagent.core.agent_runtime_helpers import convert_to_trajectory_format
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use RoveAgent against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "RoveAgent can't reach the model provider. You may be offline. "
    ...
    def _extract_api_error_context(error: Exception) -> Dict[str, Any]:
        ...
        from roveagent.core.agent_runtime_helpers import extract_api_error_context
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ROVEAGENT_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        try:
            from roveagent.clisupport import lifecycle as _lifecycle
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        from roveagent.core.agent_runtime_helpers import dump_api_request_debug
    ...
    def _redact_message_content(content):
        ...
        Respects ``ROVEAGENT_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.roveagent/sessions/session_{sid}.json``
        ...
        # untrusted input (X-RoveAgent-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ROVEAGENT_REDACT_SECRETS via
    ...
    def interrupt(
        ...
        *,
        ...
        # interrupt event rather than RoveAgent' per-thread flag.
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal RoveAgent model request this cancels only that request;
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell RoveAgent-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``ROVEAGENT_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        ...
        try:
            import os as _os
            env = _os.environ.get("ROVEAGENT_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.roveagent/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ROVEAGENT_TURN_COMPLETION_EXPLAINER`` env var overrides
        ...
        try:
            import os as _os
            env = _os.environ.get("ROVEAGENT_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
    ...
    def _format_turn_completion_explanation(
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "turn_lease":
                return (
                    prefix
                    + "the turn was stopped because another RoveAgent process "
            ...
            if cause == "locked":
                return (
                    ...
                    "(another RoveAgent process was writing to the state "
            ...
            if cause == "corrupt":
                return (
                    ...
                    "1. Run `roveagent doctor --fix`\n"
                    "2. Salvage with: sqlite3 ~/.roveagent/state.db \".recover\" "
                    "(then replace state.db)\n"
                    "3. Restore from a backup in ~/.roveagent/backups/\n"
            ...
            return (
                ...
                "Check the state database health (`roveagent doctor`), then "
    ...
    def _apply_pending_steer_to_tool_results(self, messages: list, num_tool_msgs: int) -> None:
        ...
        from roveagent.core.agent_runtime_helpers import apply_pending_steer_to_tool_results
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (ROVEAGENT_KANBAN_TASK set),
        ...
        """
        from roveagent.core.session_activity import (
        ...
        if os.environ.get("ROVEAGENT_KANBAN_TASK"):
            try:
                from roveagent.tools.kanban_tools import (
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from roveagent.core.rate_limit_tracker import parse_rate_limit_headers
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (ROVEAGENT_DEV_CREDITS_FIXTURE): inject a chosen notice state
        # each turn for repeatable testing, bypassing real headers. Throwaway scaffolding.
        try:
            from roveagent.core.credits_tracker import dev_fixture_credits_state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ROVEAGENT_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ROVEAGENT_DEV_CREDITS"))

        # ── Parse (fail-open → miss; never overwrite good state with None) ──
        try:
            from roveagent.core.credits_tracker import parse_credits_headers
        ...
        if _dev:
            # ROVEAGENT_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `roveagent logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _emit_credits_notices(self) -> None:
        ...
        if state is None:
            return
        try:
            from roveagent.core.credits_tracker import evaluate_credits_notices, is_free_tier_model, new_credits_latch
    ...
    def close(self) -> None:
        ...
        try:
            from roveagent.tools.process_registry import process_registry
        ...
        # a private embedded daemon when RoveAgent YOLO selected unrestricted
        # mode.  The import is lazy so sessions without computer_use retain
        # the narrow core footprint.
        try:
            from roveagent.tools.computer_use import release_computer_use_session
        ...
        try:
            from roveagent.clisupport.mem_trim import trim_memory
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        ...
        (GHSA-5g4g-6jrg-mw3g).
        """
        from roveagent.tools.todo_tool import MAX_TODO_RESULT_CHARS
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        from roveagent.core.system_prompt import build_system_prompt_parts
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        from roveagent.core.system_prompt import build_system_prompt
    ...
    def _sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        from roveagent.core.agent_runtime_helpers import sanitize_api_messages
    ...
    def _is_thinking_only_assistant(
        ...
        *,
        ...
        from roveagent.core.native_compaction import has_compaction_checkpoint
    ...
    def _drop_thinking_only_and_merge_users(
        ...
        *,
        ...
        from roveagent.core.agent_runtime_helpers import drop_thinking_only_and_merge_users
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        """
        from roveagent.tools.delegate_tool import _get_max_concurrent_children
    ...
    def _repair_tool_call(self, tool_name: str) -> str | None:
        ...
        from roveagent.core.agent_runtime_helpers import repair_tool_call
        return repair_tool_call(self, tool_name)

    def _invalidate_system_prompt(self):
        """Forwarder — see ``agent.system_prompt.invalidate_system_prompt``."""
        from roveagent.core.system_prompt import invalidate_system_prompt
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        from roveagent.core.process_bootstrap import build_keepalive_http_client
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        from roveagent.core.agent_runtime_helpers import create_openai_client
    ...
    def _force_close_tcp_sockets(client: Any) -> int:
        ...
        from roveagent.core.agent_runtime_helpers import force_close_tcp_sockets
    ...
    def _build_primary_client_for_active_provider(self, *, reason: str) -> Any:
        ...
        if (getattr(self, "provider", "") or "").strip().lower() == "moa":
            from roveagent.core.moa_loop import build_moa_facade
    ...
    def _cleanup_dead_connections(self) -> bool:
        ...
        from roveagent.core.agent_runtime_helpers import cleanup_dead_connections
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from roveagent.clisupport.copilot_auth import copilot_request_headers
    ...
    def _create_request_anthropic_client(self, *, reason: str) -> Any:
        ...
        if key[0] == "bedrock":
            from roveagent.core.anthropic_adapter import build_anthropic_bedrock_client
            client = build_anthropic_bedrock_client(key[1])
        else:
            from roveagent.core.anthropic_adapter import build_anthropic_client
    ...
    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        ...
        from roveagent.core.codex_runtime import run_codex_stream
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        from roveagent.core.codex_runtime import run_codex_create_stream_fallback
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``roveagent auth add xai-oauth``) whose tokens belong to
        ...
        try:
            if self.provider == "openai-codex":
                from roveagent.clisupport.auth import resolve_codex_runtime_credentials

                singleton_now = resolve_codex_runtime_credentials(
                    refresh_if_expiring=False,
                )
            else:
                from roveagent.clisupport.auth import resolve_xai_oauth_runtime_credentials
    ...
    def _try_refresh_nous_client_credentials(
        ...
        *,
        ...
        try:
            from roveagent.clisupport.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("ROVEAGENT_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.roveagent/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``roveagent setup``) updates
        ...
        if getattr(self, "_fallback_activated", False):
            return False
        try:
            from roveagent.core.credential_pool import get_env_prefer_dotenv
        ...
        if (
            ...
            if self.provider == "kimi-coding":
                from roveagent.clisupport.auth import _resolve_kimi_base_url

                base_url = _resolve_kimi_base_url(
                    api_key, pconfig.inference_base_url, env_url
                ).rstrip("/")
            elif self.provider == "zai":
                from roveagent.clisupport.auth import _resolve_zai_base_url
            ...
            try:
                from roveagent.clisupport.runtime_provider import _get_named_custom_provider
            except ImportError:
                return False
        ...
        try:
            from roveagent.core.agent_runtime_helpers import sync_credential_pool_entry_id
    ...
    def _try_refresh_vertex_client_credentials(self) -> bool:
        ...
        try:
            from roveagent.core.vertex_adapter import get_vertex_config
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from roveagent.clisupport.copilot_auth import (
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from roveagent.core.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
        ...
        from roveagent.core.anthropic_adapter import _is_oauth_token
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        from roveagent.core.auxiliary_client import (
            _AI_GATEWAY_HEADERS,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            from roveagent.clisupport.models import copilot_default_headers

            self._client_kwargs["default_headers"] = copilot_default_headers()
        elif base_url_host_matches(base_url, "api.kimi.com"):
            from roveagent.core.auxiliary_client import _AI_GATEWAY_HEADERS
            self._client_kwargs["default_headers"] = dict(_AI_GATEWAY_HEADERS)
        elif base_url_host_matches(base_url, "portal.qwen.ai"):
            self._client_kwargs["default_headers"] = _qwen_portal_headers()
        elif base_url_host_matches(base_url, "chatgpt.com"):
            from roveagent.core.codex_headers import codex_cloudflare_headers
            ...
            from roveagent.tools.xai_http import roveagent_xai_default_headers

            self._client_kwargs["default_headers"] = roveagent_xai_default_headers()
        else:
            # No URL-specific headers — check profile.default_headers before clearing.
            _ph_headers = None
            try:
                from roveagent.providers import get_provider_profile as _gpf2
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            from roveagent.core.anthropic_adapter import build_anthropic_client, _is_oauth_token
    ...
    def _recover_with_credential_pool(
        ...
        *,
        ...
        from roveagent.core.agent_runtime_helpers import recover_with_credential_pool
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        from roveagent.core.anthropic_adapter import create_anthropic_message
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        ...
        from roveagent.core.chat_completion_helpers import interruptible_api_call
    ...
    def _emit_interim_assistant_message(
        ...
        try:
            from roveagent.core.plugin_stream_hooks import enqueue_plugin_stream_hook
    ...
    def _fire_reasoning_delta(self, text: str) -> None:
        ...
        if cb is not None:
            try:
                ...
                pass
        try:
            from roveagent.core.plugin_stream_hooks import enqueue_plugin_stream_hook, stream_reasoning_deltas_enabled
    ...
    def _has_stream_consumers(self) -> bool:
        ...
        try:
            from roveagent.core.plugin_stream_hooks import has_stream_observer_hooks
    ...
    def _interruptible_streaming_api_call(
        ...
        from roveagent.core.chat_completion_helpers import interruptible_streaming_api_call
        return interruptible_streaming_api_call(self, api_kwargs, on_first_delta=on_first_delta)

    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        """Forwarder — see ``agent.chat_completion_helpers.try_activate_fallback``."""
        from roveagent.core.chat_completion_helpers import try_activate_fallback
    ...
    def _restore_primary_runtime(self) -> bool:
        ...
        from roveagent.core.agent_runtime_helpers import restore_primary_runtime
    ...
    def _try_recover_primary_transport(
        ...
        from roveagent.core.agent_runtime_helpers import try_recover_primary_transport
    ...
    def _describe_image_for_anthropic_fallback(self, image_url: str, role: str) -> str:
        ...
        description = ""
        try:
            from roveagent.tools.vision_tools import vision_analyze_tool
    ...
    def _model_supports_vision(self) -> bool:
        ...
        try:
            ...
            from roveagent.core.image_routing import _lookup_supports_vision
            cfg = load_config()
    ...
    def _get_transport(self, api_mode: str = None):
        ...
        t = cache.get(mode)
        if t is None:
            from roveagent.core.transports import get_transport
    ...
    def _try_shrink_image_parts_in_messages(
        ...
        *,
        ...
        from roveagent.core.conversation_compression import try_shrink_image_parts_in_messages
    ...
    def _build_api_kwargs(self, api_messages: list, tools_for_api: Optional[list] = None) -> dict:
        ...
        from roveagent.core.chat_completion_helpers import build_api_kwargs
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from roveagent.clisupport.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from roveagent.clisupport.models import lmstudio_model_reasoning_options
    ...
    def _ollama_supports_thinking_cached(self) -> bool:
        ...
        try:
            from roveagent.clisupport.models import ollama_model_supports_thinking
    ...
    def _resolve_lmstudio_summary_reasoning_effort(self) -> Optional[str]:
        ...
        """
        from roveagent.core.lmstudio_reasoning import resolve_lmstudio_effort
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        from roveagent.core.chat_completion_helpers import build_assistant_message
    ...
    def _needs_kimi_tool_reasoning(self) -> bool:
        ...
        """
        from roveagent.core.message_sanitization import matches_reasoning_echo_family
    ...
    def _copy_reasoning_content_for_api(self, source_msg: dict, api_msg: dict) -> None:
        ...
        from roveagent.core.agent_runtime_helpers import copy_reasoning_content_for_api
    ...
    def _reapply_reasoning_echo_for_provider(self, api_messages: list) -> int:
        ...
        from roveagent.core.agent_runtime_helpers import reapply_reasoning_echo_for_provider
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict, model: "str | None" = None) -> dict:
        ...
        if not isinstance(tool_calls, list):
            return api_msg
        from roveagent.core.transports.chat_completions import _model_consumes_thought_signature
    ...
    def _sanitize_tool_call_arguments(
        ...
        *,
        ...
        from roveagent.core.agent_runtime_helpers import sanitize_tool_call_arguments
    ...
    def _compress_context(
        ...
        *,
        ...
        from roveagent.core.portal_tags import (
        ...
        try:
            ...
            from roveagent.core.context_compressor import _DB_PERSISTED_MARKER
            from roveagent.core.conversation_compression import (
            ...
            # id rotation updated roveagent_logging._session_context (a
            ...
            try:
                from roveagent.logsetup import set_session_context
            ...
            # ROVEAGENT_SESSION_ID to the child id after an out-of-place
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        try:
            ...
            from roveagent.core.tool_dispatch_helpers import _plan_tool_batch_segments
            ...
            from roveagent.core.tool_executor import execute_tool_calls_segmented
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        from roveagent.tools.delegate_tool import (
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        from roveagent.core.agent_runtime_helpers import invoke_tool
        return invoke_tool(
            self,
            function_name,
            function_args,
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from roveagent.core.tool_executor import execute_tool_calls_concurrent
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        from roveagent.core.tool_executor import execute_tool_calls_sequential
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        from roveagent.core.chat_completion_helpers import handle_max_iterations
    ...
    def run_conversation(
        ...
        from roveagent.core.background_review import cancel_background_review_for_live_turn

        cancel_background_review_for_live_turn(self)

        from roveagent.core.aux_accounting import (
            reset_accounting_context,
            set_accounting_context,
        )
        from roveagent.core import relay_runtime
        from roveagent.core.conversation_loop import run_conversation
        from roveagent.core.portal_tags import (
            reset_conversation_context,
            set_conversation_context,
        )
        from roveagent.clisupport.observability.relay_shared_metrics import (
            finish_task_run,
            start_task_run,
        )
        from roveagent.core.subagent_lifecycle import bind_subagent_parent
        ...
        try:
            # Serialize the full load -> run -> flush region across RoveAgent
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another RoveAgent process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other RoveAgent process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another RoveAgent process on "
                    ...
                    timeout_msg = (
                        "⏳ Another RoveAgent process kept this session busy too "
            ...
            from roveagent.core.auxiliary_client import scoped_runtime_main
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        from roveagent.core.codex_runtime import run_codex_app_server_turn
...
def main(
    ...
    # Handle tool listing
    if list_tools:
        from roveagent.model_tools import get_all_tool_names, get_available_toolsets
        from roveagent.toolsets import get_all_toolsets, get_toolset_info
