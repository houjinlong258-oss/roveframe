...
metadata:
  roveagent:
    ...
    related_skills: [claude-code, roveagent-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the RoveAgent terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For RoveAgent itself, `model.provider: openai-codex` uses RoveAgent-managed Codex
OAuth from `~/.roveagent/auth.json` after `roveagent auth add openai-codex`. For the
...
## RoveAgent Gateway Caveat

When invoking the Codex CLI from a RoveAgent gateway/service context (for example,
