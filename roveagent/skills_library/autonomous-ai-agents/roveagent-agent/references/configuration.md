...
Edit with `roveagent config edit` or `roveagent config set section.key value`.
Full reference: https://roveagent-agent.nousresearch.com/docs/user-guide/configuration
...
`roveagent config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `roveagent tools` (interactive) or `roveagent tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ROVEAGENT_CORE_TOOLS` is the default bundle most platforms inherit).
