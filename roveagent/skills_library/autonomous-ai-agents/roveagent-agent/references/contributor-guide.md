...
For occasional contributors and PR authors. Full developer docs: https://roveagent-agent.nousresearch.com/docs/developer-guide/
...
├── cli.py                # Interactive CLI (RoveAgentCLI)
├── roveagent_state.py       # SQLite session store
├── agent/                # Prompt builder, context compression, memory, model routing, credential pooling, skill dispatch
├── roveagent_cli/           # CLI subcommands, config, setup, commands
...
Config: `~/.roveagent/config.yaml` (settings), `~/.roveagent/.env` (API keys) — both under `$ROVEAGENT_HOME` when it is set.
...
`_ROVEAGENT_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_roveagent_home()` for paths,
never hardcode `~/.roveagent`. For custom/local-only tools, write a plugin in
`~/.roveagent/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `roveagent_cli/commands.py`
...
- Tests auto-redirect `ROVEAGENT_HOME` to temp dirs — never touch real `~/.roveagent/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/roveagent_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_roveagent_home()` from `roveagent_constants` for all paths (profile-safe)
