...
Write plugins for the RoveAgent desktop app: statusbar items, layout panes,
...
system (`~/.roveagent/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ROVEAGENT_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ROVEAGENT_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The RoveAgent desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ROVEAGENT_HOME/desktop-plugins/` (usually
  `~/.roveagent/desktop-plugins/`).

## How to Run

1. Create `$ROVEAGENT_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.roveagent/...` by default, or `~/.roveagent/profiles/<profile>/...` under a
...
The ONLY import surface is `@roveagent/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from RoveAgent
  ...
  (`roveagent://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.roveagent/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@roveagent/plugin-sdk`, `react`, and
