...
## Layer 1 — Is this thing a RoveAgent plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the RoveAgent process**. Its LLM calls go through RoveAgent's already-configured provider. | Already uses Portal if user's RoveAgent is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside RoveAgent. That LLM call is already authenticated
via RoveAgent's provider config. The plugin's own server auth (e.g.
...
What RoveAgent Agent has that *feels* like OAuth — `roveagent login --provider nous`
opening a browser, user signs in, token lands in `~/.roveagent/auth.json` — is a
**RoveAgent-specific browser flow**. Under the hood it produces a credential
RoveAgent uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read RoveAgent's existing Portal credential out of `~/.roveagent/auth.json`.
...
rides on the credential RoveAgent already holds.

Where this could live in RoveAgent:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for RoveAgent itself.
...
1. First decide: RoveAgent plugin (runs inside RoveAgent) or separate app? If plugin,
   it already uses Portal via RoveAgent's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   RoveAgent agent itself") is fully supported. RoveAgent treats agent
...
pitch in the Tool Gateway announcement is specifically about RoveAgent Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when RoveAgent is configured to use Portal as its provider. It is
