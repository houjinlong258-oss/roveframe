...
Set via `roveagent model` (picker) or `roveagent setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://roveagent-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `roveagent auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `roveagent auth add openai-codex` |
| qwen-oauth | OAuth | `roveagent auth add qwen-oauth` |
| minimax-oauth | OAuth | `roveagent auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`roveagent auth`).
Fallback chain when the primary fails: `roveagent fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`roveagent_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   roveagent config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
