# Themes / Skins — Author a RoveAgent Color Theme

Author a RoveAgent **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`roveagent_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.roveagent/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for RoveAgent itself.
...
- Write access to the RoveAgent home dir — `~/.roveagent` by default, or `$ROVEAGENT_HOME`
  / the active profile's dir. Skins live in `<roveagent-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `roveagent config set`).
...
3. `write_file` it to `<roveagent-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `roveagent config set display.skin default` (or they can `/skin default`).
...
```
roveagent skin set <key> <hex>      # e.g. roveagent skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `roveagent skin set background "#08201f"` changes
only the background; `roveagent skin use <name>` / `roveagent skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.roveagent`** when a profile is active — resolve the real home
  from `$ROVEAGENT_HOME` first, falling back to `~/.roveagent`.
...
- **Never hand-edit `config.yaml` to activate.** Use `roveagent config set
...
- **You apply it, not the user.** `roveagent config set display.skin <name>` is
...
- `read_file` the written `<roveagent-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `roveagent config get display.skin` and confirm it reports `<name>`.
