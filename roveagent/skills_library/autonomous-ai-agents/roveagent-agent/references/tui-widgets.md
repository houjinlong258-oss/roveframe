...
Author widget apps for the RoveAgent TUI (`roveagent --tui`): glanceable ambient
...
- The TUI must be in use (`roveagent --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.roveagent/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.roveagent/logs/tui_gateway_crash.log` if a widget never appears.
