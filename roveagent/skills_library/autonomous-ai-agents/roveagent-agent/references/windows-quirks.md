...
RoveAgent runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`roveagent config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually RoveAgent's env scrubber dropping
...
RoveAgent-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every RoveAgent tool and most Windows APIs. Prefer forward slashes in code
