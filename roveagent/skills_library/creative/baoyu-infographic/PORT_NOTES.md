...
| Change | Upstream | RoveAgent |
|--------|----------|--------|
| Metadata namespace | `openclaw` | `roveagent` |
| Trigger | `/baoyu-infographic` slash command | Natural language skill matching |
| User config | EXTEND.md file (project/user/XDG paths) | Removed — not part of RoveAgent infra |
...
| File operations | Bash commands | RoveAgent file tools (write_file, read_file) |
...
Reference files can be overwritten directly (they're unchanged from upstream). SKILL.md must be manually merged since it contains RoveAgent-specific adaptations.
