#!/usr/bin/env python3
"""Google Workspace API CLI for RoveAgent Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing RoveAgent-facing JSON contract and falls back to the Python client
...
from _roveagent_home import get_roveagent_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ROVEAGENT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for RoveAgent Agent")
