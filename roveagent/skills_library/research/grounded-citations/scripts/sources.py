...
Ledger path resolution (first wins):
  --ledger PATH
  $ROVEAGENT_CITATION_LEDGER
  $ROVEAGENT_HOME/cache/citations/ledger.json
"""
...
from _roveagent_home import get_roveagent_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ROVEAGENT_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_roveagent_home() / "cache" / "citations" / "ledger.json"
