...
- Give it a name like "roveagent-agent"
- Select scopes:
...
- Give it a title like "roveagent-agent-<machine-name>"
...
uv run python "${ROVEAGENT_HOME:-$HOME/.roveagent}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _roveagent_env="${ROVEAGENT_HOME:-$HOME/.roveagent}/.env"; [ -f "$_roveagent_env" ] && grep -q "^GITHUB_TOKEN=" "$_roveagent_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_roveagent_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python "${ROVEAGENT_HOME:-$HOME/.roveagent}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
