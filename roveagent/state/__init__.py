#!/usr/bin/env python3
"""
SQLite State Store for RoveAgent Agent.
...
from roveagent.core.message_sanitization import _sanitize_surrogates
...
# transitive dependency via roveagent_state_common). run_agent keeps its own
# predating copy — roveagent_state cannot import run_agent (circular) — guarded
...
from roveagent.state.portability import SessionPortabilityMixin
from roveagent.state.schema import SessionSchemaMixin
from roveagent.state.search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``roveagent -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_roveagent_home() / "state.db"
...
#
# The ceilings above bound RoveAgent's SQLite descriptors, which is only ever part
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.roveagent`` even when a test fixture later
    redirects ``ROVEAGENT_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_roveagent_home()`` fresh so a runtime
       ``ROVEAGENT_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_roveagent_home() / "state.db"
...
# ~/.roveagent/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects ROVEAGENT_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without ROVEAGENT_HOME, a stale worktree
# without the re-pin, or a developer shell that exports ROVEAGENT_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "ROVEAGENT_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.roveagent``).  The test conftest injects the pre-sandbox production
#: root here so custom-``ROVEAGENT_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default RoveAgent root for the guard.

    Deliberately avoids ``Path.home()`` / ``roveagent_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``roveagent_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "roveagent"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "roveagent"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".roveagent"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``ROVEAGENT_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under RoveAgent test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``ROVEAGENT_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "ROVEAGENT_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``ROVEAGENT_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``roveagent`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real RoveAgent home *root*.
    ...
    under ``~/.roveagent/roveagent-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``ROVEAGENT_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``ROVEAGENT_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real RoveAgent root {root}). "
                "Tests must run against a temporary ROVEAGENT_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "ROVEAGENT_HOME. If this test genuinely needs the live "
...
# roveagent_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``roveagent sessions optimize`` (FTS merge + VACUUM)
    ...
    ``roveagent_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``roveagent_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``roveagent update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"RoveAgent-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart RoveAgent"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a RoveAgent-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `roveagent doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``roveagent_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see roveagent_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with roveagent_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``roveagent doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal RoveAgent host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``roveagent serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — RoveAgent's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``roveagent_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship roveagent_state without roveagent_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``roveagent_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the RoveAgent home tree (``get_roveagent_home()``) — the safe repair scope:
      RoveAgent owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``roveagent_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_roveagent_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. RoveAgent needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (roveagent_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_roveagent_state.py:600-632)
        ...
        probe_session_id = f"_roveagent_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, RoveAgent deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (roveagent gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.roveagent/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``roveagent sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("ROVEAGENT_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_roveagent_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("ROVEAGENT_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship roveagent_state without roveagent_cli). A
    ...
    try:
        ...
        logger.debug(
            "roveagent.clisupport.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch roveagent_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``roveagent_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from roveagent.clisupport.backup import is_zeroed_sqlite_file
    ...
    if size < 0:
        return False
    from roveagent.clisupport.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error(
                ...
                "state-snapshots via `roveagent snapshot list` / "
                "`roveagent snapshot restore <id>`.",
                path,
            )
            return None
        return _do_quarantine()
...
# ── Read-only health/stats probes (roveagent doctor, dashboards) ──────────
...
_ROVEAGENT_CMDLINE_MARKERS = ("roveagent.clisupport.main", "roveagent_cli/main", "roveagent serve",
                           "roveagent-agent", "roveagent gateway", "roveagent chat")


def _looks_like_roveagent(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a RoveAgent process?
    ...
    We only flag processes that look like RoveAgent, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _ROVEAGENT_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple roveagent processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling RoveAgent
    ...
    # `roveagent update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _handle_quarantine_if_zeroed(already_locked: bool = False):
                if (
                    ...
                    msg = (
                        ...
                        f"Restore from {snaps} via `roveagent snapshot list` / "
                        f"`roveagent snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`roveagent db optimize`),
            ...
            # ``roveagent_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `roveagent update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `roveagent sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another RoveAgent process held the "
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a RoveAgent process —
                        # only flag uninspectable holders that look like
                        # another RoveAgent instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_roveagent(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from roveagent.clisupport.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `roveagent sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`roveagent sessions optimize-storage`) ──
    #
    ...
    def _own_profile_name(self) -> Optional[str]:
        ...
        try:
            ...
            root = get_default_roveagent_root().resolve()
    ...
    # offline repair path behind ``roveagent sessions repair-routing``.
    ...
    def get_session_activity(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not row:
            return None
        from roveagent.core.session_activity import build_activity_snapshot
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``roveagent --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``roveagent serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: roveagent_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def append_messages_batch(
        ...
        def _do(conn):
            ...
            from roveagent.core.transcript_repair import resolve_and_repair_transcript_batch
    ...
    def get_messages(
        ...
        if include_compacted:
            ...
            for row in all_rows:
                ...
                if row["role"] == "user":
                    from roveagent.core.context_compressor import split_user_originated_turn
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: roveagent_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``roveagent -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``roveagent sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``ROVEAGENT_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade RoveAgent, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one RoveAgent session.

        A RoveAgent session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a RoveAgent session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `roveagent sessions vacuum` runs in a transient
