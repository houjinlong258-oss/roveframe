...
Extracted verbatim from roveagent_state.py so the SessionDB mixin modules
(roveagent_state_search / roveagent_state_schema / roveagent_state_portability) can
reference them without importing roveagent_state (which would be a cycle).
roveagent_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``roveagent sessions optimize-storage``. A legacy DB sits at
...
# `roveagent sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `roveagent db optimize`. This is
...
#
# Several independent RoveAgent processes routinely share one state.db (gateway
# service, the Desktop app's `roveagent serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `roveagent_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not roveagent_state)
# because the search/schema mixins cannot import roveagent_state (cycle).
#
...
logger = logging.getLogger("roveagent_state")
