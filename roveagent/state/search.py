...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: ROVEAGENT_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("ROVEAGENT_SEARCH_SLOW_MS", "1000"))
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another RoveAgent surface and jump
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``ROVEAGENT_DISABLE_FTS_TRIGRAM`` or not yet created), so
