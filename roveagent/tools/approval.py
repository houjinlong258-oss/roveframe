...
from roveagent.utils import env_var_enabled, is_truthy_value
...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("ROVEAGENT_YOLO_MODE", ""))
...
# RoveAgent session id (observability identity, distinct from the gateway
...
# os.environ["ROVEAGENT_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export ROVEAGENT_INTERACTIVE.
_roveagent_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "roveagent_interactive",
    default=None,
)


def set_roveagent_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["ROVEAGENT_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``ROVEAGENT_INTERACTIVE`` env var for legacy callers.
    """
    return _roveagent_interactive_ctx.set("1" if interactive else "")


def reset_roveagent_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_roveagent_interactive_context`."""
    _roveagent_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``ROVEAGENT_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _roveagent_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("ROVEAGENT_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the RoveAgent session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ROVEAGENT_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("ROVEAGENT_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("ROVEAGENT_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("ROVEAGENT_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("ROVEAGENT_CRON_SESSION")
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    ``ROVEAGENT_SESSION_PLATFORM`` like chat gateways do, but there is no human
...
def _is_single_query_approval_context() -> bool:
    ...
    ``roveagent chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``ROVEAGENT_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("ROVEAGENT_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("ROVEAGENT_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ROVEAGENT_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ROVEAGENT_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ROVEAGENT_SESSION_PLATFORM via
    ...
    if _is_unattended_platform_approval_context():
        return False
    if env_var_enabled("ROVEAGENT_GATEWAY_SESSION"):
        return True
...
def _resolve_cli_approval_callback(approval_callback=None):
    ...
    try:
        from roveagent.tools.terminal_tool import _get_approval_callback
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``ROVEAGENT_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $ROVEAGENT_HOME, or by the resolved absolute
# active profile home path such as /home/roveagent/.roveagent/config.yaml. The
# resolved-absolute form is folded into the ~/.roveagent/ patterns at detection
...
# go stale when ROVEAGENT_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ROVEAGENT_ENV_PATH = (
    r'(?:~\/\.roveagent/|'
    r'(?:\$home|\$\{home\})/\.roveagent/|'
    r'(?:\$roveagent_home|\$\{roveagent_home\})/)'
    r'\.env\b'
)
# ~/.roveagent/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _ROVEAGENT_ENV_PATH; matches the ROVEAGENT_HOME override form as
# well as ~/.roveagent/.
_ROVEAGENT_CONFIG_PATH = (
    r'(?:~\/\.roveagent/|'
    r'(?:\$home|\$\{home\})/\.roveagent/|'
    r'(?:\$roveagent_home|\$\{roveagent_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_ROVEAGENT_ENV_PATH}|'
    rf'{_ROVEAGENT_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_roveagent_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by RoveAgent: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY RoveAgent
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]roveagent[^\n]*\.env\b', "access to RoveAgent secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `roveagent` and `gateway` (e.g. `roveagent -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\broveagent\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart roveagent gateway (kills running agents)"),
    (r'\broveagent\s+update\b', "roveagent update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `roveagent gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `roveagent ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart roveagent-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart roveagent-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(roveagent|gateway|cli\.py)\b', "kill roveagent/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill roveagent` but not
    # `kill -9 $(pgrep -f roveagent)` because the substitution is opaque
    ...
    # the `roveagent gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.roveagent.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "roveagent"/"ai.roveagent" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.roveagent.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "roveagent" appear
    ...
    # an unrelated "roveagent" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:roveagent|ai\.roveagent)\b)', "stop/restart roveagent launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/RoveAgent file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.roveagent/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a RoveAgent-managed security file (~/.roveagent/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_ROVEAGENT_CONFIG_PATH}|{_ROVEAGENT_ENV_PATH})', "in-place edit of RoveAgent config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_ROVEAGENT_CONFIG_PATH}|{_ROVEAGENT_ENV_PATH})', "in-place edit of RoveAgent config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_ROVEAGENT_CONFIG_PATH}|{_ROVEAGENT_ENV_PATH})', "in-place edit of RoveAgent config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    """
    from roveagent.tools.ansi_strip import strip_ansi
    ...
    # ~/ and ~/.roveagent/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / ROVEAGENT_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) RoveAgent home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\roveagent), so folding the user home
    # first would eat the prefix the RoveAgent-home fold needs.
    command = _rewrite_resolved_roveagent_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / ROVEAGENT_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.roveagent``); the matched
...
def _rewrite_resolved_roveagent_home(command: str) -> str:
    """Rewrite the resolved absolute RoveAgent home prefix to ``~/.roveagent/``.

    Resolves the active ``ROVEAGENT_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.roveagent/`` so the static ``_ROVEAGENT_CONFIG_PATH`` / ``_ROVEAGENT_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/roveagent/.roveagent/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$ROVEAGENT_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from roveagent.constants import get_roveagent_home
        home = get_roveagent_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.roveagent")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``roveagent send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one RoveAgent ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"roveagent-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart roveagent gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.roveagent.gateway`` auto-approved
    ...
    anchored on a roveagent-gateway identifier — reusing its prose
    ...
    """
    try:
        from roveagent.cron.lifecycle_guard import contains_gateway_lifecycle_command
    except Exception:
        return False
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from RoveAgent YOLO.
...
def clear_session(session_key: str) -> None:
    ...
    try:
        from roveagent.tools.code_kernel import shutdown_kernels_for_owner
    ...
    try:
        from roveagent.tools.code_kernel_remote import shutdown_remote_kernels_for_owner
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["ROVEAGENT_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "ROVEAGENT_SPINNER_PAUSE" in os.environ:
            del os.environ["ROVEAGENT_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def _get_approval_mode() -> str:
    ...
    try:
        from roveagent.gateway.hosted_room_execution_policy import (
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses RoveAgent approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``ROVEAGENT_YOLO_MODE`` (frozen at import time
...
def _get_approval_timeout() -> int:
    ...
    try:
        ...
        return 300
    try:
        from roveagent.core.deadline import MAX_SAFE_TIMEOUT_S
...
def _smart_approve(command: str, description: str) -> str:
    ...
    try:
        from roveagent.core.auxiliary_client import _get_task_timeout, call_llm
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            ROVEAGENT_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export ROVEAGENT_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set ROVEAGENT_INTERACTIVE or "
                "ROVEAGENT_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set ROVEAGENT_INTERACTIVE or "
            "ROVEAGENT_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("ROVEAGENT_EXEC_ASK"):
        ...
        # can see (ROVEAGENT_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``ROVEAGENT_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from roveagent.clisupport.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from roveagent.core.redact import redact_sensitive_text
        from roveagent.clisupport.approval_transport import ApprovalRequest, invoke_approval_transport
    ...
    try:
        from roveagent.tools.environments.base import touch_activity_if_due
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ROVEAGENT_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # ROVEAGENT_EXEC_ASK routes through the gateway decision loop (no human
    ...
    if not is_cli and not is_gateway and not is_ask:
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    from roveagent.tools.tirith_security import check_command_security
                    ...
                    try:
                        from roveagent.clisupport.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (ROVEAGENT_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # ROVEAGENT_EXEC_ASK (and sometimes a session platform marker) can leak
