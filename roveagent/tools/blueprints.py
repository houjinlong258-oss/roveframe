...
`roveagent skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.roveagent.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.roveagent.blueprint`` is a mapping containing
    ...
    roveagent = meta.get("roveagent") if isinstance(meta, dict) else None
    blueprint = roveagent.get("blueprint") if isinstance(roveagent, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.roveagent.blueprint must be a mapping")
...
def register_blueprint_suggestion(spec: BlueprintSpec) -> Optional[Dict[str, Any]]:
    ...
    if not spec.skill_name:
        return None
    try:
        from roveagent.cron.suggestions import add_suggestion
    except Exception:  # pragma: no cover - import guard
        return None
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.roveagent.blueprint`` block) they can hand
    to ``roveagent skills publish`` to share. ``body`` is the plain-language
