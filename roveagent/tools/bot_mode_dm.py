...
teammate agent (another RoveAgent profile on this install, or an agent on a
...
DM each other via a prompt-injected ``roveagent -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``roveagent -p`` / ``roveagent peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``roveagent -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``roveagent peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'roveagent' for the default agent), or "
        ...
        if tools:
            for tool in tools:
                if (
                    ...
                    return True
        from roveagent.tools.bot_mode_probe import BOT_CHAT_TITLE, is_bot_mode_managed
...
def _peers(root: Path) -> list[str]:
    try:
        from roveagent.tools.bot_mode_probe import _peers as _probe_peers
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('roveagent' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "roveagent":
...
def _err(message: str, *, roster: list[str] | None = None, peers: list[str] | None = None) -> str:
    from roveagent.tools.bot_failure_reasons import classify_agent_error
...
def message_agent_tool(
    ...
    root = _roveagent_root(Path(home))
    ...
    if peer_match or bare_peer:
        ...
        # Pin the registry-owning profile (#93935): `roveagent peer` resolves
...
def _try_relay_delivery(
    ...
    *,
    ...
    """
    try:
        from roveagent.tools.bot_relay import (
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "roveagent-dm-*.txt"),
        (temp_root, "roveagent-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``roveagent -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative roveagent next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("roveagent", "roveagent.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from roveagent.tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("ROVEAGENT_HOME") or os.path.expanduser("~/.roveagent"))
    return acquire_turn_lock(_roveagent_root(home), argv[2])
    ...
    try:
        with _delivery_lock(argv, stdin_file=stdin_file):
            ...
            if proc.returncode != 0:
                from roveagent.tools.bot_failure_reasons import (
                    RETRY_NONE,
...
def _spawn_delivery(
    ...
    *,
    ...
    transferred = False
    try:
        from roveagent.tools.terminal_tool import terminal_tool
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("ROVEAGENT_HOME") or os.path.expanduser("~/.roveagent")
