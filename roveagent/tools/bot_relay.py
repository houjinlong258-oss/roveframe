...
RoveAgent Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own ROVEAGENT root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in roveagent_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "roveagent" if profile == "default" else profile
    if (
...
def _roveagent_cli() -> str:
    """Resolve the roveagent CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``roveagent``
    entrypoint. A bare ``"roveagent"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("roveagent.exe" if sys.platform == "win32" else "roveagent")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("roveagent")
    if found:
        return found
    return "roveagent"
...
# turns concurrently: deliveries spawn separate ``roveagent`` subprocesses, so
