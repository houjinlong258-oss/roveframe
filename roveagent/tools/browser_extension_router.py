...
- **No server-bound identity ⇒ legacy.** Generic RoveAgent callers keep the
...
def extension_controller_available(action: str) -> bool:
    ...
    try:
        ...
        if not browser_control_enabled():
            return False
        session_id = get_session_env("ROVEAGENT_SESSION_ID", "") or None
        principal_id = get_session_env("ROVEAGENT_BROWSER_CONTROL_PRINCIPAL", "") or None
        transport_family = get_session_env(
            "ROVEAGENT_BROWSER_CONTROL_TRANSPORT_FAMILY", ""
        ) or None
    ...
    *,
    ...
    if scope is None:
        ...
        if callable(lane_bound) and not lane_bound(
            ...
            return fallback()
        from roveagent.gateway.browser_control_broker import ControllerUnavailable
...
def current_tool_call_id() -> str:
    ...
    """
    try:
        from roveagent.tools.approval import _approval_tool_call_id
...
def routed_browser_handler(
    ...
    *,
    ...
    try:
        ...
        session_id = session_id or get_session_env("ROVEAGENT_SESSION_ID", "") or None
        principal_id = principal_id or get_session_env(
            "ROVEAGENT_BROWSER_CONTROL_PRINCIPAL", ""
        ) or None
