...
One ``CDPSupervisor`` runs per RoveAgent ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach RoveAgent logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "roveagent-dialog-bridge.invalid"
...
(() => {
  if (window.__roveagentDialogBridgeInstalled) return;
  window.__roveagentDialogBridgeInstalled = true;
  const ENDPOINT = "http://roveagent-dialog-bridge.invalid/";
