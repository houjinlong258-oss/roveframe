...
Usage:
    from roveagent.tools.browser_tool import browser_navigate, browser_snapshot, browser_click
...
from roveagent.core.redact import redact_cdp_url
from roveagent.constants import (
    agent_browser_runnable,
    get_roveagent_home,
    get_roveagent_home_override,
    roveagent_home_key,
    node_tool_runnable,
)
from roveagent.utils import env_int, is_truthy_value
from roveagent.clisupport.config import DEFAULT_CONFIG, cfg_get
...
def __getattr__(name: str):
    ...
    (``patch("roveagent.tools.browser_tool.requests.get")`` /
    ``patch("roveagent.tools.browser_tool.call_llm")``): first attribute access imports
    ...
    if name == "requests":
        ...
        return _requests
    if name == "call_llm":
        from roveagent.core.auxiliary_client import call_llm as _call_llm
...
# means a compromised transitive dependency could read every RoveAgent secret
...
def _build_browser_env() -> dict:
    ...
    Strips RoveAgent-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``roveagent_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    """
    from roveagent.tools.environments.local import roveagent_subprocess_env

    env = roveagent_subprocess_env(inherit_credentials=False)
...
try:
    from roveagent.tools.website_policy import check_website_access
except Exception:
...
from roveagent.core.browser_provider import BrowserProvider as CloudBrowserProvider  # noqa: F401  (legacy alias)
from roveagent.core.browser_registry import (  # noqa: F401  (test-patchable surface)
...
try:
    ...
    def _browser_registry_generation(*, scope=None):
        return (0, 0)
from roveagent.plugins.browser.browserbase.provider import (  # noqa: F401  (legacy import surface)
    BrowserbaseBrowserProvider as BrowserbaseProvider,
)
from roveagent.plugins.browser.browser_use.provider import (  # noqa: F401
    BrowserUseBrowserProvider as BrowserUseProvider,
)
from roveagent.plugins.browser.firecrawl.provider import (  # noqa: F401
    FirecrawlBrowserProvider as FirecrawlProvider,
)
from roveagent.tools.tool_backend_helpers import normalize_browser_cloud_provider
# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from roveagent.tools.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
    _is_camofox_mode = lambda: False  # noqa: E731
# Browser Use CLI (optional)
try:
    from roveagent.tools.browser_use_cli import is_browser_use_cli_mode as _is_browser_use_cli_mode
except ImportError:
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    roveagent_node_bin = str(roveagent_home / "node" / "bin")
    roveagent_node_root = str(roveagent_home / "node")
    roveagent_nm_bin = str(roveagent_home / "node_modules" / ".bin")
    return [roveagent_node_bin, roveagent_node_root, roveagent_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``roveagent doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
def _get_dialog_policy_config() -> Tuple[str, float]:
    ...
    from roveagent.tools.browser_supervisor import (
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    if not cdp_url:
        return
    try:
        from roveagent.tools.browser_supervisor import SUPERVISOR_REGISTRY  # type: ignore[import-not-found]
...
# plugins drop in under ``~/.roveagent/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active RoveAgent profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.roveagent/plugins/browser/<vendor>/``) participate
        ...
        if provider_key:
            try:
                ...
                if resolved is None:
                    ...
                    from roveagent.tools.tool_backend_helpers import selection_error
...
from roveagent.constants import is_termux as _is_termux_environment
...
# comparison sites (four here, plus roveagent_cli/tools_config.py and
# roveagent_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    if bu_mode:
        return True, "Browser Use mode: RoveAgent spawns `lightpanda serve` per session"
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when RoveAgent silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — RoveAgent-managed-Node-only setups
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the RoveAgent CLI where sys.stdout and sys.stderr both report
...
_REAL_PROFILE_SESSION = "roveagent-real-profile"
...
def _cdp_http_ready(http_cdp: str) -> bool:
    ...
    try:
        from roveagent.clisupport.browser_connect import is_browser_debug_ready
...
def _real_profile_cdp() -> tuple:
    ...
    Snapshots the user's default-Chromium profile into a roveagent-owned copy
    ...
    if not _use_real_profile():
        ...
        try:
            from roveagent.clisupport.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous roveagent process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from roveagent.clisupport.browser_connect import _PROFILE_LOCKED_PREFIX
            ...
            if err and err.startswith(_PROFILE_LOCKED_PREFIX):
                ...
                return None, (
                    ...
                    "`roveagent browser close-profile`, then retry."
                )
        ...
        from roveagent.clisupport.browser_connect import chromium_executable
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-roveagent_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a roveagent
...
# Hard ceiling for a daemon whose owning roveagent process is still alive but
...
def _emergency_cleanup_all_sessions():
    ...
    crashed roveagent processes — this way every clean roveagent exit sweeps
    ...
    try:
        from roveagent.tools.browser_lightpanda import stop_all_lightpanda

        stop_all_lightpanda()
    except Exception as e:
        logger.debug("Lightpanda cleanup on exit failed: %s", e)

    # Sweep orphans from other crashed roveagent processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live roveagent processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current roveagent PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live roveagent process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    roveagent restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning roveagent process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced roveagent PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent roveagent instances won't reap each
         other's daemons.
    ...
    try:
        from roveagent.tools.browser_lightpanda import reap_orphaned_lightpanda
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-roveagent_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live roveagent process and must not be touched — but "owner alive"
        ...
        try:
            ...
            from roveagent.tools.process_registry import ProcessRegistry
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # browser running on a roveagent-owned SNAPSHOT of their real profile —
    ...
    # ``browser.engine: lightpanda`` that endpoint is a RoveAgent-spawned
...
def _create_lightpanda_session(task_id: str) -> Dict[str, Any]:
    ...
    import uuid
    from roveagent.tools.browser_lightpanda import launch_lightpanda
...
def _local_backend_process_dead(session_info: Dict[str, Any]) -> bool:
    ...
    if not (session_info.get("features") or {}).get("lightpanda"):
        return False
    from roveagent.tools.browser_lightpanda import get_server
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the RoveAgent-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy RoveAgent-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_roveagent_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, RoveAgent-managed
    ...
    # node_modules binary, which disappears on the next ``roveagent update`` and
    ...
    # Build an extended search PATH including RoveAgent-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_roveagent_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_roveagent_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_roveagent_home() / "node")),
            ]
...
def _kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    """
    try:
        from roveagent.core.deadline import kill_process_tree as _deadline_kill_tree
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``roveagent update`` (or ``roveagent doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential RoveAgent holds, while running registry-fetched
    npm code on every ``roveagent update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `roveagent update`/`doctor
    ...
    if session_name:
        ...
        if os.path.isfile(pid_file):
            try:
                ...
                from roveagent.core import deadline as _deadline
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this roveagent PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        from roveagent.tools.spill_safety import ensure_spill_dir, write_text_exclusive

        cache_dir = get_roveagent_dir("cache/web", "web_cache")
...
def evaluate_url_safety(url: str) -> Optional[dict]:
    ...
    import urllib.parse
    from roveagent.core.redact import _PREFIX_RE
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_type
    ...
    from roveagent.core.display import (
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_console
...
def _camofox_current_page_private_url(tab_id: str, user_id: str) -> Optional[str]:
    ...
    """
    try:
        from roveagent.tools.browser_camofox import _post
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from roveagent.tools.browser_camofox import _ensure_tab, _post
    try:
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = roveagent_home / "browser_recordings"
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise RoveAgent
    ...
    """
    if _is_camofox_mode():
        from roveagent.tools.browser_camofox import camofox_vision
        return camofox_vision(question, annotate, task_id)

    import base64
    import uuid as uuid_mod
    from roveagent.constants import get_roveagent_dir
    screenshots_dir = get_roveagent_dir("cache/screenshots", "browser_screenshots")
    ...
    try:
        ...
        from roveagent.tools.vision_tools import (
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if _is_camofox_mode():
        try:
            from roveagent.tools.browser_camofox import camofox_close, camofox_soft_cleanup
    ...
    if session_info:
        ...
        # A Lightpanda session is a process RoveAgent spawned itself (Browser
        ...
        if (session_info.get("features") or {}).get("lightpanda"):
            try:
                from roveagent.tools.browser_lightpanda import stop_lightpanda
...
def _maybe_autoinstall_chromium() -> bool:
    ...
    from roveagent.tools.lazy_deps import _allow_lazy_installs
...
def check_browser_vision_requirements() -> bool:
    ...
    if not check_browser_requirements():
        return False
    try:
        from roveagent.tools.vision_tools import check_vision_requirements
    except ImportError:
        return False
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/roveagent-agent:latest")
                else:
...
from roveagent.tools.browser_extension_router import (
