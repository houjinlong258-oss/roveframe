...
_PRIVATE_BROWSER_SENTINEL = "_ROVEAGENT_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# roveagent: pin this named session to its own tab (once per daemon process)
def _roveagent_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "roveagent-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_roveagent_ensure_own_tab()
del _roveagent_ensure_own_tab
"""
...
def _blocked_url_in_code(code: str) -> Optional[str]:
    ...
    from roveagent.tools.browser_tool import evaluate_url_safety
...
def _base_subprocess_env() -> dict:
    from roveagent.tools.browser_tool import _build_browser_env

    env = _build_browser_env()
    # The browser-use CLI runs under its own Python (uv tool / uvx), which
    # may differ from RoveAgent's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at RoveAgent's venv site-packages, and a
...
def _floor_subprocess_path(path: str) -> str:
    ...
    if os.name == "nt":
        return path
    try:
        from roveagent.tools.browser_tool import _merge_browser_path
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_roveagent_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `roveagent tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """RoveAgent' own bin dir ($ROVEAGENT_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from roveagent.constants import get_roveagent_home

        return str(Path(get_roveagent_home()) / "bin")
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: RoveAgent' own ``$ROVEAGENT_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    RoveAgent-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: RoveAgent' managed uv (bootstrapped on demand via
    ``roveagent_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$ROVEAGENT_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical RoveAgent-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through roveagent tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_roveagent_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_lightpanda_cdp(
    ...
    """Point the harness at a RoveAgent-spawned ``lightpanda serve``.
    ...
    """
    try:
        from roveagent.tools.browser_tool import _get_session_info, _using_lightpanda_engine
    ...
    try:
        ...
        return (
            ...
            "to use local Chrome, or switch backends via `roveagent tools` → "
            "Browser Automation."
        )
...
def _resolve_backend_cdp(
    ...
    4. ``browser.engine: lightpanda``: a RoveAgent-spawned ``lightpanda serve``
    ...
    try:
        ...
        return (
            ...
            "switch backends via `roveagent tools` → Browser Automation."
        )
    cdp = str((session_info or {}).get("cdp_url") or "")
    if not cdp:
        return (
...
def _real_profile_consented() -> bool:
    ...
    try:
        from roveagent.tools.browser_tool import _use_real_profile
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser RoveAgent launches on a
    SNAPSHOT of their real profile (see roveagent_cli.browser_connect). Two ways
    in:
...
def browser_exec(
    ...
    from roveagent.tools.registry import tool_error, tool_result
...
def _description_header() -> str:
    ...
    try:
        from roveagent.tools.vision_tools import _should_use_native_vision_fast_path
...
def _lightpanda_engine_in_use() -> bool:
    try:
        from roveagent.tools.browser_tool import lightpanda_engine_status
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a RoveAgent-managed copy of "
