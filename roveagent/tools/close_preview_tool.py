#!/usr/bin/env python3
"""Close the RoveAgent desktop GUI's preview pane, or one of its tabs.
...
from roveagent.tools.open_preview_tool import _normalize_target
...
def close_preview_tool(url: str = "") -> str:
    ...
    if not ok:
        return tool_error("The preview pane is only available in the RoveAgent desktop app.")
...
CLOSE_PREVIEW_SCHEMA = {
    "name": "close_preview",
    "description": (
        "Close the preview pane beside the chat in the RoveAgent desktop app, or one "
