...
Lets the LLM write a Python script that calls RoveAgent tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `roveagent_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `roveagent_tools.py` with file-based RPC stubs
...
from roveagent.core.thread_scoped_output import thread_scoped_silence

# Availability gate.  On Windows we fall back to loopback TCP for the
# sandbox RPC transport (AF_UNIX is unreliable on Windows Python) — see
# ``_use_tcp_rpc`` in ``_execute_local`` below.  That makes execute_code
# available on every platform RoveAgent itself runs on.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_roveagent_dir("cache/exec", "exec_spill")
        cache_dir.mkdir(parents=True, exist_ok=True)
        digest = hashlib.sha256(
            stdout_text.encode("utf-8", errors="replace")
        ).hexdigest()[:12]
        path = cache_dir / f"stdout-{digest}.txt"
        from roveagent.tools.spill_safety import write_text_exclusive
...
# match a safe prefix, the operational ROVEAGENT_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports RoveAgent
# code can lose the DB-layer Kanban mutation guard while still inheriting
# ROVEAGENT_HOME.
#
# NB: the broad "ROVEAGENT_" prefix was deliberately removed (#27303) — it leaked
# ROVEAGENT_*-named config that lacks a secret substring (e.g. ROVEAGENT_BASE_URL,
# ROVEAGENT_KANBAN_DB, ROVEAGENT_*_WEBHOOK).  The child only needs the few
# location/profile vars in _ROVEAGENT_CHILD_ALLOWED below; ROVEAGENT_RPC_SOCKET /
# ROVEAGENT_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational ROVEAGENT_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set roveagent_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_ROVEAGENT_CHILD_ALLOWED = frozenset({
    "ROVEAGENT_HOME",
    "ROVEAGENT_PROFILE",
    "ROVEAGENT_CONFIG",
    "ROVEAGENT_ENV",
    "ROVEAGENT_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational ROVEAGENT_* vars (_ROVEAGENT_CHILD_ALLOWED) pass by exact name.
    ...
    if is_passthrough is None:
        ...
        try:
            from roveagent.tools.env_passthrough import resolve_passthrough_value
        except Exception:
            resolve_passthrough_value = lambda _name, _fallback: None  # noqa: E731
    ...
    scrubbed = {}
    # Non-secret ROVEAGENT_* vars dropped by the tightened allowlist (#27303). The
    # broad "ROVEAGENT_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like ROVEAGENT_KANBAN_DB / ROVEAGENT_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _ROVEAGENT_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("ROVEAGENT_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped ROVEAGENT_* var.
            _dropped_roveagent.append(k)
    if _dropped_roveagent:
        logger.debug(
            "execute_code: dropped %d non-allowlisted ROVEAGENT_* var(s) from the "
            ...
            len(_dropped_roveagent),
            ", ".join(sorted(_dropped_roveagent)),
        )
...
def check_sandbox_requirements() -> bool:
    ...
    try:
        from roveagent.tools.terminal_tool import (
...
# ---------------------------------------------------------------------------
# roveagent_tools.py code generator
# ---------------------------------------------------------------------------
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    roveagent_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'roveagent_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_roveagent_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the roveagent_tools.py stub module.
...
"""Auto-generated RoveAgent tools RPC stubs."""
...
def _connect():
    ...
    ROVEAGENT_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ROVEAGENT_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("ROVEAGENT_RPC_TOKEN", ""),
    }) + "\\n"
    ...
    # re-accepts (ROVEAGENT_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("ROVEAGENT_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
"""Auto-generated RoveAgent tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ROVEAGENT_RPC_DIR") or os.path.join(tempfile.gettempdir(), "roveagent_rpc")
_seq = 0
...
def _rpc_server_loop(
    ...
    """
    from roveagent.model_tools import handle_function_call
...
def _get_or_create_env(task_id: str):
    ...
    with task_lock:
        ...
        from roveagent.tools.terminal_tool import _is_container_backend as _is_container
...
def _format_interrupted_output(stdout_text: str) -> str:
    ...
    from roveagent.tools.interrupt import get_interrupt_reason
...
def _execute_remote(
    ...
    sandbox_dir = f"{temp_dir}/roveagent_exec_{sandbox_id}"
    ...
    try:
        ...
        # --- Session-kernel path (roveagent-agent#96873) -------------------
        ...
        try:
            from roveagent.tools.code_kernel_remote import execute_in_remote_kernel
        ...
        tools_src = generate_roveagent_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/roveagent_tools.py", tools_src)
        ...
        env_prefix = (
            f"ROVEAGENT_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"ROVEAGENT_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ROVEAGENT_TIMEZONE", "").strip()
        if tz:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    child_env["ROVEAGENT_RPC_SOCKET"] = rpc_endpoint
    child_env["ROVEAGENT_RPC_TOKEN"] = rpc_token
    ...
    # ROVEAGENT_TIMEZONE is an internal RoveAgent setting and must not leak
    # into child processes.
    _tz_name = os.getenv("ROVEAGENT_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("ROVEAGENT_TIMEZONE", None)

    apply_subprocess_home_env(child_env)
    # ``roveagent_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # RoveAgent's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing RoveAgent's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip RoveAgent-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-RoveAgent-
    ...
    from roveagent.tools.environments.local import _strip_roveagent_owned_pythonpath
    _strip_roveagent_owned_pythonpath(child_env)
    _roveagent_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_roveagent_python_environment(child_python):
        _pp_parts.append(_roveagent_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import roveagent_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the RoveAgent "
            "environment; roveagent root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.roveagent.gateway`, but the
    ...
    from roveagent.tools.terminal_tool import _get_env_config, _docker_has_host_access
    ...
    from roveagent.tools.approval import check_execute_code_guard
    ...
    if _guard.get("user_approved"):
        from roveagent.tools.interrupt import clear_current_thread_interrupt
    ...
    from roveagent.tools.interrupt import is_interrupted as _is_interrupted
    ...
    if _get_kernel_mode() == "session":
        ...
        from roveagent.tools.code_kernel import execute_in_session_kernel
    ...
    # --- Set up temp directory with roveagent_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="roveagent_sandbox_")
    ...
    # selector from ROVEAGENT_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"roveagent_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated roveagent_tools module.
        ...
        tools_src = generate_roveagent_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "roveagent_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ROVEAGENT_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.roveagent/.env')).
...
def _load_config() -> dict:
    ...
    ``roveagent --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (roveagent-agent's python). Reproducible and the
...
# Interpreter paths already reported as outside the RoveAgent environment —
...
def _probe_python(python_path: str, code: str, *, text: bool = False):
    ...
    """
    try:
        from roveagent.core.delegation_context import delegated_child_subprocess_env
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the roveagent root from every
...
def _uses_roveagent_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to RoveAgent's active Python environment.
...
def _resolve_child_cwd(mode: str, staging_dir: str, task_id: str = "") -> str:
    ...
    if task_id:
        ...
        try:
            from roveagent.tools.terminal_tool import get_session_cwd
        ...
        try:
            from roveagent.tools.file_tools import _registered_task_cwd_override
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``roveagent tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and roveagent-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.roveagent/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
            ...
            "RoveAgent's own python (the common case — stdlib plus RoveAgent's "
    ...
    description = (
        "Run Python that calls RoveAgent tools programmatically. Use when you "
        ...
        "loses that state.\n\n"
        f"Available via `from roveagent_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from roveagent_tools import {import_str}` "
