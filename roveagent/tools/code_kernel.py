...
"""Auto-generated RoveAgent session-kernel runner. One exec cell per request."""
import contextlib
import io
import json
import os
...
_SENTINEL = os.environ["ROVEAGENT_KERNEL_SENTINEL"]
_CAPTURE_LIMIT = {capture_limit}
_SPILL_DIR = os.environ.get("ROVEAGENT_KERNEL_SPILL_DIR", "")
...
class CellAuthority:
    ...
    def __init__(self, task_id: str):
        ...
        try:
            from roveagent.tools.thread_context import _callback_api
    ...
    def dispatch(self, tool_name: str, tool_args: dict) -> str:
        ...
        from roveagent.tools.code_execution_tool import tool_error
...
# roveagent-agent#88637 by @z80dev (stable owner id, owner-teardown disposal,
...
def _lifecycle_limits() -> Tuple[int, int]:
    from roveagent.tools.code_execution_tool import _load_config
...
def _resolve_owner(task_id: str) -> str:
    ...
    try:
        ...
        if is_delegated_child_context():
            ...
            child_id = get_session_env("ROVEAGENT_SESSION_ID", "") or (task_id or "")
...
def shutdown_kernels_for_owner(owner: str) -> None:
    ...
    (the /new + session-close disposal shape from roveagent-agent#88637).
    """
    if not owner:
        return
    with _KERNELS_LOCK:
...
def _teardown(kernel: SessionKernel) -> None:
    ...
    if kernel.proc is not None and kernel.proc.poll() is None:
        from roveagent.tools.code_execution_tool import _kill_process_group
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    client stub reconnects on its side (ROVEAGENT_RPC_PERSISTENT).
    ...
    """
    from roveagent.tools.code_execution_tool import _rpc_server_loop, tool_error
...
def _stdout_reader(kernel: SessionKernel) -> None:
    ...
    from roveagent.tools.code_execution_tool import MAX_STDOUT_BYTES
...
def _stderr_reader(kernel: SessionKernel) -> None:
    from roveagent.tools.code_execution_tool import MAX_STDERR_BYTES
...
def _spawn(kernel: SessionKernel, *, task_id: str, child_python: str,
    ...
    from roveagent.tools.code_execution_tool import (
        _build_child_env,
        generate_roveagent_tools_module,
    )

    kernel.tmpdir = tempfile.mkdtemp(prefix="roveagent_kernel_")
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()

    kernel.rpc_token = secrets.token_urlsafe(32)
    kernel.sentinel = "@@ROVEAGENT-KERNEL-" + secrets.token_urlsafe(16) + "@@"
    ...
    if _IS_WINDOWS:
        ...
        kernel.sock_path = os.path.join(_sock_tmpdir, f"roveagent_rpc_{uuid.uuid4().hex}.sock")
    ...
    with open(os.path.join(kernel.tmpdir, "roveagent_tools.py"), "w", encoding="utf-8") as f:
        f.write(tools_src)
    runner_path = os.path.join(kernel.tmpdir, "roveagent_kernel_runner.py")
    ...
    child_env["ROVEAGENT_KERNEL_SENTINEL"] = kernel.sentinel
    ...
    child_env["ROVEAGENT_KERNEL_SPILL_DIR"] = kernel.tmpdir
    ...
    child_env["ROVEAGENT_RPC_PERSISTENT"] = "1"
    ...
    with kernel.lock:
        try:
            ...
            if status == "timeout":
                ...
                from roveagent.tools.code_execution_tool import _format_interrupted_output
