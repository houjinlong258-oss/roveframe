...
Closes the gap tracked in roveagent-agent#96873: local execute_code holds a
...
"""Auto-generated RoveAgent REMOTE session-kernel runner (file cell protocol)."""
import contextlib
import io
import json
import os
...
KDIR = os.environ["ROVEAGENT_KERNEL_DIR"]
...
def _spawn_remote_kernel(env, env_type: str, owner: str, task_env_id: str,
    ...
    kernel_dir = f"{_env_temp_dir(env)}/roveagent_rkernel_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{kernel_dir}/roveagent_tools.py", tools_src)

        env_prefix = (
            f"ROVEAGENT_KERNEL_DIR={q_dir} "
            f"ROVEAGENT_RPC_DIR={shlex.quote(kernel_dir + '/rpc')} "
...
def execute_in_remote_kernel(
    ...
    *,
    ...
    """
    from roveagent.tools.code_kernel import _resolve_owner
    from roveagent.tools.code_execution_tool import (
        _rpc_poll_loop,
        _ship_file_to_remote,
    )
