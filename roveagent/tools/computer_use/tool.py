...
`health_report` (surfaced by `roveagent computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session RoveAgent runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-RoveAgent-session cached backends. Each backend owns its own cua-driver
...
# See NousResearch/roveagent-agent#67052 gap 4.
...
def _warn_bypass_escalation(session_id: str) -> None:
    ...
    try:
        from roveagent.tools.computer_use.cua_backend import _cua_configured_permission_mode
...
def _cua_permission_mode(session_id: str) -> str:
    """Map RoveAgent's explicit approval bypass onto Cua's immutable mode.

    RoveAgent has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "ROVEAGENT_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    from roveagent.tools.computer_use.cua_backend import CuaDriverBackend
                    ...
                    raise RuntimeError(
                        f"Unknown ROVEAGENT_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    RoveAgent YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated RoveAgent sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the RoveAgent process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `roveagent computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NousResearch/roveagent-agent#67052).
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NousResearch/roveagent-agent#47072 — trycua/cua#1961
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        from roveagent.tools.computer_use.vision_routing import (
...
def _route_capture_through_aux_vision(
    ...
    *,
    ...
    The captured base64 PNG is materialised to ``$ROVEAGENT_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_roveagent_dir("cache/vision", "temp_vision_images")
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in RoveAgent' media cache and return its absolute path.
    ...
    try:
        ...
        cache_dir = get_roveagent_dir("cache/images", "image_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
        try:
            captures = sorted(
...
def _spill_elements_to_file(cap: CaptureResult) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_roveagent_dir("cache/computer_use", "computer_use_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
...
def check_computer_use_requirements() -> bool:
    ...
    `roveagent computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
    if sys.platform not in ("darwin", "win32", "linux"):
        return False
    from roveagent.tools.computer_use.cua_backend import cua_driver_binary_available
    return cua_driver_binary_available()


def get_computer_use_schema() -> Dict[str, Any]:
    from roveagent.tools.computer_use.schema import COMPUTER_USE_SCHEMA
