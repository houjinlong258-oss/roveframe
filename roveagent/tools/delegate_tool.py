...
# Must match roveagent_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
_RUNTIME_PROVIDER_CUSTOM = "custom"
from roveagent.tools import file_state
from roveagent.tools.terminal_tool import set_approval_callback as _set_subagent_approval_cb
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``roveagent-cli`` (which bundles
...
def _build_child_system_prompt(
    ...
    *,
    ...
    if workspace_path and str(workspace_path).strip():
        ...
        try:
            from roveagent.core.prompt_builder import build_context_files_prompt
...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``roveagent-cli`` because those also contain
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. roveagent-cli) so that individual
    ...
    # Blocked tools also live inside mixed platform bundles (roveagent-cli,
    # roveagent-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # child RoveAgent/Qwen subagent onto the parent's Claude Messages wire (or the
    ...
    from roveagent.core.delegation_context import delegated_child_context
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.roveagent/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_roveagent_dir("cache/delegation", "delegation_cache")
    ...
    **_kwargs,
    ...
    if _subagent_id:
        if owner_session_id is None:
            try:
                ...
                owner_session_id = get_session_env("ROVEAGENT_UI_SESSION_ID", "") or None
            except Exception:
    ...
    def _attach_worktree(entry_dict: Dict[str, Any]) -> None:
        ...
        if _worktree_info is None:
            return
        try:
            from roveagent.tools import subagent_worktree
            ...
            try:
                from roveagent.tools import subagent_worktree as _sw
    ...
    try:
        ...
        if _get_worktree_isolation():
            try:
                ...
                if subagent_worktree.local_backend_active():
                    _parent_cwd = None
                    try:
                        from roveagent.tools.terminal_tool import get_session_cwd as _gsc
            ...
            if _worktree_info is not None:
                try:
                    from roveagent.tools.terminal_tool import record_session_cwd as _rsc
                ...
                from roveagent.tools.subagent_worktree import build_worktree_context_note
        ...
        if isinstance(_output_schema, dict):
            from roveagent.tools.delegation_output_schema import (
                build_retry_message,
                validate_output,
            )
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from roveagent.clisupport.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    from roveagent.tools.delegation_output_schema import coerce_output_schema
    ...
    from roveagent.tools.delegation_live_log import (
    ...
    # ROVEAGENT_SESSION_ID ContextVar and os.environ with the subagent's internal
    # id before the background-dispatch code below would read it. The
    # request-scoped chat_id binding (the raw X-RoveAgent-Session-Id on
    ...
    try:
        ...
        _origin_ui_session_id = get_session_env("ROVEAGENT_UI_SESSION_ID", "")
    except Exception:
    ...
    for i, t in enumerate(task_list):
        ...
        if _task_schema is not None:
            from roveagent.tools.delegation_output_schema import append_output_contract
    ...
    if background:
        from roveagent.tools.async_delegation import dispatch_async_delegation_batch
        ...
        if not _async_ok:
            ...
            # with that id in X-RoveAgent-Session-Id once the batch completes.
            ...
            # ROVEAGENT_SESSION_ID here would return the subagent's internal id.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `roveagent -z`, a cron job, a Kanban "
        ...
        try:
            ...
            _source = get_session_env("ROVEAGENT_SESSION_SOURCE", "")
            ...
            _origin_ui_session_id = (
                get_session_env("ROVEAGENT_UI_SESSION_ID", "") or _origin_ui_session_id
            )
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and ROVEAGENT_SESSION_KEY is not in the CLI
...
def _resolve_child_credential_pool(
    ...
    if effective_provider == "custom":
        try:
            from roveagent.core.credential_pool import get_custom_provider_pool_key, load_pool
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'roveagent auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active RoveAgent config.

    Prefer the shared persistent loader because it follows the active
    ROVEAGENT_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``ROVEAGENT_IGNORE_USER_CONFIG=1`` (``roveagent chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("ROVEAGENT_IGNORE_USER_CONFIG") == "1"
