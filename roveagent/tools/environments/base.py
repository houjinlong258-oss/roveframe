"""Base class for all RoveAgent execution environment backends.
...
from roveagent.tools.interrupt import is_interrupted, is_thread_interrupted
from roveagent.tools.environments.path_utils import (
...
# ROVEAGENT_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ROVEAGENT_DEBUG_INTERRUPT"))
    ...
    def _maybe_spill(self, text: str) -> None:
        ...
        try:
            if self._spill_fh is None:
                from roveagent.tools.spill_safety import ensure_spill_dir, open_exclusive
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ROVEAGENT_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_roveagent_home() / "sandboxes"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._roveagent_stdin_errors`` while stdin is still closed
    ...
    proc._roveagent_stdin_errors = errors
    ...
    proc._roveagent_stdin_thread = thread
    thread.start()
...
def _cwd_marker(session_id: str) -> str:
    return f"__ROVEAGENT_CWD_{session_id}__"
...
# the FIRST session's ROVEAGENT_SESSION_ID into the snapshot makes every LATER
...
# ROVEAGENT_SESSION_ID leak via the shared snapshot). Stripping them from the
...
# set), not RoveAgent' per-turn session identity.
#
# Kept in sync with gateway.session_context._VAR_MAP: every bridged name starts
# with one of these prefixes (or is ROVEAGENT_UI_SESSION_ID). Used by unit tests
# as the Python-side contract for the exclusion set; the dump path unsets by
# name/prefix instead of grepping declare lines (see below / issue #71296).
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ROVEAGENT_SESSION_|ROVEAGENT_UI_SESSION_ID|ROVEAGENT_CRON_AUTO_DELIVER_|"
    "ROVEAGENT_CRON_SESSION|ROVEAGENT_BROWSER_CONTROL_)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``ROVEAGENT_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!ROVEAGENT_SESSION_*} ${!ROVEAGENT_CRON_AUTO_DELIVER_*} "
        "${!ROVEAGENT_BROWSER_CONTROL_*} "
        # AI_AGENT / ROVEAGENT_AGENT are per-command attribution markers
        ...
        "AI_AGENT ROVEAGENT_AGENT "
        f"ROVEAGENT_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all RoveAgent backends.
    ...
    # True only when commands execute on the SAME host as the RoveAgent process
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/roveagent-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/roveagent-cwd-{self._session_id}.txt"
    ...
    def _snapshot_excluded_passthrough_names(self) -> tuple[str, ...]:
        ...
        try:
            ...
            if is_multiplex_active():
                from roveagent.tools.env_passthrough import get_all_passthrough
                names = (
                    *get_all_passthrough(),
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__roveagent_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__roveagent_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__roveagent_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__roveagent_fns\" ] && declare -f $__roveagent_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_ROVEAGENT_RUNTIME_PASSTHROUGH_{name}"
        ...
        # under RoveAgent via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the RoveAgent-specific
        # ``ROVEAGENT_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``roveagent-agent`` — see huggingface.js
        ...
        # inherited from the RoveAgent process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (RoveAgent running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-roveagent-agent}" '
            'ROVEAGENT_AGENT="${ROVEAGENT_AGENT:-true}"'
        )
        ...
        parts.append("__roveagent_ec=$?")
        # Restrict RoveAgent metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__roveagent_snap_tmp=$(mktemp {_snap_tmp_template}) && "
        ...
        parts.append("exit $__roveagent_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ROVEAGENT_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        *,
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_roveagent_home() / "cache" / "terminal-output"
        ...
        # --- Debug tracing (opt-in via ROVEAGENT_DEBUG_INTERRUPT=1) -------------
        ...
        stdin_thread = getattr(proc, "_roveagent_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        stdin_errors = getattr(proc, "_roveagent_stdin_errors", None)
        if stdin_errors:
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ROVEAGENT_CWD_{session}__ marker from stdout output.
    ...
    def execute(
        ...
        *,
        ...
        if rewrite_compound_background:
            from roveagent.tools.terminal_tool import _rewrite_compound_background
        ...
        from roveagent.core.deadline import run_bounded_sync
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from roveagent.tools.terminal_tool import _transform_sudo_command
