...
from roveagent.tools.environments.base import _file_mtime_key
...
_FORCE_SYNC_ENV = "ROVEAGENT_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.roveagent") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.roveagent to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.roveagent", container_base, 1
        )
...
def _credential_host_paths() -> set[str]:
    ...
    try:
        from roveagent.tools.credential_files import get_credential_file_mounts
    except Exception:
        return set()
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ROVEAGENT_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, roveagent_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.roveagent/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(roveagent_home=roveagent_home)

    def _sync_back_transaction(self, roveagent_home: Path | None = None) -> None:
        ...
        # against an uninitialized remote .roveagent/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (roveagent_home or get_roveagent_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="roveagent-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.roveagent/skills/a.md`` →
        ``~/.roveagent/skills/a.md``, a new remote file at
        ``/root/.roveagent/skills/b.md`` maps to ``~/.roveagent/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
