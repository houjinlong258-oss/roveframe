...
wrapper, while preserving RoveAgent' persistent snapshot behavior across sessions.
"""
...
from roveagent.tools.environments.file_sync import (
    FileSyncManager,
    iter_sync_files,
...
_SNAPSHOT_STORE = get_roveagent_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("roveagent-agent", create_if_missing=True)
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.roveagent"),
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .roveagent/ as a tar archive.

        Modal sandboxes always run as root, so /root/.roveagent is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.roveagent"
            )
