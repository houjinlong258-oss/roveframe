...
Usage:
    from roveagent.tools.file_operations import ShellFileOperations
    from roveagent.tools.terminal_tool import _active_environments
...
from roveagent.tools.binary_extensions import BINARY_EXTENSIONS
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__ROVEAGENT_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__ROVEAGENT_FENCE_" in line or "\x1b]" in line
...
NOT_REGULAR_SENTINEL = "__roveagent_not_regular__"
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from roveagent.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        """
        from roveagent.tools.tool_output_limits import get_max_line_length
    ...
    def _escape_shell_arg(self, arg: str) -> str:
        ...
        """
        from roveagent.tools.environments.local import _bash_safe_path
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        paths — and RoveAgent disables MSYS argument conversion for its bash
        ...
        """
        from roveagent.tools.environments.local import _IS_WINDOWS, _msys_to_windows_path
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.roveagent-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".roveagent-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.roveagent-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.roveagent-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('ROVEAGENT_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
        ...
        marker = stdout.find("ROVEAGENT_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("ROVEAGENT_UTF16:OK"):].strip()
        try:
    ...
    def write_file(self, path: str, content: str,
        ...
        # clean the temp up on any failure so we never leak a ``.roveagent-tmp``
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        if error or match_count == 0:
            ...
            from roveagent.tools.fuzzy_match import is_already_applied
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from roveagent.tools.patch_parser import parse_v4a_patch, apply_v4a_operations
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        # globals (`Window.roveagentDesktop` → TS2339) that are defined by the
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        if not ext:
            return False
        try:
            from roveagent.core.lsp.servers import SERVERS
        except Exception:  # noqa: BLE001
            return False
        ext_lower = ext.lower()
        for srv in SERVERS:
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        if not self._lsp_local_only():
            return False
        try:
            from roveagent.core.lsp import get_service
        except Exception:  # noqa: BLE001
            return False
        try:
            svc = get_service()
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        if pre_content is not None and post_content is not None and pre_content != post_content:
            try:
                from roveagent.core.lsp.range_shift import build_line_shift
        ...
        if not diagnostics:
            return ""
        try:
            from roveagent.core.lsp.reporter import report_for_file, truncate
    ...
    def _search_with_rg(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # nothing converts back — RoveAgent sets MSYS_NO_PATHCONV for its bash).
