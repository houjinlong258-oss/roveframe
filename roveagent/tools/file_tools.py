...
from roveagent.core.file_safety import get_read_block_error
from roveagent.tools.binary_extensions import (
    has_binary_extension,
    has_opaque_document_extension,
    is_pdf_path,
)
from roveagent.tools.file_operations import (
    ShellFileOperations,
    normalize_read_pagination,
    normalize_search_pagination,
)
from roveagent.tools import file_state
...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``roveagent_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from roveagent.constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where roveagent previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_roveagent_backend_name", None)
...
def _uses_container_paths(task_id: str = "default") -> bool:
    ...
    try:
        from roveagent.tools.terminal_tool import _is_container_backend
...
def _registered_task_cwd_override(task_id: str = "default") -> str | None:
    ...
    """
    try:
        from roveagent.tools.terminal_tool import resolve_task_overrides
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as RoveAgent.
    ...
    if env is None:
        return True
    try:
        from roveagent.tools.environments.local import LocalEnvironment
    except ImportError:
        return True
...
_roveagent_config_resolved: str | None = None
_roveagent_config_resolved_loaded = False


def _get_roveagent_config_resolved() -> str | None:
    """Return the resolved absolute path of the RoveAgent config file (cached)."""
    global _roveagent_config_resolved, _roveagent_config_resolved_loaded
    if _roveagent_config_resolved_loaded:
        return _roveagent_config_resolved
    _roveagent_config_resolved_loaded = True
    try:
        from roveagent.clisupport.config import get_config_path
        _roveagent_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _roveagent_config_resolved = str(Path(_expand_tilde("~/.roveagent/config.yaml")).resolve())
        except Exception:
            _roveagent_config_resolved = None
    return _roveagent_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the RoveAgent config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    roveagent_config = _get_roveagent_config_resolved()
    if roveagent_config and (resolved == roveagent_config or normalized == roveagent_config):
        return (
            f"Refusing to write to RoveAgent config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.roveagent/config.yaml directly or use 'roveagent config' instead."
        )
    return None
...
# .cursorrules (or a project-local .roveagent config tree) outlives the current
...
_real_roveagent_home_cached: str | None = None
_real_roveagent_home_loaded = False


def _get_real_roveagent_home() -> str | None:
    """Return the realpath of the authoritative RoveAgent home (cached)."""
    global _real_roveagent_home_cached, _real_roveagent_home_loaded
    if _real_roveagent_home_loaded:
        return _real_roveagent_home_cached
    _real_roveagent_home_loaded = True
    try:
        from roveagent.constants import get_roveagent_home
        _real_roveagent_home_cached = os.path.realpath(str(get_roveagent_home()))
    except Exception:
        try:
            _real_roveagent_home_cached = os.path.realpath(_expand_tilde("~/.roveagent"))
        except Exception:
            _real_roveagent_home_cached = None
    return _real_roveagent_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from roveagent.clisupport.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.roveagent home is governed by its own guards
    ...
    # ``.roveagent`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_roveagent_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .roveagent config dirs (e.g. <repo>/.roveagent/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.roveagent`` — matching
        # any ancestor named .roveagent would gate every write inside a
        # checkout that happens to live under ~/.roveagent (e.g. the
        # roveagent-agent repo itself at ~/.roveagent/roveagent-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".roveagent":
            return candidate
    return None
...
def _request_protected_instruction_approval(
    ...
    try:
        import roveagent.tools.approval
...
def _check_approval_required_write(paths: list[str],
    ...
    """
    try:
        from roveagent.core.file_safety import is_write_approval_required
    except Exception:
        return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side RoveAgent mirror prefix for Docker file tools."""
    try:
        from roveagent.tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of RoveAgent state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host RoveAgent process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside RoveAgent scope.
    """
    try:
        from roveagent.core.file_safety import (
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        from roveagent.tools.read_extract import (
            ANYDOC_EXTENSIONS,
        ...
        # ── RoveAgent internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under ROVEAGENT_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == ROVEAGENT_HOME (e.g. "auth.json") still hits the
        ...
        if not _partial:
            try:
                from roveagent.tools.skill_manager_tool import mark_background_review_skill_read
...
def _mark_verification_stale(
    ...
    if not paths:
        return
    try:
        from roveagent.core.coding_context import project_facts_for
        from roveagent.core.verification_evidence import mark_workspace_edited
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    if mode == "patch" and patch:
        import re as _re
        from roveagent.tools.path_security import has_traversal_component
...
def _check_file_reqs():
    ...
    from roveagent.tools import check_file_requirements
...
def _is_openai_family_main() -> bool:
    ...
    """
    try:
        from roveagent.core.auxiliary_client import _read_main_model, _read_main_provider
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with roveagent_tools.write_file() for very "
            "large files."
        )
...
def _read_file_schema_overrides():
    ...
    """
    try:
        from roveagent.tools.read_extract import hosted_ocr_available
