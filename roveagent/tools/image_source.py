...
async def _download_to_bytes(url: str) -> bytes:
    ...
    from roveagent.tools.vision_tools import _download_image
...
def _media_cache_roots() -> list:
    """Agent-managed media cache directories under ROVEAGENT_HOME (host side).
...
def _permitted_host_read_target(p: Path, ctx: ResolveContext) -> Optional[Path]:
    ...
    - Non-local backend: permitted only if the path resolves inside a media
      cache root. A container-visible cache path (e.g. ``/root/.roveagent/cache/
    ...
    from roveagent.tools.credential_files import from_agent_visible_cache_path
...
def _get_active_env(task_id: Optional[str]):
    if not task_id:
        return None
    try:
        from roveagent.tools.terminal_tool import get_active_env
...
def _ensure_container_env(task_id: Optional[str]) -> None:
    ...
    if not task_id:
        return
    try:
        from roveagent.tools.terminal_tool import ensure_task_env
...
def _finalize(
    ...
    """
    from roveagent.tools.vision_tools import _detect_image_mime_type_from_bytes
...
def _detect_video_mime(data: bytes, src: str) -> Optional[str]:
    ...
    from roveagent.tools.vision_tools import _detect_video_mime_type
