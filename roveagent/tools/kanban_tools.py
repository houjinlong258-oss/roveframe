...
running under the dispatcher (env var ``ROVEAGENT_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``roveagent chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``roveagent kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``roveagent kanban complete …``
   inside the container, where ``roveagent`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.roveagent/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``roveagent kanban …``), the dashboard
(``roveagent dashboard``), and the slash command (``/kanban …``) — all
...
from roveagent.clisupport.goals import judge_goal
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever ROVEAGENT_KANBAN_* is present but not ours."""
    try:
        from roveagent.core.delegation_context import is_dispatcher_owned_worker_context
...
def _reject_delegated_child_mutation(tool_name: str) -> Optional[str]:
    ...
    inherited ROVEAGENT_KANBAN_* env vars are not proof of dispatcher ownership.
...
def _check_kanban_mode() -> bool:
    ...
    1. ``ROVEAGENT_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``roveagent chat`` without the kanban toolset see zero
    ...
    if _is_delegated_child_context():
        return False
    if os.environ.get("ROVEAGENT_KANBAN_TASK") and _is_dispatcher_owned_worker():
        return True
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if not _is_dispatcher_owned_worker():
        ...
        return None
    env_tid = os.environ.get("ROVEAGENT_KANBAN_TASK")
    return env_tid or None


def _worker_run_id(task_id: str) -> Optional[int]:
    """Return this worker's dispatcher run id when it is scoped to task_id."""
    if os.environ.get("ROVEAGENT_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("ROVEAGENT_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("ROVEAGENT_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("ROVEAGENT_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``ROVEAGENT_KANBAN_TASK`` set
    ...
    ``ROVEAGENT_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``ROVEAGENT_KANBAN_DB`` → ``ROVEAGENT_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting RoveAgent.
    """
    from roveagent.clisupport import kanban_db as kb
...
def _goal_judge_available() -> bool:
    ...
    """
    try:
        from roveagent.core.auxiliary_client import get_text_auxiliary_client
...
#   - No-op outside dispatcher-spawned worker context (no ``ROVEAGENT_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``ROVEAGENT_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``ROVEAGENT_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``ROVEAGENT_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("ROVEAGENT_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("ROVEAGENT_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("ROVEAGENT_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    (``ROVEAGENT_KANBAN_TASK`` set) and ``agent`` exposes ``steer``. Returns True
    ...
    by ``ROVEAGENT_PROFILE``) are skipped to avoid echoing itself.
    """
    tid = os.environ.get("ROVEAGENT_KANBAN_TASK")
    ...
    own = (os.environ.get("ROVEAGENT_PROFILE") or "").strip()
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set ROVEAGENT_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # ROVEAGENT_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``roveagent-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("ROVEAGENT_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        with httpx.stream(
            "GET",
            current_url,
            headers={"User-Agent": "roveagent-kanban/attach"},
            timeout=30,
...
def _handle_attach_url(args: dict, **kw) -> str:
    ...
    The agent passes a URL; RoveAgent downloads it (with the shared size cap)
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("ROVEAGENT_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets ROVEAGENT_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    # Prefer the request-scoped api_server origin binding: ROVEAGENT_SESSION_ID
    ...
    from roveagent.tools.async_delegation import _current_origin_session_id

    session_id = (
        args.get("session_id")
        or _current_origin_session_id()
        or os.environ.get("ROVEAGENT_SESSION_ID")
    )
    ...
    try:
        ...
        try:
            ...
            if _inherit_project and project_id is None:
                _self_tid = os.environ.get("ROVEAGENT_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("ROVEAGENT_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``roveagent dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``ROVEAGENT_SESSION_PLATFORM``,
      ``ROVEAGENT_SESSION_CHAT_ID``, and ``ROVEAGENT_SESSION_CHAT_TYPE`` are set in
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``ROVEAGENT_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # poller keys on ROVEAGENT_SESSION_KEY.
            session_key = (
                get_session_env("ROVEAGENT_SESSION_KEY", "")
                or os.environ.get("ROVEAGENT_SESSION_KEY", "")
            )
            if not session_key:
        ...
        chat_type = get_session_env("ROVEAGENT_SESSION_CHAT_TYPE", "") or None
        delivery_mode = "notify+wake" if is_gateway_session else None
        thread_id = get_session_env("ROVEAGENT_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("ROVEAGENT_SESSION_USER_ID", "") or None
        user_id_alt = get_session_env("ROVEAGENT_SESSION_USER_ID_ALT", "") or None
        message_id = get_session_env("ROVEAGENT_SESSION_MESSAGE_ID", "") or ""
        notifier_profile = (
            get_session_env("ROVEAGENT_SESSION_PROFILE", "")
            or os.environ.get("ROVEAGENT_PROFILE")
        )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to ROVEAGENT_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: ROVEAGENT_KANBAN_DB env → "
    "ROVEAGENT_KANBAN_BOARD env → the 'current' symlink under the kanban "
...
KANBAN_ATTACH_URL_SCHEMA = {
    ...
    "description": (
        "Attach a file to a task by URL — RoveAgent downloads it server-side "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to ROVEAGENT_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
