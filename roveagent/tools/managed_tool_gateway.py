...
def auth_json_path():
    """Return the RoveAgent auth store path, respecting ROVEAGENT_HOME overrides."""
    return get_roveagent_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`roveagent tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a RoveAgent release, and
