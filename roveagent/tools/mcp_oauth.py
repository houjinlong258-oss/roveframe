...
URL of RoveAgent' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``RoveAgentTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "RoveAgent Agent"
...
from roveagent.constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__roveagent_user_skipped__"
...
def _get_token_dir(roveagent_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses ROVEAGENT_HOME so each profile gets its own OAuth tokens.
    Layout: ``ROVEAGENT_HOME/mcp-tokens/``
    """
    from roveagent.constants import get_roveagent_home

    base = Path(roveagent_home) if roveagent_home is not None else Path(get_roveagent_home())
...
def _cached_redirect_port(storage: "RoveAgentTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If RoveAgent restarts and chooses a
...
def _cached_redirect_uri(storage: "RoveAgentTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``roveagent mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `roveagent mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# RoveAgentTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class RoveAgentTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ROVEAGENT_HOME/mcp-tokens/<server_name>.json         -- tokens
        ROVEAGENT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ROVEAGENT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        ROVEAGENT_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, roveagent_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._roveagent_home = Path(roveagent_home) if roveagent_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._roveagent_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._roveagent_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._roveagent_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._roveagent_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # RoveAgent records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``roveagent mcp login`` /
        ``roveagent mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._roveagent_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to RoveAgent.</p></body></html>"
            ) if code else (
...
def _make_redirect_handler(port: int, redirect_uri: str | None = None):
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        """
        from roveagent.tools.mcp_dashboard_oauth import get_dashboard_oauth_flow
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://roveagent-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"RoveAgent' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `roveagent mcp login <server>` later to "
...
RoveAgentOAuthClientProvider: Any = None


def _get_roveagent_oauth_provider_class() -> type | None:
    global RoveAgentOAuthClientProvider
    if RoveAgentOAuthClientProvider is not None:
        return RoveAgentOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _RoveAgentOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._roveagent_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_roveagent_token_user_agent", None)
            if ua:
    ...
    _RoveAgentOAuthClientProvider.__name__ = "RoveAgentOAuthClientProvider"
    _RoveAgentOAuthClientProvider.__qualname__ = "RoveAgentOAuthClientProvider"
    RoveAgentOAuthClientProvider = _RoveAgentOAuthClientProvider
    return RoveAgentOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = RoveAgentTokenStorage(server_name, roveagent_home=roveagent_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; RoveAgent only decides whether a
...
# roveagent-agent.nousresearch.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://nousresearch.github.io/roveagent-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port RoveAgent picks
...
def _has_cached_client_info(storage: "RoveAgentTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "RoveAgentTokenStorage | None") -> bool:
    ...
    RoveAgent has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "RoveAgentTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI RoveAgent would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "RoveAgent Agent" / "RoveAgent" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "RoveAgent Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. RoveAgent is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: roveagent mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. RoveAgent auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older RoveAgent is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = RoveAgentTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`roveagent mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_roveagent_oauth_provider_class()
