...
`mcp_tool.py`, and `roveagent_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# RoveAgentMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_roveagent_provider_class() -> Optional[type]:
    ...
    class RoveAgentMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._roveagent_server_name = server_name
            ...
            self._roveagent_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`RoveAgentTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, RoveAgentTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._roveagent_server_name, exc,
                    )
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not RoveAgent' — mcp 2.0 builds on httpx2,
            ...
            from roveagent.tools.mcp_tool import sdk_httpx
            httpx = sdk_httpx()
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._roveagent_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from roveagent.tools.mcp_oauth import RoveAgentTokenStorage
                        if isinstance(storage, RoveAgentTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._roveagent_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from roveagent.tools.mcp_oauth import RoveAgentTokenStorage
            if not isinstance(storage, RoveAgentTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``roveagent mcp reauth``.
            ...
            back to ``roveagent mcp reauth``.
            """
            try:
                if self._roveagent_preregistered:
                    return
                ...
                # walking back into the same refusal. `roveagent mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._roveagent_server_name, cimd_url,
                    )
        ...
        async def async_auth_flow(self, request):  # type: ignore[override]
            ...
            try:
                await get_manager().invalidate_if_disk_changed(
                    self._roveagent_server_name,
                    roveagent_home=self._roveagent_home,
                )
            except Exception as exc:  # pragma: no cover — defensive
                logger.debug(
    ...
    return RoveAgentMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ROVEAGENT_PROVIDER_CLS: Optional[type] = _make_roveagent_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._roveagent_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`RoveAgentMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _ROVEAGENT_PROVIDER_CLS is None:
            logger.warning(
        ...
        from roveagent.tools.mcp_oauth import (
            RoveAgentTokenStorage,
        ...
        from roveagent.tools.mcp_oauth import apply_oauth_provider_defaults
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `roveagent mcp login "
        ...
        return _ROVEAGENT_PROVIDER_CLS(
    ...
    def remove(
        ...
        *,
        ...
        Called by ``roveagent mcp remove <name>`` and (indirectly) by
        ``roveagent mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, roveagent_home), None)

        from roveagent.tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, roveagent_home=roveagent_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, roveagent_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, roveagent_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        """
        from roveagent.tools.mcp_oauth import _get_token_dir, _safe_filename

        entry = self._entries.get(self._key(server_name, roveagent_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(roveagent_home) / f"{_safe_filename(server_name)}.json"
            try:
