...
<url>``) is spawned as a direct child of the RoveAgent process. RoveAgent's own
...
RoveAgent process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next RoveAgent startup's opt-in
