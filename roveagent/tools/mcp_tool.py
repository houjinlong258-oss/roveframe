...
transport, discovers their tools, and registers them into the roveagent-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.roveagent/config.yaml under the ``mcp_servers`` key.
...
from roveagent.tools.ansi_strip import strip_unicode_tags
...
# per-profile log file (~/.roveagent/logs/mcp-stderr.log), tagged with the
...
# importing the SDK first — so tests doing mock.patch("roveagent.tools.mcp_tool.
...
def sdk_httpx():
    ...
    side by side with RoveAgent' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach RoveAgent' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_tool_timeout(config: dict) -> float:
    ...
    if per_server is not None:
        return per_server
    try:
        from roveagent.core.deadline import resolve_timeout
...
def _workspace_folder() -> str:
    ...
    """
    try:
        from roveagent.tools.file_tools import _authoritative_workspace_root
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    RoveAgent explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so RoveAgent and
    its subprocesses can consume those credentials without duplicating them
    in every MCP server's ``env:`` block.
    """
    try:
        from roveagent.clisupport.env_loader import get_secret_source
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            roveagent_home = os.path.expanduser(
                os.getenv(
                    "ROVEAGENT_HOME", os.path.join(os.path.expanduser("~"), ".roveagent")
                )
            )
            candidates = [
                os.path.join(roveagent_home, "node", "bin", resolved_command),
                ...
                # image (which the RoveAgent Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → RoveAgent MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that RoveAgent gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the RoveAgent document cache; returns a marker with
    ...
    try:
        from roveagent.gateway.platforms.base import cache_document_from_bytes
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`roveagent_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active RoveAgent profile name once at
...
class SamplingHandler:
    ...
    async def __call__(self, context, params):
        ...
        from roveagent.core.auxiliary_client import call_llm
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through RoveAgent' existing approval
    ...
    async def __call__(self, context, params):
        ...
        try:
            from roveagent.tools.approval import request_elicitation_consent
        ...
        # the agent's contextvars (ROVEAGENT_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit ROVEAGENT_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into RoveAgent' logging (agent.log via roveagent_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `roveagent setup` to install MCP support, "
                "then retry."
            )
        ...
        from roveagent.tools.osv_check import check_package_for_malware
        try:
        ...
        # ungraceful exit of this RoveAgent process (kill -9, crash, force-quit)
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    ...
                    for _pid in new_pids:
                        try:
                            from roveagent.clisupport.process_identity import register_child
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"roveagent-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through RoveAgent' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until RoveAgent is fully
                ...
                if not self._ever_connected:
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``roveagent mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by RoveAgent' pinned ``httpx``, and the two define unrelated
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from roveagent.tools.mcp_oauth import OAuthNonInteractiveError
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `roveagent mcp login {server_name}` (or delete the tokens "
        f"file under ~/.roveagent/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent RoveAgent processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_roveagent_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ROVEAGENT_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_roveagent_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_roveagent_home_override(home_override)
        try:
            return await coro
        finally:
...
def _wrap_with_dashboard_oauth_flow(coro):
    ...
    try:
        from roveagent.tools.mcp_dashboard_oauth import (
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ROVEAGENT_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_roveagent_home()
...
def _interpolate_env_vars(value):
    ...
    """
    from roveagent.core.secret_scope import get_secret as _get_secret
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the RoveAgent config file.
    ...
    ``os.environ`` (which includes ``~/.roveagent/.env`` loaded at startup).
    """
    try:
        from roveagent.clisupport.config import load_config
        from roveagent.utils import env_var_enabled as _env_enabled
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through RoveAgent' MEDIA: tag convention and out to messaging
            ...
            # RoveAgent' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * MCP/Pydantic optional fields commonly arrive as
    ...
    def _strip_nullable_union(node):
        ...
        """
        from roveagent.tools.schema_sanitizer import strip_nullable_unions
    ...
    def _collapse_const_unions(node):
        ...
        """
        from roveagent.tools.schema_sanitizer import collapse_const_unions
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves RoveAgent's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. RoveAgent uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the RoveAgent registry schema format.
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    if registered_names:
        ...
        try:
            from roveagent.tools.mcp_schema_cache import config_fingerprint, write_cache_entry
...
def _register_from_cache_sync(name: str, config: dict, entry: dict) -> List[str]:
    ...
    """
    from roveagent.tools.registry import registry
    from roveagent.tools.mcp_schema_cache import (
        config_fingerprint,
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    lazy_server_count = 0
    try:
        from roveagent.tools.mcp_schema_cache import config_fingerprint, get_cached_entry
    ...
    from roveagent.tools.interrupt import is_interrupted as _is_interrupted, set_interrupt as _set_interrupt
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``roveagent tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the RoveAgent registry.
...
def refresh_agent_mcp_tools(
    ...
    *after* that snapshot — a slow HTTP/OAuth server that misses the bounded
    ...
    """
    from roveagent.model_tools import get_tool_definitions
    from roveagent.tools.registry import registry
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    ...
    try:
        ...
        if callable(get_mem_schemas):
            ...
            from roveagent.core.memory_manager import memory_provider_tools_enabled
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple roveagent
