...
warn / fail). RoveAgent already scans hub-installed *skills* via
``tools/skills_guard.py``; this module extends the same static-analysis
engine to ``roveagent plugins install`` and ``roveagent plugins update``, which
...
- Exempts the "reads own env secret" / "HTTP call with key" pattern
  ...
  foreign credential-store access (~/.ssh, ~/.aws, ~/.roveagent/.env),
...
Usage:
    from roveagent.tools.plugin_guard import scan_plugin, should_allow_plugin_install
...
#
# ``roveagent_env_access`` (a reference to ``~/.roveagent/.env``) is the DOCUMENTED
...
def scan_plugin(plugin_dir: Path, source: str = "") -> ScanResult:
    ...
    Args:
        plugin_dir: Path to the plugin directory (typically the temp clone,
            before it is moved into ``~/.roveagent/plugins/``).
