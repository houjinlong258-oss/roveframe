...
from roveagent.tools.environments.local import _find_shell, _resolve_safe_cwd, _sanitize_subprocess_env
...
CHECKPOINT_PATH = get_roveagent_home() / "processes.json"
...
# ---------------------------------------------------------------------------
# When RoveAgent runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=roveagent-worker-<pid>``
        ...
        if not _IS_WINDOWS:
            try:
                ...
                if binary:
                    ...
                    probe_unit = f"roveagent-probe-scope-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised RoveAgent gateway runtime.

    Both supervisor markers and ``_ROVEAGENT_GATEWAY`` are inherited by every
    ...
    """
    if os.environ.get("_ROVEAGENT_GATEWAY") != "1":
        return False

    try:
        from roveagent.gateway.restart import is_gateway_supervisor_process
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"roveagent-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def __init__(self):
        ...
        try:
            from roveagent.tools.async_delegation import restore_undelivered_completions
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from roveagent.clisupport.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        from roveagent.tools.terminal_tool import _rewrite_compound_background as _rewrite_bg
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"roveagent-worker-{session.id}.scope"
        ...
        if use_systemd_scope:
            ...
            session.systemd_unit = f"roveagent-worker-{unit_suffix}.scope"
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/roveagent_bg_{session.id}.log"
        pid_path = f"{temp_dir}/roveagent_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/roveagent_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``roveagent_cli.goals``) to support
    ...
    def wait_for_pending_completions(
        ...
        *,
        ...
        One-shot CLI runs (``roveagent -q/-Q/-z``) exit as soon as their single
        ...
        (#90879): a recipient invoked as ``roveagent -p <bot> chat -Q
    ...
    def _oneshot_completion_wait_seconds() -> float:
        ...
        """
        try:
            from roveagent.clisupport.config import DEFAULT_CONFIG, cfg_get, read_raw_config
            cfg = read_raw_config()
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `roveagent update` restarting the gateway) is still
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the RoveAgent desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.roveagent/processes.json with the raw command
...
def _model_not_found_patterns() -> "list[str]":
    ...
    """
    try:
        from roveagent.core.error_classifier import _MODEL_NOT_FOUND_PATTERNS
...
def _delegation_config() -> dict:
    ...
    """
    try:
        from roveagent.tools.delegate_tool import _load_config as _cfg
...
def _delegation_model_not_found_notice(results) -> "list[str] | None":
    ...
    lines = [
        ...
        "roveagent config get delegation.model).",
    ]
    try:
...
def _delegation_attribution_line(evt: dict) -> "str | None":
    ...
    if not task_id.startswith("sa-"):
        return None
    try:
        from roveagent.tools.delegate_tool import get_subagent_attribution
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'RoveAgent'}"
    elif _reason == "lost":
...
def _redact_process_result(result: dict) -> dict:
    ...
    if not isinstance(result, dict):
        return result
    from roveagent.core.redact import redact_sensitive_text, redact_terminal_output
