"""Central registry for all roveagent-agent tools.
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    for path in sorted(tools_path.glob("*.py")):
        ...
        if registers:
            module_names.append(f"roveagent.tools.{path.stem}")
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (roveagent_constants itself is stdlib-only, so no cycle).
        from roveagent.constants import get_roveagent_home

        return Path(get_roveagent_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
def _save_discovery_cache(cache: Dict[str, list]) -> None:
    ...
    if path is None:
        return
    try:
        from roveagent.utils import atomic_json_write  # stdlib+yaml only; no cycle
...
# timescales. Cache results for ~30 s so env-var flips via ``roveagent tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a RoveAgent-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("ROVEAGENT_SESSION_ID", ""),
            get_session_env("ROVEAGENT_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("ROVEAGENT_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``roveagent tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved ROVEAGENT_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("roveagent_plugins."):
                ...
                if override:
                    if owner is not None and not self._plugin_override_allowed(
                        ...
                        logger.error(
                            ...
                            "operator opt-in. Set "
                            "roveagent.plugins.entries.<plugin_id>.allow_tool_override: true "
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``roveagent_plugins.{name}``), not the exact module string.
                # A handler defined in ``roveagent_plugins.pkg.handlers`` is
                # still owned by the ``roveagent_plugins.pkg`` package — exact
                ...
                if (
                    ...
                    logger.error(
                        ...
                        "operator opt-in. Set "
                        "roveagent.plugins.entries.%s.allow_tool_override: true in "
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``roveagent tools enable foo``)
    ...
    def dispatch(
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            ...
            try:
                from roveagent.model_tools import _sanitize_tool_error
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from roveagent.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
