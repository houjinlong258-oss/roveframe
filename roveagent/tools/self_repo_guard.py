...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite RoveAgent's live source checkout "
        ...
        "checkout, stop RoveAgent, run the command externally, then restart "
        "RoveAgent."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(roveagent_home).expanduser() if roveagent_home else Path.home() / ".roveagent"
