...
Design contract (matches the RoveAgent "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    roveagent_meta = meta.get("roveagent") if isinstance(meta, dict) else None
    if not isinstance(roveagent_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.roveagent.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in roveagent_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.roveagent.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("roveagent", "agent", "roveagent agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'RoveAgent Agent' (proper caps) "
