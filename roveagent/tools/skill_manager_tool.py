...
~/.roveagent/skills/. Existing skills (bundled, hub-installed, or user-created)
...
from roveagent.utils import atomic_write_text, is_truthy_value
...
def mark_background_review_skill_read(path: Path) -> None:
    ...
    """
    try:
        from roveagent.tools.skill_provenance import is_background_review
...
try:
    from roveagent.tools.skills_guard import scan_skill, should_allow_install, format_scan_report
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `roveagent config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.roveagent/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch ROVEAGENT_HOME and
    ...
    live profile-scoped ROVEAGENT_HOME on every call.
    """
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from roveagent.core.skill_utils import get_all_skills_dirs
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``roveagent-agent``) are treated as permanently pinned: the system prompt
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to RoveAgent (the agent's own "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`roveagent curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `roveagent curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `roveagent curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.roveagent/skills/) first, then any
    ...
    """
    from roveagent.core.skill_utils import get_all_skills_dirs, is_excluded_skill_path
...
def _maybe_auto_propose_org_edit(name: str, skill_path: Path) -> Optional[str]:
    ...
    """
    try:
        from roveagent.core.skill_utils import is_org_mirror_path
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `roveagent sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `roveagent sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `roveagent sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `roveagent sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER RoveAgent profiles.
    ...
    # Default profile (~/.roveagent/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.roveagent/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"profiles (`roveagent -p {other_profile}`) or edit the file "
            ...
            base += (
                ...
                f"Switch profiles (`roveagent -p <name>`) to edit there, or "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    """
    try:
        from roveagent.tools.skill_linter import lint_skill  # local import: optional path
    ...
    result["lint_hint"] = (
        ...
        "to match RoveAgent skill standards."
    )
...
def _patch_skill(
    ...
    if not old_string:
        ...
        # how to recover. Upstream: NousResearch/roveagent-agent#33064.
        return {
            "success": False,
            "error": (
    ...
    from roveagent.tools.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from roveagent.tools.fuzzy_match import format_no_match_hint
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.roveagent/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `roveagent curator restore` promises the skill can be brought back. Route
    ...
    if curator_pass:
        try:
            from roveagent.tools.skill_usage import archive_skill
...
def _maybe_debounced_sync_push(skill_name: str) -> None:
    ...
    try:
        from roveagent.tools.skill_usage import is_sync_enabled
    ...
    def _fire():
        try:
            from roveagent.tools.skills_sync_client import maybe_push_skills
...
def skill_manage(
    ...
    try:
        from roveagent.tools import skill_ledger as _ledger
    ...
    if result.get("success"):
        ...
        try:
            from roveagent.tools.skill_usage import bump_patch, forget, record_created
            from roveagent.tools.skill_provenance import is_background_review
            if action == "create":
                record_created(
                    name,
                ...
                # keeps its usage record as STATE_ARCHIVED so `roveagent curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"{display_roveagent_home()}/skills/; must precede that skill's other "
