...
Trust levels:
  - builtin:   Ships with RoveAgent. Never scanned, always trusted.
...
_ROVEAGENT_CONFIG_FILES = r'\.roveagent/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.roveagent/\.env|\~/\.roveagent/\.env',
     "roveagent_env_access", "critical", "exfiltration",
     "directly references RoveAgent secrets file"),
    ...
    #     For RoveAgent/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_ROVEAGENT_CONFIG_FILES),
     "roveagent_config_mod", "high", "persistence",
     "modification language aimed at RoveAgent configuration files (verify intent)"),
    (_shell_write_re(_ROVEAGENT_CONFIG_FILES),
     "roveagent_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting RoveAgent configuration files"),
    (r'\.roveagent/config\.yaml|\.roveagent/SOUL\.md',
     "roveagent_config_ref", "low", "persistence",
     "references RoveAgent configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the RoveAgent-native name; `.clawhubignore` is honored
