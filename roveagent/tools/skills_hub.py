#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the RoveAgent Skills Hub.
...
Used by roveagent_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
from roveagent.tools.skills_guard import (
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _roveagent_home() / "skills"
...
_DYNAMIC_PATH_RESOLVERS = {
    "ROVEAGENT_HOME": _roveagent_home,
...
def _ssrf_safe_http_get(url: str, *, timeout: int = 20) -> httpx.Response:
    ...
    from roveagent.tools.url_safety import create_ssrf_safe_client
...
def _guarded_http_get(url: str, *, timeout: int = 20) -> Optional[httpx.Response]:
    ...
    from roveagent.tools.url_safety import SSRFConnectionBlocked
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            ...
            if isinstance(roveagent_meta, dict):
                tags = roveagent_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            roveagent_meta = metadata.get("roveagent", {})
            if isinstance(roveagent_meta, dict):
                raw_tags = roveagent_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.roveagent/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/roveagent-agent"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's RoveAgent-native bundle
        # under pbakaus/impeccable:.roveagent/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``roveagent update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.roveagent.upstream`` in its SKILL.md
        frontmatter:
            ...
            metadata:
              roveagent:
                upstream:
                  ...
                  path: .roveagent/skills/impeccable
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.roveagent.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(meta_block, dict):
            return None
        roveagent_meta = meta_block.get("roveagent")
        if not isinstance(roveagent_meta, dict):
            return None
        upstream = roveagent_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``roveagent_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def install_from_quarantine(
    ...
    try:
        from roveagent.tools.skill_usage import record_installed
...
# ---------------------------------------------------------------------------
# RoveAgent centralized index source
# ---------------------------------------------------------------------------

ROVEAGENT_INDEX_URL = "https://roveagent-agent.nousresearch.com/docs/api/skills-index.json"
ROVEAGENT_INDEX_TTL = 6 * 3600  # 6 hours


def _roveagent_index_cache_file() -> Path:
    return _index_cache_dir() / "roveagent-index.json"


def _load_roveagent_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ROVEAGENT_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    roveagent_index_cache_file = _roveagent_index_cache_file()
    if roveagent_index_cache_file.exists():
        try:
            age = time.time() - roveagent_index_cache_file.stat().st_mtime
            if age < ROVEAGENT_INDEX_TTL:
                return json.loads(roveagent_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("RoveAgent index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "RoveAgent index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("RoveAgent index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        roveagent_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        roveagent_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class RoveAgentIndexSource(SkillSource):
    """Skill source backed by the centralized RoveAgent Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_roveagent_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "roveagent-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "roveagent-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        RoveAgentIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "roveagent-index"
