...
409), and is driven by:
  ...
  * the ``roveagent sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``roveagent_cli/``) so the
...
is toggled via ``roveagent sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.roveagent/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``roveagent sync enable`` — it is
...
# namespace from roveagent-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the ROVEAGENT_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: ROVEAGENT_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("ROVEAGENT_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a RoveAgent Cloud instance can be set
...
# precedence as base_url: the ROVEAGENT_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   ROVEAGENT_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   ROVEAGENT_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   ROVEAGENT_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``ROVEAGENT_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a RoveAgent Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("ROVEAGENT_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``ROVEAGENT_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``roveagent sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "ROVEAGENT_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``ROVEAGENT_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``roveagent sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    RoveAgent Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("ROVEAGENT_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.roveagent/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.roveagent/skills/, NOT bundled, NOT
    ...
    """
    try:
        from roveagent.tools.skill_usage import is_bundled, is_hub_installed, _find_skill_dir
        from roveagent.core.skill_utils import is_external_skill_path
    except Exception:
        return False
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (RoveAgent Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``ROVEAGENT_SYNC_DEFAULT_OPT_IN``) so a user's skills
      follow them with no per-skill setup.

    Sorted, deduped.
    """
    try:
        from roveagent.tools.skill_usage import load_usage
    except Exception:
        return []
    usage = load_usage() or {}
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.roveagent/skills/ containing
    ...
    try:
        ...
        for skill_md in root.rglob("SKILL.md"):
            ...
            try:
                from roveagent.tools.skill_usage import _read_skill_name
...
def stable_device_id() -> str:
    ...
    ~/.roveagent/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``roveagent sync device --name`` to set an explicit
    label."""
    ...
    # RoveAgent Cloud (and any templated deployment) can seed the label
    # declaratively via ROVEAGENT_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `roveagent sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("ROVEAGENT_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.roveagent/skills/.sync_device_id, overwriting
...
# ~/.roveagent/skills/.sync_state as JSON.
#
...
# path under ~/.roveagent/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.roveagent/skills/ (posix), or None."""
    try:
        from roveagent.tools.skill_usage import _find_skill_dir
    except Exception:
        return None
    skill_dir = _find_skill_dir(skill_name)
    if skill_dir is None:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this RoveAgent speaks "
            f"{WIRE_VERSION} — update RoveAgent to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "roveagent skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (roveagent sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.roveagent/skills/. Only paths the user has opted into (``sync: true``) are
    ...
    if remote_manifest:
        try:
            from roveagent.tools.skill_usage import set_sync, is_curation_eligible, is_sync_enabled
...
def maybe_push_skills(*, message: str = "roveagent skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (ROVEAGENT_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``roveagent sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
def list_org_skill_names() -> List[str]:
    ...
    names: List[str] = []
    try:
        from roveagent.core.skill_utils import read_active_org_id
...
#
# Org skills live under a DISTINCT local namespace, ~/.roveagent/skills/_org/
...
#
# `roveagent sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.roveagent/skills/_org/<org_id>/``.
...
def _org_baseline_path(org_id: str) -> Path:
    ...
    from roveagent.core.skill_utils import ORG_BASELINE_FILE
...
def _write_active_org_marker(org_id: str) -> None:
    ...
    try:
        from roveagent.core.skill_utils import ORG_ACTIVE_MARKER
...
def _write_org_provenance(org_id: str, data: Dict[str, Any]) -> None:
    ...
    try:
        from roveagent.core.skill_utils import ORG_PROVENANCE_FILE
