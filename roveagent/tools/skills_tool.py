...
Usage:
    from roveagent.tools.skills_tool import skills_list, skill_view, check_skills_requirements
...
# Cache validation (mirrors roveagent_cli/profiles.py::_count_skills, d5eee133e):
...
def _skills_scan_signature(dirs_to_scan, disabled) -> tuple:
    ...
    """
    from roveagent.core import skill_utils as _skill_utils
...
# All skills live in ~/.roveagent/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
ROVEAGENT_HOME = get_roveagent_home()
SKILLS_DIR = ROVEAGENT_HOME / "skills"
...
def _skills_dir() -> Path:
    ...
    set ROVEAGENT_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped ROVEAGENT_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from ROVEAGENT_HOME/.env."""
    env_path = get_roveagent_home() / ".env"
    ...
    # the canonical readers in roveagent_cli/config.py).
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from roveagent.core.skill_utils import skill_matches_platform as _impl
...
def skill_matches_environment(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from roveagent.core.skill_utils import skill_matches_environment as _impl
    return _impl(frontmatter)
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set ROVEAGENT_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (ROVEAGENT_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("ROVEAGENT_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("ROVEAGENT_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        from roveagent.gateway.platforms.base import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE

        return GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
    except Exception:
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_roveagent_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.roveagent/skills/mlops/axolotl/SKILL.md -> "mlops"
    ...
    try:
        from roveagent.core.skill_utils import get_external_skills_dirs
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``ROVEAGENT_SESSION_PLATFORM``.
    """
    try:
        from roveagent.gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``ROVEAGENT_PLATFORM`` environment variable
    3. ``ROVEAGENT_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from roveagent.clisupport.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("ROVEAGENT_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.roveagent/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``roveagent skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile ROVEAGENT_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from roveagent.clisupport.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: roveagent plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    if file_path:
        from roveagent.tools.path_security import has_traversal_component, validate_within_dir
    ...
    if preprocess:
        try:
            from roveagent.core.skill_preprocessing import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from roveagent.core.skill_utils import is_valid_namespace, parse_qualified_name
            from roveagent.clisupport.plugins import discover_plugins, get_plugin_manager
            ...
            try:
                from roveagent.plugins.memory import (
        ...
        from roveagent.core.skill_utils import iter_skill_index_files
        ...
        if skill_md is not None and project_dirs:
            from roveagent.core.skill_utils import is_quarantined_project_skill
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `roveagent skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.roveagent/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `roveagent skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        if file_path and skill_dir:
            from roveagent.tools.path_security import validate_within_dir, has_traversal_component
        ...
        # Check metadata.roveagent.* first (agentskills.io convention), fall back to top-level
        roveagent_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            roveagent_meta = metadata.get("roveagent", {}) or {}

        tags = _parse_tags(roveagent_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            roveagent_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
        ...
        if available_env_names:
            try:
                from roveagent.tools.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from roveagent.tools.credential_files import register_credential_files
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `roveagent sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from roveagent.tools.skill_usage import bump_use, bump_view
