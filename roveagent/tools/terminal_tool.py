...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or RoveAgent exit
...
# ---------------------------------------------------------------------------
from roveagent.tools.interrupt import is_interrupted, _interrupt_event  # noqa: F401 — re-exported
from roveagent.tools.registry import tool_error
from roveagent.tools.shell_heredoc import strip_inert_heredoc_bodies
# display_roveagent_home imported lazily at call site (stale-module safety during roveagent update)
...
from roveagent.tools.environments.singularity import _get_scratch_dir
from roveagent.tools.tool_backend_helpers import (
    coerce_modal_mode,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of roveagent directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "roveagent-*")):
...
def _current_session_key() -> str:
    ...
    Single lookup point for the ``ROVEAGENT_SESSION_KEY`` ContextVar with the
    ...
    return get_session_env("ROVEAGENT_SESSION_KEY", "")
...
def _in_delegated_child_context() -> bool:
    ...
    inherit process-wide interactivity signals (``ROVEAGENT_INTERACTIVE=1`` set
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ROVEAGENT_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ROVEAGENT_INTERACTIVE=1).
    ...
    if _sudo_cb is not None:
        try:
            ...
            from roveagent.tools.approval import human_wait_window
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (ROVEAGENT_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive RoveAgent password prompt or the sudo -S password-pipe path.
    ...
    # silently without RoveAgent prompting.
    ...
    # ROVEAGENT_INTERACTIVE=1 (and, on a recycled worker thread, potentially a
    ...
    should_prompt_for_sudo = (
        env_var_enabled("ROVEAGENT_INTERACTIVE") or has_sudo_prompt_callback
...
from roveagent.tools.environments.base import EnvironmentConnectionError
from roveagent.tools.environments.local import LocalEnvironment as _LocalEnvironment
from roveagent.tools.environments.singularity import SingularityEnvironment as _SingularityEnvironment
from roveagent.tools.environments.ssh import SSHEnvironment as _SSHEnvironment
from roveagent.tools.environments.docker import DockerEnvironment as _DockerEnvironment
from roveagent.tools.environments.modal import ModalEnvironment as _ModalEnvironment
from roveagent.tools.environments.managed_modal import ManagedModalEnvironment as _ManagedModalEnvironment
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``roveagent-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by RoveAgent processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      RoveAgent processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling RoveAgent processes a generous grace
    ...
    try:
        from roveagent.tools.environments.docker import reap_orphan_containers, _container_identity
    except ImportError:
        return
    try:
...
def _docker_persistent_profile_scoped() -> bool:
    ...
    ``container_persistent: true`` is ONE long-lived container per RoveAgent
...
def _current_session_profile() -> str:
    """Return the active session's RoveAgent profile name, or "" when unset.
    ...
    return get_session_env("ROVEAGENT_SESSION_PROFILE", "")
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, RoveAgentSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.roveagent/.env or environment variables."
        )
...
def _plugin_env_flag(env_type: str, attr: str, default=False):
    ...
    if not env_type or env_type in _CONTAINER_BACKENDS or env_type in {"local", "ssh", "managed_modal"}:
        return default
    try:
        from roveagent.core.terminal_env_registry import provider_flag
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``roveagent
    ...
    be stale from ``roveagent setup``). Environment values for omitted terminal
    ...
    try:
        from roveagent.clisupport.config import apply_terminal_config_to_env, read_raw_config
...
def _get_env_config() -> Dict[str, Any]:
    ...
    from roveagent.clisupport.config import _is_ssh_remote_tilde_cwd
    ...
    return {
        ...
        # Startup orphan reaper for roveagent-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior RoveAgent processes that hit SIGKILL / OOM / a closed terminal
        ...
        from roveagent.tools.environments.daytona import DaytonaEnvironment as _DaytonaEnvironment
        ...
        from roveagent.tools.environments.vercel_sandbox import (
        ...
        if provider is not None:
            ...
            try:
                env_obj._roveagent_backend_name = provider.name.strip().lower()
            except AttributeError:
                pass  # test doubles may reject attributes
            return env_obj
        try:
            from roveagent.core.terminal_env_registry import plugin_backend_names
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    for task_id, env in envs_to_stop:
        ...
        try:
            from roveagent.tools.file_tools import clear_file_ops_cache
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so RoveAgent tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        effective_task_id = _resolve_container_task_id(task_id)
        if _host_local:
            # RoveAgent-owned control-plane children must run beside the current
        ...
        from roveagent.tools.approval import get_current_session_key

        session_key = get_current_session_key(default="") or (task_id or "")

        # Hard-block: gateway lifecycle commands (systemctl/launchctl/roveagent
        # restart|stop|uninstall targeting roveagent-gateway) must never run inside the
        ...
        # never restart. This mirrors the `roveagent gateway restart` guard in
        # roveagent_cli/gateway.py and the cron-path guard in roveagent_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        # Gate on the SUPERVISED-gateway probe, not the raw _ROVEAGENT_GATEWAY
        # marker: gateway.run sets it at import time, so it leaks into every
        # process that merely imports gateway.run (roveagent serve --isolated,
        # CLI, web server) which are NOT the gateway and must be able to
        # restart it. A plain foreground `roveagent gateway run` (env set, PID
        ...
        if _is_supervised_gateway_process():
            from roveagent.cron.lifecycle_guard import (
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use RoveAgent cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `roveagent gateway restart` from a "
        ...
        if env_type == "local":
            from roveagent.tools.self_repo_guard import (
        ...
        if background:
            ...
            try:
                ...
                # roveagent-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in roveagent-agent dev work (PRs #31329, "
                            ...
                            "name='github/roveagent-agent-dev', "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `roveagent -z`, a "
                        ...
                        _gw_platform = _gse("ROVEAGENT_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("ROVEAGENT_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("ROVEAGENT_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("ROVEAGENT_SESSION_USER_ID", "")
                            _gw_user_name = _gse("ROVEAGENT_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("ROVEAGENT_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                can_reprompt = (
                    has_sudo_prompt_callback or env_var_enabled("ROVEAGENT_INTERACTIVE")
                ) and not _in_delegated_child_context()
                if can_reprompt:
                    output += (
            ...
            from roveagent.tools.tool_output_limits import get_max_bytes
            ...
            from roveagent.core.redact import redact_terminal_output
            ...
            if returncode != 0 and not exit_note:
                try:
                    from roveagent.tools.terminal_hints import annotate_failure
                ...
                try:
                    from roveagent.tools.terminal_hints import annotate_masked_success
            ...
            if spill_file_path:
                try:
                    ...
                    try:
                        ...
                        pass
            try:
                from roveagent.core.verification_evidence import record_terminal_result
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from roveagent.tools.environments.docker import find_docker
            docker = find_docker()
            if not docker:
