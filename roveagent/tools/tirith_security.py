...
it is automatically downloaded from GitHub releases to $ROVEAGENT_HOME/bin/tirith.
...
def _get_roveagent_home() -> str:
    """Return the RoveAgent home directory, respecting ROVEAGENT_HOME env var."""
    return str(get_roveagent_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_roveagent_home(), ".tirith-install-failed")
...
def _roveagent_bin_dir() -> str:
    """Return $ROVEAGENT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_roveagent_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ROVEAGENT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_roveagent_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ROVEAGENT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ROVEAGENT_HOME/bin/tirith
    ...
    roveagent_bin = os.path.join(_roveagent_bin_dir(), "tirith")
    if os.path.isfile(roveagent_bin) and os.access(roveagent_bin, os.X_OK):
        _resolved_path = roveagent_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return roveagent_bin
