...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$ROVEAGENT_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   RoveAgent-owned caches (images, audio, documents, ...) instead of littering
...
from roveagent.tools.budget_config import (
...
STORAGE_DIR = "/tmp/roveagent-results"
SPILLOVER_SUBDIR = "cache/spillover"
SPILLOVER_MAX_AGE_HOURS = 24
HEREDOC_MARKER = "ROVEAGENT_PERSIST_EOF"
...
def get_spillover_dir():
    """Return $ROVEAGENT_HOME/cache/spillover as a Path (not created)."""
    from roveagent.constants import get_roveagent_home

    return get_roveagent_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $ROVEAGENT_HOME/cache/spillover.
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/roveagent-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ROVEAGENT_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $ROVEAGENT_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other RoveAgent-owned
