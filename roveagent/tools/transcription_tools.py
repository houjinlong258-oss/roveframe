...
LOCAL_STT_COMMAND_ENV = "ROVEAGENT_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "ROVEAGENT_LOCAL_STT_LANGUAGE"
...
# DeepInfra STT base URL now resolved via roveagent_cli.models.deepinfra_base_url (shared).
...
def _resolve_stt_language(
    ...
    *,
    ...
    Resolution order (first non-empty wins):
      ...
      3. ``ROVEAGENT_LOCAL_STT_LANGUAGE`` env var (legacy escape hatch)
...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "This is often a permission issue: the RoveAgent process user cannot "
            "write to the virtual environment. Try running manually as the "
            "venv owner: `stat -c '%%u' '$(dirname $(dirname $(which python3)))'` "
            "then `su - <owner> -c 'VIRTUAL_ENV=/opt/roveagent/.venv "
...
#
# The single-env-var ``ROVEAGENT_LOCAL_STT_COMMAND`` escape hatch is preserved
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from RoveAgent's remote upload cap."""
...
def _render_command_stt_template(
    ...
    def replace_match(match: "re.Match[str]") -> str:
        ...
        token = f"__ROVEAGENT_STT_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _transcribe_command_stt(
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"roveagent-cmd-stt-{provider_name}-") as tmpdir:
...
def _get_provider(stt_config: dict) -> str:
    ...
    if explicit and provider == "local":
        ...
        try:
            from roveagent.tools.tool_backend_helpers import read_selection
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set ROVEAGENT_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `roveagent plugins list` to see "
...
def _dispatch_to_plugin_provider(
    ...
    *,
    ...
    if stt_config is not None and _is_command_stt_provider_config(
        ...
        return None
    try:
        from roveagent.core.transcription_registry import get_provider
...
def _apply_pre_transcription_hook(
    *,
    ...
    """
    try:
        from roveagent.clisupport.plugins import has_hook, invoke_hook
...
def _prepare_audio_for_transcription(
    ...
    temp_dir = tempfile.mkdtemp(prefix="roveagent-silk-")
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(
            target=_watch, name="roveagent-stt-idle-unload", daemon=True
        )
...
def _transcribe_local_command(
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="roveagent-local-stt-") as output_dir:
            ...
            # Scrub RoveAgent secrets from the child env (sibling path to #56332 /
            # _run_command_stt — this local-whisper path previously inherited
            # the full process environment).
            from roveagent.tools.environments.local import roveagent_subprocess_env

            child_env = roveagent_subprocess_env(inherit_credentials=False)
            subprocess.run(
                shlex.split(command),
                check=True,
                capture_output=True,
...
def _transcribe_groq(
    ...
    *,
    ...
    ``stt.language`` (config.yaml) > ``ROVEAGENT_LOCAL_STT_LANGUAGE`` (env).
...
def _transcribe_openai(
    ...
    *,
    ...
    try:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="roveagent-stt-") as work_dir:
                try:
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `roveagent model` or set XAI_API_KEY",
        }
...
def _transcribe_deepinfra(
    ...
    *,
    ...
    ``roveagent_cli.models`` helpers so every DeepInfra surface resolves the
...
def _trim_silence_for_cloud_stt(
    ...
    work_dir = tempfile.mkdtemp(prefix="roveagent-stt-trim-")
...
def _dispatch_stt_provider(
    ...
    # unavailable). Surface it — with its `roveagent tools` remediation —
