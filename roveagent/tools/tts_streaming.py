...
The keystone of RoveAgent' conversational voice UX. `stream_tts_to_speaker`
...
from roveagent.tools.tool_backend_helpers import resolve_openai_audio_api_key
from roveagent.tools.tts_tool import _get_provider, _load_tts_config, get_env_value
...
def _resolve_key(env_var: str, provider_id: str) -> str:
    ...
    """
    try:
        from roveagent.tools.tts_tool import _resolve_provider_key
...
class ElevenLabsStreamer(StreamingTTSProvider):
    ...
    def stream(self, text: str) -> Iterator[bytes]:
        from roveagent.tools.tts_tool import (
...
class XAIStreamer(StreamingTTSProvider):
    ...
    async def _async_frames(self, text: str):
        ...
        from roveagent.tools.tts_tool import DEFAULT_XAI_VOICE_ID
        from roveagent.tools.xai_http import resolve_xai_http_credentials
