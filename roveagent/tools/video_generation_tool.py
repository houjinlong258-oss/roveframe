...
provider** — turn on a backend by enabling a plugin (``roveagent plugins
enable video_gen/<name>``) and selecting it in ``roveagent tools`` → Video
Generation.
...
from roveagent.core.video_gen_provider import (
...
def _resolve_active_provider():
    ...
    """
    try:
        from roveagent.core.video_gen_registry import get_active_provider
        from roveagent.clisupport.plugins import _ensure_plugins_discovered
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `roveagent plugins list` to see "
            f"installed video gen backends, or `roveagent tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `roveagent tools` → "
...
# `roveagent tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`roveagent tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `roveagent tools` → Video Generation."
        )
        return {
            "description": "\n".join(parts),
            "parameters": {
    ...
    if provider.name == "xai":
        ...
        try:
            from roveagent.tools.xai_http import xai_storage_notice_text
