...
# ``agent.auxiliary_client`` pulls credential_pool → roveagent_cli.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("ROVEAGENT_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: ROVEAGENT_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("ROVEAGENT_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_roveagent_dir("cache/vision", "temp_vision_images")
...
async def _download_image(image_url: str, destination: Path, max_retries: int = 3) -> Path:
    ...
    async def _ssrf_redirect_guard(response):
        ...
        """
        from roveagent.tools.url_safety import async_is_safe_url, redirect_target_from_response
...
def _resize_image_for_vision(image_path: Path, mime_type: Optional[str] = None,
    ...
    try:
        ...
        try:
            from roveagent.tools.lazy_deps import ensure as _ensure_dep
...
def _should_use_native_vision_fast_path() -> bool:
    ...
    """
    try:
        from roveagent.core.auxiliary_client import _read_main_provider, _read_main_model
        from roveagent.core.image_routing import decide_image_input_mode, _lookup_supports_vision
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_roveagent_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $ROVEAGENT_HOME/cache/vision/ and cleaned up
...
def check_vision_requirements() -> bool:
    ...
    """
    try:
        from roveagent.core.auxiliary_client import aux_probe_mode, resolve_vision_provider_client
    except ImportError:
        return False
    try:
...
async def _materialize_video_from_terminal_backend(video_source: str, task_id: Optional[str]) -> Path:
    ...
    """
    from roveagent.tools.image_source import ImageResolutionError, ResolveContext, resolve_image_source
    ...
    temp_dir = get_roveagent_dir("cache/video", "temp_video_files")
    temp_dir.mkdir(parents=True, exist_ok=True)
