"""Wake-word ("Hey RoveAgent") detection — hands-free session trigger.
...
wake word, RoveAgent opens a fresh session and captures voice via the existing
...
* **openwakeword** (default, free, no API key) — loads an ONNX model. Defaults
  to the bundled "hey roveagent" model (``tools/wakewords/``) so the wake word
...
# Minimum gap between two consecutive wake fires, so one "hey roveagent" can't
...
_DEFAULTS: Dict[str, Any] = {
    ...
    "phrase": "hey roveagent",
    "sensitivity": 0.6,
    "confirmation_frames": _DEFAULT_CONFIRMATION_FRAMES,
    "start_new_session": True,
}

# Bundled "hey roveagent" model (tools/wakewords/) — the default, so the wake word
# works out of the box. Config names in _ALIASES resolve to it, not a built-in.
_BUNDLED_MODEL_NAME = "hey_roveagent"
_BUNDLED_MODEL_ALIASES = frozenset({"", "hey_roveagent", "hey roveagent", "roveagent"})


def _bundled_wakeword_path(framework: str = "onnx") -> str:
    """Path to the shipped hey_roveagent model (.onnx/.tflite) for ``framework``."""
...
def wake_phrase(cfg: Optional[Dict[str, Any]] = None) -> str:
    ...
    return str(_get(cfg, "phrase")) or "hey roveagent"
...
def enrolled_profile_phrases() -> Dict[str, str]:
    ...
    try:
        ...
        from roveagent.clisupport.profiles import get_profile_dir, list_profiles
...
def silent_audio_hint(details: Dict[str, Any]) -> str:
    ...
    if sys.platform == "darwin":
        return (
            "Microphone delivers only silence. Grant the RoveAgent backend "
...
class _OpenWakeWordEngine(_Engine):
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Default (or explicit "hey_roveagent") → the bundled model; a built-in name
...
# ROVEAGENT_HOME. Keywords are typed phrases tokenized at RUNTIME — no
...
def _sherpa_model_root() -> Path:
    ...
    return get_roveagent_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    model's vocabulary, so "hey roveagent", "hey coder", or any other phrase works
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # wake any profile ("hey roveagent" / "hey coder" / ...). display-name →
        # profile is kept for routing the match back.
        phrase = str(_get(cfg, "phrase") or "hey roveagent").strip()
        ...
        kw = tempfile.NamedTemporaryFile(
            mode="w", suffix=".txt", prefix="roveagent-kws-", delete=False, encoding="utf-8"
        )
...
def _stt_ready() -> bool:
    ...
    """
    try:
        from roveagent.tools.transcription_tools import _get_provider, _load_stt_config, is_stt_enabled
...
def _tts_ready() -> bool:
    ...
    """
    try:
        from roveagent.tools.tts_tool import _get_provider, _load_tts_config
    ...
    try:
        from roveagent.tools.tts_tool import check_tts_requirements
...
def check_wake_word_requirements(cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if provider == "porcupine" and not (os.getenv("PORCUPINE_ACCESS_KEY") or "").strip():
        ...
        hint = (f"Wake word needs {missing} configured — run `roveagent tools` "
...
# ---------------------------------------------------------------------------
# Process-wide singleton (mirrors roveagent_cli.voice's continuous API)
# ---------------------------------------------------------------------------
...
def _lock_path() -> Path:
    ...
    return get_default_roveagent_root() / "runtime" / "wake-word.lock"
