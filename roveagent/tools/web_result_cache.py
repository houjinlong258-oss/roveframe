...
def _web_config() -> dict:
    try:
        from roveagent.tools.web_tools import _load_web_config
...
def _cache_dir() -> Optional[Path]:
    try:
        ...
        d = get_roveagent_dir("cache/web", "web_cache")
...
def extract_cache_put(
    ...
    if _is_local_dev_url(url) or _is_cache_exempt_host(url):
        return
    try:
        from roveagent.tools.web_tools import MAX_STORED_TEXT_CHARS
