...
Backend is selected during ``roveagent tools`` setup (web.backend in config.yaml).
When available, RoveAgent can route Firecrawl calls through a Nous-hosted tool-gateway
...
from roveagent.plugins.web.firecrawl.provider import (
    Firecrawl,  # noqa: F401  # re-exported for tests that mock.patch("roveagent.tools.web_tools.Firecrawl")
...
from roveagent.plugins.web.parallel.provider import (  # noqa: F401 — backward-compat names
    _get_async_parallel_client,
    _get_parallel_client,
)
from roveagent.plugins.web.exa.provider import _get_exa_client  # noqa: F401
...
from roveagent.tools.debug_helpers import DebugSession
...
from roveagent.tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    build_vendor_gateway_url,
    peek_nous_access_token as _peek_nous_access_token,
    read_nous_access_token as _read_nous_access_token,
    resolve_managed_tool_gateway,
)
from roveagent.tools.tool_backend_helpers import (  # noqa: F401
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via RoveAgent config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through RoveAgent' config/.env layer (``roveagent config set``, ``roveagent tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.roveagent/config.yaml."""
    try:
...
def _list_registered_web_providers():
    ...
    try:
        from roveagent.core.web_search_registry import list_providers
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``roveagent tools``). A
    ...
    if configured:
        ...
        from roveagent.tools.tool_backend_helpers import NOUS_MANAGED_PROVIDER

        if configured == NOUS_MANAGED_PROVIDER:
            return "firecrawl"
        return configured

    from roveagent.tools.tool_backend_helpers import selection_exists
    ...
    try:
        ...
        from roveagent.core.web_search_registry import _keyless_preference, _keyless_tier_enabled
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `roveagent tools` repaint.
        try:
            from roveagent.tools.xai_http import has_xai_credentials
...
def _rescue_eligible(provider) -> bool:
    ...
    if provider is None:
        return False
    try:
        from roveagent.plugins.web.keyless_mcp import _KEYLESS_RING, use_keyless
...
def _rescue_search(provider_name: str, original_error: str, query: str, limit: int) -> dict:
    ...
    """
    from roveagent.plugins.web.keyless_mcp import search_with_failover
...
def _rescue_extract(provider_name: str, urls: list, results: list) -> list:
    ...
    """
    from roveagent.plugins.web.keyless_mcp import extract_with_failover
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `roveagent plugins enable {disabled_key}` "
        ...
        if provider is None:
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `roveagent tools` to set one up."
                    ),
                }
        else:
            logger.info(
            ...
            from roveagent.tools.web_result_cache import (
...
async def web_extract_tool(
    ...
    try:
        ...
        if not safe_urls:
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `roveagent plugins "
                            f"enable {disabled_key}` (or remove it from "
                            "roveagent.plugins.disabled)."
                        )
                    else:
                        error_text = selection_error(
                            "web",
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`roveagent plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
            ...
            from roveagent.tools.website_policy import check_website_access as _check_site
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``roveagent doctor`` paints a
...
if __name__ == "__main__":
    ...
    from roveagent.clisupport.config import get_env_value as _gev
