...
(see ``roveagent_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<ROVEAGENT_HOME>/pending/{memory,skills}/<id>.json``
...
def _pending_dir(subsystem: str) -> Path:
    return get_roveagent_home() / "pending" / subsystem
...
def current_origin() -> str:
    ...
    """
    try:
        from roveagent.tools.skill_provenance import get_current_write_origin
