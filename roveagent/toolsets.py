...
Usage:
    from roveagent.toolsets import get_toolset, resolve_toolset, get_all_toolsets
...
_ROVEAGENT_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (ROVEAGENT_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `roveagent tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``roveagent tools`` → Video Generation."
        ),
    ...
    "business": {
        "description": "Read current business facts through the scoped RoveFrame adapter",
        "tools": [
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (ROVEAGENT_KANBAN_TASK env "
    ...
    # Coding posture (base RoveAgent — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in roveagent_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full RoveAgent toolsets (CLI + messaging platforms)
    #
    ...
    # the `roveagent send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "roveagent-cli": {
        ...
        "tools": _ROVEAGENT_CORE_TOOLS,
        "includes": []
    },

    "roveagent-cron": {
        # Mirrors roveagent-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `roveagent tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as roveagent-cli; gated by `roveagent tools`",
        "tools": _ROVEAGENT_CORE_TOOLS,
        "includes": []
    },
    ...
    "roveagent-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _ROVEAGENT_CORE_TOOLS,
        "includes": []
    },

    "roveagent-homeassistant": {
    ...
    "roveagent-email": {
        "description": "Email bot toolset - interact with RoveAgent via email (IMAP/SMTP)",
        "tools": _ROVEAGENT_CORE_TOOLS,
        "includes": []
    },

    "roveagent-mattermost": {
    ...
    "roveagent-yuanbao": {
        ...
        "module": "roveagent.tools.yuanbao_tools",
        "includes": []
    },

    "roveagent-sms": {
        "description": "SMS bot toolset - interact with RoveAgent via SMS (Twilio)",
        "tools": _ROVEAGENT_CORE_TOOLS,
        "includes": []
    },
    ...
    "roveagent-webhook": {
        ...
        "tools": _ROVEAGENT_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "roveagent-gateway": {
        ...
        "tools": [],
        "includes": ["roveagent-telegram", "roveagent-discord", "roveagent-whatsapp", "roveagent-slack", "roveagent-signal", "roveagent-bluebubbles", "roveagent-homeassistant", "roveagent-email", "roveagent-sms", "roveagent-mattermost", "roveagent-matrix", "roveagent-dingtalk", "roveagent-feishu", "roveagent-wecom", "roveagent-wecom-callback", "roveagent-weixin", "roveagent-qqbot", "roveagent-webhook", "roveagent-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``roveagent-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_ROVEAGENT_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``roveagent-gateway``
    ...
    """
    core = set(_ROVEAGENT_CORE_TOOLS)
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (roveagent-<name>).
        # Gives them _ROVEAGENT_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("roveagent-"):
            platform_name = name[len("roveagent-"):]
            try:
                from roveagent.gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_ROVEAGENT_CORE_TOOLS)
                    try:
                        from roveagent.tools.registry import registry
                        plugin_tools.update(
