"""Shared utility functions for roveagent-agent."""
...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.roveagent/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
    ...
    * A Windows rename contended by another open handle (winerror 5/32/33).
    ...
    try:
        ...
        if contended:
            ...
            from roveagent.core.retry_utils import jittered_backoff
...
def warn_if_credential_file_broadly_readable(
    ...
    *,
    ...
    Secret-bearing files that users create by hand (or that older RoveAgent
...
def atomic_roundtrip_yaml_update(
    ...
    # ``roveagent config set``'s ``_set_nested`` (#91607: /model + TUI
    # persistence route through here and used to write ``glm-5: {'3': ...}``
    # phantom siblings while the runtime kept reading the literal key).
    from roveagent.clisupport.config import _greedy_literal_match, _split_key_path
...
def atomic_roundtrip_yaml_save(
    ...
    * Non-dict values (lists, scalars) are overwritten wholesale — list
    ...
    Shares the fail-closed contract ``roveagent_cli.config.atomic_config_write``
    ...
    ``roveagent_cli.config`` itself imports from this module.
    """
    from ruamel.yaml import YAML
    from ruamel.yaml.comments import CommentedMap
    from ruamel.yaml.scalarstring import DoubleQuotedScalarString

    from roveagent.clisupport.config import require_readable_config_before_write
