...
import { Plus, X, Sparkles, Bot, RefreshCw, Factory } from 'lucide-react';
import { fmtCurrency, fmtDateTime } from '@/lib/format';
import { Link } from '@/i18n/navigation';

type Tab = 'products' | 'orders' | 'inventory';
...
export default function BusinessPage() {
  ...
  const [prodForm, setProdForm] = useState({ name: '', category: '招牌菜', price: '', cost: '', description: '' });
  ...
  }, [tab, orderStatusFilter, loadProducts, loadOrders, loadInventory]);
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      setProdForm({ name: '', category: '招牌菜', price: '', cost: '', description: '' });
      await loadProducts();
    } finally {
      setProdSaving(false);
    }
