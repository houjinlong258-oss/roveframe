...
async function sendChannel(request: NextRequest) {
  ...
  const targets: ChannelKey[] = provider ? [provider] : await listConnectedChannels(tenant.tenantId, tenant.businessId);
  if (targets.length === 0) {
  ...
  const message = text && text.trim()
    ? text.trim()
    : await buildBriefing(tenant.tenantId, tenant.businessId, locale, forwardHeaders);

  const sent: string[] = [];
  const failed: { provider: string; error: string }[] = [];
  for (const ch of targets) {
    const config = await getChannelConfig(tenant.tenantId, tenant.businessId, ch);
    if (!config) {
...
export const POST = protectBusinessMutation(
  { permission: 'channels:write', action: 'channels.send', entity: 'channels' },
  sendChannel,
);
